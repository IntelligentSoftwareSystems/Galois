...
private:
  ...
  typedef GaloisRuntime::WorkList::ChunkedFIFO<CHUNK_SIZE, ODGnode*> WLTy;

  typedef Galois::GAccumulator<size_t> Accumulator;
  typedef GaloisRuntime::PerCPU < std::vector<ODGnode*> > VecPerThrd;
  ...
  public:
    ...
    ODGnode (size_t id, KEdge* edge)
      : id (id), activeElem (edge), indeg (0) {}
    ...
    bool operator < (const ODGnode& that) const {
  ...
  struct LoopBody {
    ...
    void operator () (ODGnode* src, ContextTy& lwl) {
      ...
      // TODO: the following may be moved inside if statement above(contract edge)
      for (ODGnode::OutSet::iterator i = src->neighbor_begin ()
          , ei = src->neighbor_end (); i != ei; ++i) {
      ...
      for (std::vector<ODGnode*>::const_iterator i = addListPerThrd.get ().begin ()
          , ei = addListPerThrd.get ().end (); i != ei; ++i) {
        lwl.push (*i);
      }
...
protected:
  ...
  virtual void runMST (std::vector<KNode*>& nodes, VecKEdge_ty& edges,
    ...
    for (std::set<KEdge>::iterator e = edges.begin (), ee = edges.end ();
        e != ee; ++e) {
    ...
    for (std::vector<ODGnode>::iterator i = odgNodes.begin (), ei = odgNodes.end ();
        i != ei; ++i) {
