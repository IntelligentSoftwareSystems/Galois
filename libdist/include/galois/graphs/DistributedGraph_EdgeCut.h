 ...
 *
 * Implements the edge cut partitioning scheme for DistGraph.
 */
...
class DistGraphEdgeCut : public DistGraph<NodeTy, EdgeTy, WithInEdges> {
...
public:
   ...
   */
  DistGraphEdgeCut(const std::string& filename, const std::string&,
    ...
    bGraph.loadPartialGraph(filename, nodeBegin, nodeEnd, *edgeBegin, *edgeEnd,
  ...
  std::vector<std::pair<uint32_t, uint32_t>> getMirrorRanges() const {
    ...
    if (base_DistGraph::numOwned < numNodes) {
      mirrorRanges_vec.push_back(
          std::make_pair(base_DistGraph::numOwned, numNodes));
    }
