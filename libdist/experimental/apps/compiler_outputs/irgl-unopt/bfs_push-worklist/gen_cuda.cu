...
__global__ void BFS(CSRGraph graph, unsigned int __nowned, unsigned int * p_dist_current, Worklist2 in_wl, Worklist2 out_wl)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
...
void BFS_cuda(struct CUDA_Context * ctx)
{
  ...
  ctx->out_wl.reset();
  // FP: "7 -> 8;
  BFS <<<blocks, threads>>>(ctx->gg, ctx->nowned, ctx->dist_current.gpu_wr_ptr(), ctx->in_wl, ctx->out_wl);
  // FP: "8 -> 9;
  check_cuda_kernel;
  // FP: "9 -> 10;
