...
void PageRank_partial_cuda(unsigned int  __begin, unsigned int  __end, struct CUDA_Context * ctx)
{
  ...
  kernel_sizing(blocks, threads);
  // FP: "4 -> 5;
  PageRank_partial <<<blocks, __tb_PageRank_partial>>>(ctx->gg, ctx->nowned, __begin, __end, ctx->nout.data.gpu_wr_ptr(), ctx->sum.data.gpu_wr_ptr(), ctx->value.data.gpu_wr_ptr());
  // FP: "5 -> 6;
  check_cuda_kernel;
  // FP: "6 -> 7;
}
