...
public:
   ...
   GraphTypeReplacer(Rewriter &rewriter) :
         rewriter(rewriter) {
   }
   virtual ~GraphTypeReplacer() {
   }
   virtual void run(const MatchFinder::MatchResult &Results) {
      VarDecl * vd = const_cast<VarDecl *>(Results.Nodes.getNodeAs<VarDecl>("varDecl"));
      FunctionDecl * graphCall = const_cast<FunctionDecl *>(Results.Nodes.getNodeAs<FunctionDecl>("callExpr"));
      if (vd) {
//         llvm::outs() << "VarDecl using Galois graph object :: \n";
//         vd->dump();
//         vd->getTypeSourceInfo();
//         vd->setType(vd->getASTContext().getUIntMaxType());
         rewriter.ReplaceText(vd->getTypeSourceInfo()->getTypeLoc().getSourceRange(), getReturnType(graphCall->getNameAsString().c_str()));
...
public:
   ...
   std::string class_name;bool in_class = false;
   TypeDefCollector(CXXRecordDecl * d) :
         class_def(d) {
      class_name = class_def->getNameAsString(); //QualType::getAsString(class_def->getTypeForDecl(), Qualifiers());
/*
      llvm::outs() << "Initialized typedef collector for class :: " << class_name << " \n";
      llvm::outs() << "------------------------------------------------------\n";
      d->dump(llvm::outs());
      llvm::outs() << "------------------------------------------------------\n";
*/
   ...
   virtual bool TraverseDecl(Decl * d) {
      if (d) {
         if (isa<CXXRecordDecl>(d)) {
            ...
            std::string decl_name = cd->getNameAsString(); //QualType::getAsString(cd->getTypeForDecl(), Qualifiers());
            llvm::outs() << " Traverse class :: " << decl_name << " " << QualType::getAsString(cd->getTypeForDecl(), Qualifiers()) << " \n";
            if (decl_name.compare(class_name) == 0) {
               llvm::outs() << " Graphclass name :: " << QualType::getAsString(class_def->getTypeForDecl(), Qualifiers()) << "\n";
...
//         llvm::outs() << " Done recursing class :: " << decl->getNameAsString( ) << " \n";
//      }
//      return true;
//   }
...
//         DeclContext * parent = const_cast<DeclContext*>(decl->getParentFunctionOrMethod());
...
//         if(s.find("edge_iterator")!=s.npos)
            {
               llvm::outs() << " ExposedType :: " << QualType::getAsString(exposed_type, Qualifiers()) << ", RealType:: " << QualType::getAsString(real_type, Qualifiers()) << " ";
//            if(parent)
//               llvm::outs()<< parent->get)
               llvm::outs() << "\n";

//            llvm::outs()<< "TypeDefDecl" << QualType::getAsString(decl->getUnderlyingType().getTypePtr(), Qualifiers()) << " MappedTo " << QualType::getAsString(decl->getCanonicalDecl()->getTypeForDecl(), Qualifiers()) << "\n";
...
public:
   ...
   std::map<CXXRecordDecl *, std::vector<Type *>*> typeDecls;
public:
   GraphTypeMapper(Rewriter &rewriter) :
         rewriter(rewriter) {
   }
...
//         rewriter.ReplaceText(vd->getTypeSourceInfo()->getTypeLoc().getSourceRange(), " CLGraph ");
      ...
      graph_types.insert(const_cast<Type *>(decl->getCanonicalDecl()->getTypeForDecl()));
      {
...
//      llvm::outs() << "\nBEGIN@@@@@@@@@@@\n";
//      llvm::outs() << ast_utility.get_string(call->getLocStart(), call->getLocEnd()) << "\n";

      llvm::outs() << "Kernel name :: " << k->getNameAsString() << "\n";
//      ast_utility.print_expr(call->getLocStart(), call->getLocEnd());
//      rewriter.InsertText((*k->field_begin())->getLocStart(), "KERNEL_INSTANCE_HERE\n");
//      for(auto f : k->fields()){
...
//         size_t len = std::distance(ast_utility.get_src_ptr(call->getLocStart()), ast_utility.get_src_ptr(call->getLocEnd()));
//      }
//      llvm::outs() << "\nEND\n";
   }
};
