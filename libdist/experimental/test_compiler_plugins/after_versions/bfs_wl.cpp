...
struct InitializeGraph {
  ...
  InitializeGraph(cll::opt<unsigned int>& _src_node,
                  llvm::cl::opt<unsigned int>& _src_node, Graph* _graph)
      : local_src_node(_src_node), local_src_node(_src_node), graph(_graph) {}
  void static go(Graph& _graph) {

    galois::do_all(_graph.begin(), _graph.end(),
                   InitializeGraph{local_src_node, src_node, &_graph},
...
struct Get_info_functor : public galois::op_tag {
  ...
  unsigned operator()(GNode n) const { return graph.getHostID(n); }
  GNode getGNode(uint32_t local_id) const {
    return GNode(graph.getGID(local_id));
  }
  uint32_t getLocalID(GNode n) const { return graph.getLID(n); }
...
struct BFS {
  ...
  void static go(Graph& _graph) {
    ...
    if (personality == GPU_CUDA) {
      ...
      for (int __i = 0; __i < cuda_wl.num_in_items; ++__i)
        cuda_wl.in_items[__i] = __i;
      ...
      while (!dbag.canTerminate()) {
        ++num_iter;
        // std::cout << "[" << galois::runtime::getSystemNetworkInterface().ID
        // << "] Iter : " << num_iter << " Total items to work on : " <<
        // cuda_wl.num_in_items << "\n";
        T_compute.start();
