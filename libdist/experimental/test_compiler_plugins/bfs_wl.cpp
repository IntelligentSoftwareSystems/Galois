...
struct InitializeGraph {
  ...
  InitializeGraph(cll::opt<unsigned int> &_src_node, llvm::cl::opt<unsigned int>& _src_node, Graph* _graph) : local_src_node(_src_node), local_src_node(_src_node), graph(_graph){}
  void static go(Graph& _graph) {

    galois::do_all(_graph.begin(), _graph.end(), InitializeGraph {local_src_node, src_node, &_graph}, galois::loopname("InitGraph"));
...
struct BFS {
  ...
  void static go(Graph& _graph){
    ...
    typedef dChunkedFIFO<64> dChunk;
      ...
      #endif
      galois::for_each(_graph.begin(), _graph.end(), BFS (&_graph), galois::loopname("BFS"), galois::workList_version(), galois::write_set("sync_push", "this->graph", "struct NodeData &", "struct NodeData &" , "dist_current", "unsigned long long" , "{ galois::atomicMin(node.dist_current, y);}",  "{node.dist_current = std::numeric_limits<unsigned long long>::max()/4; }"), galois::write_set("sync_pull", "this->graph", "struct NodeData &", "struct NodeData &", "dist_current" , "unsigned long long"), Get_info_functor<Graph>(_graph));
