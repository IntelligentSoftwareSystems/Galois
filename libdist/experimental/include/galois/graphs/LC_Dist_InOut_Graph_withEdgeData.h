...
class LC_Dist_InOut {
  ...
  struct NodeImplTy : public runtime::Lockable {
    ...
    unsigned get_num_outEdges() { return len; }
    unsigned get_num_inEdges() { return len_inEdges; }
...
public:
  ...
  struct with_node_data {
    typedef LC_Dist_InOut<_node_data> type;
  };
...
private:
  ...
  LC_Dist_InOut(runtime::PerHost<LC_Dist_InOut> _self, std::string inGr,
                std::string inGrTrans)
    ...
    for (unsigned h = 0; h < runtime::NetworkInterface::Num; ++h) {
      auto p =
          block_range(fg.begin(), fg.end(), h, runtime::NetworkInterface::Num);
      Num[h] = std::distance(p.first, p.second);
    }
    std::partial_sum(Num.begin(), Num.end(), PrefixNum.begin());

    auto p  = block_range(fg.begin(), fg.end(), runtime::NetworkInterface::ID,
                         runtime::NetworkInterface::Num);
    auto pt = block_range(fgt.begin(), fgt.end(), runtime::NetworkInterface::ID,
                          runtime::NetworkInterface::Num);
    ...
    unsigned num_out = std::accumulate(
        p.first, p.second, 0, [&fg](unsigned d, const uint64_t& node) {
          return d + std::distance(fg.edge_begin(node), fg.edge_end(node));
        });
    // number of in-edges on this host
    unsigned num_in = std::accumulate(
        pt.first, pt.second, 0, [&fgt](unsigned d, const uint64_t& node) {
          return d + std::distance(fgt.edge_begin(node), fgt.edge_end(node));
        });
    ...
    // allocate Nodes
    Nodes.reserve(num);
    // std::cout << " I am : " <<runtime::NetworkInterface::ID << " with Nodes =
    ...
    for (; ii != p.second; ++ii, ++iit) {
      ...
      cur += num_o;
      cur_In += num_i;
      for (auto fgi = fg.edge_begin(*ii), fge = fg.edge_end(*ii); fgi != fge;
           ++fgi) {
        //        std::cout << *ii << " " << *fgi << " " << num_o << " " <<
        //        num_i << " " << num << " " << num_out << " " << num_in <<
        //        "\n";
        if (*fgi < num) {
...
public:
  ...
  iterator end() {
    return iterator(PrefixNum[runtime::NetworkInterface::Num - 1]);
  }

  local_iterator local_begin() {
    if (runtime::LL::getTID() == 0)
      return iterator(runtime::NetworkInterface::ID == 0
                          ? 0
                          : PrefixNum[runtime::NetworkInterface::ID - 1]);
    else
      return local_end();
  ...
  local_iterator local_end() {
    return iterator(PrefixNum[runtime::NetworkInterface::ID]);
  }
  ...
  edge_iterator in_edge_begin(GraphNode N, MethodFlag mflag = MethodFlag::ALL) {
    ...
    if (mflag != MethodFlag::SRC_ONLY && mflag != MethodFlag::NONE) {
      /** prefetch **/
      for (auto ii = np->begin_inEdges(), ee = np->end_inEdges(); ii != ee;
           ++ii) {
        prefetch(makeNodePtr(ii->dst));
      }
      for (edge_iterator ii = np->begin_inEdges(), ee = np->end_inEdges();
           ii != ee; ++ii) {
        acquireNode(makeNodePtr(ii->dst), mflag);
      }
    }
