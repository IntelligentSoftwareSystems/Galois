  ...
  void syncRecvApply(uint32_t from_id, galois::runtime::RecvBuffer& buf,
    ...
    if (num > 0) {
      ...
      if (!FnTy::reduce_batch(from_id, &val_vec[0])) {
        galois::do_all(
            galois::iterate(0, num)[&](uint32_t n) {
...
public:
  ...
  vGraph(const std::string& filename, const std::string& partitionFolder,
    ...
    uint64_t numEdges =
        g.edge_begin(*g.end()) -
        g.edge_begin(*g.begin()); // depends on Offline graph impl
  ...
  void simulate_bare_mpi_reduce_serialized() {
...
#ifdef __GALOIS_SIMULATE_COMMUNICATION_WITH_GRAPH_DATA__
      if (!FnTy::reduce_batch(x, &val_vec[0])) {
           galois::do_all(galois::iterate(0, num) [&] (uint32_t n) {
               [&](uint32_t n){
            uint32_t lid = masterNodes[x][n];
#ifdef __GALOIS_HET_OPENCL__
            CLNodeDataWrapper d = clGraph.getDataW(lid);
...
#ifdef __GALOIS_SIMULATE_COMMUNICATION_WITH_GRAPH_DATA__
      void (vGraph::*fn)(galois::runtime::RecvBuffer&) =
          &vGraph::syncRecvApplyPull<FnTy>;
#else
    void (vGraph::*fn)(galois::runtime::RecvBuffer&) =
        &vGraph::syncRecvApplyPull;
#endif
...
#ifdef __GALOIS_HET_OPENCL__
                auto val =
                    FnTy::extract((localID), clGraph.getDataR((localID)));
#else
              auto val = FnTy::extract((localID), getData(localID));
#endif
                val_vec[n] = val;
...
#ifdef __GALOIS_SIMULATE_COMMUNICATION_WITH_GRAPH_DATA__
      void (vGraph::*fn)(galois::runtime::RecvBuffer&) =
          &vGraph::syncRecvApplyPush<FnTy>;
#else
    void (vGraph::*fn)(galois::runtime::RecvBuffer&) =
        &vGraph::syncRecvApplyPush;
#endif
    ...
    void reduce(std::string loopName) {
      ...
      std::string doall_str("LAMBDA::REDUCE_" + loopName + "_" +
                            std::to_string(num_run) + "_" +
                            std::to_string(num_iter_push));
