...
struct BufferWrapperImpl {
  ...
  BufferWrapperImpl() : num_hosts(0) {
    ...
    inMsgApply.init(ctx->get_default_device(), "buffer_wrapper.cl",
                    "recvApply");
    outMsgGen.init(ctx->get_default_device(), "buffer_wrapper.cl", "prepSend");
  }
  /*
   * DistGraph - slaveNodes - nodes on other hosts that have a master here.
   * Hence, outMessageIDs DistGraph - masterNodes - nodes on othor hosts that
   * are slaves here. Hence, inMessageIDs
   * */
  void init(int nh, std::vector<std::vector<size_t>> slaveNodes,
  ...
  void sync_outgoing(GraphType& g) {
    ...
    // &DistGraph::syncPullRecvApply<FnTy>;
    auto& net = galois::runtime::getSystemNetworkInterface();

    for (size_t h = 0; h < num_hosts; ++h) {
      for (size_t i = 0; i < outMessageIDs[h].size(); ++i) {
        // static unsigned int extract(uint32_t node_id, const struct NodeData &
        // node) {
        outMessageBuffers[h][i] =
            Fn::extract(outMessageIDs[h][i], g.getData(outMessageIDs[h][i]));
      }
  ...
  template <typename GraphType, typename Fn>
  void sync_incoming(GraphType& g,
                     std::vector<std::vector<unsigned int>>& incomingMessages) {
    auto& net = galois::runtime::getSystemNetworkInterface();
    cl_int err;
    auto* ctx              = galois::opencl::getCLContext();
    ...
    //
    for (size_t h = 0; h < num_hosts; ++h) {
      // err = clEnqueueWriteBuffer(queue, devInMessageBuffers[h], CL_TRUE, 0,
      // sizeof(unsigned int)*(incomingMessages[h],size()),
      // incomingMessages[h].data(), 0, NULL, NULL);
      /*for(size_t i=0; i< outMessageIDs[h].size(); ++i){
         //      static void setVal (uint32_t node_id, struct NodeData & node,
      unsigned int y) { Fn::setVal(inMessageIDs[h][i],
      g.getData(inMessageIDs[h][i]), inMessageBuffers[h][i]);
      }*/
      // TODO RK - send messages.
    } // end for host
    this->inMsgApply();
  ...
  void allocate() {
    ...
    cl_int err;
    CL_Kernel initBuffer(getCLContext()->get_default_device(),
                         "buffer_wrapper.cl", "initBufferWrapper", false);
    CL_Kernel registerBuffer(getCLContext()->get_default_device(),
                             "buffer_wrapper.cl", "registerBuffer", false);
    {
      inMessages.counters = clCreateBuffer(
          ctx->get_default_device()->context(), CL_MEM_READ_WRITE,
          sizeof(cl_uint) * (num_hosts), 0, &err);
      inMessages.messages = clCreateBuffer(
          ctx->get_default_device()->context(), CL_MEM_READ_WRITE,
          sizeof(cl_mem) * (num_hosts), 0, &err);
      inMessages.localIDs = clCreateBuffer(
          ctx->get_default_device()->context(), CL_MEM_READ_WRITE,
          sizeof(cl_mem) * (num_hosts), 0, &err);
      inMessages.buffer_ptr =
          clCreateBuffer(ctx->get_default_device()->context(),
                         CL_MEM_READ_WRITE, sizeof(BufferWrapperGPU), 0, &err);
      ...
      //               galois::opencl::CHECK_CL_ERROR(clSetKernelArg(kernel, 0,
      //               sizeof(cl_mem), &val), "Arg-1, is NOT set!");
      ...
      for (int i = 0; i < num_hosts; ++i) {
        cl_mem msg_buffer = clCreateBuffer(
            ctx->get_default_device()->context(), CL_MEM_READ_WRITE,
            sizeof(MessageType) * (inMessageIDs[i].size()), 0, &err);
        cl_mem id_buffer = clCreateBuffer(
            ctx->get_default_device()->context(), CL_MEM_READ_WRITE,
            sizeof(MessageType) * (inMessageIDs[i].size()), 0, &err);
        err = clEnqueueWriteBuffer(queue, id_buffer, CL_TRUE, 0,
                                   sizeof(unsigned int) *
                                       (inMessageIDs[i].size()),
                                   inMessageIDs[i].data(), 0, NULL, NULL);
    ...
    {
      outMessages.counters = clCreateBuffer(
          ctx->get_default_device()->context(), CL_MEM_READ_WRITE,
          sizeof(cl_uint) * (num_hosts), 0, &err);
      outMessages.messages = clCreateBuffer(
          ctx->get_default_device()->context(), CL_MEM_READ_WRITE,
          sizeof(cl_mem) * (num_hosts), 0, &err);
      outMessages.localIDs = clCreateBuffer(
          ctx->get_default_device()->context(), CL_MEM_READ_WRITE,
          sizeof(cl_mem) * (num_hosts), 0, &err);
      outMessages.buffer_ptr =
          clCreateBuffer(ctx->get_default_device()->context(),
                         CL_MEM_READ_WRITE, sizeof(BufferWrapperGPU), 0, &err);
      ...
      for (int i = 0; i < num_hosts; ++i) {
        cl_mem msg_buffer = clCreateBuffer(
            ctx->get_default_device()->context(), CL_MEM_READ_WRITE,
            sizeof(MessageType) * (outMessageIDs[i].size()), 0, &err);
        cl_mem id_buffer = clCreateBuffer(
            ctx->get_default_device()->context(), CL_MEM_READ_WRITE,
            sizeof(MessageType) * (outMessageIDs[i].size()), 0, &err);
        err = clEnqueueWriteBuffer(queue, id_buffer, CL_TRUE, 0,
                                   sizeof(unsigned int) *
                                       (outMessageIDs[i].size()),
                                   outMessageIDs[i].data(), 0, NULL, NULL);
...
void sync_push() {
void (DistGraph::*fn)(galois::runtime::RecvBuffer&) =
&DistGraph::syncRecvApply<FnTy>; auto& net =
galois::runtime::getSystemNetworkInterface(); for (unsigned x = 0; x <
hostNodes.size(); ++x) { if (x == id) continue; uint32_t start, end;
  ...
  for(; num ; --num) {
    ...
    FnTy::reduce(getData(gid - globalOffset), val);
       ...
       /*
          ...
          ctx->get_default_device()->command_queue();      unsigned sender=0;      const
          uint32_t counter =  0; // get counters from buffers.      gDeserialize(buffer,
          sender, counter);      for(int m=0; m<counter; ++m){      gDeserialize(buffer,
          messageBuffers[sender][m].first, messageBuffers[sender][m].second);
                   }
                   int err = clEnqueueWriteBuffer(queue, gpuMessageBuffer[sender],
          CL_TRUE, 0, sizeof(MessageType)*(num_ghosts), &messageBuffers[sender], 0,
          NULL, NULL);      applyKernel.init("buffer_wrapper.cl", "syncApply");
...
protected:
  ...
  void read_node_data_impl(NodeIterator& it, NodeDataType& data) {
    ...
    fprintf(stderr, "Data read[%d], offset=%d :: val=%d \n", id,
            sizeof(NodeDataType) * id, data.dist_current);
  ...
  void write_node_data_impl(NodeIterator& it, NodeDataType& data) {
    ...
    fprintf(stderr, "Data read[%d], offset=%d :: val=%d \n", *it,
            sizeof(NodeDataType) * (*it), data.dist_current);
...
public:
  ...
  CL_LC_Graph(std::string filename, unsigned myid, unsigned numHost)
    ...
    edge_data                  = nullptr;
    gpu_struct_ptr = gpu_meta = nullptr;
    //                           DistGraph<NodeDataType, EdgeDataType>
    //                           g(filename, myid, numHost);
    HGraph g(filename, myid, numHost);
    fprintf(stderr, "Loading from hgraph\n");
    load_from_galois(g);
  }
  //                        CL_LC_Graph(std::string filename, unsigned myid,
  //                        unsigned numHost) :
  //                                    SizeEdgeData(sizeof(EdgeDataType) /
  //                                    sizeof(unsigned int)),
  //                                    SizeNodeData(sizeof(NodeDataType) /
  //                                    sizeof(unsigned int)) {
  //                        //      fprintf(stderr, "Created
  //                        LC_LinearArray_Graph with %d node %d edge data.",
  //                        (int) SizeNodeData, (int) SizeEdgeData);
  //                                       fprintf(stderr, "Loading device-graph
  //                                       [%s] with copy-optimization.\n",
  //                                       filename.c_str()); _max_degree =
  ...
  void load_from_galois(GaloisGraph& ggraph) {
    ...
    fprintf(stderr,
            ...
            sizeof(EdgeDataType), _num_owned, _global_offset);
  }
  template <typename HGraph>
  void initialize_sync_buffers(HGraph& hgraph) {
    // bufferWrapper.init(galois::runtime::NetworkInterface::Num,
    // hgraph.masterNodes, hgraph.slaveNodes);
  }
  /*
   * */
  template <typename GaloisGraph>
  void writeback_from_galois(GaloisGraph& ggraph) {
    ...
    fprintf(stderr,
            "Writeback from GaloisGraph [V=%zu,E=%zu,ND=%lu,ED=%lu, Owned=%lu, "
            "Offset=%lu].\n",
            gg_num_nodes, gg_num_edges, sizeof(NodeDataType),
            sizeof(EdgeDataType), _num_owned, _global_offset);
  }
  ...
  EdgeDataType& getEdgeData(unsigned int eid) { return edge_data[eid]; }
  ...
  void allocate_on_gpu() {
    ...
    gpu_meta = clCreateBuffer(ctx->get_default_device()->context(), flags,
                              sizeof(int) * meta_buffer_size, nullptr, &err);
  ...
  void init_graph_struct() {
    ...
    // TODO RK - cleanup this place. We are generating the code, so we do not
    // need to 'build' kernels here.
    //               size_t kernel_len = strlen(cl_wrapper_str_CL_LC_Graph) +
    //               strlen(init_kernel_str_CL_LC_Graph) + 1; char * kernel_src
    //               = new char[kernel_len]; sprintf(kernel_src, "%s\n%s",
    //               cl_wrapper_str_CL_LC_Graph, init_kernel_str_CL_LC_Graph);
    ...
    init_kernel.set_arg_list_raw(gpu_struct_ptr, gpu_meta,
                                 gpu_wrapper.node_data,
                                 gpu_wrapper.outgoing_index,
                                 gpu_wrapper.neighbors, gpu_wrapper.edge_data);
  ...
  void copy_to_host(GRAPH_FIELD_FLAGS flags = GRAPH_FIELD_FLAGS::ALL) {
    ...
    if (flags & GRAPH_FIELD_FLAGS::EDGE_DATA) {
      err = clEnqueueReadBuffer(queue, gpu_wrapper.edge_data, CL_TRUE, 0,
                                sizeof(EdgeDataType) * _num_edges, edge_data, 0,
                                NULL, NULL);
  ...
  void copy_to_device(GRAPH_FIELD_FLAGS flags = GRAPH_FIELD_FLAGS::ALL) {
    ...
    if (flags & GRAPH_FIELD_FLAGS::EDGE_DATA) {
      err = clEnqueueWriteBuffer(queue, gpu_wrapper.edge_data, CL_TRUE, 0,
                                 sizeof(EdgeDataType) * _num_edges, edge_data,
                                 0, NULL, NULL);
  ...
  size_t getGID(const NodeIterator& nt) { return 0; }
