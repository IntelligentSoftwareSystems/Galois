...
protected:
  ...
  explicit CL_Device(CL_Platform* p, cl_device_id p_id)
      : _id(0), platform(p)
        ...
        ,
        init_kernel_str(
            "__kernel void init(__global int * arr, int size, int val){const "
            "int id = get_global_id(0); if(id < size){arr[id]=val;}}")
#endif
  {
    initialize(p_id);
    ...
    cl_program init_program =
        clCreateProgramWithSource(_context, 1, &init_kernel_str, NULL, &err);
    ...
    if (err != CL_SUCCESS) {
      size_t len;
      char buffer[10 * 2048];
      clGetProgramBuildInfo(init_program, _id, CL_PROGRAM_BUILD_LOG,
...
public:
  void finish() { clFinish(command_queue()); }
  cl_device_id id() { return _id; }
  cl_command_queue command_queue() { return _queue; }
  ...
  void print() const { print_info(_id); }
  DeviceStats& stats() { return _stats; }
  ...
  ~CL_Device() { fprintf(stderr, "Released device :: %s \n", name().c_str()); }
  ...
  cl_ulong get_device_shared_memory() {
    cl_ulong ret;
    CHECK_CL_ERROR(clGetDeviceInfo(_id, CL_DEVICE_LOCAL_MEM_SIZE,
  ...
  cl_ulong get_max_allocation_size() {
    cl_ulong ret;
    CHECK_CL_ERROR(clGetDeviceInfo(_id, CL_DEVICE_MAX_MEM_ALLOC_SIZE,
                                   sizeof(cl_ulong), &ret, NULL),
                   "Global mem size");
    return ret;
  }
  cl_uint get_device_threads() {
    cl_uint num_eus;
    CHECK_CL_ERROR(clGetDeviceInfo(_id, CL_DEVICE_MAX_COMPUTE_UNITS,
                                   sizeof(cl_uint), &num_eus, NULL),
                   "clGetDeviceInfo");
    size_t max_wg_size;
    CHECK_CL_ERROR(clGetDeviceInfo(_id, CL_DEVICE_MAX_WORK_GROUP_SIZE,
...
#else
    _queue = clCreateCommandQueue(
        _context, _id, 0 /*CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE*/, &err);
#endif
  ...
  std::string name() const {
    ...
    CHECK_CL_ERROR(
        clGetDeviceInfo(_id, CL_DEVICE_NAME, 1024, string_holder, NULL),
        "clGetDeviceInfo-1");
