...
typedef worklists::PerSocketChunkFIFO<GALOIS_DEFAULT_CHUNK_SIZE> defaultWL;
...
protected:
  ...
  ForEachExecutor(T2, const FunctionTy& f, const ArgsTy& args, WArgsTy... wargs)
      : term(substrate::getSystemTermination(activeThreads)),
        ...
        origFunction(f), loopname(get_by_supertype<loopname_tag>(args).value),
        broke(false) {}
...
template <typename RangeTy, typename FunctionTy, typename ArgsTy>
void for_each_impl_dist(const RangeTy& range, const FunctionTy& fn,
                        const ArgsTy& args) {
...
void for_each_gen(const RangeTy& r, const FunctionTy& fn, const TupleTy& tpl) {
  ...
  if (forceNew) {
    ...
    runtime::for_each_impl(
        r, fn,
        std::tuple_cat(xtpl,
                       get_default_trait_values(
                           tpl, std::make_tuple(loopname_tag{}, wl_tag{}),
    ...
    runtime::for_each_impl(
        ...
        std::tuple_cat(xtpl,
                       get_default_trait_values(
                           dtpl, std::make_tuple(loopname_tag{}, wl_tag{}),
                           std::make_tuple(loopname{}, wl<defaultWL>()))));
  }
}
...
// Tag dispatching
template <typename RangeTy, typename FunctionTy, typename TupleTy>
void for_each_gen_dist(const RangeTy& r, const FunctionTy& fn,
                       const TupleTy& tpl) {

  auto dtpl = std::tuple_cat(
      tpl, get_default_trait_values(tpl, std::make_tuple(wl_tag{}),
                                    std::make_tuple(wl<defaultWL>())));
  for_each_gen_dist_impl(
      r, fn, dtpl,
      std::integral_constant<bool,
                             exists_by_supertype<op_tag, TupleTy>::value>());
}

/** For distributed worklist **/
template <typename RangeTy, typename FunctionTy, typename TupleTy>
void for_each_gen_dist_impl(const RangeTy& r, const FunctionTy& fn,
                            const TupleTy& tpl, std::true_type) {
  static_assert(!exists_by_supertype<char*, TupleTy>::value, "old loopname");
  static_assert(!exists_by_supertype<char const*, TupleTy>::value,
                "old loopname");
  static_assert(!exists_by_supertype<bool, TupleTy>::value, "old steal");
  ...
  if (exists_by_supertype<op_tag, TupleTy>::value) {
    ...
    static_assert(
        ...
        "old type trait");
    static_assert(
        !forceNew || runtime::DEPRECATED::ForEachTraits<FunctionTy>::NeedsStats,
        "old type trait");
    static_assert(!forceNew ||
                      runtime::DEPRECATED::ForEachTraits<FunctionTy>::NeedsPush,
                  "old type trait");
    ...
    typedef typename get_type_by_supertype<wl_tag, TupleTy>::type::type
        BaseWorkListTy;
    typedef
        typename std::iterator_traits<typename RangeTy::iterator>::value_type
            value_type;
    ...
    auto ztpl = std::tuple_cat(
        ytpl, std::make_tuple(
                  wl<galois::worklists::WLdistributed<WorkListTy>>(&bag)));
    auto xtpl =
        std::tuple_cat(ztpl, typename function_traits<FunctionTy>::type{});

    std::string loopName(get_by_supertype<loopname_tag>(tpl).value);
    // std::string timer_for_each_str("FOR_EACH_IMPL_" + loopName + "_" +
    ...
    runtime::for_each_impl_dist(
        r, fn,
        std::tuple_cat(xtpl,
                       get_default_trait_values(
                           ztpl, std::make_tuple(loopname_tag{}, wl_tag{}),
                           std::make_tuple(loopname{}, wl<defaultWL>()))));
    Timer_for_each_impl.stop();
    galois::runtime::reportStat(
        "(NULL)", "NUM_WORK_ITEMS_" + (helper_fn.get_run_identifier()),
        r.end() - r.begin(), 0);
    ...
    std::cout << "[" << galois::runtime::getSystemNetworkInterface().ID
              << "] worklist size : " << std::distance(bag.begin(), bag.end())
              << "\n";
#endif
    dbag.sync();
    ...
    while (!dbag.canTerminate()) {
      ...
      // std::cout << "["<< galois::runtime::getSystemNetworkInterface().ID <<"]
      // Iter : " << num_iterations <<" Total items to work on : " <<
      // local_wl.size() << "\n";
      ...
      if (!local_wl.empty()) {
        runtime::for_each_impl_dist(
            runtime::makeStandardRange(local_wl.begin(), local_wl.end()), fn,
            std::tuple_cat(xtpl,
                           get_default_trait_values(
                               ztpl, std::make_tuple(loopname_tag{}, wl_tag{}),
                               std::make_tuple(loopname{}, wl<defaultWL>()))));
    ...
    galois::runtime::reportStat(
        "(NULL)", "NUM_ITERATIONS_" + std::to_string(helper_fn.get_run_num()),
        (unsigned long)num_iterations, 0);

    // std::cout << "\n\n TERMINATING on : " << net.ID << "\n\n";

  } else {
    /**CHECK with exist with exist_by_supertype to call special gen_dist . which
     * will extract from tupe OP. get_by_super<foo_tag> **/
    static const bool forceNew = false;
    static_assert(
        !forceNew ||
            runtime::DEPRECATED::ForEachTraits<FunctionTy>::NeedsAborts,
...
template <typename RangeTy, typename FunctionTy, typename TupleTy>
void for_each_gen_dist_impl(const RangeTy& r, const FunctionTy& fn,
                            const TupleTy& tpl, std::false_type) {
  static const bool forceNew = false;
