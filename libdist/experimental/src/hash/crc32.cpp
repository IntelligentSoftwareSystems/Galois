...
namespace
{
  ...
  // Edited by Galois project team: this function is apparently never used so 
  // we commented it out to make the compiler not complain
...
uint32_t CRC32::hash(const void* data, size_t numBytes) {
  union {
    uint32_t r;
    unsigned char b[4];
  } retval;
  CRC32 crc32;
  crc32.reset();
  crc32.add(data, numBytes);
  crc32.getHash(retval.b);
  return retval.r;
}
