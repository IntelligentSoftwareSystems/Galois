...
static cll::opt<Algo> algo(
    ...
    cll::values(clEnumValN(Algo::pull, "pull",
                ...
                clEnumValN(Algo::async, "async",
                           "Asynchronous without priority version..."),
                clEnumValEnd),
    cll::init(Algo::pull));
  ...
  */
  ...
  struct Process3 {
    ...
    void operator()(const UpdateRequest& srcRq,
      ...
      if (node->residual < tolerance ||
          pri(*node) != srcRq.first) { // degree biased residual
        post += 1;
        return;
      }
      // if (total<1000)
      // std::cout<<node->id<<" picked up... residual: "<<node->residual<<",
      // priority: "<<srcRq.first<<", rsd/deg:
      // "<<(node->residual/node->deg)<<"\n";
      ...
      double sum = 0;
      for (auto jj = graph.in_edge_begin(src, flag),
                ej = graph.in_edge_end(src, flag);
           jj != ej; ++jj) {
        GNode dst    = graph.getInEdgeDst(jj);
  ...
  void operator()(Graph& graph) {
    ...
    typedef OrderedByIntegerMetric<UpdateRequestIndexer,
                                   PSchunk>::with_block_period<4>::type OBIM;
#ifdef GALOIS_USE_EXP
    ...
    galois::for_each(initialWL, Process3(this, graph, pre, post),
                     galois::wl<OBIM>(), galois::loopname("mainloop"));
    T.stop();

    for (auto N : graph) {
      auto& data = graph.getData(N);
      if (data.residual > tolerance) {
        std::cout << N << " id " << data.id << " residual " << data.residual
                  << " pr " << data.pagerank << " nout " << data.nout << " deg "
                  << data.deg << "\n";
      }
    }
...
//---------- asynchronous without priority (original copy: PagerankDelta,
//readGraph is re-written.)
struct Async {
  struct LNode {
    float value;
    unsigned int nout;
