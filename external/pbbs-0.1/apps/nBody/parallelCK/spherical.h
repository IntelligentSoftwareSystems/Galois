...
public:
  ...
  void evaluateMultipole(coeff_type array[], real_type r, real_type cosTheta,
                         coeff_type eiphi) {
  ...
  void evaluateLocal(coeff_type array[], real_type r, real_type cosTheta,
                     coeff_type eiphi) {
    const real_type s2 = sqrt((1 - cosTheta) * (1 + cosTheta));
    real_type pn       = 1.0;
  ...
  void evaluateMultipoleTheta(coeff_type multipole[],
                              coeff_type multipoleTheta[], real_type r,
                              real_type cosTheta, real_type sinTheta,
                              coeff_type eiPhi) {
    const real_type s2 = sqrt((1 - cosTheta) * (1 + cosTheta));
    real_type pn       = 1.0;
    coeff_type dummy;
    ...
    for (int m = 0, fact = 1; m < numTerms; ++m, fact += 2) {
      ...
      coeff_type Yth =
          prefactor[npn] * (p - (m + 1) * cosTheta * p1) / sinTheta * eim;
      ...
      for (int n = m + 1; n < numTerms; ++n) {
        ...
        p1             = p;
        p   = (cosTheta * (2 * n + 1) * p1 - (n + m) * p2) / (n - m + 1);
        Yth = prefactor[npm] * ((n - m + 1) * p - (n + 1) * cosTheta * p1) /
              sinTheta * eim;
...
public:
  inline int powNeg1(int i) { return (i & 1) ? -1 : 1; }

  void M2Madd(coeff_type array[], point_type newCenter, coeff_type coeff[],
              point_type center) {
    ...
    coeff_type eiphi =
        (rxy == 0) ? coeff_type(1, 0) : coeff_type(diff.x / rxy, diff.y / rxy);
        ...
        for (int n = 0; n <= j; ++n) {
          for (int m = -n; m <= fmin(k - 1, n); ++m) {
            if (j - n >= k - m) {
              ...
              const coeff_type cnm =
                  (powNeg1((m - abs(m)) / 2) * powNeg1(n) * Anm[nm] *
                   Anm[jnkm] * AnmI[jk] * multipole[nm]);
              bx += coeff[jnkms] * cnm;
            }
          }
          for (int m = k; m <= n; ++m) {
            if (j - n >= m - k) {
              ...
              const int jnkms      = (j - n) * (j - n + 1) / 2 - k + m;
              const int nm         = n * n + n + m;
              const coeff_type cnm = powNeg1(k + n + m) * Anm[nm] * Anm[jnkm] *
                                     AnmI[jk] * multipole[nm];
  ...
  void M2Ladd(coeff_type array[], point_type newCenter, coeff_type coeff[],
              point_type center) {
    vect_type diff     = newCenter - center;
    real_type r        = diff.Length();
    real_type cosTheta = diff.z / r;
    ...
    for (int j = 0; j < numTerms; ++j) {
      for (int k = 0; k <= j; k++) {
        ...
        for (int m = -(numSourceTerms + 1); m < numSourceTerms; m++) {
          int ip = (1 - 2 * ((-abs(k - m) + abs(k) + abs(m)) / 2 &
                             1)); // i^{|k-m|-|k|-|m|}
          // i^{|k-m|-|k|-|m|} * A^k_j * -1^{j}
  ...
  void L2Ladd(coeff_type array[], point_type newCenter, coeff_type coeff[],
              point_type center) {
    vect_type diff     = newCenter - center;
    real_type r        = diff.Length();
    real_type cosTheta = diff.z / r;
      ...
      for (int k = 0; k <= j; ++k) {
        ...
        for (int n = j; n < numTerms; ++n) {
          for (int m = j + k - n; m < 0; ++m) {
            ...
            const coeff_type cnm =
                powNeg1(k) * Anm[jnkm] * Anm[jk] * AnmI[nm] * multipole[jnkm];
          ...
          for (int m = 0; m <= n; ++m) {
            if (n - j >= abs(m - k)) {
              ...
              const int nm         = n * n + n + m;
              const int nms        = n * (n + 1) / 2 + m;
              const coeff_type cnm = powNeg1((m - k - abs(m - k)) / 2) *
                                     Anm[jnkm] * Anm[jk] * AnmI[nm] *
                                     multipole[jnkm];
              ax += coeff[nms] * cnm;
            }
          }
  ...
  void clearM(coeff_type array[]) {
    for (int i = 0; i < numTerms * (numTerms + 1) / 2; i++)
      array[i] = coeff_type(0.0, 0.0);
  }

  void P2Madd(coeff_type array[], real_type gamma, point_type center,
              point_type x) {
  ...
  void M2P(real_type& potential, vect_type& field, point_type x,
           coeff_type coeff[], point_type center) {
    vect_type diff     = x - center;
    real_type r        = diff.Length();
    real_type rxy      = sqrt(diff.x * diff.x + diff.y * diff.y);
    real_type cosTheta = (r == 0) ? 1.0 : diff.z / r;
    ...
    evaluateMultipoleTheta(multipole, multipoleTheta, r, cosTheta, sinTheta,
                           eiphi);
    ...
    gxth *= 2;
    gxph *= 2;
    const real_type gxx = sinTheta * cosPhi * gxr +
                          cosTheta * cosPhi / r * gxth -
                          sinPhi / r / sinTheta * gxph;
    const real_type gxy = sinTheta * sinPhi * gxr +
                          cosTheta * sinPhi / r * gxth +
                          cosPhi / r / sinTheta * gxph;
  ...
  void L2P(real_type& potential, vect3d& field, point_type x,
           coeff_type coeff[], point_type center) {
    vect_type diff     = x - center;
    real_type r        = diff.Length();
    real_type rxy      = sqrt(diff.x * diff.x + diff.y * diff.y);
