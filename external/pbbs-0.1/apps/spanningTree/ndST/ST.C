...
pair<int*, int> st(edgeArray EA) {
  ...
  parallel_doall_end vindex* hooks = newA(vindex, n);
  //  parallel_for (int i=0; i < n; i++) hooks[i] = INT_MAX;
  parallel_doall(int, i, 0, n) { hooks[i] = INT_MAX; }
  parallel_doall_end edge* st = newA(edge, m);
  ...
  parallel_doall_end _seq<int> stIdx =
      sequence::filter((int*)hooks, n, notMax());
