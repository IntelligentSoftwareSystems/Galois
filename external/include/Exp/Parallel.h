...
struct Work {
  IterTy begin, end;
  FunctionTy function;
  Work(const IterTy& b, const IterTy& e, const FunctionTy& f)
      : begin(b), end(e), function(f) {}
...
#ifdef EXP_DOALL_GALOIS
struct Init {
  Init() { galois::setActiveThreads(Exp::getNumThreads()); }
};
#endif
...
#if defined(EXP_DOALL_CILK) || defined(EXP_DOALL_CILKP) ||                     \
    defined(EXP_DOALL_OPENMP) || defined(EXP_DOALL_OPENMP_RUNTIME) ||          \
    defined(EXP_DOALL_PTHREAD_OPENMP)
struct Init {};
#endif
...
#define parallel_doall_1(type, index, begin, end)                              \
  _Pragma("omp parallel for schedule (static,1)") for (type index = begin;     \
                                                       index < end; ++index)
#define parallel_doall_obj(type, index, begin, end, obj)                       \
  _Pragma("omp parallel for") for (type index = begin; index < end; ++index)
#define parallel_doall_obj_1(type, index, begin, end, obj)                     \
...
#define parallel_doall_obj(type, index, begin, end, obj)                       \
  if (true) {                                                                  \
    _Pragma("omp parallel for") for (type index = begin; index < end;          \
                                     ++index) {                                \
      obj(index);                                                              \
    }                                                                          \
  } else                                                                       \
...
#define cilk_sync
#define parallel_doall(type, index, begin, end)                                \
  _Pragma("omp parallel for schedule (runtime)") for (type index = begin;      \
                                                      index < end; ++index)
#define parallel_doall_1(type, index, begin, end)                              \
  _Pragma("omp parallel for schedule (static,1)") for (type index = begin;     \
                                                       index < end; ++index)
...
#define parallel_doall_obj(type, index, begin, end, obj)                       \
  if (true) {                                                                  \
    Exp::do_all(boost::counting_iterator<type>(begin),                         \
                boost::counting_iterator<type>(end), obj);                     \
  } else                                                                       \
    for (type index = begin; index < end; ++index)
#define parallel_doall_obj_1(type, index, begin, end, obj)                     \
  if (true) {                                                                  \
