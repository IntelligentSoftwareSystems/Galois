 ...
 */
init_root(ProcessId) unsigned int ProcessId;
{
  int i;
...
int Log_base_2(number) int number;
{
  int cumulative;
  int out;
...
pickshell(vec, rad) real vec[]; /* coordinate vector chosen */
...
void stepsystem(ProcessId) unsigned int ProcessId;
{
  int i;
  real Cavg;
  bodyptr p, *pp;
  ...
  Local[ProcessId].workMax =
      (int)(Cavg * (ProcessId + 1) + (ProcessId == (NPROC - 1)));
...
void ComputeForces(ProcessId) unsigned int ProcessId;
{
  bodyptr p, *pp;
  vector acc1, dacc, dvel, vel1, dpos;
...
find_my_initial_bodies(btab, nbody, ProcessId) bodyptr btab;
int nbody;
unsigned int ProcessId;
{
  int Myindex;
...
find_my_bodies(mycell, work, direction, ProcessId) nodeptr mycell;
int work;
int direction;
unsigned ProcessId;
{
    ...
    for (i = 0; i < l->num_bodies; i++) {
      if (work >= Local[ProcessId].workMin - .1) {
        if ((Local[ProcessId].mynbody + 2) > maxmybody) {
          error("find_my_bodies: Processor %d needs more than %d bodies; "
                "increase fleaves\n",
                ProcessId, maxmybody);
        }
...
Housekeep(ProcessId) unsigned ProcessId;
{
