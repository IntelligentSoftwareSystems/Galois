...
hackgrav(p, ProcessId) bodyptr p;
unsigned ProcessId;
...
gravsub(p, ProcessId,
        level) register nodeptr p; /* body or cell to interact with     */
unsigned ProcessId;
int level;
{
  double sqrt();
...
hackwalk(sub, ProcessId) proced sub; /* routine to do calculation */
unsigned ProcessId;
...
walksub(n, dsq, ProcessId) nodeptr n; /* pointer into body-tree    */
...
bool subdivp(p, dsq,
             ProcessId) register nodeptr p; /* body/cell to be tested    */
