...
maketree(ProcessId) unsigned ProcessId;
{
  bodyptr p, *pp;
...
cellptr InitCell(parent, ProcessId) cellptr parent;
unsigned ProcessId;
{
  cellptr c;
  int i, Mycell;
...
leafptr InitLeaf(parent, ProcessId) cellptr parent;
unsigned ProcessId;
{
  leafptr l;
  int i, Mycell;
...
printtree(n) nodeptr n;
{
  ...
  case LEAF:
    ...
    for (k = 0; k < l->num_bodies; k++) {
      p = Bodyp(l)[k];
      printf("Body #%2d: Num = %2d, Level = %o, ", p - bodytab, k, Level(p));
      PRTV("Pos", Pos(p));
      printf("\n");
    }
    break;
...
nodeptr loadtree(p, root, ProcessId) bodyptr p; /* body to load into tree */
cellptr root;
unsigned ProcessId;
{
...
bool intcoord(xp, rp) int xp[NDIM]; /* integerized coordinate vector [0,IMAX) */
...
int subindex(x, l) int x[NDIM]; /* integerized coordinates of particle */
...
cellptr SubdivideLeaf(le, parent, l, ProcessId) leafptr le;
cellptr parent;
unsigned int l;
unsigned int ProcessId;
{
...
cellptr makecell(ProcessId) unsigned ProcessId;
{
  cellptr c;
  int i, Mycell;
...
leafptr makeleaf(ProcessId) unsigned ProcessId;
{
  leafptr le;
  int i, Myleaf;
