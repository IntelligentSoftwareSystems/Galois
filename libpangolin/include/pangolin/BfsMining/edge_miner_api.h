...
public:
	...
	static inline bool toAdd(unsigned n, const EmbeddingTy &emb, unsigned pos, VertexId src, VertexId dst, BYTE& existed, const VertexSet& vertex_set) {
...
protected:
	...
	static inline int compare(std::pair<VertexId, VertexId>& oneEdge, std::pair<VertexId, VertexId>& otherEdge) {
		swap(oneEdge);
		swap(otherEdge);
