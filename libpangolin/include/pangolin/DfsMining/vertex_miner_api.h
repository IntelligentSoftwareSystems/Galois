...
public:
	...
	static inline bool toExtend(unsigned, unsigned, const std::vector<VertexId> *) {
		return true;
	}
	...
	static inline bool toAdd(unsigned level, unsigned, VertexId vid, 
	...
	static inline unsigned getPattern(unsigned, unsigned, VertexId, 
		VertexId, BYTE, unsigned, BYTE, const std::vector<VertexId> *) {
		return 0;
	}
	...
	static inline void local_reduction(unsigned, int, Ulong &counter) { counter ++; } 

	static inline void update(unsigned, unsigned, unsigned, BYTE, unsigned) { }
...
//*/
	...
	static inline bool is_connected(unsigned level, VertexId, unsigned element_id, BYTE ccode) {
		if (level == 1) return (ccode == element_id + 1) || (ccode == 3);
		else if (level == 2) {
			return (ccode & (1 << element_id));
		} else return false;
	}
	static inline unsigned get_pattern_id(unsigned level, VertexId, BYTE ccode, unsigned previous_pid = 0, unsigned src_idx = 0) {
		unsigned pid = 0;
