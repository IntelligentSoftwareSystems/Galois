...
protected:
	...
	static inline unsigned find_motif_pattern_id(unsigned n, Graph &g, unsigned idx, VertexId dst, const EmbeddingTy& emb, BYTE *pre_pid, unsigned pos = 0) {
		unsigned pid = 0;
		if (n == 2) { // count 3-motifs
			pid = 1; // 3-chain
			if (idx == 0) {
				if (is_connected(g, emb.get_vertex(1), dst)) pid = 0; // triangle
				else if (use_wedge) pre_pid[pos] = 1; // wedge; used for 4-motif
			}
			...
			if (pid == 0) { // extending a triangle
				for (unsigned j = idx+1; j < n; j ++)
					if (is_connected(g, emb.get_vertex(j), dst)) num_edges ++;
				...
				if (num_edges == 1) {
					...
					unsigned center = 1;
					if (use_wedge) {
						if (pre_pid[pos]) center = 0;
					} else center = is_connected(g, emb.get_vertex(1), emb.get_vertex(2)) ? 1 : 0;
