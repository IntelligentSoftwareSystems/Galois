...
public:
	...
	inline void extend_vertex_multi(unsigned level, size_t chunk_begin, size_t chunk_end) {
		...
		galois::do_all(galois::iterate(begin, end), [&](const size_t& pos) {
			...
			if (n < this->max_size-1) num_new_emb[pos-begin] = 0;
			if (!is_single && n == 3 && this->max_size == 4)
				emb.set_pid(this->emb_list.get_pid(pos));
			for (unsigned i = 0; i < n; ++i) {
				if (!API::toExtend(n, emb, i)) continue;
				...
				for (auto e : this->graph.edges(src)) {
					...
					if (API::toAdd(n, this->graph, emb, i, dst)) {
						if (n < this->max_size-1) {
							...
							if (is_single) total_num += 1;
							else {
								//unsigned pid  = getPattern(n, i, dst, emb, pos);
								if (n < 4) {
									unsigned pid = this->find_motif_pattern_id(n, i, dst, emb, pos);
									accumulators[pid] += 1;
								} else quick_reduce(n, i, dst, emb, qp_lmap);
							}
						}
					}
				}
				...
				for (auto e : this->graph.edges(src)) {
					...
					if (API::toAdd(n, this->graph, emb, i, dst)) {
						...
						if (!is_single && n == 2 && this->max_size == 4)
							this->emb_list.set_pid(start, this->find_motif_pattern_id(n, i, dst, emb, start));
	...
	inline void canonical_reduce() {
		...
		galois::do_all(galois::iterate(qp_map), [&](auto& element) {
			...
			cg.clean();
		}, galois::chunk_size<CHUNK_SIZE>(), galois::loopname("CanonicalReduction"));
		qp_map.clear();
	}
	...
	void tc_solver() { // edge parallel
		galois::do_all(galois::iterate((size_t)0, this->emb_list.size()), [&](const size_t& id) {
	...
	void solver() {
		...
		for (size_t cid = 0; cid < num_blocks; cid ++) {
			...
			std::cout << "Processing the " << cid << " chunk (" 
				<< cur_size << " edges) of " << num_blocks << " blocks\n";
			unsigned level = 1;
			while (1) {
...
protected:
	...
	inline unsigned find_motif_pattern_id(unsigned n, unsigned idx, VertexId dst, const EmbeddingTy& emb, unsigned pos = 0) {
		...
		if (n == 2) { // count 3-motifs
			pid = 1; // 3-chain
			if (idx == 0) {
				if (this->is_connected(emb.get_vertex(1), dst)) pid = 0; // triangle
			...
			if (pid == 0) { // extending a triangle
				for (unsigned j = idx+1; j < n; j ++)
					if (this->is_connected(emb.get_vertex(j), dst)) num_edges ++;
				...
				if (num_edges == 1) {
					...
					} else center = this->is_connected(emb.get_vertex(1), emb.get_vertex(2)) ? 1 : 0;
