...
template <typename EmbTy, typename EleTy> std::ostream& operator<<(std::ostream& strm, const QuickPattern<EmbTy, EleTy>& qp);

template <typename EmbTy, typename EleTy>
class QuickPattern {
friend std::ostream & operator<< <>(std::ostream & strm, const QuickPattern<EmbTy, EleTy>& qp);
public:
	QuickPattern() { }
