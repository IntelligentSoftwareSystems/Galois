...
template <typename EmbTy, typename EleTy>
QuickPattern<EmbTy,EleTy>::QuickPattern(unsigned n, std::vector<bool> connected) {
	cg_id = 0;
...
template <typename EmbTy, typename EleTy>
void QuickPattern<EmbTy,EleTy>::findAutomorphisms(VertexPositionEquivalences &eq_sets) {
...
template <typename EmbTy, typename EleTy>
std::ostream & operator<<(std::ostream & strm, const QuickPattern<EmbTy, EleTy>& qp) {
	if(qp.get_size() == 0) {
		strm << "(empty)";
		return strm;
	}
...
template class QuickPattern<EdgeInducedEmbedding<StructuralElement>, StructuralElement>; //Motif
template class QuickPattern<EdgeInducedEmbedding<LabeledElement>,LabeledElement>; // FSM
