#include "pangolin/DfsMining/edgelist.h"
#include "pangolin/scan.h"
#include "pangolin/core.h"
...
void EdgeList::ord_core() {
  ...
  // for (size_t i = 0; i < num_edges; i ++) {
  galois::do_all(
      galois::iterate((size_t)0, num_edges),
      [&](const auto& i) {
        // d0[(*this)[i].src]++;
        // d0[(*this)[i].dst]++;
  ...
  */
  ...
  galois::gstl::Vector<VertexId> adj0(2 *
                                      num_edges); // TODO: use 64-bit for GSH
  // for (size_t i = 0; i < num_edges; i ++) {
...
unsigned EdgeList::generate_graph(Graph& g) {
  ...
  galois::do_all(
      ...
      galois::loopname("ComputeDegreesFromEdgelist"));
  /*
