...
struct SSSP {
  ...
  void static go(Graph& _graph){
    ...
    do{
      ...
      #endif
      galois::do_all(_graph.allNodesRange().begin(), _graph.allNodesRange().end(), SSSP { &_graph }, galois::loopname("SSSP"), galois::numrun(_graph.get_run_identifier()), galois::write_set("reduce", "this->graph", "struct NodeData &", "struct NodeData &" , "dist_current", "unsigned int" , "min",  ""));
