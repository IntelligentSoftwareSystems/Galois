...
#endif
galois::do_all(boost::make_counting_iterator(__begin), boost::make_counting_iterator(__end), FirstItr_BFS{&_graph}, galois::loopname("BFS"), galois::numrun(_graph.get_run_identifier()), galois::write_set("reduce", "this->graph", "struct NodeData &", "struct NodeData &" , "dist_current", "unsigned int" , "min",  ""));
_graph.reduce<Reduce_0>("FirstItr_BFS");

if(_graph.is_vertex_cut()) {
	_graph.broadcast<Broadcast_0>("FirstItr_BFS");
}
...
struct BFS {
  ...
  void static go(Graph& _graph){
    ...
    #endif
    galois::do_all(_graph.allNodesRange().begin(), _graph.allNodesRange().end(), BFS (&_graph), galois::loopname("BFS"), galois::write_set("reduce", "this->graph", "struct NodeData &", "struct NodeData &" , "dist_current", "unsigned int" , "min",  ""), galois::numrun(_graph.get_run_identifier()));
