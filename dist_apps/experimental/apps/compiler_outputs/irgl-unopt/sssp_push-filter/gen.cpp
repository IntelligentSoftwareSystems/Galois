...
struct InitializeGraph {
  ...
  void static go(Graph& _graph) {
    ...
    #endif
    galois::do_all(_graph.allNodesRange().begin(), _graph.allNodesRange().end(), InitializeGraph {src_node, infinity, &_graph}, galois::loopname("InitializeGraph"), galois::numrun(_graph.get_run_identifier()), galois::write_set("broadcast", "this->graph", "struct NodeData &", "struct NodeData &", "dist_current" , "unsigned int" , "set",  ""));
...
#ifdef __GALOIS_HET_CUDA__
	if (personality == GPU_CUDA) {
		std::string impl_str("CUDA_DO_ALL_IMPL_FirstItr_SSSP_" + (_graph.get_run_identifier()));
		galois::StatTimer StatTimer_cuda(impl_str.c_str());
		StatTimer_cuda.start();
...
#endif
galois::do_all(boost::make_counting_iterator(__begin), boost::make_counting_iterator(__end), FirstItr_SSSP{&_graph}, galois::loopname("SSSP"), galois::numrun(_graph.get_run_identifier()), galois::write_set("reduce", "this->graph", "struct NodeData &", "struct NodeData &" , "dist_current", "unsigned int" , "min",  ""));
_graph.reduce<Reduce_0>("FirstItr_SSSP");

if(_graph.is_vertex_cut()) {
	_graph.broadcast<Broadcast_0>("FirstItr_SSSP");
}
...
struct SSSP {
  ...
  void static go(Graph& _graph){
    ...
    #endif
    galois::do_all(_graph.allNodesRange().begin(), _graph.allNodesRange().end(), SSSP (&_graph), galois::loopname("SSSP"), galois::write_set("reduce", "this->graph", "struct NodeData &", "struct NodeData &" , "dist_current", "unsigned int" , "min",  ""), galois::numrun(_graph.get_run_identifier()));
    _graph.reduce<Reduce_0>("SSSP");
    
    if(_graph.is_vertex_cut()) {
    	_graph.broadcast<Broadcast_0>("SSSP");
    }
