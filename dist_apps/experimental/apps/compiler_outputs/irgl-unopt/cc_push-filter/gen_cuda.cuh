...
struct CUDA_Context {
	...
	Shared<unsigned int> *mirror_comp_old; // per host
};
...
void batch_get_mirror_node_comp_old_cuda(struct CUDA_Context *ctx, unsigned from_id, unsigned int *v) {
	...
	dim3 threads;
	kernel_sizing(ctx->gg, blocks, threads);
	batch_get_subset<unsigned int> <<<blocks, threads>>>(ctx->num_mirror_nodes[from_id], ctx->mirror_nodes[from_id].gpu_rd_ptr(), ctx->mirror_comp_old[from_id].gpu_wr_ptr(true), ctx->comp_old.gpu_rd_ptr());
	check_cuda_kernel;
	memcpy(v, ctx->mirror_comp_old[from_id].cpu_rd_ptr(), sizeof(unsigned int) * ctx->num_mirror_nodes[from_id]);
}
...
void batch_get_reset_node_comp_old_cuda(struct CUDA_Context *ctx, unsigned from_id, unsigned int *v, unsigned int i) {
	...
	dim3 threads;
	kernel_sizing(ctx->gg, blocks, threads);
	batch_get_reset_subset<unsigned int> <<<blocks, threads>>>(ctx->num_mirror_nodes[from_id], ctx->mirror_nodes[from_id].gpu_rd_ptr(), ctx->mirror_comp_old[from_id].gpu_wr_ptr(true), ctx->comp_old.gpu_rd_ptr(), i);
	check_cuda_kernel;
	memcpy(v, ctx->mirror_comp_old[from_id].cpu_rd_ptr(), sizeof(unsigned int) * ctx->num_mirror_nodes[from_id]);
}
...
void batch_set_node_comp_old_cuda(struct CUDA_Context *ctx, unsigned from_id, unsigned int *v) {
	...
	dim3 threads;
	kernel_sizing(ctx->gg, blocks, threads);
	memcpy(ctx->mirror_comp_old[from_id].cpu_wr_ptr(true), v, sizeof(unsigned int) * ctx->num_mirror_nodes[from_id]);
	batch_set_subset<unsigned int> <<<blocks, threads>>>(ctx->num_mirror_nodes[from_id], ctx->mirror_nodes[from_id].gpu_rd_ptr(), ctx->mirror_comp_old[from_id].gpu_rd_ptr(), ctx->comp_old.gpu_wr_ptr());
	check_cuda_kernel;
}
...
void load_graph_CUDA(struct CUDA_Context *ctx, MarshalGraph &g, unsigned num_hosts) {
	...
	ctx->mirror_comp_old = (Shared<unsigned int> *) calloc(num_hosts, sizeof(Shared<unsigned int>));
	for(uint32_t h = 0; h < num_hosts; ++h){
		if (ctx->num_mirror_nodes[h] > 0) {
			...
			ctx->mirror_comp_old[h].alloc(ctx->num_mirror_nodes[h]);
		}
	}
