...
struct ResetGraph {
  ...
  void static go(Graph& _graph) {
    	...
    	struct Reduce_0 {
    		...
    		#ifdef __GALOIS_HET_CUDA__
    			if (personality == GPU_CUDA) { batch_get_mirror_node_residual_cuda(cuda_ctx, from_id, b, o, y, s, data_mode); return true; }
    			assert (personality == CPU);
    		#endif
    			return false;
    		}
    		...
    		#ifdef __GALOIS_HET_CUDA__
    			if (personality == GPU_CUDA) { batch_get_mirror_node_residual_cuda(cuda_ctx, from_id, y); return true; }
    			assert (personality == CPU);
    		#endif
    			return false;
    		}
...
struct PageRank {
  ...
  void static go(Graph& _graph) {
    ...
    do{
      ...
      #endif
      galois::do_all(_graph.allNodesRange().begin(), _graph.allNodesRange().end(), PageRank { tolerance, alpha, &_graph }, galois::loopname("PageRank"), galois::numrun(_graph.get_run_identifier()), galois::write_set("reduce", "this->graph", "struct PR_NodeData &", "struct PR_NodeData &" , "residual", "float" , "add",  "0"));
