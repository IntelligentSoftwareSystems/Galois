...
struct BFS {
  ...
  void static go(Graph& _graph){
    ...
    do{
      ...
      #endif
      galois::do_all(_graph.allNodesRange().begin(), _graph.allNodesRange().end(), BFS { &_graph }, galois::loopname("BFS"), galois::numrun(_graph.get_run_identifier()), galois::write_set("broadcast", "this->graph", "struct NodeData &", "struct NodeData &", "dist_current" , "unsigned int" , "min",  ""));
      if(_graph.is_vertex_cut()) {
