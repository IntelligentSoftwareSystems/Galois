...
//   typedef galois::PODResizeableArray<galois::CopyableAtomic<Block>> vector_type;
//   typedef std::size_t size_type;
//   typedef typename vector_type::size_type count_type;
...
public:
  ...
  //   void test_set_indicator(size_t pos, bool val);
  //   bool nposInd();
  // #endif
  // size_t right_most_bit(uint64_t w) const;
  // size_t left_most_bit(uint64_t w) const;
  // size_t find_from_block(size_t first, bool forward) const;
  ...
  PRBCBitSet() : num_bits(numSourcesPerRound) {
    #ifdef USE_INDICATOR
      indicator = npos;
    #endif
  }
   ...
   */
  void reset() {
    std::fill(bitvec.begin(), bitvec.end(), uint64_t(0));
  }
