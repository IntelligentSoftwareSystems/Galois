...
struct PageRank_delta {
  ...
  void static go(Graph& _graph) {
    ...
    if (personality == GPU_CUDA) {
      ...
      StatTimer_cuda.start();
#if __OPT_VERSION__ <= 4
      PageRank_delta_cuda(*allNodes.begin(), *allNodes.end(), alpha, tolerance,
                          cuda_ctx);
#endif
#if __OPT_VERSION__ == 5
      PageRank_delta_cuda(*nodesWithEdges.begin(), *nodesWithEdges.end(), alpha,
                          tolerance, cuda_ctx);
#endif
  ...
  void static go(Graph& _graph, galois::DGAccumulator<unsigned int>& dga) {
    ...
    do {
      ...
      if (personality == GPU_CUDA) {
        ...
        StatTimer_cuda.start();
        int __retval = 0;
        PageRank_cuda(*nodesWithEdges.begin(), *nodesWithEdges.end(), __retval,
                      cuda_ctx);
        dga += __retval;
        StatTimer_cuda.stop();
      } else if (personality == CPU)
      ...
      {
        galois::do_all(
            nodesWithEdges, PageRank{&_graph, dga}, galois::no_stats(),
...
#endif
    {
      ...
      galois::do_all(
          ...
          PageRankSanity(tolerance, &_graph, DGA_max, DGA_min, DGA_sum,
                         ...
                         DGA_max_residual, DGA_min_residual, max_value,
                         min_value, max_residual, min_residual),
          galois::loopname("PageRankSanity"), galois::no_stats());
    ...
    auto& net = galois::runtime::getSystemNetworkInterface();
    {
      ...
      for (auto run = 0; run < numRuns; ++run) {
        ...
        PageRankSanity::go(*hg, DGA_max, DGA_min, DGA_sum, DGA_sum_residual,
                           ...
                           DGA_min_residual, max_value, min_value, max_residual,
                           min_residual);
