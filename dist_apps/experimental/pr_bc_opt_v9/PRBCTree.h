...
/*** RoundUpdate **************************************************************/
  ...
  void prepForBackPhase() {
    ...
    if (!distanceTree.empty()) {
      BitSet& curSet = (distanceTree.rbegin())->second;
        curSet.flip();
    }
    else
...
/*** BackFindMessageToSend *****************************************************/
  ...
  uint32_t backGetIndexToSend(const uint32_t roundNumber, 
    ...
    uint32_t distance = lastRound - roundNumber - (numSentSources - 1);
    if (distance == 0) {
      zeroReached = true;
      return infinity;
    }

    auto curKey = distanceTree.find(distance);

    if (curKey == distanceTree.end()){
    ...
    if (!curSet.nposInd()) {
        ...
        indexToReturn = curSet.getIndicator();
        numSentSources--;
    }
