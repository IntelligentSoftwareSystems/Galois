...
uint32_t min_node_current_degree_cuda(struct CUDA_Context *ctx, unsigned LID, uint32_t v) {
	uint32_t *current_degree = ctx->current_degree.data.cpu_wr_ptr();
  uint32_t old = current_degree[LID];
