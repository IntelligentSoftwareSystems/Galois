...
struct InitializeGraph2 {
  ...
  void static go(Graph& _graph) {
    ...
    if (personality == GPU_CUDA) {
      std::string impl_str("InitializeGraph2_" + (_graph.get_run_identifier()));
...
#ifdef __GALOIS_HET_CUDA__
    if (personality == GPU_CUDA) {
      std::string impl_str("InitializeGraph1_" + (_graph.get_run_identifier()));
      galois::StatTimer StatTimer_cuda(impl_str.c_str(), REGION_NAME);
      StatTimer_cuda.start();
      ...
      galois::do_all(
          ...
          galois::loopname(
              _graph.get_run_identifier("InitializeGraph1").c_str()));
...
#endif
      galois::do_all(
          galois::iterate(nodesWithEdges), InitializeGraph2_recovery{&_graph},
          galois::steal(), galois::no_stats(),
          galois::loopname(
...
struct InitializeGraph1_crashed {
  ...
  void static go(Graph& _graph) {
    ...
    if (personality == GPU_CUDA) {
      ...
      galois::do_all(
          galois::iterate(allNodes.begin(), allNodes.end()),
          InitializeGraph1_crashed{&_graph}, galois::no_stats(),
          galois::loopname(
...
struct InitializeGraph1_Healthy {
  ...
  void static go(Graph& _graph) {
    ...
    if (personality == GPU_CUDA) {
      ...
      galois::StatTimer StatTimer_cuda(impl_str.c_str(), REGION_NAME);
      StatTimer_cuda.start();
      InitializeGraph1_cuda(*(allNodes.begin()), *(allNodes.end()), cuda_ctx);
      StatTimer_cuda.stop();
    } else if (personality == CPU)
#endif
      galois::do_all(
          galois::iterate(allNodes.begin(), allNodes.end()),
          InitializeGraph1_Healthy{&_graph}, galois::no_stats(),
          galois::loopname(
...
struct recovery {
  ...
  void static go(Graph& _graph) {
    ...
    _graph.sync<writeSource, readSource, Reduce_min_flag, Broadcast_flag>(
        "RECOVERY_FLAG");

    // TODO: nodes with edges
    const auto& nodesWithEdges = _graph.allNodesWithEdgesRange();
    galois::do_all(
        galois::iterate(nodesWithEdges), recovery{&_graph}, galois::no_stats(),
  ...
  void static go(Graph& _graph) {
    ...
    if (personality == GPU_CUDA) {
      ...
      galois::do_all(
          ...
          KCoreStep2_recovery{&_graph}, galois::no_stats(),
          galois::loopname(_graph.get_run_identifier("RECOVERY").c_str()));
  }
  ...
  void static go(Graph& _graph, galois::DGAccumulator<unsigned int>& dga) {
    ...
    do {
      ...
      if (enableFT && (iterations == crashIteration)) {
        crashSite<recovery, InitializeGraph1_crashed, InitializeGraph1_Healthy>(
            _graph);
