...
struct ResetGraph_crashed {
  ...
  void static go(Graph& _graph) {
    ...
    galois::do_all(
        galois::iterate(allNodes.begin(), allNodes.end()),
        ResetGraph_crashed{alpha, &_graph}, galois::no_stats(),
        galois::loopname(
...
struct ResetGraph_healthy {
  ...
  void static go(Graph& _graph) {
    ...
    galois::do_all(
        galois::iterate(allNodes.begin(), allNodes.end()),
        ResetGraph_healthy{&_graph}, galois::no_stats(),
        galois::loopname(
...
struct recovery {
  ...
  void static go(Graph& _graph) {
    _graph.sync<writeAny, readAny, Reduce_max_value, Broadcast_value>(
        "RECOVERY_VALUE");
    ...
    galois::do_all(
        ...
        galois::iterate(allNodes.begin(), allNodes.end()), recovery{&_graph},
        galois::no_stats(),
    ...
    do {
      ...
      if (enableFT && (_num_iterations == crashIteration)) {
        ...
        if (recoveryScheme == CP) {
          ...
          _graph.reset_mirrorField<Reduce_add_residual>();
          galois::do_all(
              galois::iterate(nodesWithEdges), PageRank{&_graph},
              galois::steal(), galois::no_stats(),
              galois::loopname(
