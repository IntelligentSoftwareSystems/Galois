...
struct ResetGraphCrashed {
  void static go(Graph& _graph) {
    ...
    galois::do_all(
        ...
        ResetGraph{alpha, &_graph}, galois::no_stats(),
        galois::loopname(
            _graph.get_run_identifier("ResetGraph_crashed").c_str()));
  }
};
...
struct ResetGraphHealthy {
  ...
  void static go(Graph& _graph) {
    ...
    galois::do_all(
        galois::iterate(allNodes.begin(), allNodes.end()),
        ResetGraphHealthy{&_graph}, galois::no_stats(),
        galois::loopname(
            _graph.get_run_identifier("ResetGraph_healthy").c_str()));
  }
...
struct InitializeGraph_crashed {
  ...
  void static go(Graph& _graph) {
    ...
    // doing a local do all because we are looping over edges
    galois::do_all(
        galois::iterate(nodesWithEdges), InitializeGraph_crashed{&_graph},
        galois::steal(), galois::no_stats(),
        galois::loopname(
...
struct InitializeGraph_healthy {
  ...
  void static go(Graph& _graph) {
    ...
    // doing a local do all because we are looping over edges
    galois::do_all(
        galois::iterate(nodesWithEdges), InitializeGraph_healthy{&_graph},
        galois::steal(), galois::no_stats(),
        galois::loopname(
...
struct RecoveryOperator {
  ...
  void static go(Graph& _graph) {
    ...
    _graph.sync<writeSource, readAny, Reduce_max_value, Broadcast_value>(
        "PageRank-afterCrash");
  }
};
...
struct PageRankSum {
  ...
  void static go(Graph& _graph, galois::DGAccumulator<unsigned int>& dga) {
    ...
    galois::do_all(
        galois::iterate(allNodes.begin(), allNodes.end()),
        PageRankSum{alpha, tolerance, &_graph, dga}, galois::no_stats(),
...
struct PageRank {
  ...
  void static go(Graph& _graph, galois::DGAccumulator<unsigned int>& dga) {
    ...
    do {
      ...
      galois::do_all(
          galois::iterate(nodesWithEdges), PageRank{alpha, &_graph},
          galois::steal(), galois::no_stats(),
          galois::loopname(_graph.get_run_identifier("PageRank").c_str()));
      ...
      if (enableFT && (_num_iterations == crashIteration)) {
        crashSite<RecoveryOperator, InitializeGraph_crashed,
...
struct PageRankSanity {
  ...
  PageRankSanity(cll::opt<float>& _local_tolerance, Graph* _graph,
      ...
      : local_tolerance(_local_tolerance), graph(_graph),
        DGAccumulator_sum(_DGAccumulator_sum), max_value(_max_value),
...
constexpr static const char* const desc =
    "PageRank Pull Topological version on "
    "Distributed Galois.";
