...
static cll::opt<int> num_nodes(
    ...
    cll::init(-1));
static cll::opt<double> cuda_wl_dup_factor(
    "cuda_wl_dup_factor",
    cll::desc("Upper bound for duplication factor in CUDA worklist (1): "
              "worklist size = factor * graph.nnodes"),
    cll::init(1));
#endif
...
typedef galois::graphs::DistGraphEdgeCut<NodeData, unsigned int> Graph_edgeCut;
typedef galois::graphs::DistGraphHybridCut<NodeData, unsigned int>
    Graph_vertexCut;
...
struct SSSP {
  ...
  void static go(Graph& _graph) {
    ...
    galois::for_each(src_node, SSSP(&_graph), galois::workList_version(),
