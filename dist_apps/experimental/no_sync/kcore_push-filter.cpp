...
static const char* const name =
    "KCore - Distributed Heterogeneous Push Filter.";
...
struct InitializeGraph2 {
  ...
  void static go(Graph& _graph) {
    ...
    galois::do_all(
        nodesWithEdges, InitializeGraph2{&_graph}, galois::no_stats(),
...
struct InitializeGraph1 {
  ...
  void static go(Graph& _graph) {
    ...
    galois::do_all(allNodes.begin(), allNodes.end(), InitializeGraph1{&_graph},
                   galois::no_stats(),
                   galois::loopname(
...
struct KCoreStep1 {
  ...
  void static go(Graph& _graph, galois::DGAccumulator<unsigned int>& dga) {
    ...
    do {
      ...
      galois::do_all(
          allNodes, KCoreStep1{k_core_num, &_graph, dga}, galois::no_stats(),
  ...
  try {
...

    {
      ...
      for (auto run = 0; run < numRuns; ++run) {
        std::cout << "[" << net.ID << "] KCoreStep1::go run " << run
                  << " called\n";
