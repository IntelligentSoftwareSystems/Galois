...
void batch_set_node_delta_cuda(struct CUDA_Context* ctx, unsigned from_id,
                               uint64_t* bitset_comm, unsigned int* offsets,
                               float* v, size_t v_size, DataCommMode data_mode);
...
void PageRankSanity_nodesWithEdges_cuda(
    ...
    float& min_residual, float& min_value, float local_tolerance,
    struct CUDA_Context* ctx);
void PageRank_allNodes_cuda(unsigned int& DGAccumulator_accum,
                            struct CUDA_Context* ctx);
...
void PageRank_delta_nodesWithEdges_cuda(const float& local_alpha,
                                        float local_tolerance,
                                        struct CUDA_Context* ctx);
void PageRank_masterNodes_cuda(unsigned int& DGAccumulator_accum,
                               struct CUDA_Context* ctx);
void PageRank_nodesWithEdges_cuda(unsigned int& DGAccumulator_accum,
                                  struct CUDA_Context* ctx);
