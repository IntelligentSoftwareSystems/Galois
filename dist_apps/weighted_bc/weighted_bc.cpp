...
//#define __USE_BFS__ // also defined in weighted_bc_cuda.h

#ifdef __GALOIS_HET_CUDA__
#include "weighted_bc_cuda.h"
  ...
  void static go(Graph& _graph, galois::DGAccumulator<uint32_t>& dga) {
    ...
    do {
      ...
      if (personality == GPU_CUDA) {
        ...
        StatTimer_cuda.start();
        uint32_t __retval = 0;
        NumShortestPaths_nodesWithEdges_cuda(__retval, infinity,
  ...
  void static go(Graph& _graph, galois::DGAccumulator<uint32_t>& dga) {
    ...
    do {
      ...
      if (personality == GPU_CUDA) {
        ...
        StatTimer_cuda.start();
        uint32_t __retval = 0;
        DependencyPropagation_nodesWithEdges_cuda(__retval, infinity,
                                                  current_src_node, cuda_ctx);
        dga += __retval;
        StatTimer_cuda.stop();
      } else if (personality == CPU)
