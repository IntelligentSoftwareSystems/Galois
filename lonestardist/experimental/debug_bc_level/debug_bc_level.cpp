    ...
    if (src_data.current_length == r) {
      ...
      for (auto current_edge : graph->edges(src)) {
        ...
        if (dest_to_find == dst_data.current_length.load()) {
          double contrib = (1.0 + dst_data.dependency) /
...
struct Sanity {
  ...
  galois::DGReduceMax<double>& DGAccumulator_max;
  galois::DGReduceMin<double>& DGAccumulator_min;
  galois::DGAccumulator<double>& DGAccumulator_sum;

  Sanity(Graph* _graph, galois::DGReduceMax<double>& _DGAccumulator_max,
         galois::DGReduceMin<double>& _DGAccumulator_min,
  ...
  void static go(Graph& _graph, galois::DGReduceMax<double>& DGA_max,
                 galois::DGReduceMin<double>& DGA_min,
    ...
    double max_bc = DGA_max.reduce();
    double min_bc = DGA_min.reduce();
...
int main(int argc, char** argv) {
  ...
  galois::DGReduceMax<double> dga_max;
  galois::DGReduceMin<double> dga_min;
  galois::DGAccumulator<double> dga_sum;
