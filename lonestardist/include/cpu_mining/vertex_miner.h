...
public:
				...
				for (unsigned i = 0; i < n; ++i) {
					...
					for (EdgeIdx e = e0; e != e1; e ++) {
						...
						if (!is_vertexInduced_automorphism(n, emb, i, src, dst)) {
							...
							emb_list.set_idx(n, start, pos);
							emb_list.set_vid(n, start++, dst);
						}
					}
				}
			},
	...
	inline void extend_vertex_clique(unsigned level, EmbeddingList& emb_list) {
		...
		galois::do_all(galois::iterate((size_t)0, cur_size),
			[&](const size_t& pos) {
				...
				num_new_emb[pos] = 0;
				EdgeIdx e0 = graph->edge_begin(vid);
				EdgeIdx e1 = graph->edge_end(vid);
				for (EdgeIdx e = e0; e != e1; e ++) {
					auto dst = graph->getEdgeDst(e);
					#ifdef USE_DAG
