...
public:
	...
	ValueT get_label(ValueT n) { return labels_[n]; }
	ValueT getData(ValueT n) { return labels_[n]; }
	...
	IndexT out_degree(IndexT n) const { return rowptr_[n+1] - rowptr_[n]; }
	...
	void read_gr(const char *filename) {
		num_edges_ = el.size();
		labels_.resize(num_vertices_);
		for (int i = 0; i < num_vertices_; i ++) { labels_[i] = 1; }
		MakeGraphFromEL();
	}
	void print_graph() {
