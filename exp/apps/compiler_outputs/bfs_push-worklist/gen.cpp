...
struct BFS {
  ...
  void static go(Graph& _graph){
    ...
    typedef dChunkedFIFO<64> dChunk;
      ...
      #endif
      Galois::for_each(_graph.begin(), _graph.end(), BFS (&_graph), Galois::loopname("bfs"), Galois::workList_version(), Galois::write_set("sync_push", "this->graph", "struct NodeData &", "struct NodeData &" , "dist_current", "unsigned int" , "{ Galois::atomicMin(node.dist_current, y);}",  "{node.dist_current = std::numeric_limits<unsigned int>::max()/4; }"), Galois::write_set("sync_pull", "this->graph", "struct NodeData &", "struct NodeData &", "dist_current" , "unsigned int"), Get_info_functor<Graph>(_graph));
...
int main(int argc, char** argv) {
  try {
   ...
   T_total.stop();
    ...
    std::cout << "[" << net.ID << "]" << " Total Time : " << T_total.get() << " hGraph : " << T_hGraph_init.get() << " Init : " << T_init.get() << " bfs1 : " << T_bfs1.get() << " bfs2 : " << T_bfs2.get() << " bfs3 : " << T_bfs3.get() <<" bfs mean time : " << mean_time << "(msec)\n\n";
