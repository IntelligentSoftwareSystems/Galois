...
struct BFS {
  ...
  void static go(Graph& _graph){
    ...
    do{
      ...
      #endif
      Galois::do_all(_graph.begin(), _graph.end(), BFS { &_graph }, Galois::loopname("BFS"), Galois::numrun(_graph.get_run_num()), Galois::write_set("sync_pull", "this->graph", "struct NodeData &", "struct NodeData &", "dist_current" , "unsigned int" , "min",  ""));
      if(_graph.is_vertex_cut()) {
