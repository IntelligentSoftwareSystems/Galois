...
__global__ void InitializeIteration(CSRGraph graph, unsigned int __nowned, unsigned int __begin, unsigned int __end, const unsigned int  local_current_src_node, const unsigned int  local_infinity, unsigned int * p_current_length, float * p_dependency, unsigned int * p_num_predecessors, unsigned int * p_num_shortest_paths, unsigned int * p_num_successors, unsigned int * p_old_length, unsigned char * p_propogation_flag, unsigned int * p_to_add, unsigned int * p_trim)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
...
__global__ void PredecessorDecrement(CSRGraph graph, unsigned int __nowned, unsigned int __begin, unsigned int __end, unsigned int * p_num_predecessors, unsigned char * p_propogation_flag, unsigned int * p_trim)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
...
void InitializeIteration_cuda(unsigned int  __begin, unsigned int  __end, const unsigned int & local_infinity, const unsigned int & local_current_src_node, struct CUDA_Context * ctx)
{
  ...
  kernel_sizing(blocks, threads);
  // FP: "4 -> 5;
  InitializeIteration <<<blocks, threads>>>(ctx->gg, ctx->nowned, __begin, __end, local_current_src_node, local_infinity, ctx->current_length.data.gpu_wr_ptr(), ctx->dependency.data.gpu_wr_ptr(), ctx->num_predecessors.data.gpu_wr_ptr(), ctx->num_shortest_paths.data.gpu_wr_ptr(), ctx->num_successors.data.gpu_wr_ptr(), ctx->old_length.data.gpu_wr_ptr(), ctx->propogation_flag.data.gpu_wr_ptr(), ctx->to_add.data.gpu_wr_ptr(), ctx->trim.data.gpu_wr_ptr());
  // FP: "5 -> 6;
  check_cuda_kernel;
  // FP: "6 -> 7;
}
...
{
  // FP: "1 -> 2;
  PathsIncrement_cuda(0, ctx->nowned, ctx);
  // FP: "2 -> 3;
}
...
{
  // FP: "1 -> 2;
  DependencyIncrement_cuda(0, ctx->nowned, ctx);
  // FP: "2 -> 3;
}
