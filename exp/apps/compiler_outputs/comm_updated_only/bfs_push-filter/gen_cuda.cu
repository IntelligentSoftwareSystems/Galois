...
void InitializeGraph_cuda(unsigned int  __begin, unsigned int  __end, const unsigned int & local_infinity, unsigned int local_src_node, struct CUDA_Context * ctx)
{
  ...
  InitializeGraph <<<blocks, threads>>>(ctx->gg, ctx->is_updated.gpu_rd_ptr(), ctx->nowned, __begin, __end, local_infinity, local_src_node, ctx->dist_current.gpu_wr_ptr(), ctx->dist_old.gpu_wr_ptr());
  // FP: "5 -> 6;
  check_cuda_kernel;
  // FP: "6 -> 7;
}
...
void FirstItr_BFS_cuda(unsigned int  __begin, unsigned int  __end, struct CUDA_Context * ctx)
{
  ...
  // FP: "4 -> 5;
  ctx->is_updated.cpu_rd_ptr()->clear();
  FirstItr_BFS <<<blocks, __tb_FirstItr_BFS>>>(ctx->gg, ctx->is_updated.gpu_rd_ptr(), ctx->nowned, __begin, __end, ctx->dist_current.gpu_wr_ptr(), ctx->dist_old.gpu_wr_ptr());
  // FP: "5 -> 6;
  check_cuda_kernel;
  // FP: "6 -> 7;
}
...
void BFS_cuda(unsigned int  __begin, unsigned int  __end, int & __retval, struct CUDA_Context * ctx)
{
  ...
  *(retval.cpu_wr_ptr()) = 0;
  ...
  ctx->is_updated.cpu_rd_ptr()->clear();
  BFS <<<blocks, __tb_BFS>>>(ctx->gg, ctx->is_updated.gpu_rd_ptr(), ctx->nowned, __begin, __end, ctx->dist_current.gpu_wr_ptr(), ctx->dist_old.gpu_wr_ptr(), _rv);
  // FP: "5 -> 6;
  check_cuda_kernel;
  // FP: "6 -> 7;
