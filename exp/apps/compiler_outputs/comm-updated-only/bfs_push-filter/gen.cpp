...
_graph.sync_forward<Reduce_0, Broadcast_0>("BFS", bitset_dist_current);
...
struct BFS {
  ...
  void static go(Graph& _graph){
        ...
        for (unsigned int __begin2 = __begin; __begin2 < __end; __begin2+=stepSize) {
          ...
          if (__end2 > __end) __end2 = __end;
            ...
            if (personality == GPU_CUDA) {
              ...
              BFS_cuda(__begin2, __end2, __retval, cuda_ctx);
          ...
          #endif
            {
              ...
              Galois::do_all(_graph.begin() + __begin2, _graph.begin() + __end2, BFS (&_graph), Galois::loopname("BFS"), Galois::write_set("reduce", "this->graph", "struct NodeData &", "struct NodeData &" , "dist_current", "unsigned int" , "min",  ""), Galois::numrun(_graph.get_run_identifier()));
            }
        }
        _graph.sync_forward_pipe<Reduce_0, Broadcast_0>("BFS", bitset_dist_current);
      }
      } else {
    ...
    _graph.sync_forward_wait<Reduce_0, Broadcast_0>("BFS", bitset_dist_current);
