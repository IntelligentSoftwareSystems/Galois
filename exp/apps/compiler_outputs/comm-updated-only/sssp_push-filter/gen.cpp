    ...
    for (unsigned int __begin = 0; __begin < totalSize; __begin+=pipeSize) {
      ...
      if (__end > totalSize) __end = totalSize;
        ...
        bitset_dist_current_clear_cuda(cuda_ctx);
    		...
    		SSSP_cuda(__begin, __end, __retval, cuda_ctx);
      ...
      {
        bitset_dist_current.clear();
        Galois::do_all(_graph.begin() + __begin, _graph.begin() + __end, SSSP (&_graph), Galois::loopname("SSSP"), Galois::write_set("sync_push", "this->graph", "struct NodeData &", "struct NodeData &" , "dist_current", "unsigned int" , "min",  ""), Galois::numrun(_graph.get_run_identifier()));
      }
