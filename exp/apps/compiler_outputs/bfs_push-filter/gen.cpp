...
#endif
Galois::do_all(_graph.begin(), _graph.end(), FirstItr_BFS{&_graph}, Galois::loopname("FirstItr_BFS"), Galois::numrun(_graph.get_run_num()), Galois::write_set("sync_push", "this->graph", "struct NodeData &", "struct NodeData &" , "dist_current", "unsigned int" , "min",  "std::numeric_limits<unsigned int>::max()"));
_graph.sync_push<Syncer_0>("FirstItr_BFS");

if(_graph.is_vertex_cut()) {
	_graph.sync_pull<SyncerPull_vertexCut_0>("FirstItr_BFS");
}
...
struct BFS {
  ...
  void static go(Graph& _graph){
    ...
    #endif
    Galois::do_all(_graph.begin(), _graph.end(), BFS (&_graph), Galois::loopname("BFS"), Galois::numrun(_graph.get_run_num()), Galois::write_set("sync_push", "this->graph", "struct NodeData &", "struct NodeData &" , "dist_current", "unsigned int" , "min",  "std::numeric_limits<unsigned int>::max()"));
