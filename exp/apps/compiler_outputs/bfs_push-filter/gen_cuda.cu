...
void FirstItr_BFS_cuda(struct CUDA_Context * ctx)
{
  ...
  kernel_sizing(ctx->gg, blocks, threads);
  // FP: "4 -> 5;
  FirstItr_BFS <<<blocks, __tb_FirstItr_BFS>>>(ctx->gg, ctx->nowned, ctx->dist_current.gpu_wr_ptr(), ctx->dist_old.gpu_wr_ptr());
  // FP: "5 -> 6;
  check_cuda_kernel;
  // FP: "6 -> 7;
}
void BFS_cuda(int & __retval, struct CUDA_Context * ctx)
{
  ...
  *(ctx->p_retval.cpu_wr_ptr()) = __retval;
  ...
  // FP: "6 -> 7;
  BFS <<<blocks, __tb_BFS>>>(ctx->gg, ctx->nowned, ctx->dist_current.gpu_wr_ptr(), ctx->dist_old.gpu_wr_ptr(), ctx->any_retval);
  // FP: "7 -> 8;
  check_cuda_kernel;
  // FP: "8 -> 9;
