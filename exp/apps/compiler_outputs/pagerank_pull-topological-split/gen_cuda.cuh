...
void batch_get_slave_node_sum_cuda(struct CUDA_Context *ctx, unsigned from_id, float *v) {
	...
	dim3 threads;
	kernel_sizing(ctx->gg, blocks, threads);
	batch_get_subset<float> <<<blocks, threads>>>(ctx->num_slave_nodes[from_id], ctx->slave_nodes[from_id].gpu_rd_ptr(), ctx->slave_sum[from_id].gpu_wr_ptr(true), ctx->sum.gpu_rd_ptr());
	check_cuda_kernel;
...
void batch_get_reset_node_sum_cuda(struct CUDA_Context *ctx, unsigned from_id, float *v, float i) {
	...
	dim3 threads;
	kernel_sizing(ctx->gg, blocks, threads);
	batch_get_reset_subset<float> <<<blocks, threads>>>(ctx->num_slave_nodes[from_id], ctx->slave_nodes[from_id].gpu_rd_ptr(), ctx->slave_sum[from_id].gpu_wr_ptr(true), ctx->sum.gpu_rd_ptr(), i);
	check_cuda_kernel;
	memcpy(v, ctx->slave_sum[from_id].cpu_rd_ptr(), sizeof(float) * ctx->num_slave_nodes[from_id]);
}
...
void batch_set_node_sum_cuda(struct CUDA_Context *ctx, unsigned from_id, float *v) {
	...
	batch_set_subset<float> <<<blocks, threads>>>(ctx->num_slave_nodes[from_id], ctx->slave_nodes[from_id].gpu_rd_ptr(), ctx->slave_sum[from_id].gpu_rd_ptr(), ctx->sum.gpu_wr_ptr());
	check_cuda_kernel;
}
