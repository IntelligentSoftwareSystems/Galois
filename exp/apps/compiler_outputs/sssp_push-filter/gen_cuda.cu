...
__global__ void FirstItr_SSSP(CSRGraph graph, unsigned int nowned, unsigned int * p_dist_current, unsigned int * p_dist_old)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
...
__global__ void SSSP(CSRGraph graph, unsigned int nowned, unsigned int * p_dist_current, unsigned int * p_dist_old, Any any_retval)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
...
void FirstItr_SSSP_cuda(struct CUDA_Context * ctx)
{
  ...
  kernel_sizing(ctx->gg, blocks, threads);
  // FP: "4 -> 5;
  FirstItr_SSSP <<<blocks, __tb_FirstItr_SSSP>>>(ctx->gg, ctx->nowned, ctx->dist_current.gpu_wr_ptr(), ctx->dist_old.gpu_wr_ptr());
  // FP: "5 -> 6;
  check_cuda_kernel;
  // FP: "6 -> 7;
}
void SSSP_cuda(int & __retval, struct CUDA_Context * ctx)
{
  ...
  *(ctx->p_retval.cpu_wr_ptr()) = __retval;
  ...
  // FP: "6 -> 7;
  SSSP <<<blocks, __tb_SSSP>>>(ctx->gg, ctx->nowned, ctx->dist_current.gpu_wr_ptr(), ctx->dist_old.gpu_wr_ptr(), ctx->any_retval);
  // FP: "7 -> 8;
  check_cuda_kernel;
  // FP: "8 -> 9;
