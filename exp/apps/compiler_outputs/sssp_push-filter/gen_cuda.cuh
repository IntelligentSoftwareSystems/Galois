...
__global__ void batch_get_node_dist_current(index_type size, const unsigned int * __restrict__ p_master_nodes, unsigned int * __restrict__ p_master_dist_current, const unsigned int * __restrict__ p_dist_current) {
	unsigned tid = TID_1D;
...
__global__ void batch_get_reset_node_dist_current(index_type size, const unsigned int * __restrict__ p_slave_nodes, unsigned int * __restrict__ p_slave_dist_current, unsigned int * __restrict__ p_dist_current, unsigned int value) {
	unsigned tid = TID_1D;
	unsigned nthreads = TOTAL_THREADS_1D;
	index_type src_end = size;
	for (index_type src = 0 + tid; src < src_end; src += nthreads) {
...
__global__ void batch_set_node_dist_current(index_type size, const unsigned int * __restrict__ p_slave_nodes, const unsigned int * __restrict__ p_slave_dist_current, unsigned int * __restrict__ p_dist_current) {
	unsigned tid = TID_1D;
	unsigned nthreads = TOTAL_THREADS_1D;
	index_type src_end = size;
	for (index_type src = 0 + tid; src < src_end; src += nthreads) {
...
__global__ void batch_add_node_dist_current(index_type size, const unsigned int * __restrict__ p_master_nodes, const unsigned int * __restrict__ p_master_dist_current, unsigned int * __restrict__ p_dist_current) {
	unsigned tid = TID_1D;
	unsigned nthreads = TOTAL_THREADS_1D;
	index_type src_end = size;
	for (index_type src = 0 + tid; src < src_end; src += nthreads) {
...
__global__ void batch_min_node_dist_current(index_type size, const unsigned int * __restrict__ p_master_nodes, const unsigned int * __restrict__ p_master_dist_current, unsigned int * __restrict__ p_dist_current) {
	unsigned tid = TID_1D;
	unsigned nthreads = TOTAL_THREADS_1D;
	index_type src_end = size;
	for (index_type src = 0 + tid; src < src_end; src += nthreads) {
...
__global__ void batch_get_node_dist_old(index_type size, const unsigned int * __restrict__ p_master_nodes, unsigned int * __restrict__ p_master_dist_old, const unsigned int * __restrict__ p_dist_old) {
	unsigned tid = TID_1D;
	unsigned nthreads = TOTAL_THREADS_1D;
	index_type src_end = size;
	for (index_type src = 0 + tid; src < src_end; src += nthreads) {
		unsigned LID = p_master_nodes[src];
		p_master_dist_old[src] = p_dist_old[LID];
	}
}
...
void batch_get_node_dist_old_cuda(struct CUDA_Context *ctx, unsigned from_id, unsigned int *v) {
	...
	dim3 threads;
	kernel_sizing(ctx->gg, blocks, threads);
	batch_get_node_dist_old <<<blocks, threads>>>(ctx->num_master_nodes[from_id], ctx->master_nodes[from_id].gpu_rd_ptr(), ctx->master_dist_old[from_id].gpu_wr_ptr(true), ctx->dist_old.gpu_rd_ptr());
	check_cuda_kernel;
	memcpy(v, ctx->master_dist_old[from_id].cpu_rd_ptr(), sizeof(unsigned int) * ctx->num_master_nodes[from_id]);
}

__global__ void batch_get_reset_node_dist_old(index_type size, const unsigned int * __restrict__ p_slave_nodes, unsigned int * __restrict__ p_slave_dist_old, unsigned int * __restrict__ p_dist_old, unsigned int value) {
	unsigned tid = TID_1D;
	unsigned nthreads = TOTAL_THREADS_1D;
	index_type src_end = size;
	for (index_type src = 0 + tid; src < src_end; src += nthreads) {
		unsigned LID = p_slave_nodes[src];
		p_slave_dist_old[src] = p_dist_old[LID];
		p_dist_old[LID] = value;
	}
}
...
void batch_get_reset_node_dist_old_cuda(struct CUDA_Context *ctx, unsigned from_id, unsigned int *v, unsigned int i) {
	...
	dim3 threads;
	kernel_sizing(ctx->gg, blocks, threads);
	batch_get_reset_node_dist_old <<<blocks, threads>>>(ctx->num_slave_nodes[from_id], ctx->slave_nodes[from_id].gpu_rd_ptr(), ctx->slave_dist_old[from_id].gpu_wr_ptr(true), ctx->dist_old.gpu_rd_ptr(), i);
	check_cuda_kernel;
	memcpy(v, ctx->slave_dist_old[from_id].cpu_rd_ptr(), sizeof(unsigned int) * ctx->num_slave_nodes[from_id]);
}

__global__ void batch_set_node_dist_old(index_type size, const unsigned int * __restrict__ p_slave_nodes, const unsigned int * __restrict__ p_slave_dist_old, unsigned int * __restrict__ p_dist_old) {
	unsigned tid = TID_1D;
	unsigned nthreads = TOTAL_THREADS_1D;
	index_type src_end = size;
	for (index_type src = 0 + tid; src < src_end; src += nthreads) {
		unsigned LID = p_slave_nodes[src];
		p_dist_old[LID] = p_slave_dist_old[src];
	}
}
...
void batch_set_node_dist_old_cuda(struct CUDA_Context *ctx, unsigned from_id, unsigned int *v) {
	...
	batch_set_node_dist_old <<<blocks, threads>>>(ctx->num_slave_nodes[from_id], ctx->slave_nodes[from_id].gpu_rd_ptr(), ctx->slave_dist_old[from_id].gpu_rd_ptr(), ctx->dist_old.gpu_wr_ptr());
	check_cuda_kernel;
}

__global__ void batch_add_node_dist_old(index_type size, const unsigned int * __restrict__ p_master_nodes, const unsigned int * __restrict__ p_master_dist_old, unsigned int * __restrict__ p_dist_old) {
	unsigned tid = TID_1D;
	unsigned nthreads = TOTAL_THREADS_1D;
	index_type src_end = size;
	for (index_type src = 0 + tid; src < src_end; src += nthreads) {
		unsigned LID = p_master_nodes[src];
		p_dist_old[LID] += p_master_dist_old[src];
	}
}
...
void batch_add_node_dist_old_cuda(struct CUDA_Context *ctx, unsigned from_id, unsigned int *v) {
	...
	batch_add_node_dist_old <<<blocks, threads>>>(ctx->num_master_nodes[from_id], ctx->master_nodes[from_id].gpu_rd_ptr(), ctx->master_dist_old[from_id].gpu_rd_ptr(), ctx->dist_old.gpu_wr_ptr());
	check_cuda_kernel;
}

__global__ void batch_min_node_dist_old(index_type size, const unsigned int * __restrict__ p_master_nodes, const unsigned int * __restrict__ p_master_dist_old, unsigned int * __restrict__ p_dist_old) {
	unsigned tid = TID_1D;
	unsigned nthreads = TOTAL_THREADS_1D;
	index_type src_end = size;
	for (index_type src = 0 + tid; src < src_end; src += nthreads) {
		unsigned LID = p_master_nodes[src];
		p_dist_old[LID] = (p_dist_old[LID] > p_master_dist_old[src]) ? p_master_dist_old[src] : p_dist_old[LID];
	}
}
...
void batch_min_node_dist_old_cuda(struct CUDA_Context *ctx, unsigned from_id, unsigned int *v) {
	...
	kernel_sizing(ctx->gg, blocks, threads);
	memcpy(ctx->master_dist_old[from_id].cpu_wr_ptr(true), v, sizeof(unsigned int) * ctx->num_master_nodes[from_id]);
	batch_min_node_dist_old <<<blocks, threads>>>(ctx->num_master_nodes[from_id], ctx->master_nodes[from_id].gpu_rd_ptr(), ctx->master_dist_old[from_id].gpu_rd_ptr(), ctx->dist_old.gpu_wr_ptr());
	check_cuda_kernel;
}
