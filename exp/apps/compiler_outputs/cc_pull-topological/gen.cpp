...
struct ConnectedComp {
  ...
  void static go(Graph& _graph){
    ...
    do{
      ...
      #ifdef __GALOIS_HET_CUDA__
      	if (personality == GPU_CUDA) {
      		std::string impl_str("CUDA_DO_ALL_IMPL_ConnectedComp_" + std::to_string(_graph.get_run_num()));
      		Galois::StatTimer StatTimer_cuda(impl_str.c_str());
      		StatTimer_cuda.start();
      		int __retval = 0;
      ...
      #endif
      Galois::do_all(_graph.begin(), _graph.end(), ConnectedComp { &_graph }, Galois::loopname("ConnectedComp"), Galois::numrun(_graph.get_run_num()), Galois::write_set("sync_pull", "this->graph", "struct NodeData &", "struct NodeData &", "comp_current" , "unsigned int" , "min",  "std::numeric_limits<unsigned int>::max()"));
      if(_graph.is_vertex_cut()) {
      	_graph.sync_push<Syncer_vertexCut_0>("ConnectedComp");
      }
