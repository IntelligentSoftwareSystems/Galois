...
struct GBPartitionerDisk {
   ...
   struct VertexCutInfo {
      ...
      std::vector<std::unordered_set<size_t>> vertexOwnersPacked;
...
//            auto owner = &vertexOwnersPacked[n];
    ...
    * */
   size_t getEdgeOwner_old(size_t src, size_t dst, size_t num) {
      return rand() % num;
   }
   size_t getEdgeOwner(size_t src, size_t dst, size_t num) {
      static std::mt19937 gen;
      static std::uniform_int_distribution<size_t> dist(0,num-1);
      return dist(gen);
   }
   /*
...
//      std::vector<size_t> &vertexOwners = vcInfo.vertexMasters;
      for (size_t h = 0; h < num_hosts; ++h) {
         ...
         for (size_t n = 0; n < g.size(); ++n) {
            if (vcInfo.global2Local[h][n] != ~0) {
               auto owner = vcInfo.vertexMasters[n];
...
//         vcInfo.global2Local[h].clear(); //Does not force deallocation
//         std::vector<size_t>().swap(vcInfo.global2Local[h]);//forces deallocation?
         vcInfo.global2Local[h].clear();
         vcInfo.global2Local[h].shrink_to_fit();
