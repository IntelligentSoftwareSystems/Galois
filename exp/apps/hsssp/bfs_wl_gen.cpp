...
struct Get_info_functor : public Galois::op_tag {
	...
	struct Syncer_0 {
		...
		static void reduce (uint32_t node_id, struct NodeData & node, int y) {
		#ifdef __GALOIS_HET_CUDA__
		...
		#endif
				{node.dist_current = std::numeric_limits<int>::max()/4; }
		}
		typedef int ValTy;
	};
	struct SyncerPull_0 {
...
struct BFS {
  ...
  void static go(Graph& _graph){
    ...
    typedef dChunkedFIFO<64> dChunk;
      ...
      #endif
      Galois::for_each(_graph.begin(), _graph.end(), BFS (&_graph), Galois::loopname("BFS"), Galois::workList_version(), Galois::write_set("sync_push", "this->graph", "struct NodeData &", "struct NodeData &" , "dist_current", "int" , "{ Galois::atomicMin(node.dist_current, y);}",  "{node.dist_current = std::numeric_limits<int>::max()/4; }"), Galois::write_set("sync_pull", "this->graph", "struct NodeData &", "struct NodeData &", "dist_current" , "int"), Get_info_functor<Graph>(_graph));
