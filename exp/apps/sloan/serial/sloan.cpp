...
struct UpdateRequestLess {
  bool operator()(const UpdateRequest& a, const UpdateRequest& b) const {
    return a.prio <= b.prio;
		/*
		if(a.prio == b.prio)
			return graph.getData(a.node, Galois::NONE).degree <= graph.getData(b.node, Galois::NONE).degree;
    return a.prio < b.prio;
		*/
  }
};
		...
		vTsloan[0].start();
			...
			for(int i = 0; i < graph.size(); ++i){
				...
				for(std::set<UpdateRequest>::iterator ii = pq.begin(), ei = pq.end(); ii != ei; ++ii){
						std::cerr << graph.getData(ii->node, Galois::NONE).id << "(" <<  ii->prio << ") ";
				}
				std::cerr << "\n";
				*/
				//printpq();
				...
				//SNode& pdata = graph.getData(next.node, Galois::NONE);
				//while(pdata.status == NUMBERED) { 
				while(graph.getData(next.node, Galois::NONE).status == NUMBERED) { 
					//std::cerr << "[" << i << "] (" << pq.size() << ") Redundant node: " << graph.getData(next.node, Galois::NONE).id << " with status: " << graph.getData(next.node, Galois::NONE).status << " and priority: " << graph.getData(next.node, Galois::NONE).prio << "\n";
					if(pq.empty())
						break;
					next = pq.top();
					//next = *pq.begin();
					...
					//parent = pq.top();
					//pq.pop();
					//pdata = graph.getData(parent, Galois::NONE);
				}

				//std::cerr << "[" << i << "] (" << pq.size() << ") Not Redundant node: " << graph.getData(next.node, Galois::NONE).id << " with status: " << graph.getData(next.node, Galois::NONE).status << " and priority: " << graph.getData(next.node, Galois::NONE).prio << "\n";
				GNode parent = next.node; 
				...
				*/
				...
				graph.getData(parent, Galois::NONE).status = NUMBERED;
				perm.push_back(parent);

				/*
				std::cerr << "Permutation: ";
				for(std::vector<GNode>::iterator ii = perm.begin(), ei = perm.end(); ii != ei; ++ii){
						std::cerr << graph.getData(*ii, Galois::NONE).id << "(" << graph.getData(*ii, Galois::NONE).prio << ") ";
				}
				std::cerr << "\n";
				*/
				...
				for (Graph::edge_iterator ii = graph.edge_begin(parent, Galois::NONE), ei = graph.edge_end(parent, Galois::NONE); ii != ei; ++ii) {
					...
					if(cdata.status == PREACTIVE){
						...
						for (Graph::edge_iterator ij = graph.edge_begin(child, Galois::NONE), ej = graph.edge_end(child, Galois::NONE); ij != ej; ++ij) {
