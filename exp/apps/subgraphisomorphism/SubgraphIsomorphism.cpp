...
static cll::opt<bool> constructD("constructData", cll::desc("construct a data graph"), cll::init(false));
...
static cll::opt<bool> constructQ("constructQuery", cll::desc("construct a random query graph"), cll::init(true));
...
void run() {
  DGraph gD;
  if(constructD) {
    std::cout << "Generating data graph of " << numNodesD << " nodes and " << numEdgesD << " edges..." << std::endl;
    constructGraph(gD, numNodesD, numEdgesD);
  } else if(dataGraphName.size()) {
    Galois::Graph::readGraph(gD, dataGraphName);
    std::cout << "Reading data graph..." << std::endl;
  } else {
    GALOIS_DIE("Failed to construct or read data graph");
  }
  initializeGraph(gD);
  if(constructD) {
    printGraph(gD);
  ...
  QGraph gQ;
  if(constructQ) {
    std::cout << "Generating query graph of " << numNodesQ << " nodes and " << numEdgesQ << " edges..." << std::endl;
    constructGraph(gQ, numNodesQ, numEdgesQ);
  } else if(queryGraphName.size()) {
    Galois::Graph::readGraph(gQ, queryGraphName);
    std::cout << "Reading query graph..." << std::endl;
  } else {
    GALOIS_DIE("Failed to construct or read query graph");
  }
  initializeGraph(gQ);
  if(constructQ) {
    printGraph(gQ);
...
//    generator.seed(rndSeed); // FIXME: does not compile if uncomment
  ...
  if(numEdgesQ > maxNumEdgesQ) {
    numEdgesQ = maxNumEdgesQ;
  }

  unsigned int maxNumEdgesD = numNodesD * (numNodesD - 1);
  if(constructUndirected) {
    maxNumEdgesD /= 2;
  }
  if(numEdgesD > maxNumEdgesD) {
    numEdgesD = maxNumEdgesD;
  }
