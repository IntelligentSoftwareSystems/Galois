...
void readFromFilesChangeEndianessAndWrite() {
  ...
  uint64_t* fptr = (uint64_t*)m;
  uint64_t version = *fptr++;
  uint64_t sw_version = Galois::convert_bswap64(version);
  fout.write((char*)(&sw_version), sizeof(sw_version));
  assert(version == 2);
  uint64_t numNodes = *fptr++;
  uint64_t numEdges = *fptr++;
  uint64_t sw_numNodes = Galois::convert_bswap64(numNodes);
  fout.write((char*)(&sw_numNodes), sizeof(sw_numNodes));
  uint64_t sw_numEdges = Galois::convert_bswap64(numEdges);
  ...
  fptr32 += idxArraysLen;
	...
	for (int i=0; i<idxArraysLen; ++i) {
	  uint32_t sw_inIdx = Galois::convert_bswap32(inIdx[i]);
  	fout.write((char*)(&sw_inIdx), sizeof(sw_inIdx));
	}

  if (idxArraysLen % 2) {
    uint32_t t = Galois::convert_bswap32(1); //dummy
  ...
  fptr32 += numEdges;
	for (unsigned int i=0; i<numEdges; ++i) {
		uint32_t sw_ins = Galois::convert_bswap32(ins[i]);
  ...
  fptr32 += idxArraysLen;
	for (int i=0; i<idxArraysLen; ++i) {
		uint32_t sw_outIdx = Galois::convert_bswap32(outIdx[i]);
  ...
  fptr = (uint64_t*)m2;
  version = *fptr++;
  sw_version = Galois::convert_bswap64(version);
  ...
  fptr32 += numEdges;
	for (unsigned int i=0; i<numEdges; ++i) {
		uint32_t sw_outs = Galois::convert_bswap32(outs[i]);
