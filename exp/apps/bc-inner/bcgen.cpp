...
void readFromFilesChangeEndianessAndWrite() {
  ...
  uint64_t* fptr = (uint64_t*)m;
  uint64_t version = *fptr++;
  uint64_t sw_version = __bswap_64(version);
  fout.write((char*)(&sw_version), sizeof(sw_version));
  assert(version == 2);
  uint64_t numNodes = *fptr++;
  uint64_t numEdges = *fptr++;
  uint64_t sw_numNodes = __bswap_64(numNodes);
  fout.write((char*)(&sw_numNodes), sizeof(sw_numNodes));
  uint64_t sw_numEdges = __bswap_64(numEdges);
  ...
  fptr32 += idxArraysLen;
	...
	for (int i=0; i<idxArraysLen; ++i) {
	  uint32_t sw_inIdx = __bswap_32(inIdx[i]);
  	fout.write((char*)(&sw_inIdx), sizeof(sw_inIdx));
	}

  if (idxArraysLen % 2) {
    uint32_t t = __bswap_constant_32(1); //dummy
  ...
  fptr32 += idxArraysLen;
	for (int i=0; i<idxArraysLen; ++i) {
		uint32_t sw_outIdx = __bswap_32(outIdx[i]);
