...
__global__ void PageRank_pull(CSRGraph graph, int  nowned, const float  local_alpha, float local_tolerance, int * p_nout, float * p_value, WorklistT in_wl, WorklistT out_wl)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
