...
struct FirstItr_SSSP {
  ...
  void static go(Graph& _graph){
    ...
    #endif
    Galois::for_each(_graph.begin(), _graph.end(), FirstItr_SSSP { &_graph }, Galois::loopname("sssp"), Galois::write_set("sync_push", "this->graph", "struct NodeData &", "struct NodeData &" , "dist_current", "unsigned int" , "{ Galois::atomicMin(node.dist_current, y);}",  "{node.dist_current = std::numeric_limits<unsigned int>::max()/4; }"));
...
struct SSSP {
  ...
  void static go(Graph& _graph){
      ...
      #endif
      Galois::for_each(_graph.begin(), _graph.end(), SSSP { &_graph }, Galois::loopname("sssp"), Galois::write_set("sync_push", "this->graph", "struct NodeData &", "struct NodeData &" , "dist_current", "unsigned int" , "{ Galois::atomicMin(node.dist_current, y);}",  "{node.dist_current = std::numeric_limits<unsigned int>::max()/4; }"));
      _graph.sync_push<Syncer_0>();
...
int main(int argc, char** argv) {
  try {
    ...
    Galois::Timer T_total, T_hGraph_init, T_init, T_sssp1, T_sssp2, T_sssp3;
