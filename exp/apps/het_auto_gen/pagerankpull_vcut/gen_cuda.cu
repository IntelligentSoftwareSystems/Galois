...
__global__ void InitializeGraph(CSRGraphTex graph, int  nowned, const float  local_alpha, int * p_nout, float * p_sum, float * p_value)
{
  unsigned tid = TID_1D;
...
__global__ void PageRank_pull_partial(CSRGraphTex graph, int  nowned, int * p_nout, float * p_sum, float * p_value)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
...
__global__ void PageRank_pull(CSRGraphTex graph, int  nowned, const float  local_alpha, float local_tolerance, float * p_sum, float * p_value, Any any_retval)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
...
void PageRank_pull_partial_cuda(struct CUDA_Context * ctx)
{
  dim3 blocks;
  dim3 threads;
  kernel_sizing(ctx->gg, blocks, threads);
  PageRank_pull_partial <<<blocks, threads>>>(ctx->gg, ctx->nowned, ctx->nout.gpu_wr_ptr(), ctx->sum.gpu_wr_ptr(), ctx->value.gpu_wr_ptr());
  check_cuda_kernel;
}
void PageRank_pull_cuda(int & __retval, const float & local_alpha, float local_tolerance, struct CUDA_Context * ctx)
{
  dim3 blocks;
  dim3 threads;
  kernel_sizing(ctx->gg, blocks, threads);
  *(ctx->p_retval.cpu_wr_ptr()) = __retval;
  ...
  PageRank_pull <<<blocks, threads>>>(ctx->gg, ctx->nowned, local_alpha, local_tolerance, ctx->sum.gpu_wr_ptr(), ctx->value.gpu_wr_ptr(), ctx->any_retval);
  check_cuda_kernel;
