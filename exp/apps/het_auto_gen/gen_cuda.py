...
Kernel("PageRank_cuda", [('bool &', 'local_didWork'), ('const float &', 'local_alpha'), ('float', 'local_tolerance'), ('struct CUDA_Context *', 'ctx')],
[
CDecl([("dim3", "blocks", "")]),
CDecl([("dim3", "threads", "")]),
CBlock(["kernel_sizing(ctx->gg, blocks, threads)"]),
CBlock(["*(ctx->p_retval.cpu_wr_ptr()) = local_didWork"]),
...
CBlock(["local_didWork = *(ctx->p_retval.cpu_rd_ptr())"]),
CBlock(["check_cuda_kernel"], parse = False),
], host = True),
])
