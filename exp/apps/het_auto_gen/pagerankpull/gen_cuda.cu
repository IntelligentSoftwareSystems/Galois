...
__global__ void PageRank_pull(CSRGraphTex graph, int  nowned, const float  local_alpha, const float  local_tolerance, int * p_nout, float * p_value, Any any_retval)
{
  unsigned tid = TID_1D;
  unsigned nthreads = TOTAL_THREADS_1D;
