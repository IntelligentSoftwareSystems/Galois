...
class RationalWrapper: 
  ...
  void truncate (void) {
    ...
    if (std::abs (m_val.numerator ()) >= MAX_NUMERATOR || m_val.denominator () >= MAX_DENOMINATOR) {
...
public:
  ...
  RationalWrapper (double d): m_val (int64_t (d * MAX_DENOMINATOR), MAX_DENOMINATOR) {
  ...
  operator double (void) const {
    return boost::rational_cast<double> (m_val);
  }
  ...
  static RationalWrapper fabs (const RationalWrapper& r) {
    r.check ();
    return RationalWrapper (std::abs (r.m_val.numerator ()), r.m_val.denominator ());
  }
...
class DoubleWrapper: 
  public boost::less_than_comparable<DoubleWrapper>,
...
public:
  ...
  DoubleWrapper (double d): m_val (d) {
    this->truncate ();
  }
  ...
  // unary - and plus
  const DoubleWrapper& operator + (void) const {
    return *this;
  }

  DoubleWrapper operator - (void) const {
    return DoubleWrapper (-m_val);
  }
