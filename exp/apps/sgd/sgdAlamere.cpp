...
static const char* const desc = "Computes Matrix Decomposition using Stochastic Gradient D Descent";
...
static cll::opt<Algo> algo(cll::desc("Choose an algorithm:"),
    cll::values(
      clEnumVal(blockUsers, "Block by Users"),
      clEnumVal(blockMovies, "Block by Movies "),
      clEnumVal(blockBoth, "Block both Users and Movies (default)"),
...
void verify (Graph& g) {
  ...
  std::cout << "Root Mean Square Error after training: " << total_rms << std::endl;
}
...
int main(int argc, char** argv) {	
  ...
  switch (algo) {
    ...
    case Algo::blockUsers:
      GALOIS_DIE ("Not Implemented yet\n");
      break;

    case Algo::blockMovies:
      GALOIS_DIE ("Not Implemented yet\n");
      break;
