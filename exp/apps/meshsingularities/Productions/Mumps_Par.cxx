...
int call_mumps(int argc, char ** argv, int* in, int* jn, double* a, double* rhs, int n, int nz, double** matrix, GenericMatrixGenerator* matrix_generator, TaskDescription &taskDescription)
{
  DMUMPS_STRUC_C id;
...
int execute_mumps(int argc, char** argv, TaskDescription &taskDescription){
	...
	std::vector<EquationSystem*>* tier_list = matrix_generator->CreateMatrixAndRhs(taskDescription);
	srand(time(NULL));
    ...
    call_mumps(argc,argv,in,jn,a,rhs,size,nz,matrix,matrix_generator,taskDescription);
    return 0;
