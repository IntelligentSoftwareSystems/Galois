...
struct Jacobi {
  ...
  void operator()(const Graph::GraphNode& src) {
    Node& node = graph.getData(src, Galois::MethodFlag::ALL);
    node.x_prev = node.x;
    ...
    for (Graph::edge_iterator ii = graph.edge_begin(src, Galois::MethodFlag::ALL),
      ...
      sum += weight * graph.getData(dst, Galois::MethodFlag::NONE).x;
    }
...
struct ConjugateGradient {
  ...
  void operator()() {
    ...
    for (Graph::iterator ii = graph.begin(), ei = graph.end(); ii != ei; ++ii) {
      double r = graph.getData(*ii, Galois::MethodFlag::NONE).r;
      rs_old += r * r;
    }
    ...
    for (int iteration = 0; iteration < MaxIterations; ++iteration) {
      ...
      for (Graph::iterator src = graph.begin(), 
        ...
        Node& node = graph.getData(*src, Galois::MethodFlag::NONE);
        node.ap = 0;
        for (Graph::edge_iterator ii = graph.edge_begin(*src, Galois::MethodFlag::ALL),
            ei = graph.edge_end(*src, Galois::MethodFlag::ALL); ii != ei; ++ii) {
          Graph::GraphNode dst = graph.getEdgeDst(ii);
          double weight = graph.getEdgeData(ii); 
          node.ap += weight * graph.getData(dst, Galois::MethodFlag::NONE).p;
        }
...
struct GBP {
  ...
  void operator()(const Graph::GraphNode& src) {
    Node& node = src.getData(Galois::MethodFlag::NONE);
    ...
    // Incoming edges
    for (Graph::neighbor_iterator dst = graph.neighbor_begin(src, Galois::MethodFlag::ALL),
        edst = graph.neighbor_end(src, Galois::MethodFlag::ALL); dst != edst; ++dst) {
      const Edge& edge = graph.getEdgeData(*dst, src, Galois::MethodFlag::NONE);
      mu_i += edge.mean;
      J_i += edge.prec;
    }
    ...
    for (Graph::neighbor_iterator dst = graph.neighbor_begin(src, Galois::MethodFlag::NONE),
        edst = graph.neighbor_end(src, Galois::MethodFlag::NONE); dst != edst; ++dst) {
      Edge& inEdge = graph.getEdgeData(*dst, src, Galois::MethodFlag::NONE);
      Edge& outEdge = graph.getEdgeData(src, dst, Galois::MethodFlag::NONE);
...
struct GBP {
  ...
  void operator()(const Graph::GraphNode& src) {
    ...
    for (Graph::edge_iterator ii = graph.edge_begin(src, Galois::MethodFlag::ALL),
        ei = graph.edge_end(src, Galois::MethodFlag::ALL); ii != ei; ++ii) {
      Graph::GraphNode dst = graph.getEdgeDst(ii);
      const Edge& edge = graph.getEdgeData(graph.findEdge(dst, src, Galois::MethodFlag::NONE));
      node.mean += edge.mean;
      node.prec += edge.prec;
    }
    ...
    for (Graph::edge_iterator ii = graph.edge_begin(src, Galois::MethodFlag::NONE),
      ...
      Graph::GraphNode dst = graph.getEdgeDst(ii);
      Edge& inEdge = graph.getEdgeData(graph.findEdge(dst, src, Galois::MethodFlag::NONE));
