...
{
	printf(
	"Usage: train-mf-test [options] data-dir/\n"
	"options:\n"
...
mf_parameter parse_command_line(int argc, char **argv, char *Y_src, char *X1_src, char *X2_src, char *Yt_src, char *Xt_src)
{
...
void run_mf_train(mf_parameter param, smat_t &Y, smat_t &X1, smat_t &X2, smat_t &Yt, smat_t &Xt) {
	srand48(0UL);
//	std::default_random_engine generator(0);
	...
	bilinear_problem testset(&Yt, &Xt, H, k);
	mf_train_test(&prob, &param, &testset, W, H);
...
int main(int argc, char* argv[]){
	char X1_src[1024], X2_src[1024], Y_src[1024], Yt_src[1024], Xt_src[1024];
	char hostname[1024];
	...
	for(int i = 0; i < argc; i++)
		printf("%s ", argv[i]);
	puts("");
	smat_t Y,X1,X2,Yt,Xt;
	mf_parameter param = parse_command_line(argc, argv, Y_src, X1_src, X2_src, Yt_src, Xt_src); 
	...
	run_mf_train(param, Y, X1, X2, Yt, Xt); 
	return 0;
}
