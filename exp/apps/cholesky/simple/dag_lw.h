...
struct LocalWorklist {
  ...
  LocalWorklist(threadlist *neighbors, unsigned int tid,
                CompareType comparator):
  Q(comparator), neighbors(neighbors), tid(tid),
    token(~0), done_work(0) {
  ...
  void forward_token() {
    ...
    if ( token == ~(unsigned)0 ) return;  /* We don't own the token */
    /* Update token */
    if ( done_work ) {
      token = 0;
      done_work = 0;
  ...
  void set_token(unsigned int newtoken) {
    ...
    token = newtoken;
    if ( pthread_cond_broadcast(&cond) ) abort();
    if ( pthread_mutex_unlock(&lock) ) abort();
  }
  int process_token() {
    if ( token == ~(unsigned)0 ) return 1; /* Keep going */
    return token > neighbors->size()*2 ? 0 /* We're done */ : 1;
  }

  /* Work stealing */
  void steal() {
    if ( neighbors->size() < 2 ) return; /* One thread; can't steal */
    ...
    unsigned
      cone = (*neighbors)[one].peek(),
      ctwo = (*neighbors)[two].peek();
