...
#ifndef GDIST_EXP_INCLUDE_OPENCL_LC_GRAPH_2_H_
#define GDIST_EXP_INCLUDE_OPENCL_LC_GRAPH_2_H_

namespace Galois {
namespace OpenCL {

//ENV is used to determine the device for execution. Needed to initialize the structure
 ...
 */
static const char * cl_wrapper_str_LC_Graph_2 =
      "\
   ...
   uint _num_edges;\n\
    ...
    __global uint *_out_edge_data;\n\
      ...
      ";
static const char * init_kernel_str_LC_Graph_2 =
      "\
...
struct LC_Graph_2 {
   ...
   typedef GPUWrapper<EdgeDataTy> GPUEdgeType;
   typedef typename GPUWrapper<unsigned int>::HostPtrType HostPtrType;
   typedef typename GPUWrapper<unsigned int>::DevicePtrType DevicePtrType;
#endif
...
//               std::cout<<*n<<", "<<ggraph.getData(*n)<<", "<< getData()[src_node]<<"\n";
   ...
   EdgeDataType * out_edge_data() {
      return edge_data_ptr->host_data;
   }
...
//      const size_t sz = 4;
      ...
      edge_data_ptr = new GPUEdgeType(_num_edges);
//      memset(gpu_graph->host_ptr(), 0, sizeof(int) * sz);
   ...
   void init_graph_struct(){
         ...
         size_t kernel_len = strlen(cl_wrapper_str_LC_Graph_2) + strlen(init_kernel_str_LC_Graph_2) + 1;
         char * kernel_src = new char[kernel_len];
         sprintf(kernel_src, "%s\n%s", cl_wrapper_str_LC_Graph_2, init_kernel_str_LC_Graph_2);
         init_kernel.init_string(kernel_src, "initialize_graph_struct");
         init_kernel.set_arg_list(gpu_struct_ptr, gpu_graph, node_data_ptr, outgoing_index_ptr, outgoing_neighbors_ptr, edge_data_ptr);
