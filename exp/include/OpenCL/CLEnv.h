...
struct CLEnvironment{
   ...
   void init(float dev_id){
      ...
      m_program=nullptr;
      CHECK_CL_ERROR(clGetPlatformIDs(4, l_platforms, &num_platforms), "clGetPlatformIDs ");
      m_platform_id = l_platforms[DEFAULT_CL_PLATFORM_ID];
      clGetPlatformInfo(l_platforms[DEFAULT_CL_PLATFORM_ID], CL_PLATFORM_NAME, 256, string_holder, NULL);
      std::cout << "Platform: " << string_holder;
      clGetPlatformInfo(l_platforms[DEFAULT_CL_PLATFORM_ID], CL_DEVICE_VENDOR, 256, string_holder, NULL);
      std::cout << ", Vendor: " << string_holder;
      vendor_name = string_holder;
      CHECK_CL_ERROR(clGetPlatformInfo(l_platforms[DEFAULT_CL_PLATFORM_ID], CL_PLATFORM_VERSION, 256, string_holder, NULL), "clGetPlatform info, version");
      std::cout << "[Version: [" << string_holder << "] ";
      cl_uint num_devices;
      clGetDeviceIDs(l_platforms[DEFAULT_CL_PLATFORM_ID], CL_DEVICE_TYPE_ALL, 0, 0, &num_devices);
      cl_device_id * l_devices = new cl_device_id[num_devices];
      clGetDeviceIDs(l_platforms[DEFAULT_CL_PLATFORM_ID], CL_DEVICE_TYPE_ALL, num_devices, l_devices, 0);
      std::cout << ",devices=" << num_devices << " \n";
      m_device_id = l_devices[DEFAULT_CL_DEVICE_ID];
//      char string_holder[4 * 256];
      CHECK_CL_ERROR(clGetDeviceInfo(m_device_id , CL_DEVICE_NAME, 1024, string_holder, NULL), "clGetDeviceInfo-1");
      std::cout<<"Current device :: "<<string_holder<<"\n";
      cl_int err;
...
#else
      m_command_queue = clCreateCommandQueue(m_context, m_device_id, 0 /*CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE*/, &err);
#endif
