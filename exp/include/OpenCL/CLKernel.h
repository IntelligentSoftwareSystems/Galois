...
//      compiler_flags += build_args;
   ...
   static size_t workgroup_size(cl_kernel kernel) {
      ...
      CHECK_CL_ERROR(clGetKernelWorkGroupInfo(kernel, cl_env.m_device_id, CL_KERNEL_WORK_GROUP_SIZE, sizeof(size_t), &work_group_size, NULL),
...
struct CL_Kernel {
   ...
   cl_event event;
   char name[256];
   static std::string self_directory() {
      char buffer[1024];
