...
void OCFileGraph::Block::load(int fd, offset_t offset, size_t begin, size_t len, size_t sizeof_data) {
  ...
  offset_t aligned = start & ~static_cast<offset_t>(Galois::Runtime::pageSize - 1);

  int _MAP_BASE = MAP_PRIVATE;
#ifdef MAP_POPULATE
  _MAP_BASE |= MAP_POPULATE;
#endif
  m_length = len * sizeof_data + Galois::Runtime::pageSize; // account for round off due to alignment
  ...
  assert(start - aligned <= static_cast<offset_t>(Galois::Runtime::pageSize));
