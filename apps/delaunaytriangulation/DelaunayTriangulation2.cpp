...
//        double length2 = origin.x() * origin.x() + origin.y() * origin.y();
  ...
  bool findContainingElement(const Point* p, GNode& node) {
    ...
    (*rp)->acquire(Galois::CHECK_CONFLICT);
  ...
  //! Parallel operator
  GALOIS_ATTRIBUTE_NOINLINE void operator()(Point* p, Galois::UserContext<Point*>& ctx) {
    p->acquire();
    assert(!p->inMesh());

    //    std::cout << p->id() << " " << p->t().x() << " " << p->t().y() << "\n";
...
class ReadPoints {
  void addBoundaryPoints() {
    ...
    std::cout << "Using bounds " << minX << "," << minY << " " << maxX << "," << maxY << "\n";
