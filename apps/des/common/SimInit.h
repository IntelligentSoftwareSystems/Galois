...
protected:
  ...
  void createConnections (G& graph) {
    ...
    for (typename std::vector<GNode>::iterator i = allNodes.begin (), ei = allNodes.end (); i != ei; ++i) {
      ...
      SimGate_tp* sg = dynamic_cast<SimGate_tp*> (graph.getData (src, Galois::MethodFlag::NONE));
      assert (sg != NULL);
      ...
      for (typename std::vector<GNode>::iterator j = allNodes.begin (), ej = allNodes.end (); j != ej; ++j) {
        GNode& dst = *j;
        SimGate_tp* dg = dynamic_cast<SimGate_tp*> (graph.getData (dst, Galois::MethodFlag::NONE));
        assert (dg != NULL);
        ...
        if (dstGate.hasInputName (srcOutName)) {
          ...
          if (graph.findEdge(src, dst) == graph.edge_end(src)) {
            ++numEdges;
            graph.addEdge (src, dst, Galois::MethodFlag::NONE);
          }
