  ...
  template<typename ContextTy>
    void __attribute__((noinline)) operator()(GNode& src, ContextTy& lwl) {
      ...
      for (Graph::neighbor_iterator 
          dst = graph.neighbor_begin(src, Galois::Graph::ALL),
          edst = graph.neighbor_end(src, Galois::Graph::ALL);dst != edst; ++dst) {
        graph.getData(*dst);
...
#if BORUVKA_DEBUG
      std::cout << " Min edge from " << graph.getData(src).toString()
        << " to " << graph.getData(*minNeighbor).toString()
        << " " << minEdgeWeight << " " << "\n";
#endif
      //Acquire locks on neighborhood of min neighbor.
      for (Graph::neighbor_iterator 
          dst = graph.neighbor_begin(*minNeighbor, Galois::Graph::ALL),
          edst = graph.neighbor_end(*minNeighbor, Galois::Graph::ALL);
          dst != edst; ++dst) {
        graph.getData(*dst);
      ...
      for(Graph::neighbor_iterator
          mdst = graph.neighbor_begin(*minNeighbor, Galois::Graph::ALL),
          medst = graph.neighbor_end(*minNeighbor, Galois::Graph::ALL);
          mdst != medst; ++mdst){
        graph.getData(*mdst);
        int edgeWeight =
          graph.getEdgeData(*minNeighbor, *mdst, Galois::Graph::ALL);
        if(*mdst!=src){
          GNode dstNode = (*mdst);
      ...
      for(std::set<int>::iterator it = toRemove.begin(),
          endIt = toRemove.end(); it != endIt; ++it){
      ...
      for(Graph::neighbor_iterator oldMin = graph.neighbor_begin(src),
          endMin = graph.neighbor_end(src); oldMin!=endMin; ++oldMin){
...
#if BORUVKA_DEBUG
  std::cout << "Begining to process with worklist size :: " << all.size() << "\n";
...
static void makeGraph(const char* input) {
  ...
  for (InGraph::active_iterator src = in_graph.active_begin(),
      esrc = in_graph.active_end();src != esrc; ++src) {
