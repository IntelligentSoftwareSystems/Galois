...
static void setPixel(struct rgbData data[][WIDTH], int x, int y, struct rgbData color) {
  if (x < 0 || x >= WIDTH) return;
  if (y < 0 || y >= HEIGHT) return;
  data[y][x] = color;
}

static void setPixelUnsafe(struct rgbData data[][WIDTH], int x, int y, struct rgbData color) {
  data[y][x] = color;
}
...
static void drawlineUnsafe(rgbData data[][WIDTH], int x1, int y1, int x2, int y2, struct rgbData color) {
  int dx = x2 - x1;
  int dy = y2 - y1;
  int dxabs = abs(dx);
  int dyabs = abs(dy);
...
static void drawcircle(struct rgbData data[][WIDTH], int x0, int y0, int radius, rgbData color) {
  int f = 1 - radius;
  int ddF_x = 1;
  int ddF_y = -2 * radius;
  int x = 0;
