...
//    super();
  ...
  KdTree(int inSplitType, float inSplitValue):KdCell(inSplitType, inSplitValue){
  }
...
//  @Override
  ...
  bool notifyContentsRebuilt(bool changed) {
    ...
    if (splitType == LEAF) {
      ...
      float newMinHX = numeric_limits<float>::max(), newMinHY = numeric_limits<float>::max(), newMinHZ = numeric_limits<float>::max();
      for (unsigned int i=0; i<pointList->size();i++){
    	  NodeWrapper *aPointList = (*pointList)[i];
              if (aPointList == NULL) {
          continue;
  ...
  bool isOkay() {
    ...
    if (splitType == LEAF) {
      for (unsigned int i=0;i<pointList->size();i++) {
        ...
        minLight = std::min(minLight, aPointList->light->getScalarTotalIntensity());
...
//    	throw new IllegalStateException("bad min light intensity");
    	std::cout<<"bad min light intensity"<<std::endl;
    }
    if (maxCos != this->maxConeCos) {
//        throw new IllegalStateException("bad max cone cos");
    	std::cout<<"bad max cone cos"<<std::endl;
    }
...
//        throw new IllegalStateException("bad min half size");
    	std::cout<<"bad min half size"<<std::endl;
    }
    return true;
  }
};
