...
public:
  ...
  virtual void aggregate(Graph* g, size_t dim, const tensor_t& in_feats,
                         tensor_t& out_feats) {}

  // user-defined combine function
  virtual void combine(const vec_t ma, const vec_t mb, const vec_t& a,
                       const vec_t& b, vec_t& out) {}
  ...
  void init() {
    ...
    if (dataset == "reddit") {
      ...
      train_count =
          read_masks(dataset, "train", train_begin, train_end, train_mask);
    ...
    feature_dims[0] =
        read_features(dataset, input_features); // input feature dimension: D
  ...
  void append_conv_layer(size_t layer_id, bool act = false, bool norm = true,
    ...
    layers[layer_id] = new graph_conv_layer(layer_id, &g, act, norm, bias,
                                            dropout, in_dims, out_dims);
    if (layer_id > 0)
  ...
  void append_out_layer(size_t layer_id) {
    ...
    out_dims[1]              = get_out_dim(layer_id);
    layers[layer_id] =
        new softmax_loss_layer(layer_id, in_dims, out_dims, &labels);
    connect(layers[layer_id - 1], layers[layer_id]);
  }
  ...
  double evaluate(size_t begin, size_t end, size_t count, MaskList& masks,
  ...
  void train(optimizer* opt) {
    ...
    for (size_t i = 0; i < epochs; i++) {
      ...
      Tfw.start();
      train_loss =
          fprop(train_begin, train_end, train_count, train_mask); // forward
      ...
      if (do_validate) {
        ...
        Tval.start();
        double val_time = evaluate(val_begin, val_end, val_count, val_mask,
                                   val_loss, val_acc);
        Tval.stop();
...
protected:
  ...
  // (num_examples x 1). Note that labels is not one-hot encoded vector and it
  // can be computed as y.argmax(axis=1) from one-hot encoded vector (y) of
  // labels if required.
  size_t read_labels(std::string dataset_str, LabelList& labels) {
    ...
    std::cout << "Done, unique label counts: " << n
  ...
  size_t read_features(std::string dataset_str, tensor_t& feats) {
    ...
    for (size_t i = 0; i < 10; ++i)
        for (size_t j = 0; j < n; ++j)
            if (feats[i][j] > 0)
                std::cout << "feats[" << i << "][" << j << "]: " << feats[i][j]
    << std::endl;
    //*/
    in.close();
    t_read.Stop();
    std::cout << "Done, feature dimention: " << n
              << ", time: " << t_read.Millisecs() << " ms\n";
    return n;
  }
  ...
  unsigned read_graph(std::string dataset_str, Graph& graph) {
    ...
    if (filetype == "el") {
      ...
      /*
      galois::do_all(galois::iterate(graph.begin(), graph.end()), [&](const
      auto& vid) { graph.getData(vid) = 1;
          //for (auto e : graph.edges(n)) graph.getEdgeData(e) = 1;
      }, galois::chunk_size<256>(), galois::steal(),
      galois::loopname("assignVertexLabels")); std::vector<unsigned>
      degrees(graph.size()); galois::do_all(galois::iterate(graph.begin(),
      graph.end()), [&](const auto& vid) { degrees[vid] =
  ...
  void genGraph(LGraph& lg, Graph& g) {
    ...
    for (size_t i = 0; i < lg.num_vertices(); i++) {
      ...
      for (auto offset = row_begin; offset < row_end; offset++)
        g.constructEdge(offset, lg.get_dest(offset),
                        0); // do not consider edge labels now
    }
  }
    ...
    galois::do_all(
        ...
        [&](const auto& i) {
          if (masks[i] == 1) {
            int prediction =
                argmax(num_classes,
                       layers[NUM_CONV_LAYERS - 1]->next()->get_data()[i]);
            if ((label_t)prediction == labels[i])
              accuracy_all += 1.0;
          }
        },
        galois::chunk_size<256>(), galois::steal(),
