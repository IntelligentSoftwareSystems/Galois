...
inline void agg_func(FV &in, FV &out) {
...
void aggregate(Graph &g, VertexID src, FV2D &features, FV &sum) {
	unsigned degree = 0;
...
void combine(FV2D &W, FV2D &Q, FV &a, FV &b, FV &out) {
	size_t m = out.size();
	FV c(m, 0);
	FV d(m, 0);
	mvmul(Q, a, c);
...
// forward propogation
void forward(Graph &g, FV2D &h_in, FV2D &h_hidden1, FV2D &h_out, FV3D W, FV3D Q, LabelList labels, MaskList masks, AccT &loss, AccT &accuracy) {
	std::cout << "\n[debug] forward\n";
	...
	for (size_t i = 0; i < n; i ++) predictions[i] = argmax(n, h_out[i]);
	accuracy = masked_accuracy(predictions, labels, masks);
}

// evaluate, i.e. inference or predict
void evaluate(Graph &g, FV2D &h_in, FV3D W, FV3D Q, LabelList labels, MaskList masks, AccT &loss, AccT &acc) {
...
int main(int argc, char** argv) {
	...
	for (size_t i = 0; i < epochs; i++) {
		...
		// Validation
		AccT val_cost = 0, val_acc = 0;
		//Timer t_eval;
		//t_eval.Start();
		//evaluate(graph, input_features, W, Q, y_val, val_mask, cost, acc);
		//t_eval.Stop();
		//t_eval.Millisecs();
