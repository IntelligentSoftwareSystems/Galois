...
public:
  ...
  virtual void forward_propagation(const tensor_t& in_data,
                                   tensor_t& out_data) = 0;
  ...
  virtual void set_sample_mask(size_t sample_begin, size_t sample_end,
                               size_t sample_count, MaskList& masks) {
    begin_ = sample_begin;
    end_   = sample_end;
    count_ = sample_count;
    masks_ = masks;
  ...
  void set_in_data(tensor_t data) {
    ...
    // std::cout << "l0 in_grad alloc: x=" << output_dims[0] << ", y=" <<
    // output_dims[1] << "\n";
  ...
  void alloc_grad() {
    // allocate memory for intermediate gradients
    // std::cout << "l" << level_ << " out_grad alloc: x=" << output_dims[0] <<
    // ", y=" << output_dims[1] << "\n";
