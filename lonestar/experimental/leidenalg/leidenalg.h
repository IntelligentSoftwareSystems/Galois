...
constexpr static const double INF_VAL_DOUBLE =
...
void setRandomness(double rdm) { randomness = rdm; }
...
void sumClusterWeight(Graph& graph, CommArray& c_info) {
  ...
  galois::do_all(
      ...
      [&](GNode n) {
        auto& n_data = graph.getData(n);
        galois::atomicAdd(c_info[n_data.curr_subcomm_ass].degree_wt,
                          n_data.degree_wt);
      },
      galois::steal());
}
...
void setConstant(Graph& graph) { constant = calConstantForSecondTerm(graph); }

double diffModQuality(uint64_t curr_subcomm, uint64_t candidate_subcomm,
                      std::map<uint64_t, uint64_t>& cluster_local_map,
                      std::vector<uint64_t>& counter, CommArray& subcomm_info,
                      uint64_t self_loop_wt, uint64_t degree_wt) {

  double ax = subcomm_info[curr_subcomm].degree_wt - degree_wt;
  double ay = subcomm_info[candidate_subcomm].degree_wt;

  double diff =
      2.0f * (double)(counter[cluster_local_map[candidate_subcomm]] -
                      counter[cluster_local_map[curr_subcomm]] + self_loop_wt) +
      2.0f * (double)degree_wt * constant * (double)(ax - ay);
...
uint64_t maxModularity(std::map<uint64_t, uint64_t>& cluster_local_map,
                       std::vector<uint64_t>& counter, uint64_t self_loop_wt,
                       // std::vector<Comm>&c_info, uint64_t degree_wt, uint64_t
                       // sc, double constant) {
  ...
  do {
    if (sc != stored_already->first) {
      ay = c_info[stored_already->first]
               .degree_wt.load(); // Degree wt of cluster y
      eiy =
...
maxModularityWithoutSwaps(std::map<uint64_t, uint64_t>& cluster_local_map,
                          std::vector<uint64_t>& counter, uint64_t self_loop_wt,
                          // std::vector<Comm>&c_info, uint64_t degree_wt,
                          // uint64_t sc, double constant) {
                          CommArray& c_info, uint64_t degree_wt, uint64_t sc) {
...
double calModularityDelay(Graph& graph, CommArray& c_info, CommArray& c_update,
                          double& e_xx, double& a2_x,
                          double& constant_for_second_term,
                          std::vector<GNode>& local_target) {
...
double calModularity(Graph& graph, CommArray& c_info, double& e_xx,
...
double calModularityFinal(Graph& graph) {
  ...
  galois::do_all(
      ...
      [&](GNode n) {
        auto n_data = graph.getData(n);
        for (auto ii = graph.edge_begin(n); ii != graph.edge_end(n); ++ii) {
          if (graph.getData(graph.getEdgeDst(ii)).curr_subcomm_ass ==
              n_data.curr_subcomm_ass) {
  ...
  galois::do_all(
      ...
      [&](GNode n) {
        acc_e_xx += cluster_wt_internal[n];
        acc_a2_x += (double)(c_info[n].degree_wt.load()) *
                    ((double)(c_info[n].degree_wt.load()) *
                     (double)constant_for_second_term);
      },
      galois::steal());
...
double diffCPMQuality(uint64_t curr_subcomm, uint64_t candidate_subcomm,
                      std::map<uint64_t, uint64_t>& cluster_local_map,
                      std::vector<uint64_t>& counter, CommArray& subcomm_info,
                      uint64_t self_loop_wt, uint64_t flatSize) {
  ...
  double diff1 =
      2.0f * (double)(counter[cluster_local_map[candidate_subcomm]] -
                      counter[cluster_local_map[curr_subcomm]] + self_loop_wt);
  double diff2 = resolution * 0.5f *
                 (double)((size_x * (size_x - 1) + size_y * (size_y - 1)) -
                          ((new_size_x) * (new_size_x - 1) +
                           new_size_y * (new_size_y - 1)));
  ...
  do {
    if (sc != stored_already->first) {
      ...
      cur_gain = 2.0f * (double)(eiy - eix) +
                 0.5f * resolution *
                     ((double)(size_x * (size_x - 1) + size_y * (size_y - 1)) -
                      (double)((new_size_x) * (new_size_x - 1) +
                               (new_size_y) * (new_size_y - 1)));
...
uint64_t
maxCPMQualityWithoutSwaps(std::map<uint64_t, uint64_t>& cluster_local_map,
                          std::vector<uint64_t>& counter, uint64_t self_loop_wt,
                          // std::vector<Comm>&c_info, uint64_t degree_wt,
                          // uint64_t sc, double constant) {
                          CommArray& c_info, uint64_t degree_wt, uint64_t sc,
  ...
  do {
    if (sc != stored_already->first) {
      //  ay = c_info[stored_already->first].degree_wt; // Degree wt of cluster
      //  y
      ...
      if ((cur_gain > max_gain) ||
          ((cur_gain == max_gain) && (cur_gain != 0) &&
           ((int64_t)stored_already->first > max_index))) {
        max_gain  = cur_gain;
        max_index = stored_already->first;
      }
    }
...
double calCPMQuality(Graph& graph, CommArray& c_info, double& e_xx,
                     double& a2_x, double resolution) {
  ...
  galois::do_all(
      ...
      [&](GNode n) {
        acc_e_xx += cluster_wt_internal[n];
        acc_a2_x += (double)(c_info[n].flatSize) *
                    ((double)(c_info[n].flatSize - 1)) * 0.5f;
      },
      galois::steal());
...
double calCPMQualityFinal(Graph& graph) {
  ...
  galois::do_all(
      ...
      [&](GNode n) {
        ...
        galois::atomicAdd(c_info[n_data.curr_subcomm_ass].flatSize,
                          n_data.flatSize);
      },
      galois::steal());
