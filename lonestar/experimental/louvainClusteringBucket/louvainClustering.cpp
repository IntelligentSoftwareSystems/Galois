...
enum Algo { coloring, foreach, delay };
...
double algoLouvainWithLockingDelayUpdateAndNoAtomics(Graph& graph, double lower,
                                                     double threshold) {
...
galois::atomicAdd(c_update[local_target[n]].size, (uint64_t)1);
galois::atomicSubtract(c_update[n_data.curr_comm_ass].degree_wt,
n_data.degree_wt); galois::atomicSubtract(c_update[n_data.curr_comm_ass].size,
(uint64_t)1);
            */
            /* Set the new cluster id */
            // n_data.curr_comm_ass = local_target;
        ...
        [&](galois::InsertBag<GNode>& b) {
          if (b.begin() != b.end()) {
            ...
            for (auto n : b) {
              ...
              c_update[graph.getData(n).curr_comm_ass].degree_wt -=
...
void buildNextLevelGraph(Graph& graph, Graph& graph_next,
  ...
  galois::do_all(
      ...
      galois::steal(), galois::loopname("Find edges"));
...
// void runMultiPhaseLouvainAlgorithm(Graph& graph, largeArray& clusters_orig,
// uint64_t min_graph_size, double c_threshold) {
  ...
  while (true) {
    ...
    if ((*graph_curr).size() > min_graph_size) {
      ...
      case foreach:
        ...
        break;
      case delay:
        //          curr_mod = algoLouvainWithLockingDelayUpdate(*graph_curr,
        //          curr_mod, c_threshold);
        curr_mod = algoLouvainWithLockingDelayUpdateAndNoAtomics(
            *graph_curr, curr_mod, c_threshold);
        break;
      default:
        std::abort();
      }
    ...
    if (iter < max_iter && (curr_mod - prev_mod) > threshold) {
      ...
      // uint64_t num_unique_clusters =
      // renumberClustersContiguously(*graph_curr); galois::gPrint("Number of
      // unique clusters (renumber): ", num_unique_clusters, "\n");
      // buildNextLevelGraph(*graph_curr, graph_next, num_unique_clusters);
      break;
    }
  }
