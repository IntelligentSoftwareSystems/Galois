...
//! @return pair<rep, other> where rep is representative component of the
//! unioned components
...
void minToFront(KNode_tp& node) {
  ...
  typename KNode_tp::EdgeListTy::iterator min =
      std::min_element(node.edges.begin(), node.edges.end(),
...
size_t getMaxAdjSize() { return maxAdjSize; }
...
void merge(KNode_tp& master, KNode_tp& other) {
  ...
  if (sorted_tp) {
    ...
    self_begin = std::stable_partition(master.edges.begin(), master.edges.end(),
                                       NotSelfEdgeRelax<KNode_tp>());
    // relaxed version calls findROrelax because
    // results of findRO may be stale due to union
    // occuring in parallel.
  } else {
    self_begin = std::partition(master.edges.begin(), master.edges.end(),
                                NotSelfEdgeRelax<KNode_tp>());
  }

  // now move the rest of the range to other
  other.edges.splice(other.edges.begin(), master.edges, self_begin,
                     master.edges.end());
...
void _internalMergeCost(KNode_tp* rep, size_t& mergeCost, size_t& findROcost) {
  checkIsRep(rep);
  for (typename KNode_tp::EdgeListTy::iterator i  = rep->edges.begin(),
                                               ei = rep->edges.end();
       i != ei; ++i) {
    ++mergeCost;
...
template <typename KNode_tp>
void measureMergeCost(KEdge<KNode_tp>& edge, size_t& mergeCost,
                      size_t& findROcost) {
