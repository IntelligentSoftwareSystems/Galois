...
private:
  ...
  struct PrimUpdate {
    ...
    PrimUpdate (PrimNode* dst, size_t weight)
  ...
  static unsigned getID (KNode_tp* nd) {
    return nd->id;
  }
  ...
  size_t runPrim (const VecKNode_ty& nodes, const Edges_ty& edges) const {
    ...
    for (typename VecKNode_ty::const_iterator n = nodes.begin (), endn = nodes.end ();
        n != endn; ++n) {
    ...
    while (!workset.empty ()) {
      ...
      if (!src.inMST) {
        ...
        for (typename PrimNode::Adj_iterator_ty i = src.adj_begin (), endi = src.adj_end (); i != endi; ++i) {
