...
void FsmSolver(EdgeMiner& miner) {
  ...
  while (1) {
    ...
    // Sub-step 1: aggregate on quick patterns: gather embeddings into different
    // quick patterns
    miner.quick_aggregate(queue);
    miner.merge_qp_map(level + 1);
    // Sub-step 2: aggregate on canonical patterns: gather quick patterns into
    // different canonical patterns
    ...
    if (show)
      filtered_queue.printout_embeddings(level, debug);
  }
...
int main(int argc, char** argv) {
  ...
  Tinit.stop();
  std::cout << "num_vertices " << graph.size() << " num_edges "
            << graph.sizeEdges() << "\n";
