...
static cll::opt<Algo> algo(
    ...
    cll::values(clEnumValN(Algo::coloring, "Coloring",
                ...
                clEnumValN(Algo::doAll, "Doall",
                           "Using galois do_all and using tie-breaking via "
                           "degree wt for conflict mitigation"),
                clEnumValEnd),
...
double algoLouvainWithoutLocking(Graph& graph, double lower, double threshold,
                                 uint32_t& iter) {
...
void runMultiPhaseLouvainAlgorithm(Graph& graph, uint64_t min_graph_size,
  ...
  while (true) {
    ...
    if ((*graph_curr).size() > min_graph_size) {
      ...
      case doAll:
        curr_mod =
            algoLouvainWithoutLocking(*graph_curr, curr_mod, c_threshold, iter);
        break;
      case delay:
    ...
    if (iter < max_iter && (curr_mod - prev_mod) > threshold) {
      ...
      // TODO: if(iter > 1) graph_curr->deallocate(); // deallocate the previous
      // graph except the input graph Can allocate the input graph as well and
      // store communities in a largeArray
      prev_mod   = curr_mod;
