...
const char* name = "Kcl";
const char* desc = "Counts the K-Cliques in a graph using DFS traversal (using "
                   "DAG and shrinking graph)";
const char* url = 0;
...
public:
  ...
  bool toExtend(unsigned level, unsigned pos) { return pos == level; }
  ...
  void edge_process_opt() {
    ...
    assert(max_size > 3);
    // galois::for_each(galois::iterate(edge_list), [&](const Edge &edge, auto
    // &ctx) {
    galois::do_all(
    ...
    //}, galois::chunk_size<CHUNK_SIZE>(), galois::steal(),
    //galois::no_conflicts(), galois::loopname("DfsEdgeSolver"));
  }
};
