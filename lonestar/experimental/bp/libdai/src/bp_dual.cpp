...
void BP_dual::calcMessages() {
  ...
  for (size_t I = 0; I < fg().nrFactors(); I++) {
    ...
    diaforeach(const Neighbor& i, fg().nbF(I)) msgN(i, i.dual) =
        f.marginal(fg().var(i)).p();
  }
  // calculate 'm' messages and normalizers from 'n' messages
  for (size_t i = 0; i < fg().nrVars(); i++)
    diaforeach(const Neighbor& I, fg().nbV(i)) calcNewM(i, I.iter);
  // recalculate 'n' messages and normalizers from 'm' messages
  for (size_t i = 0; i < fg().nrVars(); i++)
    diaforeach(const Neighbor& I, fg().nbV(i)) calcNewN(i, I.iter);
}
...
void BP_dual::calcNewM(size_t i, size_t _I) {
  ...
  diaforeach(const Neighbor& j, fg().nbF(I)) if (j != i) { // for all j in I \ i
  ...
  // ind is the precalculated Index(i,I) i.e. to x_I == k corresponds x_i ==
  // ind[k]
...
void BP_dual::calcNewN(size_t i, size_t _I) {
  ...
  diaforeach(const Neighbor& J,
             fg().nbV(i)) if (J.node != I.node) // for all J in i \ I
      prod *= msgM(i, J.iter);
...
void BP_dual::calcBeliefV(size_t i) {
  Prob prod(fg().var(i).states(), 1.0);
  diaforeach(const Neighbor& I, fg().nbV(i)) prod *= msgM(i, I.iter);
