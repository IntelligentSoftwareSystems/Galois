...
size_t getFactorEntryForState(const FactorGraph& fg, size_t I,
...
Real BBPCostFunction::evaluate(const InfAlg& ia,
                               const vector<size_t>* stateP) const {
  Real cf               = 0.0;
...
void BBP::RegenerateT() {
  ...
  for (size_t i = 0; i < _fg->nrVars(); i++) {
    ...
    diaforeach(const Neighbor& I, _fg->nbV(i)) {
      ...
      diaforeach(const Neighbor& J, _fg->nbV(i)) if (J.node != I.node) prod *=
          _bp_dual.msgM(i, J.iter);
      _Tmsg[i][I.iter] = prod;
    }
  }
}
...
void BBP::RegenerateU() {
  ...
  for (size_t I = 0; I < _fg->nrFactors(); I++) {
    ...
    diaforeach(const Neighbor& i, _fg->nbF(I)) {
      ...
      diaforeach(const Neighbor& j, _fg->nbF(I)) if (i.node != j.node) {
...
void BBP::RegenerateS() {
  ...
  for (size_t i = 0; i < _fg->nrVars(); i++) {
    ...
    diaforeach(const Neighbor& I, _fg->nbV(i)) {
      ...
      diaforeach(const Neighbor& j, _fg->nbF(I)) if (i != j) {
  ...
  for (size_t I = 0; I < _fg->nrFactors(); I++) {
    ...
    diaforeach(const Neighbor& i, _fg->nbF(I)) {
      ...
      diaforeach(const Neighbor& J, _fg->nbV(i)) {
        if (I != J) {
          Prob prod(_fg->var(i).states(), 1.0);
          diaforeach(const Neighbor& K,
                     _fg->nbV(i)) if (K.node != I && K.node != J.node) prod *=
              _bp_dual.msgM(i, K.iter);
...
void BBP::RegenerateInputs() {
  ...
  for (size_t i = 0; i < _fg->nrVars(); i++)
    _adj_b_V_unnorm.push_back(unnormAdjoint(_bp_dual.beliefV(i).p(),
                                            _bp_dual.beliefVZ(i), _adj_b_V[i]));
  _adj_b_F_unnorm.clear();
  _adj_b_F_unnorm.reserve(_fg->nrFactors());
  for (size_t I = 0; I < _fg->nrFactors(); I++)
    _adj_b_F_unnorm.push_back(unnormAdjoint(_bp_dual.beliefF(I).p(),
...
void BBP::RegeneratePsiAdjoints() {
  ...
  for (size_t i = 0; i < _fg->nrVars(); i++) {
    ...
    diaforeach(const Neighbor& I, _fg->nbV(i)) p *= _bp_dual.msgM(i, I.iter);
    p += _init_adj_psi_V[i];
...
void BBP::calcNewN(size_t i, size_t _I) {
  ...
  diaforeach(const Neighbor& j, _fg->nbF(I)) if (j != i) {
    const Prob& p                = _Smsg[i][_I][j.iter];
    const Prob& _adj_m_unnorm_jI = _adj_m_unnorm[j][j.dual];
    LOOP_ij(new_adj_n_iI.set(xi, new_adj_n_iI[xi] +
                                     p[xij] * _adj_m_unnorm_jI[xj]););
    /* THE FOLLOWING WOULD BE ABOUT TWICE AS SLOW:
    Var vi = _fg->var(i);
    Var vj = _fg->var(j);
    new_adj_n_iI = (Factor(VarSet(vi, vj), p) *
    Factor(vj,_adj_m_unnorm_jI)).marginal(vi,false).p();
    */
  }
}
...
void BBP::calcNewM(size_t i, size_t _I) {
  ...
  _adj_psi_F[I] += (Factor( _fg->factor(I).vars(), U(I, I.dual) ) * Factor(
  _fg->var(i), _adj_m_unnorm[i][_I] )).p();
  */

  _new_adj_m[i][_I] = Prob(_fg->var(i).states(), 0.0);
  diaforeach(const Neighbor& J, _fg->nbV(i)) if (J != I) _new_adj_m[i][_I] +=
      _Rmsg[I][I.dual][J.iter] * _adj_n_unnorm[i][J.iter];
}

void BBP::calcUnnormMsgN(size_t i, size_t _I) {
  _adj_n_unnorm[i][_I] =
      unnormAdjoint(_bp_dual.msgN(i, _I), _bp_dual.zN(i, _I), _adj_n[i][_I]);
}

void BBP::calcUnnormMsgM(size_t i, size_t _I) {
  _adj_m_unnorm[i][_I] =
      unnormAdjoint(_bp_dual.msgM(i, _I), _bp_dual.zM(i, _I), _adj_m[i][_I]);
}
...
void BBP::sendSeqMsgM(size_t j, size_t _I) {
  ...
  /* THE FOLLOWING WOULD BE SLIGHTLY SLOWER:
  _adj_psi_F[I] += (Factor( _fg->factor(I).vars(), U(I, _j) ) * Factor(
  _fg->var(j), _adj_m_unnorm[j][_I] )).p() * (1.0 - props.damping);
  */
  ...
  diaforeach(const Neighbor& i, _fg->nbF(I)) {
    if (i.node != j) {
      ...
      LOOP_ij(msg.set(xi, msg[xi] + S[xij] * _adj_m_unnorm_jI[xj]););
      msg *= 1.0 - props.damping;
      /* THE FOLLOWING WOULD BE ABOUT TWICE AS SLOW:
      Var vi = _fg->var(i);
      Var vj = _fg->var(j);
      msg = (Factor(VarSet(vi,vj), S) *
      Factor(vj,_adj_m_unnorm_jI)).marginal(vi,false).p() * (1.0 -
      props.damping);
      */
#if 0
...
Real BBP::getTotalMsgM() {
  Real mag = 0.0;
  for (size_t i = 0; i < _fg->nrVars(); i++)
    diaforeach(const Neighbor& I, _fg->nbV(i)) mag +=
        _adj_m[i][I.iter].sumAbs();
  return mag;
}
...
void BBP::initCostFnAdj(const BBPCostFunction& cfn,
                        const vector<size_t>* stateP) {
...
void BBP::run() {
  ...
  case UT::SEQ_FIX: {
    ...
    do {
      ...
      for (size_t i = 0; i < _fg->nrVars(); i++)
        diaforeach(const Neighbor& I, _fg->nbV(i)) sendSeqMsgM(i, I.iter);
      /*                for( size_t i = 0; i < _fg->nrVars(); i++ )
                          diaforeach( const Neighbor &I, _fg->nbV(i) )
    ...
    if (_iters >= props.maxiter)
      if (props.verbose >= 1)
        cerr << "Warning: BBP updates limited to " << props.maxiter
             << " iterations, but using UpdateType " << updates << " with "
             << totalMessages << " messages" << endl;
    break;
  }
  case UT::PAR: {
    do {
    ...
    if (_iters == props.maxiter) {
      ...
      if (props.verbose >= 1)
        cerr << "Warning: BBP didn't converge in " << _iters
             ...
             << ", norm message mags = " << s << " -> " << new_s << ")" << endl;
    }
    break;
  }
  }
  if (props.verbose >= 3)
    cerr << "BBP::run() took " << toc() - tic << " seconds " << Iterations()
         << " iterations" << endl;
}
  ...
  if (1) {
    ...
    for (size_t i = 0; i < fg.nrVars(); i++) {
      ...
      for (size_t xi = 0; xi < fg.var(i).states(); xi++) {
        ...
        //                 psi_1_prb.normalize();
        size_t I = bp_prb->fg().nbV(
            i)[0]; // use first factor in list of neighbors of i
        Factor tmp =
  ...
  /*    if(1) {
      ...
      for(size_t i=0; i<bp_dual.nrVars(); i++) {
          ...
          diaforeach(size_t I, bp_dual.nbV(i)) {
              ...
              // compare this numerical estimate to the BBP estimate; sum the
  distances cerr << "i: " << i << ", I: " << I
...
void BBP::Properties::set(const PropertySet& opts) {
  ...
  if (!opts.hasKey("maxiter"))
    errormsg =
        errormsg + "BBP: Missing property \"maxiter\" for method \"BBP\"\n";
  if (!opts.hasKey("tol"))
    errormsg = errormsg + "BBP: Missing property \"tol\" for method \"BBP\"\n";
  if (!opts.hasKey("damping"))
    errormsg =
        errormsg + "BBP: Missing property \"damping\" for method \"BBP\"\n";
  if (!opts.hasKey("updates"))
    errormsg =
        errormsg + "BBP: Missing property \"updates\" for method \"BBP\"\n";
  if (!errormsg.empty())
