...
GraphAL& GraphAL::addEdge(size_t n1, size_t n2, bool check) {
  ...
  if (check) {
    ...
    diaforeach(const Neighbor& n, nb(n1)) if (n == n2) {
      exists = true;
      break;
    }
  }
...
SmallSet<size_t> GraphAL::nbSet(size_t n) const {
  ...
  diaforeach(const Neighbor& m, nb(n)) result |= m;
  return result;
}
...
bool GraphAL::isConnected() const {
  if (nrNodes() == 0) {
    ...
    /*  using namespace boost;
       ...
       property<vertex_distance_t, int> > boostGraphAL; typedef pair<size_t,
       size_t> E;
  ...
  if (nrNodes() == 0)
    ...
    do {
      ...
      for (size_t e = 0; e < prevLevel.size(); e++) {
        size_t n2 =
            prevLevel[e].first; // for all nodes n2 in the previous level
...
void GraphAL::printDot(std::ostream& os) const {
  ...
  for (size_t n = 0; n < nrNodes(); n++)
    os << "\tx" << n << ";" << endl;
  for (size_t n1 = 0; n1 < nrNodes(); n1++)
    diaforeach(const Neighbor& n2, nb(n1)) if (n1 < n2) os
        << "\tx" << n1 << " -- x" << n2 << ";" << endl;
  os << "}" << endl;
}
  ...
  for (size_t i1 = 0; i1 < N1; i1++)
    for (size_t i2 = 0; i2 < N2; i2++)
      for (size_t i3 = 0; i3 < N3; i3++) {
        if (i1 + 1 < N1 || periodic)
          result.addEdge(i1 * N2 * N3 + i2 * N3 + i3,
                         ((i1 + 1) % N1) * N2 * N3 + i2 * N3 + i3, N1 <= 2);
        if (i2 + 1 < N2 || periodic)
          result.addEdge(i1 * N2 * N3 + i2 * N3 + i3,
                         i1 * N2 * N3 + ((i2 + 1) % N2) * N3 + i3, N2 <= 2);
        if (i3 + 1 < N3 || periodic)
          result.addEdge(i1 * N2 * N3 + i2 * N3 + i3,
                         i1 * N2 * N3 + i2 * N3 + ((i3 + 1) % N3), N3 <= 2);
      }
  return result;
}
