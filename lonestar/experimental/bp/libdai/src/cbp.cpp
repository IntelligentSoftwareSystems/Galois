...
/// Given a sorted vector of states \a xis and total state count \a n_states,
/// return a vector of states not in \a xis
...
/// Computes log of sum of exponents, i.e., \f$\log\left(\exp(a) +
/// \exp(b)\right)\f$
...
/// Compute sum of pairwise L-infinity distances of the first \a nv factors in
/// each vector
...
void CBP::construct() {
  ...
  if (props.clamp_outfile.length() > 0) {
    _clamp_ofstream = shared_ptr<ofstream>(new ofstream(
        props.clamp_outfile.c_str(), ios_base::out | ios_base::trunc));
    *_clamp_ofstream << "# COUNT LEVEL VAR STATE" << endl;
  }
}

/// Calculates a vector of mixtures p * b + (1-p) * c
static vector<Factor> mixBeliefs(Real p, const vector<Factor>& b,
...
Real CBP::run() {
  ...
  runRecurse(bp, bp->logZ(), vector<size_t>(0), _num_leaves, choose_count,
             _sum_level, lz_out, beliefs_out);
  if (props.verbose >= 1)
    cerr << "CBP average levels = " << (_sum_level / _num_leaves)
         << ", leaves = " << _num_leaves << endl;
...
void CBP::runRecurse(InfAlg* bp, Real orig_logZ,
                     vector<size_t> clamped_vars_list, size_t& num_leaves,
  ...
  if (props.recursion == Properties::RecurseType::REC_LOGZ &&
      props.rec_tol > 0 && exp(bp->logZ() - orig_logZ) < props.rec_tol)
    found = false;
  else
  ...
  choose_count++;
  if (props.clamp_outfile.length() > 0)
    *_clamp_ofstream << choose_count << "\t" << clamped_vars_list.size() << "\t"
                     << i << "\t" << xis[0] << endl;

  if (clampingVar)
    diaforeach(size_t xi, xis) DAI_ASSERT(/*0<=xi &&*/ xi < var(i).states());
  else
  ...
  vector<size_t> cmp_xis =
      complement(xis, clampingVar ? var(i).states() : factor(i).nrStates());

  /// \idea dai::CBP::runRecurse() could be implemented more efficiently with a
  /// nesting version of backupFactors/restoreFactors
  // this improvement could also be done locally: backup the clamped factor in a
  // local variable, and restore it just before we return.
  Real lz;
  vector<Factor> b;
  ...
  // enough to recurse:
  runRecurse(bp_c, orig_logZ, clamped_vars_list, num_leaves, choose_count,
             sum_level, lz, b);
  runRecurse(cmp_bp_c, orig_logZ, clamped_vars_list, num_leaves, choose_count,
             sum_level, cmp_lz, cmp_b);
  ...
  if (props.verbose >= 2) {
    ...
    cerr << "; max_adjoint = " << maxVar << "; logZ = " << lz_out << " (in "
         << bp->logZ() << ") (orig " << orig_logZ << "); p = " << p
         << "; level = " << clamped_vars_list.size() << endl;
  }
...
// 'xis' must be sorted
bool CBP::chooseNextClampVar(InfAlg* bp, vector<size_t>& clamped_vars_list,
  ...
  if (props.choose == Properties::ChooseMethodType::CHOOSE_RANDOM) {
    if (props.clamp == Properties::ClampType::CLAMP_VAR) {
      ...
      xis.resize(1, xi);
      // DAI_ASSERT(!_clamped_vars.count(i)); // not true for >2-ary variables
      DAI_IFVERB(2, endl << "CHOOSE_RANDOM chose factor " << i << " state "
                         << xis[0] << endl);
    }
    ...
    if (props.clamp == Properties::ClampType::CLAMP_VAR) {
      ...
      i = win_k;
      xis.resize(1, win_xk);
      DAI_IFVERB(2, endl << "CHOOSE_MAXENT chose variable " << i << " state "
                         << xis[0] << endl);
      if (bp->beliefV(i).p()[xis[0]] < tiny) {
        DAI_IFVERB(
            2, "Warning: CHOOSE_MAXENT found unlikely state, not recursing");
        return false;
      }
    } else {
      Real max_ent = -1.0;
      ...
      i = win_k;
      xis.resize(1, win_xk);
      DAI_IFVERB(2, endl << "CHOOSE_MAXENT chose factor " << i << " state "
                         << xis[0] << endl);
    ...
    pair<size_t, size_t> cv =
        BBPFindClampVar(*bp, clampingVar, props.bbp_props, props.bbp_cfn, &mvo);
    ...
    i         = cv.first;
#define VAR_INFO                                                               \
  (clampingVar ? "variable " : "factor ")                                      \
      << i << " state " << xi                                                  \
      << " (p=" << (clampingVar ? bp->beliefV(i)[xi] : bp->beliefF(i)[xi])     \
      << ", entropy = "                                                        \
      << (clampingVar ? bp->beliefV(i) : bp->beliefF(i)).entropy()             \
      << ", maxVar = " << mvo << ")"
    Prob b = (clampingVar ? bp->beliefV(i).p() : bp->beliefF(i).p());
    if (b[xi] < tiny) {
      cerr << "Warning, at level " << clamped_vars_list.size()
           << ", BBPFindClampVar found unlikely " << VAR_INFO << endl;
      return false;
    }
    if (abs(b[xi] - 1) < tiny) {
      cerr << "Warning at level " << clamped_vars_list.size()
           << ", BBPFindClampVar found overly likely " << VAR_INFO << endl;
      return false;
    }
...
std::pair<size_t, size_t> BBPFindClampVar(const InfAlg& in_bp, bool clampingVar,
  ...
  if (clampingVar) {
    for (size_t i = 0; i < in_bp.fg().nrVars(); i++) {
      ...
      if (0) {
        //                 adj_psi_V *=
        //                 Prob(in_bp.fg().var(i).states(),1.0)-in_bp.beliefV(i).p();
        adj_psi_V *= in_bp.beliefV(i).p();
      }
      // try to compensate for effect on same variable (doesn't work)
      //     adj_psi_V[gibbs.state()[i]] -=
      //     bp_dual.beliefV(i)[gibbs.state()[i]]/10;
...
void CBP::Properties::set(const PropertySet& opts) {
  ...
  std::string errormsg;
  for (std::set<PropertyKey>::const_iterator i = keys.begin(); i != keys.end();
       i++) {
    if (*i == "verbose")
      continue;
    if (*i == "tol")
  ...
  if (!opts.hasKey("updates"))
    errormsg =
        errormsg + "CBP: Missing property \"updates\" for method \"CBP\"\n";
  if (!opts.hasKey("maxiter"))
    errormsg =
        errormsg + "CBP: Missing property \"maxiter\" for method \"CBP\"\n";
  if (!opts.hasKey("rec_tol"))
    errormsg =
        errormsg + "CBP: Missing property \"rec_tol\" for method \"CBP\"\n";
  if (!opts.hasKey("min_max_adj"))
    errormsg =
        errormsg + "CBP: Missing property \"min_max_adj\" for method \"CBP\"\n";
  if (!opts.hasKey("choose"))
    errormsg =
        errormsg + "CBP: Missing property \"choose\" for method \"CBP\"\n";
  if (!opts.hasKey("recursion"))
    errormsg =
        errormsg + "CBP: Missing property \"recursion\" for method \"CBP\"\n";
  if (!opts.hasKey("clamp"))
    errormsg =
        errormsg + "CBP: Missing property \"clamp\" for method \"CBP\"\n";
  if (!opts.hasKey("bbp_props"))
    errormsg =
        errormsg + "CBP: Missing property \"bbp_props\" for method \"CBP\"\n";
  if (!opts.hasKey("bbp_cfn"))
    errormsg =
        errormsg + "CBP: Missing property \"bbp_cfn\" for method \"CBP\"\n";
  if (!errormsg.empty())
