...
BipartiteGraph& BipartiteGraph::addEdge(size_t n1, size_t n2, bool check) {
  ...
  if (check) {
    ...
    diaforeach(const Neighbor& nb2, nb1(n1)) if (nb2 == n2) {
      exists = true;
      break;
    }
  }
...
SmallSet<size_t> BipartiteGraph::nb1Set(size_t n1) const {
  ...
  diaforeach(const Neighbor& n2, nb1(n1)) result |= n2;
  return result;
}

SmallSet<size_t> BipartiteGraph::nb2Set(size_t n2) const {
  SmallSet<size_t> result;
  diaforeach(const Neighbor& n1, nb2(n2)) result |= n1;
  return result;
}
...
SmallSet<size_t> BipartiteGraph::delta1(size_t n1, bool include) const {
  ...
  diaforeach(const Neighbor& n2, nb1(n1))
      diaforeach(const Neighbor& m1, nb2(n2)) if (include || (m1 != n1))
          result |= m1;
  return result;
}
...
SmallSet<size_t> BipartiteGraph::delta2(size_t n2, bool include) const {
  ...
  SmallSet<size_t> result;
  diaforeach(const Neighbor& n1, nb2(n2))
      diaforeach(const Neighbor& m2, nb1(n1)) if (include || (m2 != n2))
          result |= m2;
  return result;
}
...
bool BipartiteGraph::isConnected() const {
  if (nrNodes1() == 0) {
    ...
    typedef adjacency_list< vecS, vecS, undirectedS, property<vertex_distance_t,
    int> > boostGraph; typedef pair<size_t, size_t> E;
    ...
    */
    ...
    do {
      ...
      // For all nodes of type 2, check if they are connected with the (growing)
      // component
      for (size_t n2 = 0; n2 < nrNodes2(); n2++)
        if (!incomponent2[n2]) {
          diaforeach(const Neighbor& n1, nb2(n2)) {
      ...
      // For all nodes of type 1, check if they are connected with the (growing)
      // component
      for (size_t n1 = 0; n1 < nrNodes1(); n1++)
        if (!incomponent1[n1]) {
          diaforeach(const Neighbor& n2, nb1(n1)) {
  ...
  if (nrNodes1() == 0)
    ...
    do {
      ...
      if (levels.size() == 0) {
        ...
        diaforeach(const Neighbor& n2, nb1(n1)) newLevel.ind2.push_back(n2);
      } else {
        const levelType& prevLevel = levels.back();
        // build newLevel.ind1
        diaforeach(size_t n2,
                   prevLevel.ind2) { // for all n2 in the previous level
          diaforeach(const Neighbor& n1,
                     nb2(n2)) { // for all neighbors n1 of n2
            if (find(prevLevel.ind1.begin(), prevLevel.ind1.end(), n1) ==
                prevLevel.ind1.end()) { // n1 not in previous level
              if (find(newLevel.ind1.begin(), newLevel.ind1.end(), n1) !=
                  newLevel.ind1.end())
        ...
        // build newLevel.ind2
        diaforeach(size_t n1, newLevel.ind1) { // for all n1 in this level
          diaforeach(const Neighbor& n2,
                     nb1(n1)) { // for all neighbors n2 of n1
            if (find(prevLevel.ind2.begin(), prevLevel.ind2.end(), n2) ==
                prevLevel.ind2.end()) { // n2 not in previous level
              if (find(newLevel.ind2.begin(), newLevel.ind2.end(), n2) !=
                  newLevel.ind2.end())
    ...
    } while (((newLevel.ind1.size() != 0) || (newLevel.ind2.size() != 0)) &&
             !foundCycle);
...
void BipartiteGraph::printDot(std::ostream& os) const {
  ...
  for (size_t n2 = 0; n2 < nrNodes2(); n2++)
    os << "\ty" << n2 << ";" << endl;
  for (size_t n1 = 0; n1 < nrNodes1(); n1++)
    diaforeach(const Neighbor& n2, nb1(n1)) os << "\tx" << n1 << " -- y" << n2
                                               << ";" << endl;
  os << "}" << endl;
}
