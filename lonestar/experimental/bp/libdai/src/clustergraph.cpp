...
ClusterGraph::ClusterGraph(const std::vector<VarSet>& cls)
    : _G(), _vars(), _clusters() {
...
ClusterGraph::ClusterGraph(const FactorGraph& fg, bool onlyMaximal)
    : _G(fg.nrVars(), 0), _vars(), _clusters() {
  // copy variables
  ...
  if (onlyMaximal) {
    for (size_t I = 0; I < fg.nrFactors(); I++)
      if (fg.isMaximal(I)) {
        ...
        diaforeach(const Neighbor& i, fg.nbF(I)) _G.addEdge(i, clind, true);
      }
  } else {
    // copy clusters
...
size_t sequentialVariableElimination::operator()(
    const ClusterGraph& cl, const std::set<size_t>& /*remainingVars*/) {
  return cl.findVar(seq.at(i++));
}

size_t
greedyVariableElimination::operator()(const ClusterGraph& cl,
                                      const std::set<size_t>& remainingVars) {
  set<size_t>::const_iterator lowest = remainingVars.end();
  size_t lowest_cost                 = -1UL;
  for (set<size_t>::const_iterator i = remainingVars.begin();
       i != remainingVars.end(); i++) {
...
size_t eliminationCost_MinWeight(const ClusterGraph& cl, size_t i) {
  ...
  size_t cost = 1;
  for (SmallSet<size_t>::const_iterator it = id_n.begin(); it != id_n.end();
       it++)
...
size_t eliminationCost_MinFill(const ClusterGraph& cl, size_t i) {
  ...
  for (SmallSet<size_t>::const_iterator it1 = id_n.begin(); it1 != id_n.end();
       it1++)
