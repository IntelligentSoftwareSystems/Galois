...
/// \todo Make DAI_BP_FAST a compile-time choice, as it is a memory/speed
/// tradeoff
...
void BP::construct() {
  ...
  for (size_t i = 0; i < nrVars(); ++i) {
    ...
    diaforeach(const Neighbor& I, nbV(i)) {
      ...
      if (props.updates == Properties::UpdateType::SEQMAX)
        _edge2lut[i].push_back(_lut.insert(
            make_pair(newEP.residual, make_pair(i, _edges[i].size() - 1))));
    }
  }
  ...
  for (size_t I = 0; I < nrFactors(); I++)
    diaforeach(const Neighbor& i, nbF(I)) _updateSeq.push_back(Edge(i, i.dual));
}
...
Real BP::run() {
  ...
  for (; _iters < props.maxiter && maxDiff > props.tol &&
    ...
    if (props.updates == Properties::UpdateType::SEQMAX) {
      if (_iters == 0) {
        // do the first pass
        for (size_t i = 0; i < nrVars(); ++i)
          diaforeach(const Neighbor& I, nbV(i)) calcNewMessage(i, I.iter);
      ...
      for (size_t i = 0; i < nrVars(); ++i)
        diaforeach(const Neighbor& I, nbV(i)) updateMessage(i, I.iter);
      ...
      for (size_t i = 0; i < nrVars(); ++i) {
        edge2handle.push_back(
            std::vector<PairingHeap<Task>::Handle>(nbV(i).size()));
        diaforeach(const Neighbor& I, nbV(i)) {
          if (_iters == 0) {
          ...
          if (J.iter != _I) {
            diaforeach(const Neighbor& j, nbF(J)) {
              ...
              if (j != i) {
                ...
                edge2handle[j][_J] =
                    heap.add(Task(j, _J, task.second.round, d));
              }
            }
          }
        }
      ...
      if (_iters == 0) {
        ...
        // for( size_t i = 0; i < nrVars(); ++i ) {
        //    diaforeach( const Neighbor &I, nbV(i) ) {
        //          Real d = -dist(newMessage(i,I.iter), message(i, I.iter),
        //          DISTLINF); Task t(i, I.iter, 0, d); initial.push_back(t);
      ...
      galois::Timer t;
      t.start();
      Exp::PriAuto<64, Indexer, OBIM, TaskLess, TaskGreater>::for_each(
          initial.begin(), initial.end(), Process(*this, edgeData, count));
      t.stop();
      GlobalTime += t.get();
    } else {
      abort();
    ...
    galois::for_each(integers.begin(), integers.end(),
                     ComputeDiffs(*this, accMaxDiff));
...
void BP::calcBeliefV(size_t i, Prob& p) const {
  ...
  diaforeach(const Neighbor& I, nbV(i)) if (props.logdomain) p +=
      newMessage(i, I.iter);
  else p *= newMessage(i, I.iter);
}
...
void BP::updateMessage(size_t i, size_t _I) {
  ...
  if (props.damping == 0.0) {
    ...
    if (props.logdomain)
      message(i, _I) = (message(i, _I) * props.damping) +
                       (newMessage(i, _I) * (1.0 - props.damping));
    else
      message(i, _I) = (message(i, _I) ^ props.damping) *
                       (newMessage(i, _I) ^ (1.0 - props.damping));
    if (props.updates == Properties::UpdateType::SEQMAX)
