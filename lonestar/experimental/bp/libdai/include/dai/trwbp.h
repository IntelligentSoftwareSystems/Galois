...
/// \file
/// \brief Defines class TRWBP, which implements Tree-Reweighted Belief
/// Propagation
...
/// Approximate inference algorithm "Tree-Reweighted Belief Propagation" [\ref
/// WJW03]
 ...
 * variables \f$i\f$. The update equation is given by: \f[ m_{I\to i}(x_i)
 * \propto \sum_{x_{N_I\setminus\{i\}}} f_I(x_I)^{1/c_I} \prod_{j\in
 * N_I\setminus\{i\}} m_{I\to j}^{c_I-1} \prod_{J\in N_j\setminus\{I\}} m_{J\to
 * j}^{c_J} \f] After convergence, the variable beliefs are calculated by: \f[
 * b_i(x_i) \propto \prod_{I\in N_i} m_{I\to i}^{c_I} \f] and the factor beliefs
 * are calculated by: \f[ b_I(x_I) \propto f_I(x_I)^{1/c_I} \prod_{j \in N_I}
 * m_{I\to j}^{c_I-1} \prod_{J\in N_j\setminus\{I\}} m_{J\to j}^{c_J} \f] The
 * logarithm of the partition sum is approximated by: \f[ \log Z = \sum_{I}
 * \sum_{x_I} b_I(x_I) \big( \log f_I(x_I) - c_I \log b_I(x_I) \big) + \sum_{i}
 * (c_i - 1) \sum_{x_i} b_i(x_i) \log b_i(x_i) \f] where the variable weights
 * are defined as \f[ c_i := \sum_{I \in N_i} c_I \f]
 *
...
public:
  ...
  virtual TRWBP* construct(const FactorGraph& fg,
