...
/** \note This class is meant as a base class for BBPCostFunction, which
 * provides additional functionality.
 */
DAI_ENUM(BBPCostFunctionBase, CFN_GIBBS_B, CFN_GIBBS_B2, CFN_GIBBS_EXP,
         CFN_GIBBS_B_FACTOR, CFN_GIBBS_B2_FACTOR, CFN_GIBBS_EXP_FACTOR,
         CFN_VAR_ENT, CFN_FACTOR_ENT, CFN_BETHE_ENT);
...
public:
  ...
  /// Evaluates cost function in state \a stateP using the information in
  /// inference algorithm \a ia
...
private:
  ...
  /// Pointer to the approximate inference algorithm (currently, only BP objects
  /// are supported)
  const InfAlg* _ia;
  //@}
   ...
   *  than updating _adj_n (and friends) which are unused in the sequential
   * algorithm. \pre RegenerateInputs() should be called first
   */
  ...
  /** Increases variable factor adjoint according to eqn. (27) in [\ref EaG09]
   * and calculates the new variable->factor message adjoint according to eqn.
   * (29) in [\ref EaG09].
   */
   ...
   *  calculates the new factor->variable message adjoint according to the
   * r.h.s. of eqn. (30) in [\ref EaG09].
   */
  void calcNewM(size_t i, size_t _I);
  /// Calculates unnormalized variable->factor message adjoint from the
  /// normalized one
  void calcUnnormMsgN(size_t i, size_t _I);
  /// Calculates unnormalized factor->variable message adjoint from the
  /// normalized one
  ...
  //@{
  /// Helper function for sendSeqMsgM(): increases factor->variable message
  /// adjoint by \a p and calculates the corresponding unnormalized adjoint
  void incrSeqMsgM(size_t i, size_t _I, const Prob& p);
  //  DISABLED BECAUSE IT IS BUGGY:
  //  void updateSeqMsgM( size_t i, size_t _I );
  /// Sets normalized factor->variable message adjoint and calculates the
  /// corresponding unnormalized adjoint
  ...
  /// Computes the adjoint of the unnormed probability vector from the
  /// normalizer and the adjoint of the normalized probability vector
  ...
  Real getUnMsgMag();
  /// Calculates averaged L1 norms of current and new normalized message
  /// adjoints
  void getMsgMags(Real& s, Real& new_s);
  /// Returns indices and magnitude of the largest normalized factor->variable
  /// message adjoint
  void getArgmaxMsgM(size_t& i, size_t& _I, Real& mag);
  /// Returns magnitude of the largest (in L1-norm) normalized factor->variable
  /// message adjoint
  Real getMaxMsgM();

  /// Calculates sum of L1 norms of all normalized factor->variable message
  /// adjoints
  Real getTotalMsgM();
  /// Calculates sum of L1 norms of all updated normalized factor->variable
  /// message adjoints
  Real getTotalNewMsgM();
  /// Calculates sum of L1 norms of all normalized variable->factor message
  /// adjoints
  Real getTotalMsgN();

  /// Returns a vector of Probs (filled with zeroes) with state spaces
  /// corresponding to the factors in the factor graph \a fg
  std::vector<Prob> getZeroAdjF(const FactorGraph& fg);
  /// Returns a vector of Probs (filled with zeroes) with state spaces
  /// corresponding to the variables in the factor graph \a fg
...
public:
  ...
  /// \name Initialization
  //@{
  /// Initializes from given belief adjoints \a adj_b_V, \a adj_b_F and initial
  /// factor adjoints \a adj_psi_V, \a adj_psi_F
  ...
  /// Initializes from given belief adjoints \a adj_b_V and \a adj_b_F (setting
  /// initial factor adjoints to zero)
  void init(const std::vector<Prob>& adj_b_V,
            const std::vector<Prob>& adj_b_F) {
    init(adj_b_V, adj_b_F, getZeroAdjV(*_fg), getZeroAdjF(*_fg));
  }

  /// Initializes variable belief adjoints \a adj_b_V (and sets factor belief
  /// adjoints and initial factor adjoints to zero)
  void init_V(const std::vector<Prob>& adj_b_V) {
    init(adj_b_V, getZeroAdjF(*_fg));
  }

  /// Initializes factor belief adjoints \a adj_b_F (and sets variable belief
  /// adjoints and initial factor adjoints to zero)
  void init_F(const std::vector<Prob>& adj_b_F) {
    init(getZeroAdjV(*_fg), adj_b_F);
  }

  /// Initializes with adjoints calculated from cost function \a cfn, and state
  /// \a stateP
  /** Uses the internal pointer to an inference algorithm in combination with
   * the cost function and state for initialization. \param cfn Cost function
   * used for initialization; \param stateP is a Gibbs state and can be NULL; it
   * will be initialised using a Gibbs run.
   */
  void initCostFnAdj(const BBPCostFunction& cfn,
...
public:
  ...
  /* PROPERTIES(props,BBP) {
     ...
     /// Damping constant (0 for none); damping = 1 - lambda where lambda is the
  damping constant used in [\ref EaG09] Real damping;
  ...
  */
  struct Properties {
    ...
    Real tol;
    /// Damping constant (0 for none); damping = 1 - lambda where lambda is the
    /// damping constant used in [\ref EaG09]
    Real damping;
    /// Update schedule
    UpdateType updates;
...
/// Function to verify the validity of adjoints computed by BBP using numerical
/// differentiation.
/** Factors containing a variable are multiplied by small adjustments to verify
 * accuracy of calculated variable factor adjoints. \param bp BP object; \param
 * state Global state of all variables; \param bbp_props BBP parameters; \param
 * cfn Cost function to be used; \param h Size of perturbation. \relates BBP
 */
Real numericBBPTest(const InfAlg& bp, const std::vector<size_t>* state,
                    const PropertySet& bbp_props, const BBPCostFunction& cfn,
                    Real h);
