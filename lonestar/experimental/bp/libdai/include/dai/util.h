...
/// \file
/// \brief Defines general utility functions and adds an abstraction layer for
...
#ifdef DAI_DEBUG
/// \brief "Print variable". Prints the text of an expression, followed by its
/// value (only if DAI_DEBUG is defined)
/**
...
/// Real number (alias for \c double, which could be changed to <tt>long
/// double</tt> if necessary)
typedef double Real;
...
inline Real log(Real x) { return std::log(x); }

/// Returns logarithm of \a x, or 0 if \a x == 0
inline Real log0(Real x) { return x ? std::log(x) : 0; }

/// Returns exponent of \a x
inline Real exp(Real x) { return std::exp(x); }
...
/** We use the (experimental) TR1 unordered_map implementation included in
 * modern GCC distributions or in boost versions 1.37 and higher.
 */
...
inline int rnd(int n) { return rnd_int(0, n - 1); }

/// Converts a variable of type \a T to a \c std::string by using a \c
/// boost::lexical_cast
template <class T>
...
/// Converts a variable of type std::string to \a T by using a \c
/// boost::lexical_cast
template <class T>
...
std::ostream& operator<<(std::ostream& os, const std::map<T1, T2>& x) {
  os << "{";
  for (typename std::map<T1, T2>::const_iterator it = x.begin(); it != x.end();
       it++)
 ...
 *  \param singleDelim if \c true, any single delimiter forms a boundary between
 * two tokens; if \c false, a maximal group of consecutive delimiters forms a
 * boundary between two tokens \param delim delimiter characters
 */
...
/**
 *  - DISTL1 is the \f$\ell_1\f$ distance (sum of absolute values of pointwise
 * difference);
 *  - DISTLINF is the \f$\ell_\infty\f$ distance (maximum absolute value of
 * pointwise difference);
 *  - DISTTV is the total variation distance (half of the \f$\ell_1\f$
 * distance);
 *  - DISTKL is the Kullback-Leibler distance (\f$\sum_i p_i (\log p_i - \log
 * q_i)\f$).
 *  - DISTHEL is the Hellinger distance (\f$\frac{1}{2}\sum_i
 * (\sqrt{p_i}-\sqrt{q_i})^2\f$).
 */
