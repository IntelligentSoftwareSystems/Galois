...
public:
  ...
  struct Properties {
    ...
    /// Number of "burn-in" iterations after each (re)start (for which no
...
public:
  /// Default constructor
  Gibbs()
      : DAIAlgFG(), _sample_count(0), _var_counts(), _factor_counts(),
        _iters(0), _state(), _max_state(), _max_score(-INFINITY) {}
  ...
  Gibbs(const FactorGraph& fg, const PropertySet& opts)
      : DAIAlgFG(fg), _sample_count(0), _var_counts(), _factor_counts(),
        _iters(0), _state(), _max_state(), _max_score(-INFINITY) {
    setProperties(opts);
    construct();
  }
  ...
  virtual Gibbs* construct(const FactorGraph& fg,
  ...
  //@{
  /// Draw the current joint state of all variables from a uniform random
  /// distribution
  void randomizeState();
...
private:
  ...
  void construct();
  /// Updates all counts (_sample_count, _var_counts, _factor_counts) based on
  /// current state
  void updateCounts();
  /// Calculate conditional distribution of variable \a i, given the current
  /// state
  Prob getVarDist(size_t i);
  /// Draw state of variable \a i randomly from its conditional distribution and
  /// update the current state
  void resampleVar(size_t i);
  /// Calculates linear index into factor \a I corresponding to the current
  /// state
  size_t getFactorEntry(size_t I);
  /// Calculates the differences between linear indices into factor \a I
  /// corresponding with a state change of variable \a i
  size_t getFactorEntryDiff(size_t I, size_t i);
};

/// Runs Gibbs sampling for \a maxiter iterations (of which \a burnin for
/// burn-in) on FactorGraph \a fg, and returns the resulting state
/** \relates Gibbs
 */
