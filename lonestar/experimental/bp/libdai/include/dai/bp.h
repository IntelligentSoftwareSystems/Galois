 ...
 *  and factors (more precisely, for the subset of variables depending on the
 * factor). There are two variants, the sum-product algorithm (corresponding to
 *  finite temperature) and the max-product algorithm (corresponding to
 *  zero temperature).
 *
 *  The messages \f$m_{I\to i}(x_i)\f$ are passed from factors \f$I\f$ to
 * variables \f$i\f$. In case of the sum-product algorith, the update equation
 * is: \f[ m_{I\to i}(x_i) \propto \sum_{x_{N_I\setminus\{i\}}} f_I(x_I)
 * \prod_{j\in N_I\setminus\{i\}} \prod_{J\in N_j\setminus\{I\}} m_{J\to j}\f]
 *  and in case of the max-product algorithm:
 *    \f[ m_{I\to i}(x_i) \propto \max_{x_{N_I\setminus\{i\}}} f_I(x_I)
 * \prod_{j\in N_I\setminus\{i\}} \prod_{J\in N_j\setminus\{I\}} m_{J\to j}\f]
 *  In order to improve convergence, the updates can be damped. For improved
 * numerical stability, the updates can be done in the log-domain alternatively.
 *
 ...
 *    \f[ b_I(x_I) \propto f_I(x_I) \prod_{j\in N_I} \prod_{J\in
 * N_j\setminus\{I\}} m_{J\to j}(x_j) \f] The logarithm of the partition sum is
 * calculated by: \f[ \log Z = \sum_i (1 - |N_i|) \sum_{x_i} b_i(x_i) \log
 * b_i(x_i) - \sum_I \sum_{x_I} b_I(x_I) \log \frac{b_I(x_I)}{f_I(x_I)} \f]
 *
 *  For the max-product algorithm, a heuristic way of finding the MAP state (the
 *  joint configuration of all variables which has maximum probability) is
 * provided by the findMaximum() method, which can be called after convergence.
 *
 *  \note There are two implementations, an optimized one (the default) which
 * caches IndexFor objects, and a slower, less complicated one which is easier
 * to maintain/understand. The slower one can be enabled by defining DAI_BP_FAST
 * as false in the source file.
 */
...
protected:
  ...
  struct Task {
    ...
    Real dist;
    Task() {}
    Task(size_t ii, size_t _II, int r, Real dd)
        : i(ii), _I(_II), round(r), dist(dd) {}
    bool operator<(const Task& t) const { return dist < t.dist; }
  ...
  struct TaskLess
    ...
    bool operator()(const Task& a, const Task& b) { return a.dist < b.dist; }
  };
  ...
  struct Process {
    ...
    Process(BP& p, std::vector<std::vector<EdgeData>>& e, unsigned& c)
        : parent(p), edgeData(e), count(c) {}
  ...
  struct ComputeDiffs {
    ...
    ComputeDiffs(BP& p, galois::GReduceMax<Real>& a)
        : parent(p), accMaxDiff(a) {}
  ...
  struct Initialize {
    ...
    Initialize(BP& p, std::vector<Task>& i,
               std::vector<std::vector<EdgeData>>& e)
        : parent(p), initial(i), edgeData(e) {}
  ...
  size_t _iters;
  /// The history of message updates (only recorded if \a recordSentMessages is
  /// \c true)
...
public:
  ...
  /// Default constructor
  BP()
      : DAIAlgFG(), _edges(), _edge2lut(), _lut(), _maxdiff(0.0), _iters(0U),
  ...
  BP(const FactorGraph& fg, const PropertySet& opts)
      : DAIAlgFG(fg), _edges(), _maxdiff(0.0), _iters(0U), _sentMessages(),
        _oldBeliefsV(), _oldBeliefsF(), _updateSeq(), props(),
        recordSentMessages(false) {
    setProperties(opts);
    construct();
  }
  ...
  /// Copy constructor
  BP(const BP& x)
      : DAIAlgFG(x), _edges(x._edges), _edge2lut(x._edge2lut), _lut(x._lut),
        _maxdiff(x._maxdiff), _iters(x._iters), _sentMessages(x._sentMessages),
        _oldBeliefsV(x._oldBeliefsV), _oldBeliefsF(x._oldBeliefsF),
        _updateSeq(x._updateSeq), props(x.props),
  ...
  /** \pre Assumes that run() has been called and that \a props.inference == \c
   * MAXPROD
   */
  std::vector<std::size_t> findMaximum() const {
    return dai::findMaximum(*this);
  }
  virtual void init();
...
protected:
  /// Returns constant reference to message from the \a _I 'th neighbor of
  /// variable \a i to variable \a i
  const Prob& message(size_t i, size_t _I) const {
    return _edges[i][_I].message;
  }
  /// Returns reference to message from the \a _I 'th neighbor of variable \a i
  /// to variable \a i
  Prob& message(size_t i, size_t _I) { return _edges[i][_I].message; }
  /// Returns constant reference to updated message from the \a _I 'th neighbor
  /// of variable \a i to variable \a i
  const Prob& newMessage(size_t i, size_t _I) const {
    return _edges[i][_I].newMessage;
  }
  /// Returns reference to updated message from the \a _I 'th neighbor of
  /// variable \a i to variable \a i
  Prob& newMessage(size_t i, size_t _I) { return _edges[i][_I].newMessage; }
  /// Returns constant reference to cached index for the edge between variable
  /// \a i and its \a _I 'th neighbor
  const ind_t& index(size_t i, size_t _I) const { return _edges[i][_I].index; }
  /// Returns reference to cached index for the edge between variable \a i and
  /// its \a _I 'th neighbor
  ind_t& index(size_t i, size_t _I) { return _edges[i][_I].index; }
  /// Returns constant reference to residual for the edge between variable \a i
  /// and its \a _I 'th neighbor
  const Real& residual(size_t i, size_t _I) const {
    return _edges[i][_I].residual;
  }
  /// Returns reference to residual for the edge between variable \a i and its
  /// \a _I 'th neighbor
  Real& residual(size_t i, size_t _I) { return _edges[i][_I].residual; }
  void runProcess(const Task&, std::vector<std::vector<EdgeData>>&, unsigned&,
                  galois::UserContext<Task>&);
  void runComputeDiffs(size_t v, galois::GReduceMax<Real>& acc);
  void runInitialize(size_t, std::vector<Task>&,
                     std::vector<std::vector<EdgeData>>& edgeData);

  /// Calculate the product of factor \a I and the incoming messages
  /** If \a without_i == \c true, the message coming from variable \a i is
   * omitted from the product \note This function is used by calcNewMessage()
   * and calcBeliefF()
   */
  virtual Prob calcIncomingMessageProduct(size_t I, bool without_i,
                                          size_t i) const;
  /// Calculate the updated message from the \a _I 'th neighbor of variable \a i
  /// to variable \a i
  virtual void calcNewMessage(size_t i, size_t _I);
  /// Replace the "old" message from the \a _I 'th neighbor of variable \a i to
  /// variable \a i by the "new" (updated) message
  void updateMessage(size_t i, size_t _I);
  /// Set the residual (difference between new and old message) for the edge
  /// between variable \a i and its \a _I 'th neighbor to \a r
  void updateResidual(size_t i, size_t _I, Real r);
  /// Finds the edge which has the maximum residual (difference between new and
  /// old message)
