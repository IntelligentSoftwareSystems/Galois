...
/** A directed cyclic graph has nodes connected by directed edges, such that
 * there is no directed cycle of edges n1->n2->n3->...->n1. Nodes are indexed by
 * an unsigned integer. If there are nrNodes() nodes, they are numbered
 * 0,1,2,...,nrNodes()-1. An edge from node \a n1 to node \a n2 is represented
 * by a Edge(\a n1,\a n2).
 *
 *  DAG is implemented as a sparse adjacency list, i.e., it stores for each node
 * a list of its parents and a list of its children. Both lists are implemented
 * as a vector of Neighbor structures (accessible by the pa() and ch() methods).
 * Thus, each node has two associated variables of type DAG::Neighbors, which
 * are vectors of Neighbor structures, describing their parent and children
 * nodes.
 */
class DAG {
private:
...
public:
  ...
  DAG(size_t nr, EdgeInputIterator begin, EdgeInputIterator end,
      bool check = true)
      : _pa(), _ch() {
  ...
  /// Adds a node without parents and children and returns the index of the
  /// added node.
  size_t addNode() {
   ...
   * become parents of the added node. \param end Points just beyond the last
   * index of the nodes that should become parents of the added node. \param
   ...
   * and it would not introduce a cycle.
   */
  ...
  //@{
  /// Removes node \a n and all ingoing and outgoing edges; indices of other
  ...
  /** \note The time complexity is linear in the number of children of \a n1 or
   * the number of parents of \a n2, whichever is smaller
   */
  ...
  /// Returns the set of ancestors of node \a n, i.e., all nodes \a a such that
  /// there exists a directed path from \a a to \a n (excluding \a n itself)
  std::set<size_t> ancestors(size_t n) const;

  /// Returns the set of descendants of node \a n, i.e., all nodes \a d such
  /// that there exists a directed path from \a n to \a d (excluding \a n
  /// itself)
  std::set<size_t> descendants(size_t n) const;

  /// Returns whether there exists a directed path from node \a n1 to node \a n2
  /// (which may consist of zero edges)
  ...
  bool operator==(const DAG& x) const {
    ...
    for (size_t n1 = 0; n1 < nrNodes(); n1++) {
      ...
      diaforeach(const Neighbor& n2,
                 pa(n1)) if (!x.hasEdge(n2, n1)) return false;
      diaforeach(const Neighbor& n2,
                 x.pa(n1)) if (!hasEdge(n2, n1)) return false;
    }
    return true;
  }
  //@}
...
template <typename EdgeInputIterator>
void DAG::construct(size_t nr, EdgeInputIterator begin, EdgeInputIterator end,
                    bool check) {
  _pa.clear();
  _pa.resize(nr);
  _ch.clear();
