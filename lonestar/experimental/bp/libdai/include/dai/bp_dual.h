...
/// \idea BP_dual replicates a large part of the functionality of BP; would it
/// not be more efficient to adapt BP instead? \author Frederik Eaton
...
/** BP_dual calculates "dual" versions of BP messages (both messages from
 * factors to variables and messages from variables to factors), and
 * normalizers, given an InfAlg. These are computed from the variable and factor
 * beliefs of the InfAlg. This class is used primarily by BBP.
 *
...
protected:
  ...
  /// Groups together the data structures for storing the two types of messages
  ...
  /// Groups together the data structures for storing the two types of beliefs
  /// and their normalizers
  struct beliefs {
...
public:
  /// Construct BP_dual object from (converged) InfAlg object's beliefs and
  /// factors.
  ...
  /// Returns reference to normalizer for factor->variable message (\a I -> \a
  /// i)
  Real& zM(size_t i, size_t _I) { return _msgs.Zm[i][_I]; }
  /// Returns constant reference to normalizer for factor->variable message (\a
  /// I -> \a i)
  const Real& zM(size_t i, size_t _I) const { return _msgs.Zm[i][_I]; }
  /// Returns reference to normalizer for variable -> factor message (\a i -> \a
  /// I)
  Real& zN(size_t i, size_t _I) { return _msgs.Zn[i][_I]; }
  /// Returns constant reference to normalizer for variable -> factor message
  /// (\a i -> \a I)
  const Real& zN(size_t i, size_t _I) const { return _msgs.Zn[i][_I]; }

  /// Returns belief of variable \a i
  Factor beliefV(size_t i) const {
    return Factor(_ia->fg().var(i), _beliefs.b1[i]);
  }
  /// Returns belief of factor \a I
  Factor beliefF(size_t I) const {
    return Factor(_ia->fg().factor(I).vars(), _beliefs.b2[I]);
  }
