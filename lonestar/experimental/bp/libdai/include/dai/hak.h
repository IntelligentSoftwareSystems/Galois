...
/// \file
/// \brief Defines class HAK, which implements a variant of Generalized Belief
/// Propagation. \idea Implement more general region graphs and corresponding
/// Generalized Belief Propagation updates as described in [\ref YFW05]. \todo
/// Use ClusterGraph instead of a vector<VarSet> for speed. \todo Optimize this
/// code for large factor graphs. \todo Implement GBP parent-child  algorithm.
...
/// Approximate inference algorithm: implementation of single-loop ("Generalized
/// Belief Propagation") and double-loop algorithms by Heskes, Albers and Kappen
/// [\ref HAK03]
...
public:
  ...
  struct Properties {
     ...
     *   - LOOP one cluster for each loop of length at most \a
     * Properties::loopdepth, and in addition one cluster for each maximal
     * factor
     *   - BETHE Bethe approximation (one outer region for each maximal factor,
     * inner regions are single variables)
     */
...
public:
  ...
  //@{
  /// Returns reference to message from outer region \a alpha to its \a _beta
  /// 'th neighboring inner region
  Factor& muab(size_t alpha, size_t _beta) { return _muab[alpha][_beta]; }
  /// Returns reference to message the \a _beta 'th neighboring inner region of
  /// outer region \a alpha to that outer region
  ...
  Real doGBP();
  /// Runs double-loop algorithm (as described in section 4.2 of [\ref HAK03]),
  /// which always convergences
  Real doDoubleLoop();
  //@}
...
private:
  ...
  void construct();
  /// Recursive procedure for finding clusters of variables containing loops of
  /// length at most \a length
   ...
   *  \return allcl all clusters of variables with loops of length at most \a
   * length passing through root
   */
  void findLoopClusters(const FactorGraph& fg, std::set<VarSet>& allcl,
