...
/// \file
/// \brief Defines class MR, which implements loop corrections as proposed by
/// Montanari and Rizzo
...
/** \author Bastian Wemmenhove wrote the original implementation before it was
 * merged into libDAI
 */
...
private:
  ...
  /// tJ[i][_j] is the hyperbolic tangent of the interaction between spin \a i
  /// and its neighbour G.nb(i,_j)
...
public:
  /// Default constructor
  MR()
      : DAIAlgFG(), supported(), G(), tJ(), theta(), M(), cors(), Mag(),
        _maxdiff(), _iters(), props() {}

  /// Construct from FactorGraph \a fg and PropertySet \a opts
  /** \param fg Factor graph.
   *  \param opts Parameters @see Properties
   *  \note This implementation only deals with binary variables and pairwise
   * interactions. \throw NOT_IMPLEMENTED if \a fg has factors depending on
   * three or more variables or has variables with more than two possible
   * states.
   */
...
private:
  ...
  /// Calculate \f$ \Omega^{(i)}_{j,l} \f$ as defined in [\ref MoR05] eqn.
  /// (2.15)
  Real Omega(size_t i, size_t _j, size_t _l);

  /// Calculate \f$ T^{(i)}_A \f$ as defined in [\ref MoR05] eqn. (2.17) with
  /// \f$ A = \{l_1,l_2,\dots\} \f$
  /** \param i variable index
   *  \param A subset of neighbors of variable \a i
   */
  ...
  /// Calculates \f$ \Gamma^{(i)}_{j,l_1l_2} \f$ as defined in [\ref MoR05] eqn.
  /// (2.16)
  Real Gamma(size_t i, size_t _j, size_t _l1, size_t _l2);

  /// Calculates \f$ \Gamma^{(i)}_{l_1l_2} \f$ as defined in [\ref MoK07] on
  /// page 1141
  Real Gamma(size_t i, size_t _l1, size_t _l2);

  /// Approximates moments of variables in \a A
  /** Calculate the moment of variables in \a A from M and cors, neglecting
   * higher order cumulants, defined as the sum over all partitions of A into
   * subsets of cardinality two at most of the product of the cumulants (either
   * first order, i.e. M, or second order, i.e. cors) of the entries of the
   * partitions.
   *
   *  \param i variable index
   *  \param A subset of neighbors of variable \a i
