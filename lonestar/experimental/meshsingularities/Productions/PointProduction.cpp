...
int PointProduction::getInterfaceSize() const
{
	return this->interfaceSize;
}

int PointProduction::getLeafSize() const
{
	return this->leafSize;
}

int PointProduction::getA1Size() const
{
	return this->a1Size;
}

int PointProduction::getANSize() const
{
	return this->anSize;
}
		...
		std::vector<double> *result, int num) const
{
	...
	for (int j=0; j<this->getInterfaceSize(); ++j) {
		(*result)[totalOffset+j+leafOffset+this->getInterfaceSize()] = leaf->system->rhs[j+leafOffset+this->getInterfaceSize()];
	}

	for (int j=0; j<leafOffset; ++j) {
		(*result)[totalOffset+j+this->getInterfaceSize()] = leaf->system->rhs[j];
	}
}
		...
		std::vector<double> *result, int num) const
{
	...
	for (int j=0; j<this->getInterfaceSize(); ++j) {
		...
		an->rhs[j+offset+leafOffset+this->getInterfaceSize()] = leaf->system->rhs[j+leafOffset+this->getInterfaceSize()];
	}
...
std::vector<EquationSystem*>* PointProduction::preprocess(std::vector<EquationSystem*>* input) const
{
