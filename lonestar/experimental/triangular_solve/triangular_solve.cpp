...
// In particular, right solve with CSR and left solve with CSC require a
// transpose. Upper/lower triangular will need slightly different loop bounds,
...
static char const* desc = "Back substitution to solve x.T A = b.T for sparse "
...
int main(int argc, char** argv) noexcept {
  ...
  galois::for_each(
      ...
      [&](auto node, auto& context) noexcept {
        ...
        // Usual CAS loop isn't necessary here since the dependency tracking
        // guarantees that this is the last operation run on this atomic.
        rhs[node].store(rhs[node].load(std::memory_order_relaxed) /
                            graph.getEdgeData(*edge_iterator),
                        std::memory_order_relaxed);
        ++edge_iterator;
        while (edge_iterator < edge_end) {
          auto neighbor = graph.getEdgeDst(*edge_iterator);
          assert(neighbor > node);
          atomic_relaxed_plus_eq(rhs[neighbor],
                                 -rhs[node].load() *
                                     graph.getEdgeData(*edge_iterator));
          auto& other_counter =
              graph.getData(neighbor, galois::MethodFlag::UNPROTECTED);
      ...
      galois::loopname("back_substitution"), galois::no_conflicts(),
