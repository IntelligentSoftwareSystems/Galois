...
void SP_algorithm(Graph& graph, galois::GAccumulator<unsigned int>& nontrivial,
  ...
  // a->i(t) - sigma a->i (t-1) < E on all the edges, the iteration has converged
  // and generated sigma* a->i = sigma a->i(t), goto 2 2) if t = tmax return
  // un-converged.  if (t < tmax) then return the set of fixed point warnings
  // sigma* a->i = sigma a->i (t)
