...
static cll::opt<BFSAlgo> algo(cll::desc("Choose an algorithm:"),
    cll::values(
      clEnumVal(serialSloan, "Boost Serial Sloan"),
...
struct BoostSloan {
  std::string name() const { return "Boost Serial Sloan"; }
	...
	static void 
  sloan_ordering(BGraph& g,
									Vertex s,
									Vertex e,
									std::vector<Vertex>::iterator permutation, 
	...
	{
		//typedef typename boost::property_traits<boost::property_map<BGraph, boost::vertex_degree_t>::type >::value_type Degree;
		typedef boost::property_traits<boost::property_map<BGraph, boost::vertex_priority_t>::type >::value_type Degree;
		typedef boost::property_traits<boost::property_map<BGraph, boost::vertex_color_t>::type >::value_type ColorValue;
		//typedef typename boost::property_traits<boost::property< boost::vertex_color_t, boost::default_color_type> >::value_type ColorValue;
		typedef boost::color_traits<ColorValue> Color;
		typedef boost::graph_traits<BGraph>::vertex_descriptor Vertex;
		typedef std::vector<size_type>::iterator vec_iter;

		typedef boost::property_map<BGraph, boost::vertex_index_t>::const_type VertexID;
		...
		boost::iterator_property_map<vec_iter, VertexID, size_type, size_type&> dist_pmap(dist.begin(), get(boost::vertex_index, g)); 

		boost::breadth_first_search(g, e, boost::visitor(boost::make_bfs_visitor(boost::record_distances(dist_pmap, boost::on_tree_edge()))));

		//Creating a property_map for the indices of a vertex
		boost::property_map<BGraph, boost::vertex_index_t>::type index_map = get(boost::vertex_index, g);
		...
		for (boost::tie(ui, ui_end) = boost::vertices(g); ui != ui_end; ++ui)
		{
			...
			//std::cerr << "[" << index_map[*ui] << "]: " << get(priority, *ui) << " cdge: " << cdeg << " dist: " << dist[index_map[*ui]] << " prio: " << W1*dist[index_map[*ui]]+W2*cdeg<< " real: " << W1*dist[index_map[*ui]]-W2*cdeg << " skata: " << skata << "\n";
		}

		//Priority list
		typedef boost::indirect_cmp<boost::property_map<BGraph, boost::vertex_priority_t>::type, std::greater<Degree> > Compare;
		...
		while ( !priority_list.empty() ) 
		{
			...
			/*
			std::cerr << "Queue: ";
			for(std::list<Vertex>::iterator ii = priority_list.begin(), ee = priority_list.end(); ii != ee; ++ii){
			...
			*permutation++ = u;                      //Puts u to the first position in the permutation-vector
			...
			for(std::vector<Vertex>::iterator ii = skato, ee = permutation; ii != ee; ++ii){
	...
	void operator()(const unsigned int& source) const {
		...
		boost::property_map<BGraph,boost::vertex_degree_t>::type deg = get(boost::vertex_degree, *bgraph);
		for (boost::tie(ui, ui_end) = boost::vertices(*bgraph); ui != ui_end; ++ui)
			deg[*ui] = boost::degree(*ui, *bgraph);

		boost::property_map<BGraph, boost::vertex_index_t>::type index_map = boost::get(boost::vertex_index, *bgraph);
		...
		Vertex e = boost::pseudo_peripheral_pair(*bgraph, s, ecc, get(boost::vertex_color, *bgraph), get(boost::vertex_degree, *bgraph) );
		...
		//Sloan ordering
		//boost::sloan_ordering(*bgraph, s, e, sloan_order.begin(), get(boost::vertex_color, *bgraph), get(boost::vertex_degree, *bgraph), get(boost::vertex_priority, *bgraph));
		sloan_ordering(*bgraph, s, e, sloan_order.begin(), get(boost::vertex_color, *bgraph), get(boost::vertex_degree, *bgraph), get(boost::vertex_priority, *bgraph));

		/*
		std::cout << "Sloan ordering starting at: " << s << std::endl;

		for (std::vector<Vertex>::const_iterator i = sloan_order.begin(); i != sloan_order.end(); ++i)
