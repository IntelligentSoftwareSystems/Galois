...
struct SerialSloan {
  ...
  struct sloanFn {
    ...
    void operator()(UpdateRequest& next,
      ...
      pdata.mutex.lock();
      // std::cerr << "[" << perm.size() << "] Extracting node: " << pdata.id <<
      ...
      if (status != NUMBERED) {
        ...
        // std::cerr << "[" << perm.size() << "] Will process node: " <<
        ...
        if (status == PREACTIVE) {
          for (Graph::edge_iterator
            ...
            if (cdata.status != NUMBERED) {
              ...
              // std::cerr << "[" << perm.size() << "] Adding: " << cdata.id <<
              // " with status: " << cdata.status << " and priority: " <<
              // cdata.prio << "\n";
            }
            cdata.mutex.unlock();
        ...
        for (Graph::edge_iterator
          ...
          if (cdata.status == PREACTIVE) {
            ...
            ctx.push(UpdateRequest(child, cdata.prio));
            // std::cerr << "[" << perm.size() << "] Adding: " << cdata.id << "
            // with status: " << cdata.status << " and priority: " << cdata.prio
            // << "\n";
            ...
            for (Graph::edge_iterator
              ...
              if (gdata.status != NUMBERED) {
                ...
                // std::cerr << "[" << perm.size() << "] Adding: " << gdata.id
                // << " with status: " << gdata.status << " and priority: " <<
                // gdata.prio << "\n";
              }
              gdata.mutex.unlock();
            }
      ...
      // else
      // std::cerr << "[" << perm.size() << "] Will throw node: " << pdata.id <<
      // " with status: " << pdata.status << " and priority: " << pdata.prio <<
      // "\n";
    }
    ...
    */
    ...
    static void go(GNode source) {
      ...
      // graph.getData(source).status = galois::GAtomic<unsigned
      // int>(PREACTIVE);

      // std::cout << "max dist is " << maxDist << "\n";
      galois::for_each(UpdateRequest(source, graph.getData(source).prio),
                       sloanFn(), galois::loopname("Sloan"),
                       galois::wl<OBIM>());
      // std::cout << "max dist is " << maxDist << "\n";
    }
  };
