...
public:
	l2r_bilinear_ls_fun_full_weight_fast(const bilinear_problem *prob, double Cp, double Cn){
		this->prob = prob;
...
#pragma omp parallel for schedule(dynamic,50) shared(Y,barXv)
	...
	void grad(double *w, double *g){
		...
		Z.val_t = z; smat_x_dmat(Z, prob->H, prob->k, ZH);
...
void bilinear_train(const bilinear_problem *prob, const bilinear_parameter *param, double *w, double *walltime, double *cputime) {
	...
	switch(param->solver_type)
	{
		...
		case L2R_BILINEAR_LS_FULL_WEIGHTED:
			{
				fun_obj=new l2r_bilinear_ls_fun_full_weight_fast(prob, param->Cp, param->Cn);
				//TRON tron_obj(fun_obj, param->eps, param->max_tron_iter, param->max_cg_iter);
				TRON tron_obj(fun_obj, primal_solver_regression_tol, param->max_tron_iter, param->max_cg_iter);
				tron_obj.set_print_string(liblinear_print_string);
				tron_obj.tron(w, false);  // prob->W is the initial
