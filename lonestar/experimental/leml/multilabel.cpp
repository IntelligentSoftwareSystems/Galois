...
void multilabel_train(multilabel_problem *prob, multilabel_parameter *param, double *W, double *H){
	...
	if(param->solver_type == L2R_BILINEAR_LS_FULL || param->solver_type == L2R_BILINEAR_LS_FULL_WEIGHTED)
		threshold = 0.5;
	...
	for(int iter = 1; iter <= param->maxiter; iter++) {
		...
		if(param->solver_type == L2R_WSABIE) {
			...
			printf("ML-iter %d Time %.5g norm %.5g ", 
					iter, Wtime+Htime, norm(H, Y.cols*X.cols));
