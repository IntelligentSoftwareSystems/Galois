...
struct BVHnode: public Box3d {
  ...
  size_t descendents(void) const {
    return m_descendants;
  }

  BVHnode* leftChild(void) const {
    return m_left;
  }

  BVHnode* rightChild(void) const {
    return m_right;
  }
};
          ...
          if (nB != nullptr) {
            ...
            if (nC == nA) {
              if (nA < nB) {
                ...
                assert(clus);
                bvhAlloc.construct(clus, static_cast<BVHnode*>(*nA), static_cast<BVHnode*>(*nB));
...
BVHnode* clusterNaiveSerial(const V& bodies, A& bvhAlloc) {!activeSet.empty());
  ...
  while (activeSet.size() > 1) {
    ...
    for (auto i = activeSet.cbegin(), end_i = activeSet.cend(); i != end_i; ++i) {
      auto j = i;
      ++j;
      for (; j != end_i; ++j) {
        BVHnode* nA = *i;
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Clustering " << numBodies << " bodies randomly placed" << std::endl;
