...
GraphWrapper testGraphConstruction() {
  ...
  Attr& attrE1 = (gw.g)->getEdgeData(
      (gw.g)->findEdge(gw.edges["e1"].src, gw.edges["e1"].dst));
  ...
  // find outgoing edges
  assert((gw.g)->findEdge(gw.edges["e1"].src, gw.edges["e1"].dst) !=
         (gw.g)->edge_end(gw.edges["e1"].src));
  assert((gw.g)->findEdge(gw.edges["e2"].src, gw.edges["e2"].dst) !=
         (gw.g)->edge_end(gw.edges["e2"].src));
  assert((gw.g)->findEdge(gw.edges["e3"].src, gw.edges["e3"].dst) !=
         (gw.g)->edge_end(gw.edges["e3"].src));
  assert((gw.g)->findEdge(gw.edges["e4"].src, gw.edges["e4"].dst) !=
         (gw.g)->edge_end(gw.edges["e4"].src));
  assert((gw.g)->findEdge(gw.edges["e5"].src, gw.edges["e5"].dst) !=
         (gw.g)->edge_end(gw.edges["e5"].src));
  if (DIRECTED) {
    // non-existing edge
    assert((gw.g)->findEdge(gw.nodes["n2"], gw.nodes["n1"]) ==
           (gw.g)->edge_end(gw.nodes["n2"]));
  } else {
    // implied by edge 4
    assert((gw.g)->findEdge(gw.nodes["n2"], gw.nodes["n1"]) !=
           (gw.g)->edge_end(gw.nodes["n2"]));
  ...
  // find incoming edges
  assert((gw.g)->findInEdge(gw.edges["e1"].dst, gw.edges["e1"].src) !=
         (gw.g)->in_edge_end(gw.edges["e1"].dst));
  assert((gw.g)->findInEdge(gw.edges["e2"].dst, gw.edges["e2"].src) !=
         (gw.g)->in_edge_end(gw.edges["e2"].dst));
  assert((gw.g)->findInEdge(gw.edges["e3"].dst, gw.edges["e3"].src) !=
         (gw.g)->in_edge_end(gw.edges["e3"].dst));
  assert((gw.g)->findInEdge(gw.edges["e4"].dst, gw.edges["e4"].src) !=
         (gw.g)->in_edge_end(gw.edges["e4"].dst));
  assert((gw.g)->findInEdge(gw.edges["e5"].dst, gw.edges["e5"].src) !=
         (gw.g)->in_edge_end(gw.edges["e5"].dst));
  if (DIRECTED) {
    // non-existing in_edge
    assert((gw.g)->findInEdge(gw.nodes["n1"], gw.nodes["n2"]) ==
           (gw.g)->in_edge_end(gw.nodes["n1"]));
  } else {
    // the smae as edge 4
    assert((gw.g)->findInEdge(gw.nodes["n1"], gw.nodes["n2"]) !=
           (gw.g)->in_edge_end(gw.nodes["n1"]));
  ...
  assert((gw.g)->findEdgeSortedByDst(gw.edges["e1"].src, gw.edges["e1"].dst) !=
         (gw.g)->edge_end(gw.edges["e1"].src));
  assert((gw.g)->findEdgeSortedByDst(gw.edges["e2"].src, gw.edges["e2"].dst) !=
         (gw.g)->edge_end(gw.edges["e2"].src));
  assert((gw.g)->findEdgeSortedByDst(gw.edges["e3"].src, gw.edges["e3"].dst) !=
         (gw.g)->edge_end(gw.edges["e3"].src));
  assert((gw.g)->findEdgeSortedByDst(gw.edges["e4"].src, gw.edges["e4"].dst) !=
         (gw.g)->edge_end(gw.edges["e4"].src));
  assert((gw.g)->findEdgeSortedByDst(gw.edges["e5"].src, gw.edges["e5"].dst) !=
         (gw.g)->edge_end(gw.edges["e5"].src));
  if (!DIRECTED) {
    // the same as edge 4
    assert((gw.g)->findEdgeSortedByDst(gw.nodes["n2"], gw.nodes["n1"]) !=
           (gw.g)->edge_end(gw.nodes["n2"]));
  } else {
    // non-existing edge
    assert((gw.g)->findEdgeSortedByDst(gw.nodes["n2"], gw.nodes["n1"]) ==
...
void testPagerank(GraphWrapper& gw) {
  ...
  assert(pr[0].n == gw.nodes["n0"] && pr[1].n == gw.nodes["n1"] &&
         pr[2].n == gw.nodes["n2"]);
...
void testReachability(GraphWrapper& gw) {
  ...
  assert(3 == reach.num);
  assert(reach.nodes[0] != reach.nodes[1] && reach.nodes[0] != reach.nodes[2] &&
         reach.nodes[1] != reach.nodes[2]);
  assert(reach.nodes[0] == gw.nodes["n0"] || reach.nodes[1] == gw.nodes["n0"] ||
         reach.nodes[2] == gw.nodes["n0"]);
  assert(reach.nodes[0] == gw.nodes["n1"] || reach.nodes[1] == gw.nodes["n1"] ||
         reach.nodes[2] == gw.nodes["n1"]);
  assert(reach.nodes[0] == gw.nodes["n2"] || reach.nodes[1] == gw.nodes["n2"] ||
         reach.nodes[2] == gw.nodes["n2"]);
...
void testSearchSubgraph(GraphWrapper& gw) {
  ...
  for (auto i = 0; i < 3; ++i) {
    for (auto j = 0; j < 3; ++j) {
      res_ull[i][gw2.inv_nodes[result[i * 3 + j].nQ]] =
          gw.inv_nodes[result[i * 3 + j].nD];
    }
  }
  assert(res_ull[0] != res_ull[1] && res_ull[0] != res_ull[2] &&
         res_ull[1] != res_ull[2]);
  ...
  for (auto i = 0; i < 3; ++i) {
    for (auto j = 0; j < 3; ++j) {
      res_vf2[i][gw2.inv_nodes[result[i * 3 + j].nQ]] =
          gw.inv_nodes[result[i * 3 + j].nD];
    }
  }
  assert(res_vf2[0] != res_vf2[1] && res_vf2[0] != res_vf2[2] &&
         res_vf2[1] != res_vf2[2]);
...
void testCoarsening() {
  ...
  for (int i = 0; i < 9; i++) {
    std::string nName = "f" + std::to_string(i);
    setNodeAttr(fgw.g, fgw.nodes[nName], "color",
                const_cast<ValAltTy>(colors[i % 3].c_str()));
  }
  ...
  assert(getNodeAttr(cgw.g, nl.nodes[0], "color") !=
         getNodeAttr(cgw.g, nl.nodes[1], "color"));
  assert(getNodeAttr(cgw.g, nl.nodes[0], "color") !=
         getNodeAttr(cgw.g, nl.nodes[2], "color"));
  assert(getNodeAttr(cgw.g, nl.nodes[1], "color") !=
         getNodeAttr(cgw.g, nl.nodes[2], "color"));

  Edge cge[4] = {{cgw.nodes["red"], cgw.nodes["blue"]},
  ...
  for (auto i = 0; i < 4; ++i) {
    ...
    // must be in cge
    assert(el.edges[i] == cge[0] || el.edges[i] == cge[1] ||
           el.edges[i] == cge[2] || el.edges[i] == cge[3]);
  }
