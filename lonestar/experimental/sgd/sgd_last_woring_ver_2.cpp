...
struct print_nodes {
  Graphp g;
  print_nodes(Graphp _g) : g(_g) {}
  void operator()(DGNode& node, galois::UserContext<DGNode>& ctx) {
...
pow(movie_data.updates + 1, 1.5)); double* __restrict__ user_latent =
  ...
  for(unsigned int i = 0; i < LATENT_VECTOR_SIZE; i++)
    {
      ...
      movie_latent[i] += step_size * (cur_error * prev_user_val  - LAMBDA *
prev_movie_val); user_latent[i]  += step_size * (cur_error * prev_movie_val -
LAMBDA * prev_user_val);
    }
}*/
...
struct sgd_algo {
     ...
     */
      ...
      if (edge_it == g->edge_end(movie)) // galois::MethodFlag::NONE))
      {
        ...
        if (movie_data.updates < MAX_MOVIE_UPDATES)
          ctx.push(movie);
        // else
        //  std::cout<<"Done with this movie: "<<track++<<endl;
      } else {
        ctx.push(movie);
      }
    }
  ...
  void operator()(Graphp g) {
    ...
    galois::for_each(g->begin(), g->end(), print_nodes(g), "Printing Process");
    // galois::for_each(g->begin(), g->end(), Process(this,g), "SGD Process");
