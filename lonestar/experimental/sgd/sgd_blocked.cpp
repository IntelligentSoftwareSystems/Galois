...
typedef typename galois::graphs::ThirdGraph<Node, uint32_t,
  ...
  struct Process : public galois::runtime::Lockable {
    ...
    void operator()(const DGNode& movie, galois::UserContext<DGNode>& ctx) {
      ...
      /*
       ...
       networkHostID); if(movie_data.edge_offset == movie_data.number_of_edges)
       ++movie_data.updates;
...
void runBlockSlices(Graphp g) {
  ...
  */
  ...
  struct create_nodes {
    ...
    // void operator ()(const int& item, const galois::UserContext<FileGNode>&
    // ctx) {
       ...
       //  for all movie and user nodes in the graph
          ...
          for(int i=0;i<LATENT_VECTOR_SIZE;i++) {
              data.latent_vector[i] = lv[i];
                ...
                unsigned int num_edges = g.edge_end(gnode) -
  g.edge_begin(gnode); numRatings += num_edges; if(num_edges > 0)
                   ++num_movie_nodes;
          else
          ++num_user_nodes;
  ...
  static void create_remote_graph_edges(Graphp dgraph) {
    ...
    for (auto ii = fgraph.begin(); ii != fgraph.end(); ++ii) {
      ...
      for (FGraph::edge_iterator jj = vv; jj != ev; ++jj) {
        ...
        dgraph->addEdge(mapping[*ii], mapping[fgraph.getEdgeDst(jj)],
                        edge_data);
        count++;
        n.number_of_edges += 1;
      }
  ...
  */
  ...
  static void create_dist_graph(Graphp dgraph, std::string inputFile) {
    ...
    /*  cout << "movie node size = "<<size<<endl;
      ...
      (last-first)); galois::for_each(first, last, create_nodes(dgraph,lk));
  */
    /*****************************************************************
    ...
    // "<<std::distance(graph.begin(),graph.end())<<endl; if(networkHostID == 0)
    // {
    //	std::cout << "number of node on host = " << networkHostID << " are: = "
    //<< last - first << "\n";
    printf("host: %u creating rest of the nodes\n", networkHostID);
    // typedef
    // galois::worklists::BulkSynchronous<galois::worklists::PerSocketChunkLIFO<256>
    // > WL; typedef galois::worklists::PerSocketChunkFIFO<64> PSchunk; typedef
    // galois::worklists::LIFO<FileGNode, true> chunk; typedef
    // galois::graphs::FileGraph::iterator IterTy;
    galois::for_each(first, last, create_nodes(dgraph, lk));
    // galois::for_each<>(boost::counting_iterator<int>(0),
    // boost::counting_iterator<int>(100), create_nodes(dgraph,lk));
