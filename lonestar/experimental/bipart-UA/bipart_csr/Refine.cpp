...
int calculate_cutsize(GGraph& g) {
  ...
  GNodeBag bag;
  galois::do_all(
      galois::iterate((uint64_t)0, g.hedges),
      [&](GNode n) {
...
void projectPart(MetisGraph* Graph) {
  ...
  galois::do_all(
      galois::iterate(*fineGraph),
      [&](GNode n) {
        if (n < fineGraph->hedges)
          return;
...
void initGains(GGraph& g, int pass) {
  ...
  galois::do_all(
      galois::iterate((uint64_t)g.hedges, g.size()),
      [&](GNode n) {
...
void parallel_refine_KF(GGraph& g, float tol, unsigned refineTo) {
  ...
  unsigned Size =
      g.hnodes; // std::distance(g.cellList().begin(), g.cellList().end());
...
void parallel_make_balance(GGraph& g, float tol, int p) {
  ...
  while (1) {
    ...
    if (bal < lo) {
      ...
      // placing each node in an appropriate bucket using the gain by weight
      // ratio
      galois::do_all(galois::iterate(g), [&](GNode n) {
        if (n < g.hedges)
          return;
...
void make_balance(GGraph& g, float tol, int p) {
  ...
  galois::do_all(
      ...
      galois::loopname("make balance"));
  // std::cout<<"weight of 0 : "<< nodeSize.reduce() - accum.reduce()<<"  1:
  // "<<accum.reduce()<<"\n";
  const int hi = (1 + tol) * nodeSize.reduce() / (2 + tol);
  const int lo = nodeSize.reduce() - hi;
  int bal      = accum.reduce();
  ...
  while (1) {
    ...
    if (bal < lo) {
      ...
      std::sort(
          nodeListo.begin(), nodeListo.end(), [&g](GNode& lpw, GNode& rpw) {
            if (fabs((float)((g.getData(lpw).getGain()) *
                             (1.0f / g.getData(lpw).getWeight())) -
                     (float)((g.getData(rpw).getGain()) *
                             (1.0f / g.getData(rpw).getWeight()))) < 0.00001f)
