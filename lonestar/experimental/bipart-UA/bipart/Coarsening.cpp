...
// hyper edge matching
template <MatchingPolicy matcher>
void parallelHMatchAndCreateNodes(MetisGraph* graph, int iter) {
      ...
      [&](GNode item) {
       ...
       // }
        ...
        if (!edges.empty()) {
          ...
          // create cell node
          for (auto pp : edges) {
            coarseGGraph->getData(N).nodeid =
                std::min(coarseGGraph->getData(N).nodeid,
                         fineGGraph->getData(pp).nodeid);
...
void moreCoarse(MetisGraph* graph, int iter) {
      ...
      [&](GNode item) {
        ...
        for (auto edge : fineGGraph->edges(item)) {
          ...
          if (!fineGGraph->getData(e).isMatched()) {
            ...
            //           if (coarseGGraph->getData(nn).getWeight() + w > LIMIT)
            //           continue;
  ...
  for (auto c : bag) {
    ...
    //  coarseGGraph->getData(nn).nodeid =
    //  std::min(coarseGGraph->getData(nn).nodeid,
    //  fineGGraph->getData(c).nodeid);
    int ww = coarseGGraph->getData(nn).getWeight();
...
// Coarsening phaseII
void coarsePhaseII(MetisGraph* graph, int iter) {
...
void findMatching(MetisGraph* coarseMetisGraph, scheduleMode sch, int iter) {
  ...
  switch (sch) {
  case PLD:
    parallelHMatchAndCreateNodes<PLD_f>(coarseMetisGraph, iter);
    break;
  case RAND:
    parallelHMatchAndCreateNodes<RAND_f>(coarseMetisGraph, iter);
    break;
  case PP:
    parallelHMatchAndCreateNodes<PP_f>(coarseMetisGraph, iter);
    break;
  case WD:
    parallelHMatchAndCreateNodes<WD_f>(coarseMetisGraph, iter);
    break;
  case RI:
    parallelHMatchAndCreateNodes<RI_f>(coarseMetisGraph, iter);
    break;
  case MRI:
    parallelHMatchAndCreateNodes<MRI_f>(coarseMetisGraph, iter);
    break;
  case MWD:
    parallelHMatchAndCreateNodes<MWD_f>(coarseMetisGraph, iter);
    break;
  case DEG:
    parallelHMatchAndCreateNodes<DEG_f>(coarseMetisGraph, iter);
    break;
  case MDEG:
    parallelHMatchAndCreateNodes<MDEG_f>(coarseMetisGraph, iter);
    break;
  }
...
MetisGraph* coarsen(MetisGraph* fineMetisGraph, unsigned coarsenTo,
  ...
  unsigned size = std::distance(fineMetisGraph->getGraph()->cellList().begin(),
                                fineMetisGraph->getGraph()->cellList().end());
  ...
  while (size > coarsenTo) {
    ...
    newSize     = std::distance(coarseGraph->getGraph()->cellList().begin(),
                            coarseGraph->getGraph()->cellList().end());
    if (newSize < coarsenTo)
      break;
    int netsize = std::distance(coarseGraph->getGraph()->getNets().begin(),
                                coarseGraph->getGraph()->getNets().end());
