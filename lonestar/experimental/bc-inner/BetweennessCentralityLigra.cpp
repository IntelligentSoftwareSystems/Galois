...
static cll::opt<Algo>
    algo("algo", cll::desc("Choose an algorithm:"),
         cll::values(clEnumValN(Algo::ligra, "ligra",
                                "Using Ligra programming model"),
...
struct LigraAlgo : public galois::ligraGraphChi::ChooseExecutor<UseGraphChi> {
  ...
  struct BackwardPass {
    ...
    bool operator()(GTy& graph, typename GTy::GraphNode src,
      ...
      SNode& ddata = graph.getData(dst, galois::MethodFlag::UNPROTECTED);
      float oldValue =
          ddata.dependencies.atomicIncrement(sdata.dependencies.read());
      return oldValue == 0.0;
    }
  };
  ...
  void operator()(Graph& graph, GNode source) {
    ...
    while (!frontier->empty()) {
      ...
      this->outEdgeMap(memoryLimit, graph, ForwardPass(), *frontier, *output,
                       false);
    ...
    for (int r = round - 2; r >= 0; --r) {
      ...
      this->inEdgeMap(memoryLimit, graph, BackwardPass(), *frontier, output,
                      false);
      delete frontier;
      frontier = levels[r];
      // galois::do_all(*frontier, [&](GNode n) {
...
void run() {
  ...
  if (!skipVerify) {
    ...
    for (typename Graph::iterator ii = graph.begin(), ei = graph.end();
         ii != ei && count < 20; ++ii, ++count) {
      std::cout << count << ": " << std::setiosflags(std::ios::fixed)
                << std::setprecision(6)
                << graph.getData(*ii).dependencies.read() << " "
                << (int)round(1.0 / graph.getData(*ii).numPaths.read()) << "\n";
    }
    count = 0;
    // for (typename Graph::iterator ii = graph.begin(), ei = graph.end(); ii !=
    // ei; ++ii, ++count)
    //   std::cout << ((count % 128 == 0) ? "\n" : " ") << (int)round(1.0 /
    //   graph.getData(*ii).numPaths.read());
    std::cout << "\n";
  }
}
