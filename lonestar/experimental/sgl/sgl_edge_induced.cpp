...
const char* name = "Sgl";
const char* desc =
    "Listing occurances of a given pattern in a graph using DFS extension";
const char* url = 0;
...
public:
  ...
  bool toAdd(unsigned n, const BaseEmbedding& emb, VertexId dst, unsigned pos) {
    ...
    if (get_degree(graph, dst) < get_degree(query_graph, next_qnode))
      return false;
  ...
  void vertex_process_naive() {
    ...
    galois::do_all(
        ...
        [&](const auto& src) {
          ...
          if (get_degree(graph, src) < get_degree(query_graph, curr_qnode))
            return;
          emb_list->init_vertex(src);
          dfs_extend_base(1, 0, *emb_list);
        },
        galois::chunk_size<CHUNK_SIZE>(), galois::steal(),
        galois::no_conflicts(), galois::loopname("DfsVertexNaiveSolver"));
  }
  ...
  void edge_process_naive() {
    ...
    galois::do_all(
        galois::iterate((size_t)0, edge_list.size()),
        [&](const size_t& pos) {
          Edge edge = edge_list.get_edge(pos);
          if (get_degree(graph, edge.src) < get_degree(query_graph, qnode0))
            return;
          if (get_degree(graph, edge.dst) < get_degree(query_graph, qnode1))
            return;
          EmbeddingList* emb_list = emb_lists.getLocal();
          if ((fv &&
               automorph_group_id[qnode0] != automorph_group_id[qnode1]) ||
              edge.src < edge.dst) {
            emb_list->init_edge(edge);
            // if (debug) std::cout << "new embedding: src = " << edge.src << ",
            // dst = " << edge.dst << "\n";
            dfs_extend_base(1, 0, *emb_list);
        ...
        galois::chunk_size<CHUNK_SIZE>(), galois::steal(),
        galois::no_conflicts(), galois::loopname("DfsEdgeNaiveSolver"));
  }
  inline void dfs_extend_base(unsigned level, unsigned pos,
    ...
    if (n == max_size - 1) {
      for (auto q_edge : query_graph->edges(next_qnode)) {
        ...
        unsigned q_order =
            matching_order_map[q_dst]; // using query vertex id to get its
                                       // matching order
        if (q_order < n) {
          ...
          for (auto d_edge : graph->edges(d_src)) {
            ...
            if (toAdd(n, emb, d_dst, q_order)) {
              ...
              // if (debug) std::cout << "\t extending: dst = " << d_dst << ",
              // src = " << d_src << "\n";
              total_num += 1; // if size = max_size, no need to add to the
    ...
    for (auto q_edge : query_graph->edges(next_qnode)) {
      VertexId q_dst   = query_graph->getEdgeDst(q_edge);
      unsigned q_order = matching_order_map[q_dst]; // using query vertex id to
                                                    // get its matching order
      if (q_order < n) {
        auto d_src = emb.get_vertex(q_order);
        // if (debug) std::cout << "\textending: q_order = " << q_order << ",
        // src = " << d_src << "\n";
        for (auto d_edge : graph->edges(d_src)) {
          auto d_dst = graph->getEdgeDst(d_edge);
          // if (debug) std::cout << "\tnew node dst = " << d_dst << "\n";
          if (toAdd(n, emb, d_dst, q_order)) {
            // if (debug) std::cout << "\textending: dst = " << d_dst << ", src
            // = " << d_src << "\n";
...
int main(int argc, char** argv) {
  ...
  Tinitial.stop();
  assert(k > 2);
  std::cout << "Data_graph: num_vertices " << data_graph.size() << " num_edges "
            << data_graph.sizeEdges() << "\n";
  std::cout << "Query_graph: num_vertices " << query_graph.size()
            << " num_edges " << query_graph.sizeEdges() << "\n";
