...
const char* name = "Sgl";
const char* desc =
    "Listing occurances of a given pattern in a graph using BFS extension";
const char* url = 0;
...
public:
  ...
  bool toAdd(unsigned n, const BaseEmbedding& emb, VertexId dst, unsigned pos) {
    ...
    if (debug) {
      ...
      std::cout << "\t n = " << n << ", pos = " << pos << ", src = " << src
                << ", dst = " << dst << "\n";
    }
    // std::cout << ", deg(d) = " << get_degree(graph, dst) << ", deg(q) = " <<
    // get_degree(query_graph, pos+1);
  ...
  inline void extend_vertex(BaseEmbeddingQueue& in_queue,
    ...
    galois::do_all(
        ...
        [&](const BaseEmbedding& emb) {
          ...
          VertexId next_qnode = get_query_vertex(
              n); // using matching order to get query vertex id
            ...
            if (q_order < n) {
              ...
              for (auto d_edge : graph->edges(d_vertex)) {
                ...
                if (toAdd(n, emb, d_dst, q_order)) {
                  if (n < max_size - 1) { // generate a new embedding and add it
                                          // to the next queue
...
int main(int argc, char** argv) {
  ...
  for (size_t i = 0; i < data_graph.size(); ++i) {
    if (get_degree(data_graph, i) < get_degree(query_graph, curr_qnode))
      continue;
    EmbeddingType emb;
    emb.push_back(i);
    queue.push_back(emb);
