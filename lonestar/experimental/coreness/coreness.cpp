...
enum Algo { naive = 0, peeling, hindex };

static cll::opt<std::string>
    inputFName(cll::Positional, cll::desc("<input graph>"), cll::Required);
static cll::opt<unsigned int> numThreads("t", cll::desc("Number of threads"),
                                         cll::init(56));
static cll::opt<Algo>
    algo("algo", cll::desc("Choose an algorithm:"),
         cll::values(clEnumVal(naive, "naive"), clEnumVal(peeling, "peeling"),
...
void initialize(Graph& graph) {
  galois::do_all(
      ...
      [&](GNode cNode) {
        ...
        cNDegree.store(
            std::distance(graph.edge_begin(cNode), graph.edge_end(cNode)));
      },
...
uint8_t parFindKCore(Graph& graph, DegreeTy k) {
  ...
  // Perform trimming.
  galois::for_each(
      galois::iterate(cur.begin(), cur.end()),
      [&](GNode cNode, auto& ctx) {
        ...
        for (auto e : graph.edges(cNode)) {
          ...
          auto oldVal          = galois::atomicSubtract(nghDegree, 1ul);
          if (oldVal == k + 1) {
            ctx.push(neigh);
          }
        }
 ...
 *         returns true. Otherwise, return false.
 */
int8_t parFindKCore(Graph& graph, DegreeTy k, aDegreeTy& nextK,
  ...
  // Perform trimming.
  galois::for_each(
      galois::iterate(curr.begin(), curr.end()),
      [&](GNode cNode, auto& ctx) {
        /* If the current node has degress that
           lower than k,
           then ignores and removes from
...
DegreeTy peelingCoreness(Graph& graph, galois::InsertBag<GNode>& curr) {
  ...
  do {
    cand_k    = minDegree;
    minDegree = MAX_DEGREE;
    if (!parFindKCore(graph, cand_k, minDegree, curr))
      break;
...
bool H(Graph& graph, VecByteTy& isDead, GNode cNode,
  ...
  adjHs.clear();
  adjHs.resize(
      std::distance(graph.edge_begin(cNode), graph.edge_end(cNode)) + 1, 0);
...
DegreeTy hIndexCoreness(Graph& graph, ThreadLocalData& nodesThreadLocal) {
  ...
  galois::do_all(
      ...
      galois::loopname("HIndexCorenessInitialization"), galois::no_conflicts(),
      galois::steal());
  ...
  do {
    ...
    galois::do_all(
        ...
        galois::loopname("HIndexCoreness"), galois::steal(),
...
int main(int argc, char** argv) {
  ...
  std::cout << "Graph [" << inputFName << "] has coreness of " << coreness
            << std::endl;
  if (outputP) {
    std::ofstream fp;
