...
void init(Graph& graph, Miner& miner, PatternMap3D& pattern_map,
          PatternQueue& queue) {
  ...
  if (show)
    printf("\n=============================== Init "
           "===============================\n\n");
  // classify each edge into its single-edge pattern accodding to its
  ...
  for (auto src : graph) {
    ...
    for (auto e = first; e != last; ++e) {
      ...
      if (src_label <=
          dst_label) { // when src_label == dst_label (the edge will be added
                       // twice since the input graph is symmetrized)
        // if (src_label < dst_label || (src_label == dst_label && src < dst)) {
        if (pattern_map.count(src_label) == 0 ||
            pattern_map[src_label].count(elabel) == 0 ||
            pattern_map[src_label][elabel].count(dst_label) == 0)
          single_edge_dfscodes++;
        LabEdge* eptr = &(miner.edge_list[*e]);
        pattern_map[src_label][elabel][dst_label].push(
            2, eptr, 0); // single-edge embedding: (num_vertices, edge,
                         // pointer_to_parent_embedding)
        num_embeddings++;
      }
    }
  }
  int dfscodes_per_thread =
      (int)ceil((single_edge_dfscodes * 1.0) / numThreads);
  ...
  // for each single-edge pattern, generate a DFS code and push it into the task
  // queue
  for (EmbeddingList_iterator3 fromlabel = pattern_map.begin();
       fromlabel != pattern_map.end(); ++fromlabel) {
    for (EmbeddingList_iterator2 elabel = fromlabel->second.begin();
...
void FsmSolver(Graph& graph, Miner& miner) {
  ...
  init(graph, miner, pattern_map,
       task_queue); // insert single-edge patterns into the queue
