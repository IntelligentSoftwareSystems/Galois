...
struct CL_Kernel_Helper {
  ...
  static void build_program_source(const char* file_name,
                                   const char* _flags = "") {
    cl_int err;
    std::string compiler_flags(_flags);
    //      compiler_flags +=
    //      device_id->get_platrform()->get_cl_compiler_flags();
    compiler_flags += "-I. ";
    ...
    /*std::cout<<"================KERNEL===============\n";
     std::cout<<src<<"\n";
     std::flush(std::cout);*/
    cl_env.m_program = clCreateProgramWithSource(
        cl_env.m_context, 1, (const char**)&src, NULL, &err);
    ...
    err = clBuildProgram(cl_env.m_program, 1, &dev_id, compiler_flags.c_str(),
                         NULL, NULL);
    if (err != CL_SUCCESS) {
      size_t len = 0;
      cl_int build_status;
      cl_int err2 = clGetProgramBuildInfo(cl_env.m_program, cl_env.m_device_id,
                                          CL_PROGRAM_BUILD_STATUS, 0,
                                          &build_status, NULL);
      //      CHECK_CL_ERROR(err2, "Failed to build program executable from
      //      source");
      cl_int err3  = clGetProgramBuildInfo(cl_env.m_program, cl_env.m_device_id,
                                          CL_PROGRAM_BUILD_LOG, 0, NULL, &len);
      char* buffer = new char[len];
      fprintf(stderr, "Build log length:: %zd\n", len);
      err3 = clGetProgramBuildInfo(cl_env.m_program, cl_env.m_device_id,
    ...
    }
    {
      ...
      size_t bin_sz;
      err = clGetProgramInfo(cl_env.m_program, CL_PROGRAM_BINARY_SIZES,
                             sizeof(size_t), &bin_sz, NULL);

      // Read binary (PTX file) to memory buffer
      unsigned char* bin = (unsigned char*)malloc(bin_sz);
      err = clGetProgramInfo(cl_env.m_program, CL_PROGRAM_BINARIES,
  ...
  static void build_string_source(const char* src, const char* flags = "") {
    ...
    //      compiler_flags +=
    //      device_id->get_platrform()->get_cl_compiler_flags(); compiler_flags
    //      += " "; compiler_flags += build_args;
    compiler_flags += "-I.";
    ...
    err = clBuildProgram(cl_env.m_program, 0, NULL, compiler_flags.c_str(),
                         NULL, NULL);
    if (err != CL_SUCCESS) {
      size_t len;
      char buffer[10 * 2048];
      clGetProgramBuildInfo(cl_env.m_program, cl_env.m_device_id,
   ...
   *
   **********************************************************************/
  static cl_kernel load_kernel_string(const char* src,
                                      const char* kernel_method_name) {
    build_string_source(src, "");
    int err;
    cl_kernel kernel =
        clCreateKernel(cl_env.m_program, kernel_method_name, &err);
   ...
   *
   **********************************************************************/
  static cl_kernel load_kernel(const char* kernel_file_name,
                               const char* kernel_method_name) {
  ...
  static size_t workgroup_size(cl_kernel kernel) {
    ...
    CHECK_CL_ERROR(clGetKernelWorkGroupInfo(
                       kernel, cl_env.m_device_id, CL_KERNEL_WORK_GROUP_SIZE,
  ...
  static size_t local_memory_size() {
    cl_ulong local_mem;
    CHECK_CL_ERROR(clGetDeviceInfo(cl_env.m_device_id, CL_DEVICE_LOCAL_MEM_SIZE,
  ...
  static cl_uint get_device_eu() {
    cl_uint num_eus;
    CHECK_CL_ERROR(clGetDeviceInfo(cl_env.m_device_id,
...
struct CL_Kernel {
  ...
  void init(const char* filename, const char* kernel_name) {
    ...
    kernel =
        galois::opencl::CL_Kernel_Helper::load_kernel(filename, kernel_name);
  }
  /*************************************************************************
   *
   *************************************************************************/
  void init_string(const char* src, const char* kernel_name) {
    ...
    global = local = 0;
    kernel =
        galois::opencl::CL_Kernel_Helper::load_kernel_string(src, kernel_name);
  }
  /*************************************************************************
   *
   *************************************************************************/
  ...
  cl_ulong get_shared_memory_usage() {
    cl_ulong mem_usage;
    CHECK_CL_ERROR(clGetKernelWorkGroupInfo(kernel, cl_env.m_device_id,
  ...
  void set_arg_max_shmem(unsigned int index) {
    size_t sz = CL_Kernel_Helper::get_device_shared_memory() -
                get_shared_memory_usage();
    galois::opencl::CHECK_CL_ERROR(clSetKernelArg(kernel, index, sz, nullptr),
  ...
  void operator()() {
    galois::opencl::CHECK_CL_ERROR(
        clEnqueueNDRangeKernel(cl_env.m_command_queue, kernel, 1, nullptr,
  ...
  void operator()(cl_event& e) {
    galois::opencl::CHECK_CL_ERROR(
        clEnqueueNDRangeKernel(cl_env.m_command_queue, kernel, 1, NULL, &global,
                               &local, 0, NULL, &e),
        name);
  }
  /*************************************************************************
