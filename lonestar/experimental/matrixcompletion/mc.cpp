...
static cll::opt<unsigned>
    usersPerBlockSlice("usersPerBlk", cll::desc("[users per block slice]"),
                       cll::init(2048));
static cll::opt<unsigned>
    moviesPerBlockSlice("moviesPerBlk", cll::desc("[movies per block slice]"),
                        cll::init(350));
...
static cll::opt<int> shiftFactor("shiftRating",
                                 cll::desc("Shift ratings down by"),
                                 cll::init(0));
...
static cll::opt<Algo> algo(
    ...
    cll::values(
        ...
        clEnumVal(blockAndSliceUsers,
                  "Block by Users and Movies, Slice by Users"),
        clEnumVal(
            blockAndSliceBoth,
            "Block by Users and Movies, Slice by Users and Movies (default)"),
        clEnumVal(sliceMarch, "Marching Slices version"),
        clEnumVal(sliceJump, "Jumping Slices version"), clEnumValEnd),
...
typedef typename galois::graphs::LC_CSR_Graph<Node, int>::with_numa_alloc<
    true>::type ::with_no_lockable<false>::type Graph;
...
void verify(Graph& g) {
  ...
  galois::do_all(g, [&g, &rms](GNode n) {
    for (auto e     = g.edge_begin(n, galois::MethodFlag::UNPROTECTED),
              e_end = g.edge_end(n, galois::MethodFlag::UNPROTECTED);
         e != e_end; ++e) {
...
struct sgd_node_movie {
  ...
  void operator()(GNode node, Context& cnx) {
    for (auto ii = g.edge_begin(node), ee = g.edge_end(node); ii != ee; ++ii)
      doGradientUpdate(g.getData(node), g.getData(g.getEdgeDst(ii)),
                       g.getEdgeData(ii), step_size);
  }

  static void go(Graph& g, unsigned int numMovieNodes,
                 unsigned int numUserNodes, const LearnFN* lf) {
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      galois::for_each(Movies.begin(), Movies.end(),
                       sgd_node_movie(g, step_size));
    }
  }
};
...
struct sgd_node_movie_pri {
  ...
  void operator()(GNode node, Context& cnx) {
    for (auto ii = g.edge_begin(node), ee = g.edge_end(node); ii != ee; ++ii) {
      double e1 = doGradientUpdate(g.getData(node), g.getData(g.getEdgeDst(ii)),
                                   g.getEdgeData(ii), step_size);
      double e2 = g.getEdgeData(ii) -
                  calcPrediction(g.getData(node), g.getData(g.getEdgeDst(ii)));
      if (std::abs(e1 - e2) > 20) {
  ...
  static void go(Graph& g, unsigned int numMovieNodes,
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      //   std::random_shuffle(Movies.begin(), Movies.end());
      galois::for_each(Movies.begin(), Movies.end(),
                       sgd_node_movie_pri(g, step_size),
...
struct sgd_edge_movie {
  ...
  void operator()(GNode node, Context& cnx) {
    auto ii = g.edge_begin(node, galois::MethodFlag::UNPROTECTED),
         ee = g.edge_end(node, galois::MethodFlag::UNPROTECTED);
    if (ii == ee)
      return;
  ...
  void operator()(ThreadWorkItem& workItem) const {
    ...
    while (currentBlockSliceEnd < userRangeEnd) {
      ...
      for (; movie_it != end_movie_it; ++movie_it) {
        ...
        unsigned int currentBlockSliceEndUserId =
            currentBlockSliceEnd + NUM_MOVIE_NODES; // + 1;
  ...
  for (unsigned int update = 0; update < MAX_MOVIE_UPDATES; update++) {
    ...
    for (unsigned int j = 0; j < numWorkItems; j++) {
      ...
      for (unsigned int k = 0; k < numWorkItems; k++) {
        ...
        // ") " << (long) (wi.timeTaken/1000) << "/" << (long) wi.updates;
...
struct sgd_march {
  ...
  SpinLock* locks;
  double step_size;
  sgd_march(Graph& g, SpinLock* locks, double ss)
      : g(g), locks(locks), step_size(ss) {}
  ...
  void operator()(ThreadWorkItem& workItem) const {
    ...
    while (sliceUpdates < MAX_MOVIE_UPDATES * workItem.numSlices) {
      ...
      // if(workItem.id == 0)
      //  printf("Currslice %d Slice from %d to ", currentSliceId,
      ...
      for (; movie_it != end_movie_it; ++movie_it) {
        ...
        for (; edge_it != edge_end; ++edge_it, ++movie_data.edge_offset) {
          ...
          // NUM_MOVIE_NODES); if(workItem.id == 13) printf("updates %d\n",
          // updates);
        }
    ...
    workItem.timeTaken = timer.get_usec();
    workItem.updates   = updates;
    std::cout << workItem.id << " " << workItem.updates << " "
              << workItem.timeTaken / 1000000 << " " << conflicts << std::endl;
    // printf("Worker %d took %u to do %u updates.\n", workItem.id,
    // workItem.timeTaken/1000000, workItem.updates);
  }
};
...
void runSliceMarch(Graph& g, const LearnFN* lf) {
  ...
  // move the edge iterators of each movie to the start of the current block
  // advances the edge iterator until it reaches the userRangeStart field of the
  // ThreadWorkItem userRangeStart isn't needed after this point
  galois::do_all(workItems + 0, workItems + numWorkItems,
                 advance_edge_iterators(g));
  galois::do_all(workItems + 0, workItems + numWorkItems,
                 sgd_march(g, locks, step_size));
...
typedef struct SliceInfo {
  ...
  void print() {
    printf("id: %d x: %d y: %d userStart: %d userEnd: %d movieStart: %d "
           "movieEnd: %d updates: %d\n",
...
struct calculate_user_offsets {
  ...
  calculate_user_offsets(Graph& _g, SliceInfo* _slices, unsigned _mps,
                         unsigned _nxs, unsigned _nys)
      : g(_g), slices(_slices), moviesPerSlice(_mps), numXSlices(_nxs),
        numYSlices(_nys), numSlices(_nxs * _nys) {}
  ...
  void operator()(GNode movie) const {
    ...
    numXSlices, numXSlices, sliceY);
    }*/

    // for each edge in the range
    Graph::edge_iterator edge_it = g.edge_begin(
                             movie, galois::MethodFlag::UNPROTECTED),
                         edge_end =
                             g.edge_end(movie, galois::MethodFlag::UNPROTECTED);
    ...
    for (int i = 0, offset = 0; i < numXSlices; i++, s++) {
      ...
      if (user >= userIdToUserNode(s->userStart) &&
          user < userIdToUserNode(s->userEnd)) {
        s->userOffsets[pos] = offset;

        // move the edge_it beyond slice's range
        while (edge_it != edge_end &&
               g.getEdgeDst(edge_it) < userIdToUserNode(s->userEnd)) {
          ++edge_it;
          ++offset;
        }
      } else {
...
struct sgd_slice_jump {
  ...
  double step_size;
  sgd_slice_jump(Graph& g, SpinLock* _xLocks, SpinLock* _yLocks,
                 SliceInfo* _slices, unsigned _numXSlices, unsigned _numYSlices,
                 double _step_size)
      : g(g), xLocks(_xLocks), yLocks(_yLocks), slices(_slices),
        numXSlices(_numXSlices), numYSlices(_numYSlices),
        numSlices(_numXSlices * _numYSlices), step_size(_step_size) {}

  // Preconditions: row and column of slice are locked
  // Postconditions: increments update count, does sgd update on each movie and
  // user in the slice
  ...
  inline int getNextSlice(SliceInfo* sp) const {
    ...
    while (foundWork || workSearchTries < 2 * numSlices) {
      ...
      nextSliceId++;
      if (nextSliceId == numSlices)
        nextSliceId = 0; // wrap around

      SliceInfo& nextSlice = slices[nextSliceId];

      if (nextSlice.updates < MAX_MOVIE_UPDATES &&
          xLocks[nextSlice.x].try_lock()) {
  ...
  void operator()(SliceInfo* startSlice) const {
    ...
    printf("Slice: (%d, %d) Edges: %d Slices: %d Time: %f\n", startSlice->x,
           startSlice->y, tot_updates, slices_updated,
           timer.get_usec() / 1000000.0);
...
void runSliceJump(Graph& g, const LearnFN* lf) {
  ...
  const unsigned numXSlices =
      std::max(NUM_USER_NODES / usersPerBlockSlice, threadCount + 1);
  const unsigned numYSlices =
      std::max(NUM_MOVIE_NODES / moviesPerBlockSlice, threadCount + 1);
  ...
  for (unsigned i = 0; i < numSlices; i++) {
    ...
    si.userEnd =
        si.x == numXSlices - 1 ? NUM_USER_NODES : (si.x + 1) * usersPerSlice;
    si.movieStart = si.y * moviesPerSlice;
    si.movieEnd =
        si.y == numYSlices - 1 ? NUM_MOVIE_NODES : (si.y + 1) * moviesPerSlice;
    si.updates = 0;
  ...
  galois::do_all(g.begin(), g.begin() + NUM_MOVIE_NODES,
                 calculate_user_offsets(g, slices, moviesPerSlice, numXSlices,
                                        numYSlices));
  ...
  galois::do_all(&startSlices[0], &startSlices[threadCount],
                 sgd_slice_jump(g, xLocks, yLocks, slices, numXSlices,
                                numYSlices, step_size));
}
...
int main(int argc, char** argv) {
  ...
  if (shiftFactor != 0)
    for (auto ii = g.begin(), ee = g.end(); ii != ee; ++ii)
      for (auto eii = g.edge_begin(*ii), eee = g.edge_end(*ii); eii != eee;
           ++eii)
  ...
  std::cout << "SUMMARY Movies " << numMovieNodes << " Users " << numUserNodes
            << " Ratings " << g.sizeEdges() << " usersPerBlockSlice "
            << usersPerBlockSlice << " moviesPerBlockSlice "
            << moviesPerBlockSlice << " Time " << timer.get() / 1000.0
            << std::endl;
