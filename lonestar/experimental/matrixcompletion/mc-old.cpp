...
void runBlockSlices(Graph& g, unsigned int threadCount,
                    unsigned int usersPerBlockSlice) {
...
void runSliceMarch(Graph& g, unsigned int threadCount,
                   unsigned int usersPerBlockSlice) {
  unsigned numWorkItems = threadCount;
  ThreadWorkItem workItems[numWorkItems];
  unsigned int moviesPerThread = NUM_MOVIE_NODES / numWorkItems;
...
int main(int argc, char** argv) {
  if (argc < 4) {
    std::cout << "Usage: <input binary gr file> <thread count> <users per "
                 "block slice>"
              << std::endl;
    return -1;
  }
  ...
  std::cout << "SUMMARY Movies " << numMovieNodes << " Users " << numUserNodes
            << " Ratings " << g.sizeEdges() << " Threads " << threadCount
            << " SliceSz " << usersPerBlockSlice << " Time "
            << timer.get() / 1000.0 << std::endl;
