...
enum Algo { coloring, foreach, delay, delayPart, doall };
...
static cll::opt<Algo> algo(
    ...
    cll::values(clEnumValN(Algo::coloring, "Coloring",
                ...
                clEnumValN(Algo::delay, "Delay",
                           ...
                           "delay the updation"),
                clEnumValN(Algo::delayPart, "DelayPart",
                           "Using galois for_each for conflict mitigation but "
                           "delay the updation"),
...
// double algoLouvainWithLocking(Graph &graph, largeArray& clusters, double
// lower, double threshold) {
    ...
    galois::for_each(
        ...
        [&](GNode n, auto& ctx) {
          ...
          if (local_target != n_data.curr_comm_ass && local_target != -1) {
            ...
            // n_data.degree_wt); galois::atomicAdd(c_update[local_target].size,
            // (uint64_t)1);
...
double algoLouvainWithLockingDelayUpdate(Graph& graph, double lower,
  ...
  // std::vector<int64_t> local_target(graph.size(), -1);
  while (true) {
    num_iter++;
    ...
    galois::do_all(
        ...
        [&](GNode n) {
          ...
          if (degree > 0) {
            ...
            // Find the max gain in modularity
            // local_target[n] = maxModularity(cluster_local_map, counter,
            // self_loop_wt, c_info, n_data.degree_wt, n_data.curr_comm_ass,
            // constant_for_second_term);
            local_target[n] = maxModularityWithoutSwaps(
                cluster_local_map, counter, self_loop_wt, c_info,
                n_data.degree_wt, n_data.curr_comm_ass,
                constant_for_second_term);
...
double algoLouvainWithoutLockingDelayPartUpdate(Graph& graph, double lower,
                                                double threshold,
                                                uint32_t& iter) {
  galois::gPrint("Inside algoLouvainWithoutLockingDelayPart\n");
...
*/
      ...
      galois::do_all(
          ...
          [&](GNode n) {
            ...
            /* Update c info: Asynchronously TODO: Is this correct? */
            // galois::atomicAdd(c_info[n].size,  c_update[n].size.load());
            // galois::atomicAdd(c_info[n].degree_wt,
            // c_info[n].degree_wt.load());
          },
          galois::loopname("louvain algo: Phase 1"));
      ...
      /*							 galois::do_all(galois::iterate(bag[idx]),
                         [&](GNode n) {
                           ...
                           n_data.prev_comm_ass = n_data.curr_comm_ass;
                           n_data.curr_comm_ass = local_target[n];
                           //galois::atomicAdd(c_info[n].size,
         c_update[n].size.load());
                           //galois::atomicAdd(c_info[n].degree_wt,
         c_info[n].degree_wt.load()); c_info[n].size += c_update[n].size.load();
                           c_info[n].degree_wt += c_update[n].degree_wt.load();
      ...
      */
      ...
      /*    double e_xx = 0;
          double a2_x = 0;
          curr_mod = calModularityDelay(graph, c_info, c_update, e_xx, a2_x,
         constant_for_second_term, local_target); galois::gPrint(num_iter, " ",
         e_xx, "        ", a2_x, "        ", lower, "      ", prev_mod, " ",
         curr_mod, "\n");
      */
      /*    if((curr_mod - prev_mod) < threshold_mod){
            galois::gPrint("Modularity gain: ", (curr_mod - prev_mod) , " < ",
         threshold_mod, " \n"); prev_mod = curr_mod; flag_break = true; break;
          }
      ...
      */
      ...
      // galois::do_all(galois::iterate(toProcess),
      //             [&](GNode n) {
      // galois::atomicAdd(c_info[n].size,  c_update[n].size.load());
      // galois::atomicAdd(c_info[n].degree_wt, c_info[n].degree_wt.load());
...
// double algoLouvainWithColoring(Graph &graph, largeArray& clusters, double
// lower, double threshold) {
          ...
          [&](GNode n) {
            ...
            if (n_data.colorId == c) {
              ...
              if (degree > 0) {
                ...
                cluster_local_map[graph.getData(n).curr_comm_ass] =
                    0;                // Add n's current cluster
                counter.push_back(0); // Initialize the counter to zero (no
                                      // edges incident yet)
              ...
              if (local_target != n_data.curr_comm_ass && local_target != -1) {
                ...
                // galois::atomicAdd(c_info[local_target].degree_wt,
                // n_data.degree_wt);
                // galois::atomicAdd(c_info[local_target].size, (uint64_t)1);
                // galois::atomicSubtract(c_info[n_data.curr_comm_ass].degree_wt,
                // n_data.degree_wt);
  ...
  while (true) {
    ...
    if ((*graph_curr).size() > min_graph_size) {
      ...
      case delay:
        ...
        break;
      case delayPart:
        curr_mod = algoLouvainWithoutLockingDelayPartUpdate(
    ...
    if (!enable_VF && phase == 1) {
      ...
      // galois::gPrint("ARRAY unique cluster: ",
...
int main(int argc, char** argv) {
  ...
  // runMultiPhaseLouvainAlgorithm(graph, clusters_orig, min_graph_size,
  // c_threshold);
