...
void AigParser::connectOutputs() {
	...
	for (int in = 0; in < o; in++) {
		...
		aig::NodeData & outputNodeData = aigGraph.getData( outputNode, galois::MethodFlag::WRITE );
		//outputNodeData.nFanin = 1;

		int rhs = outputs[in];
		aig::GNode inputNode = aig.getNodes()[rhs / 2];
		aig::NodeData & inputNodeData = aigGraph.getData( inputNode, galois::MethodFlag::WRITE );
...
void AigParser::connectAnds() {
	...
	// Each andDef is composed by three nodes A B C, A is the AND itself, B and C are the two input nodes.
	for ( auto andDef : this->ands ) {

		int A = std::get<0>( andDef );
		aig::GNode andNode = aig.getNodes()[ A/2 ];
		aig::NodeData & andData = aigGraph.getData( andNode, galois::MethodFlag::WRITE );
		//andData.nFanin = 2;
		...
		aigGraph.getEdgeData( aigGraph.addMultiEdge( lhsNode, andNode, galois::MethodFlag::UNPROTECTED ) ) = !(B % 2);
		aigGraph.getEdgeData( aigGraph.addMultiEdge( rhsNode, andNode, galois::MethodFlag::UNPROTECTED ) ) = !(C % 2);
...
void AigParser::connectAndsWithFanoutMap() {
	...
	for ( auto andDef : this->ands ) {
		...
		aigGraph.getEdgeData( aigGraph.addMultiEdge( lhsNode, andNode, galois::MethodFlag::UNPROTECTED ) ) = lhsPol;
		aigGraph.getEdgeData( aigGraph.addMultiEdge( rhsNode, andNode, galois::MethodFlag::UNPROTECTED ) ) = rhsPol;
