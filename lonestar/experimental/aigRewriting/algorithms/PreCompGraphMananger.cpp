...
const unsigned short PreCompGraphMananger::aigSubgraphs[3562] =
{
...
PreCompGraphMananger::PreCompGraphMananger( NPNMananger & npnMananger ) : npnMananger( npnMananger ), table( npnMananger.getNFuncs() ), classes( 222 ) {

    this->nTravIds = 0;
    this->forestSize = 0;
    this->forest = (ForestNode*) malloc( sizeof( ForestNode ) * 1800 ); // Value based on the execution of ABC rewrite command.
...
PreCompGraphMananger::~PreCompGraphMananger() {
...
void PreCompGraphMananger::loadPreCompGraphFromArray() {
...
ForestNode * PreCompGraphMananger::addForestNode( ForestNode * p0, ForestNode * p1, int fExor, int Level, int Volume ) {
    ...
    if ( fExor ) {
        ...
        uTruth = (isForestNodeComplement(p0)? ~forestNodeRegular(p0)->uTruth : forestNodeRegular(p0)->uTruth) & 
                 (isForestNodeComplement(p1)? ~forestNodeRegular(p1)->uTruth : forestNodeRegular(p1)->uTruth) & 0xFFFF;
    }
    ...
    if ( uTruth != this->npnMananger.getCanons()[ uTruth ] ) {
        return pNew;
    }
...
ForestNode * PreCompGraphMananger::addForestVar( unsigned uTruth ) {
...
void PreCompGraphMananger::addForestNodeToTable( unsigned uTruth, ForestNode * node ) {
...
int PreCompGraphMananger::getForestNodeVolume( ForestNode * p0, ForestNode * p1 ) {
...
void PreCompGraphMananger::getVolumeRec( ForestNode * node, int * volume ) {
...
void PreCompGraphMananger::incTravId() {
...
bool PreCompGraphMananger::isForestNodeComplement( ForestNode * node ) { 
    return (bool) (((unsigned long int)node) & 01u);
}

ForestNode * PreCompGraphMananger::forestNodeRegular( ForestNode * node ) { 
    return (ForestNode*) ((unsigned long int)(node) & ~01u);  
}

ForestNode * PreCompGraphMananger::forestNodeComplement( ForestNode * node ) { 
    return (ForestNode*) ((unsigned long int)(node) ^ 01u);
}

ForestNode * PreCompGraphMananger::forestNodeComplementCond( ForestNode * node, int c )  {
    return (ForestNode *) ((unsigned long int)(node) ^ (c));  
}

void PreCompGraphMananger::processDecompositionGraphs() {

    DecGraph * decGraph;
    ForestNode * node;
    unsigned char * map = this->npnMananger.getMap();
    unsigned short * mapInv = this->npnMananger.getMapInv();
    unsigned short * canons = this->npnMananger.getCanons();

    // put the nodes into the structure
    for ( int i = 0; i < this->npnMananger.getNFuncs(); i++ ) {
...
DecGraph * PreCompGraphMananger::processNode( ForestNode * node ) {
...
DecEdge PreCompGraphMananger::processNodeRec( ForestNode * node, DecGraph * decGraph ) {
...
ForestNode * PreCompGraphMananger::getForest() {
	return this->forest;
}

std::vector< ForestNode* > & PreCompGraphMananger::getTable() {
	return this->table;
}

std::vector< std::vector< ForestNode* > > & PreCompGraphMananger::getClasses() {
	return this->classes;
}
...
DecNode * DecGraph::getNodes() {
	return this->pNodes;
}

DecNode * DecGraph::getNode( int i ) {
    return this->pNodes + i;
}
...
void DecGraph::setRootEdge( DecEdge eRoot ) {
	this->eRoot = eRoot;
}

DecEdge DecGraph::getRootEdge() {
	return this->eRoot;
}

int DecGraph::getLeaveNum() {
    return this->nLeaves;
}

int DecGraph::getNodeNum() {
    return this->nSize;
}

bool DecGraph::isConst() {
    return this->fConst;
}

bool DecGraph::isVar() {
    return this->eRoot.Node < (unsigned) this->nLeaves;
}

unsigned DecGraph::isComplement() {
    return this->eRoot.fCompl;
}

int DecGraph::nodeToInt( DecNode * node ) {
    return node - this->pNodes;
}
...
unsigned DecGraph::deriveTruth() {
    ...
    // check for a literal
    if ( this->isVar() ) {
        return this->isComplement() ? ~uTruths[ this->varToInt() ] : uTruths[ this->varToInt() ];
    }
    ...
    for ( int i = this->nLeaves; (i < this->nSize) && ((node = this->getNode( i )), 1); i++ ) {
