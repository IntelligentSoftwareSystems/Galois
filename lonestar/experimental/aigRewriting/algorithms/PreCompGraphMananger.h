/*
 ...
 Possani Precomputed graphs September 22, 2017.
 ABC-based implementation on Galois.

*/

#ifndef PRECOMPGRAPHMANANGER_h_
#define PRECOMPGRAPHMANANGER_h_
...
private:	
    ...
    std::vector< ForestNode* > table;                   // the hash table of nodes by their canonical form
	std::vector< std::vector< ForestNode* > > classes;  // the nodes of the equivalence classes
    int forestSize;
    int nTravIds;

	ForestNode * addForestNode( ForestNode * p0, ForestNode * p1, int fExor, int Level, int Volume );
...
public:
    ...
    PreCompGraphMananger( NPNMananger & npnMananger );
    ~PreCompGraphMananger();
