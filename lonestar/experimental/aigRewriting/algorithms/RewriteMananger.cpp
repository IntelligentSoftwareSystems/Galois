...
RewriteMananger::RewriteMananger( aig::Aig & aig, CutMananger & cutMan, NPNMananger & npnMan, PreCompGraphMananger & pcgMan, int triesNGraphs, bool useZeros, bool updateLevel ) :
...
RewriteMananger::~RewriteMananger() {
	// TODO
}

aig::GNode RewriteMananger::rewriteNode( ThreadContextData * threadCtx, aig::GNode node, GNodeVector & fanoutNodes ) {
	...
	threadCtx->bestMFFCIds.clear();
	threadCtx->bestMFFCBannedIds.clear();
    ...
    for ( cut = cutsBegin; cut != nullptr; cut = cut->nextCut ) {
		...
		if ( currentGraph != nullptr && bestGain < currentGain ) {
			...
			threadCtx->bestMFFCIds = threadCtx->currentMFFCIds;
			threadCtx->bestMFFCBannedIds = threadCtx->currentMFFCBannedIds;
        }
    }
	...
	for ( int id : threadCtx->bestMFFCBannedIds ) {
		threadCtx->bestMFFCIds.erase( id );
	}
...
void RewriteMananger::lockFaninCone(  aig::Graph & aigGraph, aig::GNode node, Cut * cut ) {
...
int RewriteMananger::labelMFFC( ThreadContextData * threadCtx, aig::GNode node, int threadId, int travId ) {

	aig::Graph & aigGraph = this->aig.getGraph();
    int nConeSize1, nConeSize2;

    threadCtx->currentMFFCIds.clear();
	...
	threadCtx->currentMFFCIds.insert( nodeData.id );
...
int RewriteMananger::refDerefMFFCNodes( ThreadContextData * threadCtx, aig::GNode node, int threadId, int travId, bool reference, bool label ) {
    ...
    if ( reference ) {
        ...
        if ( --lhsData.nFanout == 0 ) {
			threadCtx->currentMFFCIds.insert( lhsData.id );
            counter += refDerefMFFCNodes( threadCtx, lhsNode, threadId, travId, reference, label );
        }
        if ( --rhsData.nFanout == 0 ) {
			threadCtx->currentMFFCIds.insert( rhsData.id );
            counter += refDerefMFFCNodes( threadCtx, rhsNode, threadId, travId, reference, label );
        }
    }
...
DecGraph * RewriteMananger::evaluateCut( ThreadContextData * threadCtx, aig::GNode root, Cut * cut, int nNodesSaved, int maxLevel, int & bestGain ) {
	...
	threadCtx->currentMFFCBannedIds.clear();
    ...
    for ( int i = 0; i < nSubgraphs; i++ ) {
        ...
        if ( bestGain < nNodesSaved - nNodesAdded ) {
            ...
            bestGraph = currentGraph;
			threadCtx->currentMFFCBannedIds = threadCtx->currentGraphBannedIds;
        }
    }
...
*/
int RewriteMananger::decGraphToAigCount( ThreadContextData * threadCtx, aig::GNode root, DecGraph * decGraph, int maxNode, int maxLevel ) {
    ...
    aig::GNode rhsAnd;
    bool lhsPol, rhsPol;
    int newLevel, oldLevel, counter = 0;
    aig::Graph & aigGraph = this->aig.getGraph();

	threadCtx->currentGraphBannedIds.clear();
    ...
    for ( int i = decGraph->getLeaveNum(); (i < decGraph->getNodeNum()) && (( node = decGraph->getNode( i ) ), 1); i++ ) {
        ...
        if ( curAnd ) {
            ...
            if ( isMFFC ) {
				...
				threadCtx->currentGraphBannedIds.insert( curAndData.id );
...
aig::GNode RewriteMananger::updateAig( ThreadContextData * threadCtx, aig::GNode oldRoot, DecGraph * decGraph, GNodeVector & fanoutNodes, bool isOutputCompl, bool updateLevel, int gain ) {

	aig::Graph & aigGraph = this->aig.getGraph();

	for ( int id : threadCtx->bestMFFCIds ) {
...
*/
aig::GNode RewriteMananger::decGraphToAig( ThreadContextData * threadCtx, DecGraph * decGraph ) {
...
aig::GNode RewriteMananger::createAndNode( ThreadContextData * threadCtx, aig::GNode lhsAnd, aig::GNode rhsAnd, bool lhsAndPol, bool rhsAndPol ) {
	...
	auto idIt = threadCtx->bestMFFCIds.begin(); // reuse an ID from deleted MFFC
	int id = (*idIt);
	threadCtx->bestMFFCIds.erase( idIt ); // remove the reused ID from the available IDs set
	...
	//this->aig.getFanoutMap( id ).clear(); // It was already done in the for of updateAig method
    //this->cutMan.recycleNodeCuts( id );

    return newAnd;
}

void RewriteMananger::addNewSubgraph( ThreadContextData * threadCtx, aig::GNode oldNode, aig::GNode newNode, GNodeVector & fanoutNodes, bool isNewRootComplement, bool updateLevel ) {

	int fanoutNodesSize = fanoutNodes.size();
    assert( oldNodeData.nFanout == fanoutNodesSize );

    aig::GNode fanoutNode;
    aig::GNode newFanoutNode;
    aig::GNode otherNode;
    bool otherNodePol;
    bool newNodePol;
    bool oldNodePol;
...
void RewriteMananger::deleteOldMFFC( aig::Graph & aigGraph, aig::GNode oldNode ) {
...
void RewriteMananger::deleteOldMFFCRec( aig::Graph & aigGraph, aig::GNode oldNode ) {
...
aig::Aig & RewriteMananger::getAig() {
	return this->aig;
}

CutMananger & RewriteMananger::getCutMan() {
	return this->cutMan;
}

NPNMananger & RewriteMananger::getNPNMan() {
	return this->npnMan;
}

PreCompGraphMananger & RewriteMananger::getPcgMan() {
	return this->pcgMan;
}

PerThreadContextData & RewriteMananger::getPerThreadContextData() {
	return this->perThreadContextData;
}

bool RewriteMananger::getUseZerosFlag() {
	return this->useZeros;
}

bool RewriteMananger::getUpdateLevelFlag() {
	return this->updateLevel;
}

long double RewriteMananger::getRewriteTime() {
	return this->rewriteTime;
}

void RewriteMananger::setRewriteTime( long double time ) {
	this->rewriteTime = time;
}

struct RewriteOperator {
	
	RewriteMananger & rwtMan;
	CutMananger & cutMan;
	...
	void operator()( aig::GNode node, galois::UserContext< aig::GNode > & ctx ) {
		...
		if ( nodeData.type == aig::NodeType::AND ) {
			...
			if ( nodeData.nFanout < 1000 ) {
				...
				//if ( scheduleFanoutNodes && !isBarrier ) {
				...
				//if ( nodeData.counter == 3 && !isBarrier ) {
				if ( nodeData.counter == 3 ) {
