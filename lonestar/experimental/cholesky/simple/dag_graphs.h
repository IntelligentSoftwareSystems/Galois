...
struct MutableGraph {
  ...
  nodedata_t *node_data(node_t node) {
    return &nodes[node].data.data;
  }

  NodeData &_node_data(node_t node) {
  ...
  edge_t find_edge(node_t src, node_t dest) const {
    ...
    if ( SortNeighbors ) {
      typename EdgeList::const_iterator ii = nodes[src].neighbors.begin(),
        ...
        ci = std::lower_bound(ii, ei, dest, edge_lt_node<Edge>);
...
struct CRSGraph {
  ...
  CRSGraph(Graph &orig) {
    ...
    for ( node_t node = 0; node < nodecount; node++ ) {
      node_t prev = INVALID;
      for ( edge_t ii = edge_begin(node), ei = edge_end(node);
            ii != ei; ii++ ) {
        node_t dest = edge_dest(node, ii);
