...
struct PerThread {
  ...
  PerThread(Graph *graph) {
    ...
    if ( !etree_stack || !etree_mark ) abort();
...
node_t *make_etree(const Graph *graph) {
  ...
  for ( node_t n = 0; n < graph->nodecount; n++ ) {
    parent[n] = INVALID;
    ancestor[n] = INVALID;
    for ( edge_t ci = graph->edge_begin(n), ce = graph->edge_end(n);
          ci != ce; ci++ ) {
...
unsigned etree_reach(const Graph *graph, node_t k, const node_t *parent,
  ...
  for ( edge_t ci = graph->edge_begin(k), ce = graph->edge_end(k);
        ci != ce; ci++ ) {
...
void do_node(PerThread<Graph> *mypts, Graph *graph, node_t node) {
  ...
  for ( ; stackpos < graph->nodecount; stackpos++ ) {
    ...
    // Find diagonal entry
    edge_t coldiag_edge = graph->edge_end(colnode)-1;//graph->graph->node_data(colnode)->diag; // ORDERING ASSUMPTION
    //printf("%d %d: %d %d\n", colnode, coldiag_edge, graph->edge_dest(colnode, coldiag_edge), colnode);
...

    {
      ...
      //printf("  L[%4d,%4d] /= L[%4d,%4d] == %10.5f => %10.5f\n", colnode, node, colnode, colnode, coldiag, x);
    }
  ...
  *selfdata_p = selfdata; // FIXME: Remove this, but avoid perf penalty
  ...
  //printf(" L[%4d,%4d] = sqrt(L[%4d,%4d] == %10.5f) => %10.5f\n", node, node, node, node, selfdata, *selfdata_p);
}
