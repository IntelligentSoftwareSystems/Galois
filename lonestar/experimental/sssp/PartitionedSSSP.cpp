...
enum Algo { normal, part, part2, part3, part4, part5, part6, part7 };

static cll::opt<std::string>
    filename(cll::Positional, cll::desc("<input graph>"), cll::Required);
static cll::opt<std::string> partsname(cll::Positional,
                                       cll::desc("<part base graph>"));
...
static cll::opt<unsigned int>
    reportNode("reportNode", cll::desc("Node to report distance to"),
               cll::init(1));
static cll::opt<unsigned int>
    deltaShift("delta", cll::desc("Shift value for the delta step"),
               cll::init(10));
static cll::opt<unsigned int>
    blockShift("block", cll::desc("Shift value for the block"), cll::init(10));
static cll::opt<unsigned int>
    partitionShift("part", cll::desc("Shift value for the partition"),
                   cll::init(10));
static cll::opt<Algo> algo(
    ...
    cll::values(
        ...
        clEnumValN(Algo::normal, "normal", ""), clEnumValEnd),
    cll::init(Algo::normal));
...
struct AsyncAlgo {
  ...
  typedef galois::graphs::LC_PartitionedInlineEdge_Graph<
      Node, uint32_t>::template with_out_of_line_lockable<true>::
      template with_compressed_node_ptr<true>::template with_numa_alloc<
          !WithPartitioning>
          Graph;
  ...
  std::string name() const {
    return WithPartitioning ? "partitioned" : "not partitioned";
  }
  ...
  // Operator with partitioning
  void relaxEdge(Graph& graph, Node& sdata, typename Graph::edge_iterator ii,
                 int scale, int cur, Bags& bags,
                 galois::UserContext<UpdateRequest>& pusher) {
    GNode dst = graph.getEdgeDst(ii);
    // int id = graph.idFromNode(dst) / scale;
    int id = graph.idFromNode(dst) >> partitionShift;
  ...
  void relaxNode(Graph& graph, GNode src, Dist prevDist, Pusher& pusher) {
    ...
    for (typename Graph::edge_iterator ii = graph.edge_begin(src, flag),
                                       ei = graph.edge_end(src, flag);
         ii != ei; ++ii) {
  ...
  // With partitioning
  void relaxNode(Graph& graph, GNode src, Dist prevDist, int scale, int cur,
                 Bags& bags, galois::UserContext<UpdateRequest>& pusher) {
    const galois::MethodFlag flag = galois::MethodFlag::UNPROTECTED;
    Node& sdata                   = graph.getData(src, flag);
  ...
  struct ProcessWithPartitioning {
    ...
    ProcessWithPartitioning(AsyncAlgo* s, Graph& g, int _scale, int _cur,
                            Bag& _next, Bags& _bags)
        : self(s), graph(g), scale(_scale), cur(_cur), next(_next), bags(_bags),
          count(0) {}
    ...
    void galoisParallelBreakReceiveRemaining(const UpdateRequest& req) {
      next.push(req);
    }
    // static_assert(galois::has_parallel_break_receive_remaining<ProcessWithPartitioning>::value,
    // "Oops!");
  };
  ...
  struct ProcessWithoutPartitioning {
    ...
    ProcessWithoutPartitioning(AsyncAlgo* s, Graph& g, int _scale, int _cur,
                               Bag& _next, Bags& _bags)
        : self(s), graph(g) {}
    void operator()(UpdateRequest& req,
                    galois::UserContext<UpdateRequest>& ctx) {
  ...
  typedef typename boost::mpl::if_c<WithPartitioning, ProcessWithPartitioning,
                                    ProcessWithoutPartitioning>::type Process;
  ...
  void operator()(Graph& graph, GNode source) {
    ...
    std::cout << "INFO: Using delta-step of " << (1 << deltaShift) << " ("
              << deltaShift << ")\n";
    if (WithPartitioning) {
      std::cout << "INFO: Using block-step of " << (1 << blockShift) << " ("
                << blockShift << ")\n";
      std::cout << "INFO: Using partition size of " << (1 << partitionShift)
                << " (" << partitionShift << ")\n";
    }
    ...
    if (WithPartitioning)
      total =
          (graph.size() + (1 << partitionShift) - 1) / (1 << partitionShift);
    if (WithPartitioning) {
    ...
    for (int count = 0; count < total; cur = (cur + 1) % total) {
      ...
      galois::for_each(bags[cur],
                       Process(this, graph, rangeScale, cur, next, bags),
                       galois::wl<OBIM>());
      rounds += 1;
...
struct Algo2 {
  ...
  typedef galois::graphs::LC_PartitionedInlineEdge_Graph<Node, uint32_t>::
      with_out_of_line_lockable<true>::with_compressed_node_ptr<true>
  ...
  struct UpdateRequest : public UpdateRequestCommon<GNode> {
    int partition;
    UpdateRequest(const GNode& n, Dist w, int p)
        : UpdateRequestCommon<GNode>(n, w), partition(p) {}
  ...
  template <typename Pusher>
  void relaxEdge(Graph& graph, Node& sdata, Graph::edge_iterator ii,
                 Pusher& pusher) {
    GNode dst = graph.getEdgeDst(ii);
    // int id = graph.idFromNode(dst) / scale;
    int id = graph.idFromNode(dst) >> partitionShift;
  ...
  void relaxNode(Graph& graph, GNode src, Dist prevDist, Pusher& pusher) {
    ...
    for (Graph::edge_iterator ii = graph.edge_begin(src, flag),
                              ei = graph.edge_end(src, flag);
         ii != ei; ++ii) {
      relaxEdge(graph, sdata, ii, pusher);
    }
  ...
  void operator()(Graph& graph, GNode source) {
    ...
    typedef PerSocketChunkFIFO<128> Chunk;
    typedef Partitioned<Partitioner, Chunk>::with_block_period<
        blockPeriod>::with_max_value<maxValue>
        Part;
    typedef OrderedByIntegerMetric<UpdateRequestIndexer<UpdateRequest>, Part,
                                   10>
        OBIM;
    ...
    std::cout << "INFO: Using delta-step of " << (1 << deltaShift) << " ("
              << deltaShift << ")\n";
    std::cout << "INFO: Using block-step of " << (1 << blockPeriod) << " ("
              << blockPeriod << ")\n";
    std::cout << "INFO: Using partition size of " << (1 << partitionShift)
              << " (" << partitionShift << ")\n";
...
struct Algo3 {
  ...
  template <typename Pusher>
  void relaxEdge(Graph& graph, Node& sdata, const GNode& dst, const Dist& d,
                 Pusher& pusher) {
    Node& ddata  = graph.getData(dst, galois::MethodFlag::UNPROTECTED);
    Dist newDist = sdata.dist + d;
    Dist oldDist;
  ...
  void relaxNode(Graph& graph, GNode src, Dist prevDist, int curId,
                 Graph::edge_iterator begin, Pusher1& pusher1,
                 Pusher2& pusher2) {
    const galois::MethodFlag flag = galois::MethodFlag::UNPROTECTED;
    Node& sdata                   = graph.getData(src, flag);
    ...
    for (Graph::edge_iterator ii = begin, ei = graph.edge_end(src, flag);
         ii != ei; ++ii) {
      GNode dst = graph.getEdgeDst(ii);
      int id    = graph.idFromNode(dst) >> partitionShift;
      if (id == curId) {
  ...
  struct Process {
    ...
    Process(Algo3* s, Graph& g, int i, Bag* ib, Bag* b)
        : self(s), graph(g), id(i), innerBag(ib), bag(b) {}

    void operator()(UpdateRequest& req, galois::UserContext<UpdateRequest>&) {
      if (First)
        self->relaxNode(
            graph, req.n, req.w, id,
            graph.edge_begin(req.n, galois::MethodFlag::UNPROTECTED), *innerBag,
            *bag);
      else
  ...
  void operator()(Graph& graph, GNode source) {
    ...
    galois::do_all(
        ...
        graph.out_edges(source, galois::MethodFlag::UNPROTECTED).end(),
        [&](Graph::edge_iterator ii) {
          self->relaxEdge(graph, sourceData, graph.getEdgeDst(ii),
                          graph.getEdgeData(ii), *cur);
        });
    ...
    while (!cur->empty()) {
      ...
      for (int i = 0; i < total; ++i) {
        ...
        if (i == 0)
          galois::for_each(*cur, Process<true>(this, graph, i, innerNext, next),
                           galois::wl<OBIM>());
        else
          galois::for_each(*cur,
                           Process<false>(this, graph, i, innerNext, next),
                           galois::wl<OBIM>());
        clearTime.start();
        cur->clear();
        clearTime.stop();
...
struct Algo4 {
  ...
  typedef galois::graphs::LC_PartitionedInlineEdge_Graph<
      Node, uint32_t>::with_no_lockable<true>::with_compressed_node_ptr<true>
      //::template with_numa_alloc<!WithPartitioning> XXX
      Graph;
  typedef typename Graph::GraphNode GNode;
  ...
  template <typename Pusher>
  void relaxEdge(Graph& graph, const GNode& src, const GNode& dst,
                 const Dist& newDist, Pusher& pusher) {
    const galois::MethodFlag flag = galois::MethodFlag::UNPROTECTED;
    ...
    while (newDist < (oldDist = ddata.dist)) {
      if (__sync_bool_compare_and_swap(&ddata.dist, oldDist, newDist)) {
        if (trackBadWork && oldDist != DIST_INFINITY)
          *BadWork += 1;
        pusher.push(
            UpdateRequest(dst, newDist, graph.edge_begin(dst, flag), 0));
        return;
      }
    }
  }
  ...
  template <typename Pusher1, typename Pusher2>
  void relaxNode(Graph& graph, const UpdateRequest& req, Pusher1& pusher1,
                 Pusher2& pusher2) {
    const galois::MethodFlag flag = galois::MethodFlag::UNPROTECTED;
    ...
    for (Graph::edge_iterator ii = req.begin,
                              ei = graph.edge_end(req.src, flag);
         ii != ei; ++ii) {
      if (check && *srcDist != req.w) {
        if (trackBadWork)
          *WLEmptyWork += 1;
  ...
  struct Process {
    ...
    void operator()(const UpdateRequest& req,
                    galois::UserContext<UpdateRequest>& ctx) {
  ...
  void operator()(Graph& graph, GNode source) {
    ...
    galois::do_all(
        ...
        graph.out_edges(source, galois::MethodFlag::UNPROTECTED).end(),
        [&](Graph::edge_iterator ii) {
          self->relaxEdge(graph, source, graph.getEdgeDst(ii),
                          sourceData.dist + graph.getEdgeData(ii), *cur);
        });
...
struct Algo6 {
  ...
  void readGraphParts(const std::string& name) {
    ...
    int numParts = 0;
    std::string base;
    for (std::string::const_reverse_iterator ii = name.rbegin(),
                                             ei = name.rend();
         ii != ei; ++ii, --index) {
      if (state == 0) {
        if (isdigit(*ii)) {
    ...
    while (true) {
      ...
      for (unsigned int i = 1; i < parts.size(); ++i) {
        ...
        galois::do_all(self->parts[i], [&](GNode nn) {
          GNode n =
              self->parts[i - 1].nodeFromId(self->parts[i].idFromNode(nn));
          Dist newDist = self->parts[i - 1].getData(n).dist;
          if (newDist != self->parts[i].getData(nn).dist) {
            self->parts[i].getData(nn).dist = newDist;
            if (self->parts[i].edge_begin(nn,
                                          galois::MethodFlag::UNPROTECTED) !=
                self->parts[i].edge_end(nn, galois::MethodFlag::UNPROTECTED))
              cur->push(UpdateRequest(nn, newDist));
          }
        });
        mainTime.start();
        galois::for_each(*cur, Process(this, parts[i], next),
                         galois::wl<OBIM>());
        mainTime.stop();
        // std::cout << "(" << std::distance(cur->begin(), cur->end()) << ", "
        // << mainTime.get() << ") ";
...
struct Algo7 {
  ...
  typedef galois::graphs::LC_PartitionedInlineEdge_Graph<Node, uint32_t>::
      with_no_lockable<true>::with_compressed_node_ptr<true>::with_numa_alloc<
          true>
          Graph;
  typedef typename Graph::GraphNode GNode;
  typedef UpdateRequestCommon<GNode> UpdateRequest;
  ...
  struct UpdateRequest2 : public UpdateRequestCommon<GNode> {
    ...
    UpdateRequest2(GNode n, Dist w, typename Graph::edge_iterator b)
        : UpdateRequestCommon<GNode>(n, w), begin(b) {}
  };
  ...
  template <typename Pusher>
  void relaxEdge(Graph& graph, typename Graph::edge_iterator ii,
                 const Dist& newDist, Pusher& pusher) {
    GNode dst   = graph.getEdgeDst(ii);
    Node& ddata = graph.getData(dst, galois::MethodFlag::UNPROTECTED);
    Dist oldDist;
  ...
  template <typename Pusher>
  void relaxEdge2(Graph& graph, typename Graph::edge_iterator ii,
                  const Dist& newDist, Pusher& pusher) {
    GNode dst   = graph.getEdgeDst(ii);
    Node& ddata = graph.getData(dst, galois::MethodFlag::UNPROTECTED);
    Dist oldDist;
    while (newDist < (oldDist = ddata.dist)) {
      if (__sync_bool_compare_and_swap(&ddata.dist, oldDist, newDist)) {
        ...
        typename Graph::edge_iterator next =
  ...
  void relaxNode(Graph& graph, const UpdateRequest& req,
                 typename Graph::edge_iterator begin, Pusher1* pusher1,
                 Pusher2& pusher2) {}
  ...
  struct Process {
    ...
    Process(Algo7* s, Graph& g, galois::InsertBag<UpdateRequest2>* b)
    ...
    void operator()(const UpdateRequest2& req,
                    galois::UserContext<UpdateRequest2>& ctx) {
      const galois::MethodFlag flag = galois::MethodFlag::UNPROTECTED;
      Node& sdata                   = graph.getData(req.n, flag);
      volatile Dist* sdist          = &sdata.dist;
      ...
      for (typename Graph::edge_iterator ii = req.begin,
                                         ei = graph.edge_end(req.n, flag);
           ii != ei; ++ii) {
        if (req.w != *sdist) {
          if (trackBadWork)
  ...
  void operator()(Graph& graph, GNode source) {
    ...
    galois::do_all(
        ...
        graph.out_edges(source, galois::MethodFlag::UNPROTECTED).end(),
        [&](Graph::edge_iterator ii) {
          self->relaxEdge(graph, ii, sourceData.dist + graph.getEdgeData(ii),
                          initial);
        });
    galois::for_each(initial, Process(this, graph, &next),
                     galois::loopname("A1"), galois::wl<OBIM>());
    galois::for_each(next, Process(this, graph, nullptr),
                     galois::loopname("A2"), galois::wl<OBIM>());
  }
};
