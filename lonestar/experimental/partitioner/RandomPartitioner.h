...
struct RandomPartitioner {
  ...
  /*
   * Edges have been assigned. Now, go over each partition, for any vertex in
   * the partition create a new local-id, and update all the edges to the new
   * local-ids.
   * */
  //   void assignVertices(OfflineGraph & g, VertexCutInfo & vcInfo, size_t
  //   num_hosts) {
  ...
  //               std::cout << "Host " << h << " Mapped Global:: " << vertex <<
  //               " to Local:: " << vcInfo.hostGlobalToLocalMapping[h][vertex]
  //               << "\n";
  //            }
  //         }
  //
  ...
  //         std::cout << "Host :: " << i << " , Vertices:: " <<
  //         vcInfo.verticesPerHost[i].size() << ", Edges:: " <<
  //         vcInfo.edgesPerHost[i] << "\n";
  //      }
  //      std::cout << "Vertices - Created ::" << verticesSum << " , Actual :: "
  //      << g.size() << ", Ratio:: " << verticesSum / (float) (g.size()) <<
  //      "\n";
  //   }
  /*
   * Write both the metadata as well as the partition information.
   * */
  /* void writePartitions(std::string & basename, OfflineGraph & g,
VertexCutInfo & vcInfo, size_t num_hosts) {
      //Create graph
      ...
      for (auto n = g.begin(); n != g.end(); ++n) {
         ...
         for (auto e = g.edge_begin(*n); e != g.edge_end(*n); ++e) {
            ...
            newEdges[owner].push_back(std::pair<size_t, size_t>(new_src,
new_dst));
         }
      }
      for (size_t i = 0; i < num_hosts; ++i) {
...
//         sprintf(meta_file_name, "partition_%zu_of_%zu.gr.meta", i,
num_hosts); std::ofstream meta_file(meta_file_name, std::ofstream::binary); auto
numEntries = vcInfo.hostGlobalToLocalMapping[i].size();
         meta_file.write(reinterpret_cast<char*>(&(numEntries)),
sizeof(numEntries)); for (auto n : vcInfo.hostGlobalToLocalMapping[i]) { auto
owner = *vcInfo.vertexOwners[n.first].begin();
            meta_file.write(reinterpret_cast<const char*>(&n.first),
sizeof(n.first)); meta_file.write(reinterpret_cast<const char*>(&n.second),
sizeof(n.second)); meta_file.write(reinterpret_cast<const char*>(&owner),
sizeof(owner));
         }
         meta_file.close();
...
#if _HAS_EDGE_DATA
            newEdges.push_back(
                NewEdgeData(new_src, new_dst, g.getEdgeData<EdgeDataType>(e)));
#else
