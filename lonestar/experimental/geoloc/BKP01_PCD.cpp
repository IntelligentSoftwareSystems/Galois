...
struct EstimateLocation 
{
	...
	void operator()( const GNode& n, galois::UserContext<GNode>& ctx ) const {
		...
		if ( node.type == 'U' ) {
			...
			//float *vector = new float[ accum_set_size.reduce() ];	// TODO: precisa de ser array dinâmico
...
printf( "Geometric median | old = %.2f | new = %f\n", node.geometric_median, geometric_median );
			...
			if ( node.geometric_median > geometric_median ) {
				node.geometric_median = geometric_median;
				node.location.x = ( 1 - gamma ) * T.x + gamma * node.location.x;	// Equation 11 in [1]
				node.location.y = ( 1 - gamma ) * T.y + gamma * node.location.y;	// Equation 11 in [1]
