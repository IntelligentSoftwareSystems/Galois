...
enum Algo { nodeiteratorpre, nodeiteratorpre_moreCompute };
...
static cll::opt<Algo> algo(
    ...
    cll::values(
        clEnumValN(Algo::nodeiteratorpre_moreCompute,
                   "nodeiteratorpre_moreCompute",
                   "Node Iterator with pushing singleton and more compute"),
        clEnumValN(Algo::nodeiteratorpre, "nodeiteratorpre",
                   "Node Iterator (default)"),
        clEnumValEnd),
    cll::init(Algo::nodeiteratorpre));
static cll::opt<unsigned int>
    k("k", cll::desc("max number of vertices in k-motif(default value 0)"),
      cll::init(0));
...
bool isPresentOnLeft(const VecTy& vec, const ElemTy& elem, const KeyTy& key) {
  return (std::find(vec.begin(), vec.begin() + key, elem) !=
          (vec.begin() + key));
}
...
bool edgeNotInTuple(const VecTy& vec, const ElemTy& elem,
                    const StInfoTy& st_info, const KeyTy& key) {
...
void nodeIteratingAlgoWithStruct(Graph& graph) {
  ...
  struct SubGraphTuple {
    ...
    SubGraphTuple(const VecGNodeTy& v1, const uint8_t& k1,
                  const VecUnsignedTy& s1)
        : vertices(v1), key(k1), st_info(s1) {}
  };
  ...
  galois::do_all(
      ...
      [&](const GNode& n) {
        ...
        // Graph::edge_iterator ea = first + ndata.ea; // std::advance(first,
        // ndata.ea); Graph::edge_iterator bb = first + ndata.bb;
        // //std::advance(first, ndata.bb);
        ...
        for (; bb != last; ++bb) {
          ...
          items_active.push(
              SubGraphTuple(VecGNodeTy{n, B}, 0, VecUnsignedTy{1, 0}));
          items_active.push(
              SubGraphTuple(VecGNodeTy{n, B}, 1, VecUnsignedTy{1, 0}));
        }
      },
  ...
  if (k > 2) {
    ...
    galois::for_each(
        ...
        [&](const SubGraphTuple& sg, auto& ctx) {
          ...
          /*
           * If n is duplicated, find the first instance of n else it will find
           * n: n is gauranteed to be present in the tuple
           */
          auto first_instance_of_n = std::distance(
              sg.vertices.begin(),
              std::find(sg.vertices.begin(), sg.vertices.end(), n));
          /*
           * Find the largest element on the right side of the key
           * If no duplicates are present than first_instance_of_n == sg.key
           */
          // auto first_instance_of_n  = std::distance(sg.vertices.begin(),
          // first_instance_of_n_it);
          if (sg.vertices.begin() + first_instance_of_n + 1 <
              sg.vertices.end()) {
            auto max_elem_after_n =
                std::max_element(sg.vertices.begin() + first_instance_of_n + 1,
                                 sg.vertices.end());
          ...
          Graph::edge_iterator bb =
              lowerBound(first, last, LessThan<Graph>(graph, max_elem));
          ...
          for (; bb != last; ++bb) {
            ...
            if (edgeNotInTuple(verts, dst, st_info, sg.key)) {
              ...
              if (verts[tupleSize - 1] == dst) {
                /*
                 * Must be increasing order. Should never be equal, since
                 * edgeNotInTuple should discard it
                 */
  ...
  // std::cout << "Num " << k << "-motif: " <<
  // std::distance(items_final.begin(), items_final.end()) << "\n";
...
void kMotif_moreCompute(Graph& graph) {
  ...
  struct SubGraphTuple {
    VecGNodeTy vertices;
    VecUnsignedTy st_info;
    SubGraphTuple(const VecGNodeTy& v1, const VecUnsignedTy& s1)
        : vertices(v1), st_info(s1) {}
  };
  galois::InsertBag<SubGraphTuple> items;
  galois::InsertBag<SubGraphTuple> items_active;
  ...
  galois::do_all(
      ...
      [&](const GNode& n) {
        ...
        for (; bb != last; ++bb) {
          GNode B = graph.getEdgeDst(bb);
          items_active.push(
              SubGraphTuple(VecGNodeTy{n, B}, VecUnsignedTy{1, 0}));
        }
      },
      galois::chunk_size<512>(), galois::steal(),
      galois::loopname("nodeIteratingAlgoWithStruct"));
      ...
      galois::do_all(
          ...
          [&](const SubGraphTuple& sg) {
            ...
            for (auto n : local_set) {
              auto first_instance_of_n_it =
                  std::find(sg.vertices.begin(), sg.vertices.end(), n);
              auto key =
                  std::distance(sg.vertices.begin(), first_instance_of_n_it);
              ...
              auto max_elem = sg.vertices[0];
              /*
               * If n is duplicated, find the first instance of n else it will
               * find n: n is gauranteed to be present in the tuple
               */
              // auto first_instance_of_n = std::distance(sg.vertices.begin(),
              // std::find(sg.vertices.begin(), sg.vertices.end(), n));
              /*
              ...
              if (sg.vertices.begin() + key + 1 < sg.vertices.end()) {
                auto max_elem_after_n = std::max_element(
                    sg.vertices.begin() + key + 1, sg.vertices.end());
                if (max_elem_after_n != sg.vertices.end()) {
                  max_elem = std::max(*max_elem_after_n, max_elem);
                }
              }
                  ...
                  if (uniqueInTuple(verts) == k) {
                    ...
                    for (auto v : local_set) {
                      auto first_instance_of_v_it =
                          std::find(verts.begin(), verts.end(), v);
                      auto i =
                          std::distance(verts.begin(), first_instance_of_v_it);
                      auto edge_support =
                          std::count(st_info.begin() + 2, st_info.end(), i);
                      if (edge_support < k - 2) {
                        can_expand = true;
                    ...
                    if (can_expand) {
                      ...
                      itemSizeBytes +=
                          (sizeof(SubGraphTuple) + sizeof(VecGNodeTy) +
                           sizeof(GNode) * verts.size() +
                           sizeof(VecUnsignedTy) +
                           sizeof(uint8_t) * st_info.size());
