...
static cll::opt<double> resolution("resolution",
cll::desc("Resolution for CPM quality function."),
cll::init(1.0));

static cll::opt<double> randomness("randomness",
...
constexpr static const double DOUBLE_MAX = std::numeric_limits<double>::max()/4;
...
void sumClusterWeight(GraphTy& graph, CommArrayTy& c_info) {
  ...
  for(GNode n = 0; n < graph.size(); ++n) {
      ...
      if(n_data.curr_comm_ass != UNASSIGNED)
        galois::atomicAdd(c_info[n_data.curr_comm_ass].degree_wt, n_data.degree_wt);
...
uint64_t maxCPMQuality(std::map<uint64_t, uint64_t> &cluster_local_map, 
  ...
  do {
    if(sc != stored_already->first) {
      ...
      cur_gain = 2.0f*(double)(eiy - eix) - resolution * node_wt * (double)(size_y - size_x);
...
#if 0
uint64_t maxCPMQuality(std::map<uint64_t, uint64_t> &cluster_local_map, std::vector<EdgeTy> &counter, EdgeTy self_loop_wt,
                       CommArray &c_info, EdgeTy degree_wt, uint64_t sc, double constant) {
  ...
  double size_x = c_info[sc].node_wt;
	double size_y = 0;
  ...
  do {
    if(sc != stored_already->first) {
      ...
      if(size_y < size_x || (size_y == size_x && stored_already->first < sc)){
				stored_already++;	
				continue;
			}
...
template<typename GraphTy, typename CommArrayTy> 
double calCPMQuality(GraphTy& graph, CommArrayTy& c_info, double& e_xx, double& a2_x, double& constant_for_second_term) {
...
/**
 * TODO: Must be over number of clusters
 * and not nodes of graph 
 */
galois::do_all(galois::iterate(graph),
                [&](GNode n) {
                  acc_e_xx += cluster_wt_internal[n];
                  acc_a2_x += (double) (c_info[n].node_wt) * ((double) (c_info[n].node_wt - 1)* 0.5f);
                  //acc_a2_x += (double) (c_info[n].node_wt) * ((double) (c_info[n].node_wt) * resolution);
                });
  ...
  /**
   * This 1/(2 * total_edge_wt_of_graph) 
   */ 
  //double normalization_factor = calConstantForSecondTerm(graph);  
  //mod  = mod * normalization_factor;
...
uint64_t maxModularity(std::map<uint64_t, uint64_t> &cluster_local_map, std::vector<EdgeTy> &counter, EdgeTy self_loop_wt,
                       CommArrayTy &c_info, EdgeTy degree_wt, uint64_t sc, double constant) {
...
template<typename CommArrayTy> 
uint64_t maxModularityWithoutSwaps(std::map<uint64_t, uint64_t> &cluster_local_map, std::vector<EdgeTy> &counter, uint64_t self_loop_wt,
                                  CommArrayTy &c_info, EdgeTy degree_wt, uint64_t sc, double constant) {
...
template<typename GraphTy, typename CommArrayTy> 
double calModularityDelay(GraphTy& graph, CommArrayTy& c_info, CommArrayTy& c_update, double& e_xx, double& a2_x, double& constant_for_second_term, std::vector<uint64_t>& local_target) {
  using GNode = typename GraphTy::GraphNode;
  /* Variables needed for Modularity calculation */
  double mod = -1;
...
template<typename GraphTy, typename CommArrayTy> 
double calModularity(GraphTy& graph, CommArrayTy& c_info, double& e_xx, double& a2_x, double& constant_for_second_term) {
  using GNode = typename GraphTy::GraphNode;
  /* Variables needed for Modularity calculation */
  double mod = -1;
...
void checkModularity(GraphTy& graph, largeArray& clusters_orig) {
  ...
  uint64_t num_unique_clusters = renumberClustersContiguously(graph);
				...
				uint64_t generateRandonNumberDouble(double min, double max) {
					...
					std::uniform_real_distribution<> dis(min, max);// distribution in range [min, max]
					return dis(gen);
				}

				template<typename CommArrayTy>
				double diffCPMQuality(uint64_t curr_subcomm, uint64_t candidate_subcomm, std::map<uint64_t, uint64_t> &cluster_local_map, std::vector<EdgeTy> &counter, CommArrayTy &subcomm_info, EdgeTy self_loop_wt){
				...
				uint64_t getRandomSubcommunity(GraphTy& graph, uint64_t n, CommArrayTy &subcomm_info, uint64_t total_node_wt, uint64_t total_degree_wt, double constant_for_second_term){
					using GNode = typename GraphTy::GraphNode;
					//uint64_t rand_subcomm = UNASSIGNED;
					...
					for(auto ii = graph.edge_begin(n); ii != graph.edge_end(n); ++ii) {
						GNode dst = graph.getEdgeDst(ii);
						EdgeTy edge_wt = graph.getEdgeData(ii, flag_no_lock); // Self loop weights is recorded	
					...
					// uint64_t best_subcomm = n_data.curr_subcomm_ass;
				...
				//		if(subcomm_info[subcomm].internal_edge_wt < resolution * (double)subcomm_node_wt * ((double)total_node_wt - (double)subcomm_node_wt))
						if(subcomm_info[subcomm].internal_edge_wt < constant_for_second_term * (double) subcomm_degree_wt * ((double)total_degree_wt - (double)subcomm_degree_wt)) 
								continue;
						// {

						//   quality_value_increment = subcomm_info[subcomm].internal_edge_wt - n_data.node_wt * subcomm_node_wt * resolution;

						//   if (quality_value_increment > max_quality_value_increment)
						//   {
						//       best_subcomm = subcomm;
						//       max_quality_value_increment = quality_value_increment;
						//   }

						//   if (quality_value_increment >= 0)
						//       //total_transformed_quality_value_increment += FastMath.fastExp(qualityValueIncrement / randomness);
						// }
				...
				uint64_t getRandomSubcommunity2(GraphTy &graph, typename GraphTy::GraphNode n, CommTy& subcomm_info, uint64_t total_node_wt,  uint64_t total_degree_wt, uint64_t comm_id, double constant_for_second_term){
					 ...
					 * cluster.
					*/
					cluster_local_map[n_data.curr_subcomm_ass] = 0; //Add n's current subcommunity
					counter.push_back(0); //Initialize the counter to zero (no edges incident yet)
					...
					double total_transformed_quality_value_increment = 0;
					double quality_value_increment = 0;
					std::vector<double> cum_transformed_quality_value_increment_per_cluster(num_unique_clusters);
					for(auto pair: cluster_local_map){
						auto subcomm = pair.first;
						...
						if(subcomm_info[subcomm].internal_edge_wt >= constant_for_second_term * (double) subcomm_degree_wt * ((double)total_degree_wt - (double)subcomm_degree_wt)){
							...
							quality_value_increment = counter[pair.second] - n_data.node_wt * subcomm_node_wt * resolution;
							...
							if (quality_value_increment >= 0)
									total_transformed_quality_value_increment += std::exp(quality_value_increment / randomness);
						}
						cum_transformed_quality_value_increment_per_cluster[pair.second] = total_transformed_quality_value_increment;
				 ...
				 /*
					...
					*/
					...
					{
							r = total_transformed_quality_value_increment * generateRandonNumberDouble(0.0, 1.0);
							min_idx = -1;
				...
				//	using Comm = CommTy;
					...
					for(uint64_t i = 0; i < cluster_nodes.size(); ++i) {
						 ...
						 * (externalEdgeWeightPerCluster[j] >= clusterWeights[j] * (totalNodeWeight - clusterWeights[j]) * resolution
						 */ 
					//	if(nodeEdgeWeightWithinCluster >= resolution * (double)node_wt * ((double)total_node_wt - (double)node_wt)) // well-connected
						if(nodeEdgeWeightWithinCluster >= constant_for_second_term * (double)degree_wt * ((double)total_degree_wt - (double)degree_wt))
							...
							if((int64_t) new_subcomm_ass != -1 && new_subcomm_ass != graph.getData(n).curr_subcomm_ass){ 
							 ...
							 /*
								...
								*/
								...
								subcomm_info[new_subcomm_ass].size += 1;
				 ...
				 /*
					* Map each neighbor's subcommunity to local number: Subcommunity --> Index
					* To calculate the number of unique subcommmunities inside community
					*/ 
					/*std::map<uint64_t, uint64_t> cluster_local_map; 
					uint64_t num_unique_subcomm = 0;
					for(uint64_t i = 0; i < cluster_nodes.size(); ++i) {
						GNode n = cluster_nodes[i];
						auto& n_data = 	graph.getData(n); 
						auto stored_already = cluster_local_map.find(n_data.curr_subcomm_ass); // Check if it already exists
						if(stored_already != cluster_local_map.end()) {
							n_data.curr_subcomm_ass = stored_already->second;
						} 
						else {
							cluster_local_map[n_data.curr_subcomm_ass] = num_unique_subcomm;
							n_data.curr_subcomm_ass = num_unique_subcomm; 
							num_unique_subcomm++;
						}
					}
					return num_unique_subcomm;*/
				}
				...
				void refinePartition(GraphTy &graph, double constant_for_second_term){
					...
					for(GNode n: graph){
						...
						comm_info[n_data.curr_comm_ass].node_wt += n_data.node_wt;
  				...
  				subcomm_info.allocateBlocked(graph.size()+1);	
					...
					galois::do_all(galois::iterate((uint64_t)0, (uint64_t)graph.size()),
					 ...
					 /**
						* Adjust cluster IDs for all the clusters
    */ 
   /*uint64_t num_subclusters = 0; 
   for (uint64_t c = 0; c < cluster_bags.size(); ++c){
     
     //uint64_t num = renumberClustersContiguouslySubcomm(graph, cluster_bags[c]);
     //assert(comm_info[c].num_subcomm == num);
     for(GNode n : cluster_bags[c]){
       auto& n_data = graph.getData(n);
       n_data.curr_subcomm_ass = num_subclusters + n_data.curr_subcomm_ass;
     }
     num_subclusters += comm_info[c].num_subcomm;
   }
   return num_subclusters;*/
}
 ...
 */ 
template<typename GraphTy>
void buildNextLevelGraph(GraphTy& graph, GraphTy& graph_next, uint64_t num_unique_clusters) {
  using GNode = typename GraphTy::GraphNode;
...
void buildNextLevelGraphSubComm(GraphTy &graph, GraphTy &graph_next, 
  ...
  for(GNode n = 0; n < graph.size(); ++n) {
      auto n_data = graph.getData(n, flag_no_lock);
      original_comm_ass[n_data.curr_subcomm_ass] = graph.getData(n_data.curr_comm_ass).curr_subcomm_ass;
...
//	std::cout <<" c2:" << calConstantForSecondTerm(graph_next) << std::endl;
  TimerGraphBuild.stop();
  galois::gPrint("Graph construction done\n");
}
