...
double calCPMQuality(GraphTy& graph, CommArrayTy& c_info, double& e_xx,
  ...
  // galois::gPrint("e_xx : ", e_xx, " ,constant_for_second_term : ",
  // constant_for_second_term, " a2_x : ", a2_x, "\n"); mod = e_xx - a2_x;
...
void checkModularity(GraphTy& graph, largeArray& clusters_orig) {
  ...
  // galois::gPrint("Number of unique clusters (renumber) ARR: ",
  // renumberClustersContiguouslyArray(clusters_orig), "\n");
  galois::do_all(galois::iterate(graph), [&](GNode n) {
...
uint64_t getRandomSubcommunity(GraphTy& graph, uint64_t n,
  ...
  for (auto pair : cluster_local_map) {
    ...
    //		if(subcomm_info[subcomm].internal_edge_wt < resolution *
    //(double)subcomm_node_wt * ((double)total_node_wt -
    //(double)subcomm_node_wt))
    ...
    //   quality_value_increment = subcomm_info[subcomm].internal_edge_wt -
    //   n_data.node_wt * subcomm_node_wt * resolution;
    ...
    //       FastMath.fastExp(qualityValueIncrement / randomness);
    // }
...
uint64_t getRandomSubcommunity2(GraphTy& graph, typename GraphTy::GraphNode n,
                                CommTy& subcomm_info, uint64_t total_node_wt,
...
void mergeNodesSubset(GraphTy& graph,
                      ...
                      uint64_t comm_id, uint64_t total_node_wt,
                      uint64_t total_degree_wt, CommTy& subcomm_info,
                      double constant_for_second_term) {
  ...
  for (uint64_t i = 0; i < cluster_nodes.size(); ++i) {
     ...
     */
    //	if(nodeEdgeWeightWithinCluster >= resolution * (double)node_wt *
    //((double)total_node_wt - (double)node_wt)) // well-connected
  ...
  for(uint64_t i = 0; i < cluster_nodes.size(); ++i) {
      ...
      auto& n_data = 	graph.getData(n);
      auto stored_already = cluster_local_map.find(n_data.curr_subcomm_ass); //
  Check if it already exists if(stored_already != cluster_local_map.end()) {
          n_data.curr_subcomm_ass = stored_already->second;
      }
      else {
...
void refinePartition(GraphTy& graph, double constant_for_second_term) {
  ...
  for (uint64_t c = 0; c < cluster_bags.size(); ++c){
    ...
    //uint64_t num = renumberClustersContiguouslySubcomm(graph,
  cluster_bags[c]);
