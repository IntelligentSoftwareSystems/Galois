...
//TODO: Make comm id of uniform type: uint32_t
//TODO: Make community typedef for easy switch
struct Node{
...
double algoLeidenWithLocking(Graph &graph, double lower, double threshold, uint32_t& iter) {
  ...
  // for(auto i = 0; i < 10; ++i){
  //   galois::gPrint("node wt : ", c_info[i].node_wt.load(), "\n");
  // }
...
//  }
  ...
  constant_for_second_term = calConstantForSecondTerm(graph);
	...
	if(iter >1){
		...
		for(auto n:graph){
			...
			c_info[n_data.curr_comm_ass].size += 1;
  ...
  while(true) {
    ...
    galois::for_each(galois::iterate(graph),
                  ...
                  /* Update cluster info */
                  if(local_target != n_data.curr_comm_ass && local_target != UNASSIGNED) {
                    ...
                    galois::atomicSubtract(c_info[n_data.curr_comm_ass].node_wt, n_data.node_wt);

                    /* Set the new cluster id */
                    n_data.curr_comm_ass = local_target;
                  }
                 }, galois::loopname("leiden algo: Phase 1"),
                    galois::no_pushes());
    ...
    //curr_mod = calCPMQuality(graph, c_info, e_xx, a2_x, constant_for_second_term);
    ...
    if((*graph_curr).size() > min_graph_size){
        ...
        switch (algo) {
          case coloring:
                ...
                galois::gPrint("Not Supported\n");
                std::abort();
                break;
          case foreach:
                curr_mod = algoLeidenWithLocking(*graph_curr, curr_mod, c_threshold, iter);
                break;
          case doall:
...
/*    uint64_t num_unique_clusters = renumberClustersContiguously(*graph_curr);
    galois::gPrint("Number of unique clusters (renumber) -> : ", num_unique_clusters, "\n");
      if(!enable_VF && phase == 1){
        ...
        galois::do_all(galois::iterate(*graph_curr),
                      [&](GNode n) {
                        ...
                        clusters_sub_orig[n] = (*graph_curr).getData(n, flag_no_lock).curr_subcomm_ass;
                      });
      } else {
        galois::do_all(galois::iterate((uint64_t)0, num_nodes_orig),
                      [&](GNode n) {
                        if(clusters_orig[n] != UNASSIGNED){
                          ...
                          clusters_orig[n] = (*graph_curr).getData(clusters_sub_orig[n], flag_no_lock).curr_comm_ass;
                          clusters_sub_orig[n] = (*graph_curr).getData(clusters_sub_orig[n], flag_no_lock).curr_subcomm_ass;
                        }
                      });

      }*/
      //galois::gPrint("ARRAY unique cluster: ", renumberClustersContiguouslyArray<Graph>(clusters_orig), "\n");
      ...
      //galois::gPrint("Num unique sub cluster (Refine) : ", num_unique_subclusters, "\n");

			uint64_t num_unique_subclusters = renumberClustersContiguouslySubcomm(*graph_curr);
      ...
      std::vector<uint64_t> cluster_node_wt(num_unique_subclusters, 0);
			...
			buildNextLevelGraphSubComm(*graph_curr, graph_next, num_unique_subclusters, original_comm_ass, cluster_node_wt);
      prev_mod = curr_mod;
