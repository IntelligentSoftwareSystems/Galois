...
void runMultiPhaseLouvainAlgorithm(Graph& graph, uint64_t min_graph_size,
  ...
  while (true) {
    ...
    if ((*graph_curr).size() > min_graph_size) {
      ...
      switch (algo) {
      case coloring:
        // curr_mod = algoLouvainWithColoring(*graph_curr, curr_mod,
        // c_threshold, iter);
    ...
    if (iter < max_iter && (curr_mod - prev_mod) > threshold) {
         ...
         unique clusters (renumber) -> : ", num_unique_clusters, "\n");
            if(!enable_VF && phase == 1){
              ...
              galois::do_all(galois::iterate(*graph_curr),
                            [&](GNode n) {
                              clusters_orig[n] = (*graph_curr).getData(n,
         flag_no_lock).curr_comm_ass; clusters_sub_orig[n] =
         (*graph_curr).getData(n, flag_no_lock).curr_subcomm_ass;
                            });
            } else {
              galois::do_all(galois::iterate((uint64_t)0, num_nodes_orig),
                            [&](GNode n) {
                              if(clusters_orig[n] != UNASSIGNED){
                                ...
                                clusters_orig[n] =
         (*graph_curr).getData(clusters_sub_orig[n],
         flag_no_lock).curr_comm_ass; clusters_sub_orig[n] =
         (*graph_curr).getData(clusters_sub_orig[n],
         flag_no_lock).curr_subcomm_ass;
                              }
                            });

            }*/
      // galois::gPrint("ARRAY unique cluster: ",
      // renumberClustersContiguouslyArray<Graph>(clusters_orig), "\n");
      ...
      // galois::gPrint("Num unique sub cluster (Refine) : ",
...
int main(int argc, char** argv) {
  ...
  runMultiPhaseLouvainAlgorithm(*graph_curr, min_graph_size, c_threshold,
                                clusters_orig, clusters_sub_orig);
  Tmain.stop();
