...
struct reduceGhostCells_struct {
  ...
  void static go(pGraph<Graph>& _g) {
    galois::do_all(_g.g.begin() + _g.numOwned, _g.g.begin() + _g.numNodes,
                   ...
                   galois::loopname("ReduceGhost Cells"));
  }
  void operator()(GNode src) const {
    ...
    // remoteReplicas_residual[x][src - g->numNodes] =
...
struct applyResidual_struct {
  ...
  void static go(std::vector<unsigned>& _remoteReplicas_vec,
                 std::vector<PRTy>& _remoteReplicas_residual_vec,
                 pGraph<Graph>& _g) {
    // galois::do_all(_remoteReplicas_vec.begin(), _remoteReplicas_vec.end(),
    // applyResidual_struct { &_remoteReplicas_vec,
    // &_remoteReplicas_residual_vec, &_g }, galois::loopname("Apply residual
    // struct"));
    galois::do_all(
        boost::counting_iterator<unsigned>(0),
        boost::counting_iterator<unsigned>(_remoteReplicas_vec.size()),
        applyResidual_struct{&_remoteReplicas_vec,
                             &_remoteReplicas_residual_vec, &_g},
        galois::loopname("Apply residual struct"));
  }
  ...
  void operator()(unsigned i) const {
    ...
    // g->g.getData(g->G2L((*RR_vec)[i])).residual.store(old +
    // (*RR_residual_vec)[i]);
  }
};
void receiveGhostCellVectors(galois::runtime::RecvBuffer& buff) {
  ...
  if (galois::runtime::NetworkInterface::ID == 1)
    for (auto x : residual_vec)
      std::cout << x << "\n";
  std::cout << "Size : " << remoteReplicas_L[fromHostID].size()
            << " Size res : " << residual_vec.size() << "\n";
#endif

  applyResidual_struct::go(remoteReplicas_L[fromHostID], residual_vec, *p);
  /*
   unsigned i = 0;
   for(auto GID = remoteReplicas[fromHostID].begin(); GID !=
   remoteReplicas[fromHostID].end(); ++GID, ++i){
   //if(net.ID == 1)
...
void sendGhostCellVectors(galois::runtime::NetworkInterface& net,
                          pGraph<Graph>& g) {

  unsigned remoteHostID = 0;
  for (auto x = remoteReplicas_residual.begin();
       x != remoteReplicas_residual.end(); ++x, ++remoteHostID) {
...
void inner_main() {
  ...
  galois::Timer T_total, T_graph_load, T_pagerank, T_pagerank_perIter,
      T_graph_init;
  T_total.start();
  ...
  if (personality == CPU) {
    ...
    for (auto x = remoteReplicas_residual.begin();
         x != remoteReplicas_residual.end(); ++x) {
      (*x).clear();
    }
  ...
  for (int i = 0; i < maxIterations; ++i) {
    ...
    case CPU:
      ...
      if (my_host_id == 1) {
        ...
        for (auto x = remoteReplicas_L.begin(); x != remoteReplicas_L.end();
             ++x)
          for (auto y = x->begin(); y != x->end(); ++y)
      ...
      if (my_host_id == 0) {
        ...
        for (auto n = g.g.begin() + g.numOwned; n != g.g.begin() + g.numNodes;
          ...
          std::cout << *n << " R : " << sdata.residual
                    << " L2G vec : " << g.L2G[*n - g.numOwned]
                    << " From vec : " << sdata.residual << " \n";
        }
      }
#endif
      break;
    ...
    // Iteration : " << i << " Time : " << T_pagerank_perIter.get() << "
    // (msec)\n";
    barrier.wait();
  }
