...
static const char* const desc =
    "Data Driven Bellman-Ford SSSP on Distributed Galois.";
...
struct InitializeGraph {
  ...
  void static go(Graph& _graph) {
    ...
    struct SyncerPull_0 {
      static int extract(const struct NodeData& node) {
        return node.dist_current;
      }
      static void setVal(struct NodeData& node, int y) {
        node.dist_current = y;
      }
      typedef int ValTy;
    };
...
Then call the same for_each, but in Executor_ForEach.h inside function
for_each_gen check if the foo_tag exists (using exists_by_suoertype), if exists
then call the separate for_each_gen_exp else just continue the same path.

use get_by_super to extract the functor and call it on all the items in the bag
to get the ID they belong to.

Also, no change is required to for_each_imp as it simply forwards the
arguments.. so it's cool. just simply pass bag to it.
...
struct Get_info_functor : public galois::op_tag {
  ...
  struct Syncer_0 {
    ...
    static void reduce(struct NodeData& node, int y) {
      galois::min(node.dist_current, y);
    }
    static void
    reset(struct NodeData&
              node) { /*node.dist_current = std::numeric_limits<int>::max();*/
    }
    typedef int ValTy;
  };
  ...
  void static sync_graph_static(Graph& _g) { _g.sync_push<Syncer_0>(); }
};
...
struct SSSP {
  ...
  void static go(Graph& _graph) {
    ...
    if (galois::runtime::getSystemNetworkInterface().ID == 0)
      ...
      _graph.getData(src_node).dist_current =
          std::numeric_limits<int>::max() / 4;

    galois::for_each(WorkItem(src_node), SSSP(&_graph),
