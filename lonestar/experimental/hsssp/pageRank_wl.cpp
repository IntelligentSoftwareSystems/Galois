...
struct InitializeGraph_ZeroResidual {
  ...
  void static go(Graph& _graph) {
    galois::do_all(_graph.begin(), _graph.end(),
                   InitializeGraph_ZeroResidual{&_graph},
                   galois::loopname("Init zero residual"));
  }
...
struct InitializeGraph {
  ...
  void static go(Graph& _graph) {
    ...
    struct SyncerPull_1 {
      static unsigned int extract(GNode src, const struct PR_NodeData& node) {
        return node.nout;
      }
      static void setVal(GNode src, struct PR_NodeData& node, unsigned int y) {
        node.nout = y;
      }
