...
static const char* const name =
    "BFS - Distributed Heterogeneous using vertex cut";
...
struct InitializeGraph {
  ...
  void static go(Graph& _graph) {
    ...
    _graph.sync_pull<SyncerPull_0>("InitGraph");
  }

  void operator()(GNode src) const {
    NodeData& sdata    = graph->getData(src);
    sdata.dist_current = std::numeric_limits<unsigned long long>::max() / 4;
    if (graph->getGID(src) == local_src_node) {
...
int main(int argc, char** argv) {
  try {
    ...
    galois::StatTimer StatTimer_init("TIMER_GRAPH_INIT"),
        StatTimer_total("TIMER_TOTAL"), StatTimer_hg_init("TIMER_HG_INIT");
