...
static const char* const name =
    "ConnectedComp - Distributed Heterogeneous with worklist.";
  ...
  void static go(Graph& _graph) {
    ...
    if (personality == GPU_CUDA) {
      ...
      galois::for_each(
          ...
          galois::loopname("ConnectedComp"), galois::workList_version(),
          galois::write_set("sync_push", "this->graph", "struct NodeData &",
                            "struct NodeData &", "comp_current",
                            "unsigned long long",
                            "{ galois::atomicMin(node.comp_current, y);}",
                            "{node.comp_current = std::numeric_limits<unsigned "
                            "long long>::max()/4; }"),
