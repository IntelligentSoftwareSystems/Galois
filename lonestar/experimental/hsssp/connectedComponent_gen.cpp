...
struct InitializeGraph {
  ...
  void static go(Graph& _graph) {
    struct SyncerPull_0 {
      static uint32_t extract(uint32_t id, const struct CC_NodeData& node) {
        return node.id;
      }
      static void setVal(uint32_t id, struct CC_NodeData& node, uint32_t y) {
        node.id = y;
      }
    ...
    galois::do_all(
        ...
        galois::write_set("sync_pull", "this->graph", "struct CC_NodeData &",
                          "struct CC_NodeData &", "id", "uint32_t"),
        galois::write_set("sync_pull", "this->graph", "struct CC_NodeData &",
                          "struct CC_NodeData &", "comp", "uint32_t"));
...
struct LabelPropAlgo {
  ...
  void static go(Graph& _graph) {
    struct Syncer_0 {
      ...
      static void reduce(uint32_t id, struct CC_NodeData& node, uint32_t y) {
        galois::atomicMin(node.comp, y);
      }
      static void reset(uint32_t id, struct CC_NodeData& node) {
        node.comp = std::numeric_limits<uint32_t>::max();
      }
      typedef uint32_t ValTy;
    };
    galois::do_all(
        _graph.begin(), _graph.end(), LabelPropAlgo{&_graph},
        galois::loopname("LabelPropAlgo"),
        galois::write_set("sync_push", "this->graph", "struct CC_NodeData &",
                          "struct std::atomic<unsigned int> &", "comp",
                          "uint32_t", "{galois::min(node.comp, y);}",
