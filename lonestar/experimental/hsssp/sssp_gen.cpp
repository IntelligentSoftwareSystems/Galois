...
#endif
        galois::do_all(
            _graph.begin(), _graph.end(), SSSP{&_graph},
...
int main(int argc, char** argv) {
  try {
    ...
    */
    ...
    std::cout << "[" << net.ID << "]" << " Total Time : " << T_total.get() << "
   offlineGraph : " << T_offlineGraph_init.get() << " DistGraph : " <<
   T_DistGraph_init.get() << " Init : " << T_init.get() << " sssp2 : " <<
   T_sssp2.get() << " (msec)\n\n";
    ...
    std::cout << "[" << net.ID << "]" << " Total Time : " << T_total.get() << "
   offlineGraph : " << T_offlineGraph_init.get() << " DistGraph : " <<
   T_DistGraph_init.get() << " Init : " << T_init.get() << " sssp3 : " <<
   T_sssp3.get() << " (msec)\n\n";
   ...
   T_total.stop();
    ...
    std::cout << "[" << net.ID << "]" << " Total Time : " << T_total.get() << "
   offlineGraph : " << T_offlineGraph_init.get() << " DistGraph : " <<
   T_DistGraph_init.get() << " Init : " << T_init.get() << " sssp1 : " <<
   T_sssp1.get() << " sssp2 : " << T_sssp2.get() << " sssp3 : " << T_sssp3.get()
   <<" sssp mean time (3 runs ) (" << maxIterations << ") : " << mean_time <<
   "(msec)\n\n";
*/
    if (verify) {
      for (auto ii = hg.begin(); ii != hg.end(); ++ii) {
