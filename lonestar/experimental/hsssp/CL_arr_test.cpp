...
struct InitializeArray {
  ...
  char* get_kstring() const {
    return "__kernel void init_kernel(__global int * arr, int num){ const uint "
           "my_id = get_global_id(0); if(my_id<num){arr[my_id]=my_id;} }";
  }
  char* get_kname() const { return "init_kernel"; }
...
struct VerifyArray {
  ...
  char* get_kstring() const {
    return "__kernel void verify_kernel(__global int * arr, __global int * "
           "fcounter, int num){ const uint my_id = get_global_id(0); "
           "if(my_id<num){atomic_add(fcounter, arr[my_id]!=0);} }";
  }
  char* get_kname() const { return "verify_kernel"; }
  CL_Kernel get_kernel(int num_items) const {
    CL_Kernel k(getCLContext()->get_default_device());
    k.init_string(get_kstring(), get_kname());
