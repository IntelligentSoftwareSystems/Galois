...
struct EdgeDataWrapper {
  ...
  void readIn(FileGraph& g) {
    ...
    data     = reinterpret_cast<ETy*>(
        Galoisruntime::MM::largeInterleavedAlloc(sizeof(ETy) * numEdges));
...
protected:
  ...
  uint64_t raw_neighbor_end(uint32_t N) const { return EdgeIndData[N]; }

  uint64_t getEdgeIdx(uint32_t src, uint32_t dst) {
    for (uint64_t ii = raw_neighbor_begin(src), ee = raw_neighbor_end(src);
         ii != ee; ++ii)
      if (EdgeDst[ii] == dst)
        return ii;
...
public:
  ...
  ~LC_CSR_Graph() {
    ...
    if (EdgeDst)
      Galoisruntime::MM::largeInterleavedFree(EdgeDst,
                                              sizeof(uint32_t) * numEdges);
    if (EdgeIndData)
      Galoisruntime::MM::largeInterleavedFree(EdgeIndData,
                                              sizeof(uint64_t) * numNodes);
    if (NodeData)
      Galoisruntime::MM::largeInterleavedFree(NodeData,
                                              sizeof(NodeInfo) * numNodes);
  }
  ...
  edge_data_reference getEdgeData(GraphNode src, GraphNode dst,
  ...
  void fromFile(const std::string& fname) {
    ...
    NodeData = reinterpret_cast<NodeInfo*>(
        Galoisruntime::MM::largeInterleavedAlloc(sizeof(NodeInfo) * numNodes));
    EdgeIndData = reinterpret_cast<uint64_t*>(
        Galoisruntime::MM::largeInterleavedAlloc(sizeof(uint64_t) * numNodes));
    EdgeDst = reinterpret_cast<uint32_t*>(
        Galoisruntime::MM::largeInterleavedAlloc(sizeof(uint32_t) * numEdges));
...
public:
  ...
  ~LC_CSRInline_Graph() {
    // TODO(ddn): call destructors of user data
    if (EdgeData)
      Galoisruntime::MM::largeInterleavedFree(EdgeData,
                                              sizeof(*EdgeData) * numEdges);
    if (NodeData)
      Galoisruntime::MM::largeInterleavedFree(NodeData,
                                              sizeof(*NodeData) * numEdges);
  }
  ...
  iterator begin() const { return iterator(&NodeData[0]); }
  ...
  void fromFile(const std::string& fname) {
    ...
    numEdges = graph.sizeEdges();
    NodeData = reinterpret_cast<NodeInfo*>(
        Galoisruntime::MM::largeInterleavedAlloc(sizeof(*NodeData) * numNodes));
    EdgeData = reinterpret_cast<EdgeInfo*>(
        Galoisruntime::MM::largeInterleavedAlloc(sizeof(*EdgeData) * numEdges));
    std::vector<NodeInfo*> node_ids;
    node_ids.resize(numNodes);
    for (FileGraph::iterator ii = graph.begin(), ee = graph.end(); ii != ee;
         ++ii) {
...
public:
  ...
  ~LC_Linear_Graph() {
    // TODO(ddn): call destructors of user data
    if (nodes)
      Galoisruntime::MM::largeInterleavedFree(nodes,
                                              sizeof(NodeInfo*) * numNodes);
    if (Data)
      Galoisruntime::MM::largeInterleavedFree(
          Data, sizeof(NodeInfo) * numNodes * 2 + sizeof(EdgeInfo) * numEdges);
  }
  ...
  iterator begin() const { return nodes; }

  iterator end() const { return &nodes[numNodes]; }
  ...
  void fromFile(const std::string& fname) {
    ...
    numNodes = graph.size();
    numEdges = graph.sizeEdges();
    Data     = Galoisruntime::MM::largeInterleavedAlloc(
        sizeof(NodeInfo) * numNodes * 2 + sizeof(EdgeInfo) * numEdges);
    nodes = reinterpret_cast<NodeInfo**>(
        Galoisruntime::MM::largeInterleavedAlloc(sizeof(NodeInfo*) * numNodes));
...
protected:
  ...
  void distribute(FileGraph& graph,
  ...
  struct AllocateNodes {
    ...
    AllocateNodes(Galoisruntime::PerThreadStorage<DistributeInfo>& d,
                  Galoisruntime::PerThreadStorage<Header*>& h, NodeInfo** n,
                  FileGraph& g)
    ...
    void operator()(unsigned int tid, unsigned int num) {
      ...
      size_t size = sizeof(Header) * 2 + sizeof(NodeInfo) * d.numNodes * 2 +
  ...
  struct AllocateEdges {
    ...
    AllocateEdges(Galoisruntime::PerThreadStorage<DistributeInfo>& d,
                  NodeInfo** n, FileGraph& g)
...
public:
  ...
  class local_iterator
      : public std::iterator<std::forward_iterator_tag, GraphNode> {
  ...
  public:
    ...
    local_iterator(const Galoisruntime::PerThreadStorage<Header*>* _headers,
                   int _tid)
    ...
    // local_iterator(const iterator& it): headers(it.headers), tid(it.tid),
    // p(it.p), v(it.v) { }
    ...
    bool operator==(const local_iterator& rhs) const {
      return (headers == rhs.headers && tid == rhs.tid && p == rhs.p &&
              v == rhs.v);
    }
    bool operator!=(const local_iterator& rhs) const {
      return !(headers == rhs.headers && tid == rhs.tid && p == rhs.p &&
  ...
  void fromFile(const std::string& fname) {
    ...
    nodes       = reinterpret_cast<NodeInfo**>(
        Galoisruntime::MM::largeInterleavedAlloc(size));
