...
void bots_get_architecture(char* str) {
  ...
  uname(&architecture);
  snprintf(str, BOTS_TMP_STR_SZ, "%s-%s;%d", architecture.sysname,
           architecture.machine, ncpus);
}
...
void bots_get_load_average(char* str) {
  ...
  snprintf(str, BOTS_TMP_STR_SZ, "%.2f;%.2f;%.2f", loadavg[0], loadavg[1],
           loadavg[2]);
}
#else
/* ****************************************************************** */
...
void bots_print_results() {
  ...
  sprintf(str_number_of_tasks_per_second, "%3.2f",
          (float)bots_number_of_tasks / bots_time_program);
  ...
  if (bots_print_header) {
    ...
    case 1:
      break;
    case 2:
      fprintf(stdout, "Benchmark;Parameters;Model;Cutoff;Resources;Result;\
...
Nodes;Nodes/Sec;\n");
  ...
  case 1:
    ...
    break;
  case 2:
    fprintf(stdout, "%s;%s;%s;%s;%s;%s;", str_name, str_parameters, str_model,
            str_cutoff, str_resources, str_result);
    fprintf(stdout, "%s;%s;%s;", str_time_program, str_time_sequential,
            str_speed_up);
    fprintf(stdout, "%s;%s;", str_number_of_tasks,
            str_number_of_tasks_per_second);
