...
static cll::opt<unsigned> length("len", cll::desc("Length of the array"),
                                 cll::init(10000));
static cll::opt<unsigned> LEAF_SIZE("leaf", cll::desc("recursion leaf size"),
                                    cll::init(64));
...
enum Algo { SERIAL, STL, CILK, GALOIS, GALOIS_STACK, GALOIS_GENERIC };
...
cll::opt<Algo> algorithm(
    ...
    cll::values(
        ...
        clEnumVal(GALOIS_STACK,
                  "galois stack-based typeddivide and conquer implementation"),
        clEnumVal(
            GALOIS_GENERIC,
            "galois stack-based generic divide and conquer implementation"),
        clEnumValEnd),
...
void mergeHalves(T* array, T* tmp_array, const size_t beg, const size_t mid,
                 const size_t end, const C& cmp) {

  std::merge(array + beg, array + mid, array + mid, array + end,
             tmp_array + beg);
  std::copy(tmp_array + beg, tmp_array + end, array + beg);
}

template <typename T, typename C>
void splitRecursive(T* array, T* tmp_array, const size_t beg, const size_t end,
                    const C& cmp) {
...
struct SplitGalois {
  ...
  SplitGalois(T* array, T* tmp_array, const C& cmp)
      : array(array), tmp_array(tmp_array), cmp(cmp) {}
#endif
...
#if defined(__INTEL_COMPILER) && __INTEL_COMPILER <= 1310
  MergeGalois(T* array, T* tmp_array, const C& cmp)
      : array(array), tmp_array(tmp_array), cmp(cmp) {}
#endif
...
template <typename T, typename C>
void mergeSortGaloisStack(T* array, T* tmp_array, const size_t L,
                          const C& cmp) {
...
struct MergeGaloisGeneric : public galois::runtime::TreeTaskBase {
  ...
  MergeGaloisGeneric(T* array, T* tmp_array, const C& cmp, size_t beg,
                     size_t mid, size_t end)
      : galois::runtime::TreeTaskBase(), array(array), tmp_array(tmp_array),
        cmp(cmp), beg(beg), mid(mid), end(end) {}
...
struct SplitGaloisGeneric : public galois::runtime::TreeTaskBase {
  ...
  SplitGaloisGeneric(T* array, T* tmp_array, const C& cmp, size_t beg,
                     size_t end)
      : galois::runtime::TreeTaskBase(), array(array), tmp_array(tmp_array),
        cmp(cmp), beg(beg), end(end) {}
...
template <typename T, typename C>
void mergeSortGaloisGeneric(T* array, T* tmp_array, const size_t L,
                            const C& cmp) {
  SplitGaloisGeneric<T, C> init{array, tmp_array, cmp, 0, L};
  abort();
  // FIXME: galois::runtime::for_each_ordered_tree_generic (init,
  // "mergesort-stack");
}
