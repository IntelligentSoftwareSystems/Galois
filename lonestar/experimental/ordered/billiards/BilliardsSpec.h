...
class BilliardsSpec : public Billiards {
  ...
  struct OpFunc {
    ...
    OpFunc(Table& table, const FP& endtime, Accumulator& iter)
        : table(table), endtime(endtime), iter(iter) {}

    template <typename C>
    void operator()(Event e, C& ctx) {
      // using const Event& in above param list gives error in the lambda
      // (despite using mutable), why???
      ...
      auto oncommit = [this, &ctx, e, notStale, b1, b2](void) mutable {
        ...
        using AddListTy =
            std::vector<Event, galois::PerIterAllocTy::rebind<Event>::other>;
        auto alloc = ctx.getPerIterAlloc();
        AddListTy addList(alloc);
        table.addNextEvents(e, addList, endtime);

        for (auto i = addList.begin(), endi = addList.end(); i != endi; ++i) {
...
public:
  ...
  size_t runSim(Table& table, std::vector<Event>& initEvents, const FP& endtime,
    ...
    galois::runtime::for_each_ordered_rob(
        galois::runtime::makeStandardRange(initEvents.begin(),
                                           initEvents.end()),
        Event::Comparator(), BilliardsLevelExec::VisitNhood(graph, nodes),
