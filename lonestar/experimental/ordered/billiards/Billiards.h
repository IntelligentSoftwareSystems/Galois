...
static const char* const desc =
    "Simulates elastic collisions between billiard balls on a table";
static const char* const url = "billiards";

static cll::opt<unsigned> sectorSize("secsize",
                                     cll::desc("size of a (square) sector"),
                                     cll::init(100));
static cll::opt<unsigned>
    xSectors("xsec", cll::desc("number of sectors along length"), cll::init(1));
static cll::opt<unsigned>
    ySectors("ysec", cll::desc("number of sectors along height"), cll::init(1));
static cll::opt<unsigned> numballs("balls",
                                   cll::desc("number of balls on the table"),
                                   cll::init(100));
static cll::opt<unsigned> endtime("end", cll::desc("simulation end time"),
                                  cll::init(100));

// static cll::opt<bool> runFlat("flat", cll::desc("Run simulation without
// introducing sectors/partitions"), cll::init(false));

static cll::opt<bool>
    veriFlat("vflat", cll::desc("Verify against serial flat simulation"),
             cll::init(false));
...
private:
  ...
  void verify(const Tbl_t& initial, Tbl_t& final, size_t numEvents,
              const FP& endtime);
...
// #define CUSTOM_TESTS
  ...
  void runImpl(void) {
    ...
    std::cout << "Number of initial events = " << initEvents.size()
              << std::endl;

    galois::preAlloc(
        (galois::getActiveThreads() * unsigned(endtime) * numballs * 10) /
    ...
    timer.start();
    size_t numEvents = static_cast<Derived*>(this)->runSim(
        table, initEvents, unsigned(endtime), enablePrints, logEvents);
    timer.stop();
    galois::reportPageAlloc("MeminfoPost");

    std::cout << "Billiards " << version()
              << ", number of events processed=" << numEvents << std::endl;

    if (!skipVerify) {
      if (veriFlat) {
        verify<typename Tbl_t::SerialFlatTable>(
            static_cast<const Table<Ball_t>&>(verCopy),
            static_cast<Table<Ball_t>&>(table), numEvents, unsigned(endtime));

      } else {
        verify<typename Tbl_t::SerialTable>(verCopy, table, numEvents,
                                            unsigned(endtime));
      }
    }
  }
...
class BilliardsSerialPQ : public Billiards<BilliardsSerialPQ<Tbl_t>, Tbl_t> {
  ...
  using PriorityQueue =
      std::priority_queue<Event, std::vector<Event>, Event::ReverseComparator>;
...
template <typename Derived, typename Tbl_t>
template <typename T>
void Billiards<Derived, Tbl_t>::verify(const Tbl_t& initial, Tbl_t& final,
                                       size_t numEvents, const FP& endtime) {
  ...
  timer.start();
  size_t serEvents =
      serial.runSim(serialTable, initEvents, endtime, false, true);
  timer.stop();
  ...
  if (serEvents != numEvents) {
    std::cerr
        << "WARNING: number of events differ from verification run, normal="
  ...
  if (!result) {
    std::cerr << "ERROR, Comparison against serial ordered simulation failed "
                 "due to above differences"
              << std::endl;

    // std::cout << "<<<<<<<<<<< Event logs from Original Run >>>>>>>>>>>>>" <<
    // std::endl; final.printEventLogs ();
    //
    // std::cout << "<<<<<<<<<<< Event logs from Verification Run >>>>>>>>>>>>>"
    // << std::endl; serialTable.printEventLogs ();
    ...
    std::cout << "OK, Result verified against serial ordered simulation"
              << std::endl;
  }
}
