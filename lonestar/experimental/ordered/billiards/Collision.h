...
public:
  ...
  //! happen
  //!
  //! Dist { (b1.pos () + dt1*b1.vel ()), (b2.pos () + dt2*b2.vel ()) } <=
  //! (b1.radius () + b2.radius ()) here dt1 = t - b1.time (); dt2 = t - b2.time
  //! (), t being the time of collision
  //!
  //! => mag ( (b1.pos () + 1t*b1.vel ()) - (b2.pos () + dt2*b2.vel ()) ) <=
  //! (b1.radius () + b2.radius ())
  //!
  //! Solving the resulting quadratic equation gives us the result

  static std::pair<bool, FP> computeCollisionTime(const Ball& ball1,
                                                  const Ball& ball2) {
    ...
    // FP discr = (diffVdiffD * diffVdiffD) - (diffV.magSqrd ()) *
    // (diffD.magSqrd () - sumRadiusSqrd);
    ...
    FP discr = sumRadiusSqrd * (diffV.magSqrd()) -
  ...
  static std::pair<bool, FP> computeCollisionTime(const Ball& ball,
                                                  const Cushion& cush) {
    return computeCollisionTime(ball, cush.getLineSegment());
  }

  static std::pair<bool, FP>
  computeCollisionTime(const Ball& ball, const LineSegment& lineSeg,
                       const bool useGhostPos = false) {
  ...
  static void simulateCollision(Ball& ball1, Ball& ball2, const FP& time) {
    ...
    FP v1_tangent_after = ((b1mass - b2mass) * v1_tangent_before +
                           (FP(2.0) * b2mass) * v2_tangent_before) /
                          sumMass;

    FP v2_tangent_after = ((b2mass - b1mass) * v2_tangent_before +
                           (FP(2.0) * b1mass) * v1_tangent_before) /
                          sumMass;
  ...
  static bool isValidCollision(const Ball* b1, const Cushion* c,
                               const FP& time) {
    assert(b1);
    assert(c);
    ...
    if (FPutils::almostEqual(c->getLineSegment().distanceFrom(p),
                             b1->radius())) {
      return true;
    }
...
private:
  ...
  //! may lie in the direction of the ball, or exactly opposite. When the point
  //! of intersection is opposite to the direction of velocity of a ball, then
  //! usually that means the collision time is in the past of the ball and it's
  //! not a valid collision. Also, dot product of the distance vector (between
  //! current position and position at the time of collision) and velocity
  //! should be negative i.e. both are in opposite directions. We show below
  //! that if this dot product is negative, then the collision is invalid and
  ...
  //! thus if dist.vel < 0.0 then it must be the case that (collisionTime <
  //! b.time)

  static bool isInFuture(const FP& ballTime, const FP& collisionTime) {
    assert(ballTime >= FP(0.0));
    // assert (ballTime != collisionTime);
    return (collisionTime >= ballTime ||
            FPutils::almostEqual(collisionTime, ballTime));
  }

  // If a ball b has just collided with cushion c (or another ball b2)
  // the equations in computeCollisionTime will show that ball b can collid e
  // with c i.e. repeat the same collision that has just taken place To avoid
  // such repetitions, we check if the ball is moving towards or way from the
  // cushion. This can be done by dot product of velocity and inward (right)
  // normal of cushion. If dot product is -ve, then ball is moving towards the
  // cushion, else it's moving away from it.
  //
  ...
  // If dot product of b1 with C is positive (b1's tangential component) or dot
  // product of b2 with C is negative, then we can say that balls are
  // approaching each other. Actually, if the difference of tangential
  // components is positive then the balls are approaching each other
...
public:
  ...
  // We don't want to create a collision with the object involved in previous
  // collision
  template <typename I, typename T = typename std::remove_pointer<
                            typename std::iterator_traits<I>::value_type>::type>
  static galois::optional<Event>
  computeNextEvent(const Event::EventKind& kind, const Ball* b,
                   const I collObjsBeg, const I collObjsEnd, const FP& endtime,
                   const Event* prevEvent, const Sector* sector) {

    // std::cout << "Computing future events for ball: " << b->str () <<
    // std::endl;
    ...
    for (I i = collObjsBeg; i != collObjsEnd; ++i) {
      ...
      // the object under test is not the same as the one involved in a previous
      // collision event if (static_cast<const CollidingObject*> (underTest) !=
      // static_cast<const CollidingObject*> (b)
      // && prevEventObj != underTest) {

      if (static_cast<const CollidingObject*>(underTest) !=
          static_cast<const CollidingObject*>(b)) {
    ...
    // std::cout << "At time: " << std::fixed << double (p.second) << " Ball b="
    // << b->str () << " can collide with=" << underTest->str () << std::endl;
    // }
    //
    // }
    //
