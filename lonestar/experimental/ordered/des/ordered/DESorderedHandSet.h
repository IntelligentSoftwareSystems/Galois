...
struct SimObjInfo : public TypeHelper<> {
  ...
  struct MarkedEvent {
    ...
    explicit MarkedEvent(const Event_ty& _event) : event(_event), flag(false) {}
  ...
  typedef std::set<MarkedEvent, Cmp_ty, galois::FixedSizeAllocator<MarkedEvent>>
      PQ;
  // typedef std::priority_queue<MarkedEvent, std::vector<MarkedEvent>,
  // Cmp_ty::RevCmp> PQ;
  ...
  void recv(const Event_ty& event) {
    ...
    assert(Cmp_ty::compare(event, lastInputEvents[dstIn]) >=
           0); // event >= last[dstIn]
  ...
  bool canAddMin() const {
    mutex.lock();
    bool ret = !pendingEvents.empty() && !(pendingEvents.begin()->isMarked()) &&
               isReady(*pendingEvents.begin()) && pendingEvents.begin()->mark();
    mutex.unlock();
...
class DESorderedHandSet : public des::AbstractMain<TypeHelper<>::SimInit_ty>,
                          public TypeHelper<> {
  ...
  struct OpFuncSet {
    ...
    OpFuncSet(Graph& graph, std::vector<SimObjInfo>& sobjInfoVec,
      ...
      while (epi < DEFAULT_EPI) {
        ...
        for (AddList_ty::local_iterator a    = newEvents.get().begin(),
          ...
          if (sinfo.canAddMin()) {
            ...
            // std::cout << "### Adding: " << static_cast<const Event_ty&>
            // (sinfo.getMin ()).detailedString () << std::endl;
          }
        }
        ...
        if (srcInfo.canAddMin()) {
          ...
          // std::cout << "%%% Adding: " << static_cast<const Event_ty&>
          // (srcInfo.getMin ()).detailedString () << std::endl;
        } else {
          break;
        }
  ...
  virtual void runLoop(const SimInit_ty& simInit, Graph& graph) {
    ...
    for (std::vector<Event_ty>::const_iterator
      ...
      if (sinfo.canAddMin()) {
        ...
        // std::cout << "Initial source found: " << Event_ty (sinfo.getMin
        // ()).detailedString () << std::endl;
      }
    }
