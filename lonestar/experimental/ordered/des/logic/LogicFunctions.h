...
/**
 * Interface of a functor for modeling the funciton of a one input one, output
 * logic gate. Each implementation of this interface is a different kind of one
 * input gate, e.g. an inverter, buffer etc
 */
...
/**
 * Interface of a functor for modeling functionality a logic gate with two
 * inputs and one output. Each implementation of this interface describes a two
 * input gate
 */
 ...
 * Buffer
 */
struct BUF : public OneInputFunc,
             public std::unary_function<LogicVal, LogicVal> {
  LogicVal _buf_(const LogicVal& in) const { return in; }

  virtual LogicVal operator()(const LogicVal& in) const { return _buf_(in); }
 ...
 * Inverter
 */
struct INV : public OneInputFunc,
             public std::unary_function<LogicVal, LogicVal> {
  ...
  virtual LogicVal operator()(const LogicVal& in) const { return _not_(in); }
...
struct AND2 : public TwoInputFunc,
              public std::binary_function<LogicVal, LogicVal, LogicVal> {
 ...
 * OR with two inputs
 */
struct OR2 : public TwoInputFunc,
             public std::binary_function<LogicVal, LogicVal, LogicVal> {
 ...
 * XOR with two inputs
 */
struct XOR2 : public TwoInputFunc,
              public std::binary_function<LogicVal, LogicVal, LogicVal> {
