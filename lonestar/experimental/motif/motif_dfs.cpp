...
public:
  ...
  void reduction(unsigned level, EmbeddingList& emb_list, VertexId src,
                 VertexId dst, unsigned previous_pid) {
    if (max_size == 3 && emb_list.get_label(dst) == 3) {
      emb_list.inc_tri_count();
    } else if (max_size == 4) {
      if (src < dst && previous_pid == 0 &&
          emb_list.get_label(dst) == 3) { // clique
        accumulators[5] += 1;
  ...
  void update(unsigned level, unsigned vid, unsigned start,
              unsigned previous_pid, EmbeddingList& emb_list) {
    unsigned pid = 0;
