...
static cll::opt<unsigned>
    k("k", cll::desc("trussNum of k-truss (default value 3)"), cll::init(3));
static cll::opt<std::string>
    outName("o", cll::desc("output file for the edgelist of resulting truss"));
// typedef galois::graphs::LC_CSR_Graph<uint32_t,
// void>::with_numa_alloc<true>::type ::with_no_lockable<true>::type Graph;
...
void TrussJacobi(Graph& g) {
  ...
  galois::do_all(
      ...
      [&](const GNode& src) {
        for (auto e : g.edges(src, galois::MethodFlag::UNPROTECTED)) {
          auto dst = g.getEdgeDst(e);
          if (dst > src)
            cur.push_back(std::make_pair(src, dst));
        }
      },
      galois::steal());
  ...
  while (true) {
    galois::do_all(
        galois::iterate(cur),
        [&](NodePair& e) {
          EdgeVec& w = isSupportNoLessThanJ(g, e.first, e.second, k - 2)
                           ? next
                           : unsupported;
          w.push_back(e);
        },
