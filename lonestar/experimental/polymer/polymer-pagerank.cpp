...
static cll::opt<Algo> algo("algo", cll::desc("Choose an algorithm:"),
    cll::values(
      ...
      clEnumValN(Algo::polypush, "polypush", "polymer-like push-based algorithm"),
      clEnumValN(Algo::polypull, "polypull", "polymer-like pull-based algorithm"),
      clEnumValN(Algo::polypull2, "polypull2", "polymer-like more partitions pull-based algorithm"),
      clEnumValN(Algo::duppull, "duppull", "numa-awared pull-based algorithm with duplicates"),
      clEnumValN(Algo::push, "push", "Use precomputed data perform push-based algorithm"),
...
struct PolyPush {
  ...
  struct LocalData {
    ...
    LocalData(unsigned int nnodes1, unsigned int max_nodes_per_package1, unsigned int max_edges_per_package1): 
    ...
    void alloc() {
      ...
      nodeList = (unsigned int * ) numa_alloc_local( (nnodes + 1) * sizeof(unsigned int));
      edgeList = (unsigned int * ) numa_alloc_local( max_edges_per_package * sizeof(unsigned int));
      outDegree = (unsigned int * ) numa_alloc_local( (nnodes + 1) * sizeof(unsigned int));
      inDegree = (unsigned int * ) numa_alloc_local( (nnodes + 1) * sizeof(unsigned int));
    ...
    void next_reset(){
      ...
      //printf(" %d %d %f %f\n",Bit_next[0], Bit_next[max_nodes_per_package - 1], PR_next[0], PR_next[max_nodes_per_package - 1] );
    }
  };
  ...
  typedef galois::graphs::LC_InlineEdge_Graph<LNode,void>
    ::with_no_lockable<true>::type
    ::with_numa_alloc<false>::type    // interleaved allocate
    InnerGraph;
  ...
  struct Initialize {
    ...
    void operator()(Graph::GraphNode n) const {
      LNode& data = g.getData(n, galois::MethodFlag::NONE);
      unsigned int outs = std::distance(g.edge_begin(n, galois::MethodFlag::NONE), g.edge_end(n, galois::MethodFlag::NONE));
      data.outDegree = outs;
      unsigned int ins = std::distance(g.in_edge_begin(n, galois::MethodFlag::NONE), g.in_edge_end(n, galois::MethodFlag::NONE));
      data.inDegree = ins;
      GNode start = *g.begin();
  ...
  struct Copy {
    ...
    Copy(Graph& g, PerPackageData& pData, unsigned int* nodesP, unsigned int nPackages): graph(g) , packageData(pData), nodesPerPackage(nodesP), nPackages(nPackages) {
    ...
    void operator()(unsigned tid, unsigned numT) {
      if(galois::runtime::LL::isPackageLeader(tid)){
        ...
        for(auto ii = graph.begin(), end = graph.end(); ii != end ; ii++)
        {
          ...
          if(sdata.id >= nodesStartPackage[currPackage] && sdata.id < nodesStartPackage[currPackage + 1] )
            sdata.pagerank = packageData.getLocal()->PR_curr[sdata.id - nodesStartPackage[currPackage]];
        }
  ...
  struct distributeEdges{
    ...
    void operator() (unsigned tid, unsigned numT){
      if(galois::runtime::LL::isPackageLeader(tid)){
        ...
        //printf("tid: %d, edgesPerPackage: %d", tid, edgesPerPackage[currPackage]);
        ...
        //std::cerr << "tid: " << tid <<"range:"<<rangeLow<<"-"<<rangeHi<< std::endl;
        //printf("work0.3 %d %d\n", rangeLow, rangeHi);
        unsigned int indegree_temp = 0;

        //std::cerr << "tid: " << tid <<"edgeCount:"<<edgeCount<< " inDegree "<< indegree_temp<<std::endl;
        edgeCount = 0;
        for (auto ii = graph.begin( ), end = graph.end( ); ii != end; ii++)  {
          GNode src = *ii;
          LNode& sdata = graph.getData(src, galois::MethodFlag::NONE);
  ...
  struct PartitionInfo{
    ...
    PartitionInfo(unsigned int nnodes, unsigned int numThreads, unsigned int nPackages, unsigned int coresPerPackage, unsigned int coresLastPackage):
      nnodes(nnodes), numThreads(numThreads), nPackages(nPackages), coresPerPackage(coresPerPackage), coresLastPackage(coresLastPackage){
    ...
    void partitionByDegree(Graph& graph, unsigned int numThreads, unsigned int *nodesPerThread, unsigned int *edgesPerThread) {
     ...
     //parallel_for(intT i = 0; i < n; i++) degrees[i] = GA.V[i].getInDegree();}
      ...
      for (auto ii = graph.begin(), ei = graph.end(); ii != ei; ++ii)  {
        ...
        if ((accum[counter] >= averageDegree && counter < numThreads - 1 )|| ii == ei - 1) {
    ...
    void subPartitionByDegree(Graph& graph, unsigned int nCores, unsigned int *nodesPerCore, unsigned int *nodesStartCore) {
      unsigned int n = graph.size();
    ...
    void partition(Graph& graph) {  
      ...
      for(unsigned int i = 0; i < nPackages; i++) {
        ...
        if(i > 0)
          nodesStartPackage[i] = nodesStartPackage[i - 1] + nodesPerPackage[i - 1];
      }
      nodesStartPackage[nPackages] = nodesStartPackage[nPackages - 1] + nodesPerPackage[nPackages - 1];

      subPartitionByDegree(graph,coresPerPackage, nodesPerCore, nodesStartCore);
      subPartitionByDegree(graph,coresLastPackage, nodesPerCoreLastPkg, nodesStartCoreLastPkg);

      //printf("nodesStartPackage: %d %d %d\n", nodesStartPackage[0], nodesStartPackage[1], nodesStartPackage[2]);
      /*for(int i = 0; i < numThreads; i++)
      {
  ...
  struct Process{
    ...
    Barrier& gbarrier;
    Process(PerPackageData &p, PartitionInfo& partitionInfo, Barrier& gbarrier, PolyPush* self): 
    ...
    void operator() (unsigned tid, unsigned numT) {
      ...
      unsigned int threadStart = (currPackage == nPackages - 1)?partitionInfo.nodesStartCoreLastPkg[tid - leader] : partitionInfo.nodesStartCore[tid - leader];
      unsigned int nodesCurrCore = (currPackage == nPackages - 1)?partitionInfo.nodesPerCoreLastPkg[tid - leader] : partitionInfo.nodesPerCore[tid - leader];
      ...
      for(unsigned int i = 0 ;i < nPackages;i++){
        if(threadStart >= nodesStartPackage[i] && threadStart < nodesStartPackage[i + 1]){
          activePackage = i;
  ...
  struct Process2{
    ...
    PartitionInfo& partitionInfo;
    Barrier& gbarrier;
    Process2(PerPackageData &p, PartitionInfo& partitionInfo, Barrier& gbarrier, PolyPush* self): 
      packageData(p), partitionInfo(partitionInfo), gbarrier(gbarrier), self(self) {}
    ...
    void damping(float * PR_next, unsigned tid, PolyPush *self,PerPackageData& packageData){
      ...
      nodesPerCore = partitionInfo.nodesPerPackage[currPackage]/coresCurrPackage;
      float diff;
      if(localtid != coresCurrPackage - 1){
        for(unsigned int i = localtid * nodesPerCore; i < (localtid + 1) * nodesPerCore; i++) {
        ...
        for(unsigned int i = localtid * nodesPerCore; i < partitionInfo.nodesPerPackage[currPackage]; i++)
        {
          PR_next[i] = alpha2 * PR_next[i] + 1 - alpha2;
          /*diff = packageData.getLocal()->PR_curr[i] - PR_next[i];
          self->max_delta.update(diff);
  ...
  void operator()(Graph& graph) {
    ...
    PartitionInfo partitionInfo(nnodes, numThreads, nPackages, coresPerPackage, coresLastPackage);
    ...
    for(int i = 0; i < nPackages; i++){
      max_nodes_per_package = (max_nodes_per_package > nodesPerPackage[i])? max_nodes_per_package: nodesPerPackage[i];
      max_edges_per_package = (max_edges_per_package > edgesPerPackage[i])? max_edges_per_package: edgesPerPackage[i];   
    }

    PerPackageData packageData(nnodes, max_nodes_per_package, max_edges_per_package);

    galois::on_each(distributeEdges(graph, packageData, nodesPerPackage, edgesPerPackage));
    unsigned int iteration = 0;
...
struct PolyPull {
  ...
  struct PartitionInfo{
    ...
    PartitionInfo(unsigned nnodes, unsigned numThreads, unsigned nPackages, unsigned coresPerPackage, unsigned coresLastPackage):
      nnodes(nnodes), numThreads(numThreads), nPackages(nPackages), coresPerPackage(coresPerPackage), coresLastPackage(coresLastPackage){
    ...
    void partitionByDegree(Graph& graph, unsigned numThreads, unsigned *nodesPerThread, unsigned *edgesPerThread) {
      int n = graph.size();
      //int *degrees = new int [n];
    ...
    void subPartitionByDegree(Graph& graph, int nCores, unsigned *nodesPerCore, unsigned *nodesStartCore) {
  ...
  struct Process{
    ...
    PartitionInfo& partitionInfo;
    Barrier& gbarrier;
    Process(PerPackageData &p, PartitionInfo& partitionInfo, Barrier& gbarrier, PolyPull* self): 
      packageData(p), partitionInfo(partitionInfo), gbarrier(gbarrier), self(self){}
    void update(float *PR_next, unsigned offset,float value){
      writeAdd(PR_next + offset, value);
      //PR_next[offset] += value;
    ...
    void operator() (unsigned tid, unsigned numT) {
      ...
      unsigned threadStart = (currPackage == nPackages - 1)?partitionInfo.nodesStartCoreLastPkg[tid - leader] : partitionInfo.nodesStartCore[tid - leader];
      unsigned nodesCurrCore = (currPackage == nPackages - 1)?partitionInfo.nodesPerCoreLastPkg[tid - leader] : partitionInfo.nodesPerCore[tid - leader];
  ...
  struct Process2{
    ...
    PartitionInfo& partitionInfo;
    Barrier& gbarrier;
    Process2(PerPackageData &p, PartitionInfo& partitionInfo, Barrier& gbarrier, PolyPull* self): 
      packageData(p), partitionInfo(partitionInfo), gbarrier(gbarrier), self(self) {}
    ...
    void damping(float * PR_next, unsigned tid, PolyPull *self,PerPackageData& packageData){
      ...
      float diff;
      if(localtid != coresCurrPackage - 1){
        for(unsigned i = localtid * nodesPerCore; i < (localtid + 1) * nodesPerCore; i++) {
          PR_next[i] = alpha2 * PR_next[i] + 1 - alpha2;
        ...
        for(unsigned i = localtid * nodesPerCore; i < partitionInfo.nodesPerPackage[currPackage]; i++)
        {
          PR_next[i] = alpha2 * PR_next[i] + 1 - alpha2;
          /*diff = std::fabs(packageData.getLocal()->PR_curr[i] - PR_next[i]);
          self->max_delta.update(diff);
...
/*struct PolyPull2 {
  ...
  struct distributeEdges{
    ...
    distributeEdges(Graph &g, PerPackageData& p, unsigned int *nodes, unsigned int* edges, unsigned superPartition): 
    ...
    void operator() (unsigned tid, unsigned numT){
      if(galois::runtime::LL::isPackageLeader(tid)){
        ...
        unsigned int rangeHi = rangeLow + nodesPerPackage[currPackage * superPartition];
  ...
  struct PartitionInfo{
    unsigned int numThreads, nnodes, nPackages, coresPerPackage, coresLastPackage, superPartition;
    unsigned *nodesPerPackage;
    unsigned *nodesStartPackage;
    unsigned *edgesPerPackage;
    unsigned *nodesPerCore;
    ...
    unsigned *nodesPerCoreLastPkg;
    unsigned *nodesStartCoreLastPkg;
    PartitionInfo(unsigned nnodes, unsigned numThreads, unsigned nPackages, unsigned coresPerPackage, unsigned coresLastPackage, unsigned superPartition):
      nnodes(nnodes), numThreads(numThreads), nPackages(nPackages), coresPerPackage(coresPerPackage), coresLastPackage(coresLastPackage),superPartition(superPartition){
    ...
    void partition(Graph& graph) {  
      ...
      for(int i = 0; i < nPackages * superPartition;i += superPartition) {
        ...
        for(int x = 1; x < superPartition; x++)
        {
          ...
          nodesStartPackage[i + x] = nodesStartPackage[i] + x * nodesPerPackage[i] / superPartition;
        }
  ...
  struct Process{
    ...
    PartitionInfo& partitionInfo;
    Barrier& gbarrier;
    Process(PerPackageData &p, PartitionInfo& partitionInfo, Barrier& gbarrier, PolyPull2* self): 
      packageData(p), partitionInfo(partitionInfo), gbarrier(gbarrier), self(self){}
    void update(float *PR_next, unsigned offset,float value){
      writeAdd(PR_next + offset, value);
      //PR_next[offset] += value;
  ...
  struct Process2{
    ...
    PartitionInfo& partitionInfo;
    Barrier& gbarrier;
    Process2(PerPackageData &p, PartitionInfo& partitionInfo, Barrier& gbarrier, PolyPull2* self): 
      packageData(p), partitionInfo(partitionInfo), gbarrier(gbarrier), self(self) {}
...
struct DupPull {    // still buggy
    ...
    void operator()(unsigned tid, unsigned numT) {
      if(galois::runtime::LL::isPackageLeader(tid)){
        ...
        for(auto ii = graph.begin(), end = graph.end(); ii != end ; ii++)
        {
          ...
          if(sdata.id >= nodesStartPackage[currPackage] && sdata.id < nodesStartPackage[currPackage + 1] )
            sdata.pagerank = packageData.getLocal()->PR_next[sdata.id - nodesStartPackage[currPackage]];
        }
  ...
  struct Process{
    ...
    PartitionInfo& partitionInfo;
    Barrier& gbarrier;
    Process(PerPackageData &p, PartitionInfo& partitionInfo, Barrier& gbarrier, DupPull* self): 
      packageData(p), partitionInfo(partitionInfo), gbarrier(gbarrier), self(self) {}
    void update(float *PR_next, unsigned offset,float value){
      writeAdd(PR_next + offset, value);
      //PR_next[offset] += value;
    ...
    void operator() (unsigned tid, unsigned numT) {
      ...
      unsigned coresCurrPackage = (currPackage == nPackages - 1)?partitionInfo.coresLastPackage : partitionInfo.coresPerPackage;
      ...
      else threadEnd = (tid - leader + 1) * nodesPerCore;
  ...
  struct Process2{
    ...
    PartitionInfo& partitionInfo;
    Barrier& gbarrier;
    Process2(PerPackageData &p, PartitionInfo& partitionInfo, Barrier& gbarrier, DupPull* self): 
      packageData(p), partitionInfo(partitionInfo), gbarrier(gbarrier), self(self) {}
    ...
    void damping(float * PR_next, unsigned tid, DupPull *self,PerPackageData& packageData){
      ...
      if(localtid != coresCurrPackage - 1){
        for(unsigned i = localtid * nodesPerCore; i < (localtid + 1) * nodesPerCore; i++) {
          PR_next[i] = alpha2 * PR_next[i] + 1 - alpha2;
          diff = std::fabs(packageData.getLocal()->PR_dup[i + offset] - PR_next[i]);
          self->max_delta.update(diff);
          self->sum_delta.update(diff);
        }
      }
    ...
    void operator()(unsigned tid, unsigned numT){
      ...
      if(galois::runtime::LL::isPackageLeader(tid)){
        ...
        for(unsigned i = 0; i < nPackages; i++)
        {
          if(i == currPackage)
          {
            memcpy(Bit_curr + offset, packageData.getLocal()->Bit_next, nodesCurrPackage * sizeof(bool));
            memcpy(PR_curr + offset, packageData.getLocal()->PR_next, nodesCurrPackage * sizeof(float));
          }
          else
          {
            memcpy(Bit_curr + partitionInfo.nodesStartPackage[i], packageData.getRemoteByPkg(i)->Bit_next, partitionInfo.nodesPerPackage[i] * sizeof(bool));
            memcpy(PR_curr + partitionInfo.nodesStartPackage[i], packageData.getRemoteByPkg(i)->PR_next, partitionInfo.nodesPerPackage[i] * sizeof(float));
          }
        }
...
struct PushAlgo {
  ...
  struct Reset {
    ...
    void operator()(Graph::GraphNode n) const {
      LNode& data = g.getData(n, galois::MethodFlag::NONE);
      data.value[(iteration+1) & 1] = data.value[(iteration+1) & 1] * alpha2 + 1-alpha2;
  ...
  struct Process {
    ...
    Process(PushAlgo* s, Graph& g, unsigned int i): self(s), graph(g), iteration(i) { }
...
struct PrtRsd {
  ...
  void operator()(Graph& graph) {
    ...
    T.start();
    galois::for_each(initialWL, Process3(this, graph, pre, post), galois::wl<dOBIM>(), galois::loopname("mainloop"));   
    T.stop();
  }
...
int main(int argc, char **argv) {
  ...
  switch (algo) {
    ...
    case Algo::polypush: run<PolyPush>(); break;
    case Algo::polypull: run<PolyPull>(); break;
    //case Algo::polypull2: run<PolyPull2>(); break;
    case Algo::duppull: run<DupPull>(); break;
    case Algo::push: run<PushAlgo>(); break;
