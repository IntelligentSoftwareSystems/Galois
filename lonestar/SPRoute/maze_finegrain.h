...
void mazeRouteMSMD_finegrain(int iter, int expand, float costHeight, int ripup_threshold, int mazeedge_Threshold, Bool Ordering, int cost_type, 
    ...
    //galois::substrate::PerThreadStorage<THREAD_LOCAL_STORAGE>* thread_local_storage)
{
    ...
    if (cost_type == 2) {
        ...
        for(int i=0; i<forange; i++) {
            if(i<hCapacity)
                h_costTable[i] = (round_num > 50)? 10 : costHeight/(exp((float)(hCapacity-i)*LOGIS_COF)+1) + 1;
                else
        ...
        for(int i=0; i<forange; i++) {
            if(i<vCapacity)
                v_costTable[i] = (round_num > 50)? 10 : costHeight/(exp((float)(vCapacity-i)*LOGIS_COF)+1) + 1;
                else
                        ...
                        if(d1_push == curr_d1 && d1_push < return_dist.load())
                        {
                            ...
                            if(curX<regionX2)
                            {
                                ...
                                if(d1[curY][tmpX]>tmp && tmp < return_dist) // right neighbor been put into heap1 but needs update
                                {
                                    ...
                                    //pq_grid grid_push = {&(d1[curY][tmpX]), tmp};
                                    ctx.push(pq_grid(&(d1[curY][tmpX]), tmp));
                                    //printf("right push Y: %d X: %d tmp: %f HV: false hyperH: %d\n", curY, tmpX, tmp, true);
                                }
                            }
...
void mazeRouteMSMD_finegrain_spinlock(int iter, int expand, float costHeight, int ripup_threshold, int mazeedge_Threshold, Bool Ordering, int cost_type, 
    galois::InsertBag<int>* net_shuffle)
{
    //LOCK = 0;
   ...
   //galois::runtime::profileVtune( [&] (void) {
    ...
    if(acc_cnt != 0) {
        ...
        cout << " avg dist " << acc_dist / acc_cnt << " avg length " << (float)acc_length / acc_cnt << " acc_cnt: " << acc_cnt << endl;
...
void mazeRouteMSMD_finegrain_doall(int iter, int expand, float costHeight, int ripup_threshold, int mazeedge_Threshold, Bool Ordering, int cost_type, 
    galois::InsertBag<int>* net_shuffle)
{
    //LOCK = 0;
    galois::StatTimer timer_finegrain("fine grain function", "fine grain maze");
        ...
        for(edgeREC=0; edgeREC<num_edges; edgeREC++)
        {
            ...
            if(treeedge->len > mazeedge_Threshold) // only route the non-degraded edges (len>0)
            {
                ...
                if(enter)
                {
                    ...
                    while(!cur->empty()) {
                        //std::cout << "bag size: " << bagsize.load() << " unique items: " << dynBS.count() << std::endl;
                        //bagsize = 0;
                        //dynBS.reset();
                        ...
                        //while( pop_heap2[ind1]==FALSE) // stop until the grid position been popped out from both heap1 and heap2
                        {
                            ...
                            float curr_d1 = d1[curY][curX];
                            //std::cout << "d1_push: " << d1_push << " d1: " << d1[curY][curX] << std::endl;
                            //if(netID == 2 && edgeID == 26) 
                            //printf("netID: %d edgeID:%d curX curY %d %d, d1_push: %f, curr_d1: %f\n", netID, edgeID, curX, curY, d1_push, curr_d1);
