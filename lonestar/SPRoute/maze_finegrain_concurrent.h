...
void setupHeap_nopq1clear(int netID, int edgeID, galois::InsertBag<Concurrent_pq_grid>& pq1, std::vector<int>& v2, int regionX1, 
    int regionX2, int regionY1, int regionY2, float** d1, int** corrEdge, bool** inRegion, int concurrentID)
{
                            ...
                            if(visited[nbr]==FALSE)
                            {
                                ...
                                if(treeedges[edge].route.routelen>0) // not a degraded edge
                                {
                                    ...
                                    if(inRegion[treenodes[nbr].y][treenodes[nbr].x])
                                    {
                                        ...
                                        //if(netID == 252163 && edgeID == 51)
                                        //    printf("y: %d x: %d\n", nbrY, nbrX);
                                        pq1.push(Concurrent_pq_grid(&(d1[nbrY][nbrX]), 0, concurrentID));
                                    ...
                                    if(route->type==MAZEROUTE)
                                    {
                                        for(j=1; j<route->routelen; j++) // don't put edge_n1 and edge_n2 into heap1
                                        {
                                            ...
                                            if(inRegion[y_grid][x_grid])
                                            {
                                                ...
                                                //if(netID == 252163 && edgeID == 51)
                                                //    printf("y: %d x: %d\n", y_grid, x_grid);
                                                pq1.push(Concurrent_pq_grid(&(d1[y_grid][x_grid]), 0, concurrentID));
...
void trace_back(CONCURRENT_NET_STORAGE* concurrent_net_storage, int* concurrentID2netID, int* concurrentID2edgeID, int concurrentNet_cnt) {
	...
	for(int concurrentID = 0; concurrentID < concurrentNet_cnt; concurrentID++ ) {
		...
		std::atomic<int>& return_ind1 = concurrent_net_storage[concurrentID].return_ind1;
		std::atomic<float>& return_dist = concurrent_net_storage[concurrentID].return_dist;
		...
		galois::LargeArray<galois::substrate::SimpleLock>& nodelock = concurrent_net_storage[concurrentID].nodelock;
...
void concurrent_maze(galois::InsertBag<pq_grid> pq1, CONCURRENT_NET_STORAGE* concurrent_net_storage) {
            ...
            [&] (const auto& top, auto& ctx)
            {
                ...
                if(d1_push == curr_d1 && d1_push < return_dist.load())
                {
                    ...
                    if(curX>regionX1)
                    {
                        ...
                        {
                            ctx.push(Concurrent_pq_grid(&(d1[curY][tmpX]), tmp, concurrentID));
                            nodelock[curY*xGrid+curX-1].lock();
                            if(d1[curY][tmpX]>tmp && tmp < return_dist) // left neighbor been put into heap1 but needs update
                            {
                                d1[curY][tmpX] = tmp;
                    ...
                    if(curX<regionX2)
                    {
                        ...
                        if(d1[curY][tmpX]>tmp && tmp < return_dist) {
                            ctx.push(Concurrent_pq_grid(&(d1[curY][tmpX]), tmp, concurrentID));
                            nodelock[curY*xGrid+curX+1].lock();
                            if(d1[curY][tmpX]>tmp && tmp < return_dist) // right neighbor been put into heap1 but needs update
                            {
                                d1[curY][tmpX] = tmp;
                                parentX3[curY][tmpX] = curX;
                                parentY3[curY][tmpX] = curY;
                    ...
                    if(curY>regionY1)
                    {
                        ...
                        if(d1[tmpY][curX]>tmp && tmp < return_dist) {
                            ctx.push(concurrent_pq_grid(&(d1[tmpY][curX]), tmp, concurrentID));

                            nodelock[(curY - 1)*xGrid+curX].lock();
                            if(d1[tmpY][curX]>tmp && tmp < return_dist) // bottom neighbor been put into heap1 but needs update
                            {
                                d1[tmpY][curX] = tmp;
                                parentX1[tmpY][curX] = curX;
                                parentY1[tmpY][curX] = curY;
                    ...
                    if(curY<regionY2)
                    {
                        ...
                        if(d1[tmpY][curX]>tmp && tmp < return_dist) {
                            ctx.push(Concurrent_pq_grid(&(d1[tmpY][curX]), tmp, concurrentID));
                            nodelock[(curY+1)*xGrid+curX].lock();
                            if(d1[tmpY][curX]>tmp && tmp < return_dist) // top neighbor been put into heap1 but needs update
                            {
...
void mazeRouteMSMD_finegrain_concurrent(int iter, int expand, float costHeight, int ripup_threshold, int mazeedge_Threshold, Bool Ordering, int cost_type, 
   int nConcurrentNets)
{
    //LOCK = 0;
    galois::StatTimer timer_finegrain("fine grain function", "fine grain concurrent maze");
    ...
    CONCURRENT_NET_STORAGE* concurrent_net_storage = new CONCURRENT_NET_STORAGE [nConcurrentNets];
   ...
   //galois::runtime::profileVtune( [&] (void) {
    ...
    galois::StatTimer timer_adjusttree("adjust tree", "fine grain concurrent maze");
    galois::StatTimer timer_updateusage("update usage", "fine grain concurrent maze");
    galois::StatTimer timer_checkroute2dtree("checkroute2dtree", "fine grain concurrent maze");
    ...
    galois::StatTimer timer_init_int("big int initialize", "fine grain concurrent maze");
    ...
    for(int nidRPC = 0; nidRPC < numValidNets; nidRPC++)
    {
        ...
        bool* inRegion_alloc = concurrent_net_storage[concurrentNet_cnt].inRegion_alloc;

        galois::LargeArray<galois::substrate::SimpleLock>& nodelock = concurrent_net_storage[concurrentNet_cnt].nodelock;
            ...
            if(treeedge->len > mazeedge_Threshold) // only route the non-degraded edges (len>0)
            {
                ...
                if(enter)
                {
                    ...
                    timer_setupheap.start();
                        setupHeap_nopq1clear(netID, edgeID, pq1, v2, regionX1, regionX2, regionY1, regionY2, d1, corrEdge, inRegion, concurrentNet_cnt);
                    ...
                    std::atomic<int>& return_ind1 = concurrent_net_storage[concurrentNet_cnt]->return_ind1;
                    return_ind1 = 0;
                    std::atomic<float>& return_dist = concurrent_net_storage[concurrentNet_cnt]->return_dist;
                    ...
                    trace_back(concurrent_net_storage, concurrentID2netID, concurrentID2edgeID, concurrentNet_cnt);
