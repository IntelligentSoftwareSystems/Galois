...
static cll::opt<Algo> algo(
    ...
    cll::values(clEnumValN(Algo::foreach, "Foreach", "Using galois for_each for conflict mitigation")
                ),
...
void runMultiPhaseLouvainAlgorithm(Graph& graph, uint64_t min_graph_size, double c_threshold, largeArray& clusters_orig) {
  ...
  while(true){
    ...
    if(iter < max_iter && (curr_mod - prev_mod) > threshold) {
      ...
      galois::gPrint("Number of unique sub cluster (Refine) : ", num_unique_subclusters, "\n");
