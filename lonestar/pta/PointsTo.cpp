...
const char* desc =
    "Performs inclusion-based points-to analysis over the input constraints.";
const char* url = NULL;

static cll::opt<std::string> input(cll::Positional, cll::desc("<constraints>"),
                                   cll::Required);
...
struct Node {
  ...
  Node(unsigned lid) : id(lid) { priority = 0; }
};
...
struct UpdateRequest {
  ...
  UpdateRequest(GNode& N, unsigned int W) : n(N), w(W) {}
};
...
typedef galois::worklists::OrderedByIntegerMetric<
    UpdateRequestIndexer, galois::worklists::dChunkedFIFO<1024>>
    OBIM;
...
public:
  ...
  unsigned propagate(GNode& src, GNode& dst,
    ...
    if (srcid != dstid) {
      unsigned srcreprid = getFinalRepresentative(srcid);
      unsigned dstreprid = getFinalRepresentative(dstid);
      if (srcreprid != dstreprid &&
          !result[srcreprid].isSubsetEq(result[dstreprid])) {
        // std::cout << "debug: unifying " << dstreprid << " by " << srcreprid
        // << std::endl;
  ...
  template <typename C>
  void processLoadStore(PointsToConstraints& constraints, C& ctx,
                        galois::MethodFlag flag = galois::MethodFlag::WRITE) {
    // add edges to the graph based on points-to information of the nodes
    // and then add the source of each edge to the updates.
    for (PointsToConstraints::iterator ii = constraints.begin();
         ii != constraints.end(); ++ii) {
      unsigned src, dst;
      // std::cout << "debug: Processing constraint: "; ii->print();
      ii->getSrcDst(src, dst);
      unsigned srcrepr = getFinalRepresentative(src);
      unsigned dstrepr = getFinalRepresentative(dst);
      if (ii->getType() == PtsToCons::Load) {
        ...
        for (std::vector<unsigned>::iterator pointee = ptstoOfSrc.begin();
             pointee != ptstoOfSrc.end(); ++pointee) {
          unsigned pointeerepr = getFinalRepresentative(*pointee);
          if (pointeerepr != dstrepr &&
            ...
            // std::cout << "debug: adding edge from " << *pointee << " to " <<
            // dst << std::endl;
            ctx.push(UpdateRequest(
                nn, graph.getData(nn, galois::MethodFlag::WRITE).priority));
          }
        }
      } else { // store.
        ...
        for (std::vector<unsigned>::iterator pointee = ptstoOfDst.begin();
             pointee != ptstoOfDst.end(); ++pointee) {
          unsigned pointeerepr = getFinalRepresentative(*pointee);
          if (srcrepr != pointeerepr &&
            ...
            // std::cout << "debug: adding edge from " << src << " to " <<
            // *pointee << std::endl;
            newedgeadded = true;
          }
        }
        if (newedgeadded) {
          ctx.push(UpdateRequest(
              nnsrcrepr,
              graph.getData(nnsrcrepr, galois::MethodFlag::WRITE).priority));
        }
      }
    }
  }

  void processLoadStoreSerial(
      PointsToConstraints& constraints, UpdatesVec& updates,
    ...
    for (PointsToConstraints::iterator ii = constraints.begin();
      ...
      if (ii->getType() == PtsToCons::Load) {
        ...
        for (std::vector<unsigned>::iterator pointee = ptstoOfSrc.begin();
          ...
          if (pointeerepr != dstrepr &&
              graph.findEdge(nodes[pointeerepr], nodes[dstrepr]) ==
                  graph.edge_begin(nodes[pointeerepr])) {
            GNode& nn = nodes[pointeerepr];
            graph.addEdge(nn, nndstrepr, flag);
            // std::cout << "debug: adding edge from " << *pointee << " to " <<
            // dst << std::endl;
            updates.push_back(UpdateRequest(
                nn,
                graph.getData(nn, galois::MethodFlag::UNPROTECTED).priority));
          }
        }
      } else { // store.
        std::vector<unsigned> ptstoOfDst;
        ...
        if (newedgeadded) {
          updates.push_back(UpdateRequest(
              nnsrcrepr,
              graph.getData(nnsrcrepr, galois::MethodFlag::UNPROTECTED)
                  .priority));
        }
      }
    }
  ...
  void processAddressOfCopy(PointsToConstraints& constraints,
    ...
    for (PointsToConstraints::iterator ii = constraints.begin();
      ...
      if (ii->getType() == PtsToCons::AddressOf) {
        ...
        // std::cout << "debug: adding edge from " << src << " to " << dst <<
        // std::endl;
        updates.push_back(UpdateRequest(
            nn, graph.getData(nn, galois::MethodFlag::UNPROTECTED).priority));
      }
    }
  }

  void runSerial(PointsToConstraints& addrcopyconstraints,
    ...
    processLoadStoreSerial(loadstoreconstraints, updates,
                           // required when there are zero copy constraints
                           // which keeps updates empty.
                           galois::MethodFlag::UNPROTECTED);

    // std::cout << "debug: no of addr+copy constraints = " <<
    // addrcopyconstraints.size() << ", no of load+store constraints = " <<
    // loadstoreconstraints.size() << std::endl;  std::cout << "debug: no of
    // nodes = " << nodes.size() << std::endl;

    while (!updates.empty()) {
      // std::cout << "debug: Iteration " << ++niteration << ", updates.size="
      // << updates.size() << "\n";
      GNode src = updates.back().n;
      updates.pop_back();

      // std::cout << "debug: processing updates element " << graph.getData(src,
      // galois::MethodFlag::UNPROTECTED).id << std::endl;
      for (Graph::edge_iterator
        ...
        unsigned newptsto =
            propagate(src, dst, galois::MethodFlag::UNPROTECTED);
        if (newptsto) {
      ...
      if (++nnodesprocessed > THRESHOLD_LOADSTORE || updates.empty()) {
        nnodesprocessed = 0;
        processLoadStoreSerial(
            loadstoreconstraints,
            updates); // add edges to graph, add their sources to updates.
  ...
  void runParallel(PointsToConstraints& addrcopyconstraints,
                   PointsToConstraints& loadstoreconstraints) {
    UpdatesVec updates;
    // unsigned niteration = 0;
    ...
    // galois::for_each<LocalQueues<dChunkedFIFO<1024> > >(updates.begin(),
    // updates.end(), Process());  galois::for_each<FIFO<> >(updates.begin(),
    // updates.end(), Process());  galois::for_each<ChunkedFIFO<1024>
    // >(updates.begin(), updates.end(), Process());
    // galois::for_each<dChunkedFIFO<1024> >(updates.begin(), updates.end(),
    // Process());  galois::for_each<ChunkedLIFO<32> >(updates.begin(),
    // updates.end(), Process());  galois::for_each<LocalQueues<ChunkedLIFO<32>
    // >
    // >(updates.begin(), updates.end(), Process());
    // galois::for_each<LocalQueues<FIFO<1024> > >(updates.begin(),
    // updates.end(), Process());  galois::for_each<LocalQueues<ChunkedFIFO<32>
    // >
    // >(updates.begin(), updates.end(), Process());
    // galois::for_each<LocalQueues<dChunkedLIFO<32>, FIFO<> >
    // >(updates.begin(), updates.end(), Process());
    // galois::for_each<LocalQueues<ChunkedLIFO<1024>, FIFO<> >
    // >(updates.begin(), updates.end(), Process());
    // galois::for_each<LocalQueues<ChunkedFIFO<1024>, FIFO<> >
    // >(updates.begin(), updates.end(), Process());
    ...
    galois::for_each(
        ...
        [&](const UpdateRequest& req, auto& ctx) {
          if (++nfired < THRESHOLD_LOADSTORE) {
            ...
            for (Graph::edge_iterator
              ...
              GNode dst = graph.getEdgeDst(ii);
              unsigned newptsto =
                  this->propagate(src, dst, galois::MethodFlag::WRITE);
              if (newptsto) {
        ...
        galois::loopname("Main"), galois::wl<OBIM>());
  }

  void checkReprPointsTo() {
    for (unsigned ii = 0; ii < result.size(); ++ii) {
      unsigned repr = getFinalRepresentative(ii);
      if (repr != ii && !result[ii].isSubsetEq(result[repr])) {
        std::cout << "ERROR: pointsto(" << ii
                  << ") is not less than its representative pointsto(" << repr
                  << ").\n";
      }
    }
  }
  ...
  unsigned countPointsToFacts() {
    ...
    for (PointsToInfo::iterator ii = result.begin(); ii != result.end(); ++ii) {
      unsigned repr = getFinalRepresentative(ii - result.begin());
  ...
  void process(const UpdatesVec&
                   updates) { // updates of nodes that are sources of new edges.
    ...
    for (const UpdateRequest& up : updates) {
      Node& nn        = graph.getData(up.n, galois::MethodFlag::UNPROTECTED);
  ...
  // it is okay not to detect all cycles as it is only an efficiency concern.
  bool cycleDetect(unsigned nodeid, unsigned& cyclenode) {
    ...
    for (Graph::edge_iterator
             ii = graph.edge_begin(nn, galois::MethodFlag::UNPROTECTED),
             ei = graph.edge_end(nn, galois::MethodFlag::UNPROTECTED);
         ii != ei; ++ii) {
      Node& nn =
  ...
  void cycleCollapse(unsigned repr) {
    ...
    for (std::vector<unsigned>::iterator ii = ancestors.begin();
         ii != ancestors.end(); ++ii) {
      if (*ii == repr) {
        // std::cout << "debug: collapsing cycle for " << repr << std::endl;
        // cycle exists between nodes ancestors[*ii..end].
        for (std::vector<unsigned>::iterator jj = ii; jj != ancestors.end();
             ++jj) {
          unsigned jjrepr =
              getFinalRepresentative(*jj); // jjrepr has no representative.
    ...
    // ancestors.clear();	// since collapse is called from top level
    // process(), the ancestors need to be cleared for the next element in the
    // updates.
  }
...
unsigned readConstraints(const char* file,
  ...
  for (unsigned ii = 0; ii < nconstraints; ++ii) {
    ...
    sscanf(cstr.c_str(), "%d,%d,%d,%d,%d", &consno, &src, &dst,
...
int main(int argc, char** argv) {
  ...
  numNodes =
      readConstraints(input.c_str(), addrcopyconstraints, loadstoreconstraints);
  ...
  for (PointsToInfo::iterator ii = result.begin(); ii != result.end();
       ++ii, ++nodeid) {
    ii->init(numNodes);

    GNode src = pta.graph.createNode(Node(nodeid));
    pta.graph.addNode(src);
    nodes[nodeid] = src;
  }
  ...
  if (numThreads) {
    ...
    pta.runParallel(addrcopyconstraints, loadstoreconstraints);
  } else {
    std::cout << "-------- Sequential version.\n";
    pta.runSerial(addrcopyconstraints, loadstoreconstraints);
  }
  T.stop();

  // std::cout << "No of points-to facts computed = " << countPointsToFacts() <<
  // std::endl;  checkReprPointsTo();  printPointsToInfo(result);
