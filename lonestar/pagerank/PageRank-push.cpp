...
const char* desc = "Computes page ranks a la Page and Brin. This is a push-style algorithm.";
const char* url  = 0;
...
static cll::opt<Algo> algo("algo", cll::desc("Choose an algorithm:"),
                           cll::values(clEnumVal(Async, "Async"), clEnumVal(Sync, "Sync"), clEnumValEnd),
                           cll::init(Sync));
...
static cll::opt<unsigned int> maxIterations("maxIterations", cll::desc("Maximum iterations, Sync version only"),
                                            cll::init(MAX_ITER));
...
typedef galois::graphs::LC_CSR_Graph<LNode, void>::with_numa_alloc<true>::type ::with_no_lockable<true>::type Graph;
...
void asyncPageRank(Graph& graph) {
  ...
  galois::for_each(galois::iterate(graph),
                   ...
                   galois::loopname("Async"), galois::no_conflicts(), galois::wl<WL>());
}
...
void syncPageRank(Graph& graph) {
  ...
  galois::do_all(galois::iterate(graph), [&](const GNode& src) { activeNodes.push(src); });
  ...
  for (; !activeNodes.empty() && iter < maxIterations; ++iter) {
    ...
    galois::do_all(galois::iterate(activeNodes),
                   ...
                   galois::steal(), galois::chunk_size<CHUNK_SIZE>(), galois::loopname("IterActive"));
    ...
    galois::do_all(galois::iterate(updates),
                   [&](const Update& up) {
                     ...
                     for (auto jj = up.beg; jj != up.end; ++jj) {
                       ...
                       if (std::fabs(old) <= tolerance && std::fabs(old + up.delta) >= tolerance) {
                         activeNodes.push(dst);
                       }
                     }
                   },
                   galois::steal(), galois::chunk_size<CHUNK_SIZE>(), galois::loopname("PushRes"));
...
int main(int argc, char** argv) {
  ...
  galois::preAlloc(numThreads +
                   (5 * graph.size() * sizeof(typename Graph::node_data_type)) / galois::runtime::pagePoolSize());
  ...
  galois::do_all(galois::iterate(graph), [&graph](GNode n) { graph.getData(n).init(); }, galois::no_stats(),
