...
static cll::opt<Algo> algo("algo", cll::desc("Choose an algorithm:"),
  cll::values(
    ...
    clEnumValN(Algo::blockEdge, "blockEdge", "Edge blocking (default)"),
...
void executeUntilConverged(const StepFunction& sf, Graph& g, Fn fn) {
  ...
  for (int round = 0; ; round += deltaRound) {
    ...
    galois::gPrint("R: ",  curRound,
                     " elapsed (ms): ", curElapsed,
                     " GFLOP/s: ", gflops);
    if (useExactError) {
      galois::gPrint(" RMSE (R ", curRound, "): ", std::sqrt(error/g.sizeEdges()), "\n");
    } else {
      galois::gPrint(" Approx. RMSE (R ", (curRound - 1), ".5): ", std::sqrt(std::abs(error/g.sizeEdges())), "\n");
    }

    galois::gPrint("Error Change : ", std::abs((last - error)/last), "\n");
    if (!isFinite(error))
      break;
    if (fixedRounds <= 0 && (round >= maxUpdates || std::abs((last - error) / last) < tolerance))
      break;
    if (sf.isBold()) {
    ...
    void operator()(LatentValue* steps, int maxUpdates,
      ...
      galois::for_each(
        ...
        [&] (GNode src, auto& ctx) {
          ...
          GNode dst = g.getEdgeDst(ii);
          LatentValue error = doGradientUpdate(g.getData(src, galois::MethodFlag::UNPROTECTED).latentVector,
...
class SGDEdgeMovie {
 ...
 public:
  ...
  std::string name() const { return "SGDEdgeMovie"; }
    ...
    void operator()(LatentValue* steps, int maxUpdates, galois::GAccumulator<double>* errorAccum) {
      ...
      galois::for_each(
          ...
          [&] (GNode src, auto& ctx) {
            ...
            //Do not need lock on the source node, since only one thread can work
            //on a given src(item).
            ...
            //Take lock on the destination as multiple source may update the same destination.
            auto& dstData = g.getData(g.getEdgeDst(ii));
            LatentValue error = doGradientUpdate(srcData.latentVector, dstData.latentVector, lambda,
                                                 g.getEdgeData(ii), stepSize);
 ...
 public:
  void operator()(Graph& g, const StepFunction& sf) {
    ...
    galois::runtime::reportStat_Single("SGDEdgeMovie", "EdgesVisited", edgesVisited.reduce());
  }
};
...
class BlockEdgeAlgo {
 ...
 public:
  ...
  std::string name() const { return "blockEdge"; }
 ...
 public:
  void operator()(Graph& g, const StepFunction& sf) {
    ...
    galois::runtime::reportStat_Single("blockEdgeAlgo", "EdgesVisited", 
                                       edgesVisited.reduce());
  }
};
...
void run() {
  ...
  // algorithm call
  galois::StatTimer timer("AlgoTimer");
  timer.start();
  algo(g, *sf);
  timer.stop();
