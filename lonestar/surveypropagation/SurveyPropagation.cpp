...
static cll::opt<int> M(cll::Positional, cll::desc("<num clauses>"),
                       cll::Required);
static cll::opt<int> N(cll::Positional, cll::desc("<num variables>"),
                       cll::Required);
...
class SurveyPropagation {
  ...
  void construct_random_formula(Graph& initGraph) {
    ...
    for (int m = 0; m < M; ++m) {
      ...
      while (touse.size() != (unsigned)K) {
        ...
        int newK =
            (int)(((double)rand() / ((double)RAND_MAX + 1)) * (double)(N));
        if (std::find(touse.begin(), touse.end(), newK) == touse.end()) {
          touse.push_back(newK);
          initGraph.getEdgeData(initGraph.addEdge(
              clauses[m], literalsN[newK], galois::MethodFlag::UNPROTECTED)) =
              SPEdge((bool)(rand() % 2));
        }
      }
    }
  ...
  void save_to_FileGraph(galois::graphs::FileGraphWriter& p, Graph& initGraph) {
    ...
    for (int m = 0; m < M; ++m) {
      for (auto e :
           initGraph.edges(clauses[m], galois::MethodFlag::UNPROTECTED)) {
        auto dst = initGraph
                       .getData(initGraph.getEdgeDst(e),
  ...
  void print_formula() {
    for (unsigned m = 0; m < clauses.size(); ++m) {
      ...
      for (auto ii : graph.edges(N, galois::MethodFlag::UNPROTECTED)) {
        ...
        SPNode& V = graph.getData(graph.getEdgeDst(ii),
                                  galois::MethodFlag::UNPROTECTED);
        std::cout << "v" << nameSPNode[V.id];
        if (solved[V.id])
          std::cout << "[" << (value[V.id] ? 1 : 0) << "]";
        std::cout << "{" << E.eta << "," << Bias[V.id] << ","
                  << (value[V.id] ? 1 : 0) << "}";
        std::cout << " ";
      }
      std::cout << " )\n";
  ...
  void SP_algorithm() {
    ...
    galois::for_each(
        ...
        [&, self = this](const GNode& p, auto& ctx) {
          ...
          // 	//	for (Graph::neighbor_iterator jii =
          // graph.neighbor_begin(*bii),
    ...
    galois::do_all(
        ...
        [&](GNode i) {
          ...
          for (auto aii : graph.edges(i, galois::MethodFlag::UNPROTECTED)) {
            ...
            SPEdge& aie =
                graph.getEdgeData(aii, galois::MethodFlag::UNPROTECTED);
  ...
  bool survey_inspired_decimation() {
    ...
    galois::preAlloc(numThreads +
                     120 * (graph.size()) / galois::runtime::pagePoolSize());
...
public:
  void run() {
    ...
    galois::StatTimer T;
    T.start();
    galois::runtime::profileVtune([&]() { survey_inspired_decimation(); },
                                  "Main Loop");
    T.stop();
