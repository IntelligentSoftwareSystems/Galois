...
typedef galois::graphs::LC_Linear_Graph<Node, int32_t>::with_numa_alloc<
    true>::type Graph;
...
void checkAugmentingPath(Config& app) {
  // Use id field as visited flag
  for (Graph::iterator ii = app.graph.begin(), ee = app.graph.end(); ii != ee;
       ++ii) {
    GNode src                 = *ii;
  ...
  while (!queue.empty()) {
    ...
    for (auto ii : app.graph.edges(src)) {
      ...
      if (app.graph.getData(dst).id == 0 && app.graph.getEdgeData(ii) > 0) {
...
void checkHeights(Config& app) {
  for (Graph::iterator ii = app.graph.begin(), ei = app.graph.end(); ii != ei;
       ++ii) {
    GNode src = *ii;
...
Graph::edge_iterator findEdgeLog2(Graph& g, GNode dst, Graph::edge_iterator i,
  ...
  struct EdgeDstIter
    ...
    using Base =
        boost::iterator_facade<EdgeDstIter, GNode,
                               boost::random_access_traversal_tag, GNode>;
...
void checkConservation(Config& orig, Config& app) {
  ...
  id = 0;
  for (Graph::iterator ii = orig.graph.begin(), ei = orig.graph.end(); ii != ei;
       ++ii, ++id) {
  ...
  for (Graph::iterator ii = app.graph.begin(), ei = app.graph.end(); ii != ei;
    ...
    for (auto jj : app.graph.edges(src)) {
      ...
      int64_t ocap =
          orig.graph.getEdgeData(findEdge(orig.graph, map[srcId], map[dstId]));
      int64_t delta = 0;
      if (ocap > 0)
    ...
    if (node.excess != sum) {
      std::cerr << "Not pseudoflow: " << node.excess << " != " << sum << " at "
                << node << "\n";
      abort();
    }
  }
...
void verify(Config& orig, Config& app) {
  // FIXME: doesn't fully check result
  checkHeights(app);
  checkConservation(orig, app);
  checkAugmentingPath(app);
}

void reduceCapacity(const Graph::edge_iterator& ii, const GNode& src,
                    const GNode& dst, int64_t amount, Config& app) {
  Graph::edge_data_type& cap1 = app.graph.getEdgeData(ii);
  Graph::edge_data_type& cap2 =
      app.graph.getEdgeData(findEdge(app.graph, dst, src));
  cap1 -= amount;
  cap2 += amount;
}
...
struct FindWork {
  WLTy& wl;
  Config& app;
  FindWork(WLTy& w, Config& c) : wl(w), app(c) {}

  void operator()(const GNode& src) const {
    Node& node = app.graph.getData(src, galois::MethodFlag::UNPROTECTED);
    if (src == app.sink || src == app.source ||
        node.height >= (int)app.graph.size())
      return;
    if (node.excess > 0)
      wl.push_back(src);
  }
...
template <typename IncomingWL, typename Config>
void globalRelabel(IncomingWL& incoming, Config& app) {
  ...
  T1.start();
  galois::do_all_local(app.graph, ResetHeights(app),
  ...
  switch (detAlgo) {
  case nondet:
    galois::for_each(app.sink, UpdateHeights<nondet>(app),
                     galois::loopname("UpdateHeights"),
                     galois::wl<galois::worklists::BulkSynchronous<>>());
    //      galois::for_each(app.sink, UpdateHeights<nondet>(),
    //      galois::loopname("UpdateHeights"));
    break;
  case detBase:
    galois::for_each(app.sink, UpdateHeights<detBase>(app), galois::wl<DWL>(),
                     galois::loopname("UpdateHeights"));
    break;
  case detDisjoint:
    galois::for_each(app.sink, UpdateHeights<detDisjoint>(app),
                     galois::wl<DWL>(), galois::loopname("UpdateHeights"));
    break;
  default:
  ...
  T2.start();
  galois::do_all_local(app.graph, FindWork<IncomingWL>(incoming, app),
                       galois::loopname("FindWork"));
  T2.stop();
}

void acquire(const GNode& src, Config& app) {
...
void relabel(const GNode& src, Config& app) {
  ...
  for (auto ii : app.graph.edges(src, galois::MethodFlag::UNPROTECTED)) {
    ...
    if (cap > 0) {
      const Node& dnode =
          app.graph.getData(dst, galois::MethodFlag::UNPROTECTED);
...
bool discharge(const GNode& src, Config& app, galois::UserContext<GNode>& ctx) {
  ...
  while (true) {
    ...
    Graph::edge_iterator ii = app.graph.edge_begin(src, flag),
    ...
    for (; ii != ee; ++ii, ++current) {
      ...
      reduceCapacity(ii, src, dst, amount, app);
...
struct Process {
  ...
  struct DeterministicId {
    Process& owner;
    DeterministicId(Process& ow) : owner(ow) {}
    uintptr_t operator()(const GNode& item) const {
      return owner.app.graph.getData(item, galois::MethodFlag::UNPROTECTED).id;
    }
  };
  ...
  struct ParallelBreak {
    ...
    Process& owner;
    ParallelBreak(Process& ow) : owner(ow) {}
    bool operator()() {
      if (owner.app.global_relabel_interval > 0 &&
          self.counter.accum.reduce() >= owner.app.global_relabel_interval) {
        owner.app.should_global_relabel = true;
        return true;
      }
      return false;
    }
  ...
  typedef std::tuple<galois::parallel_break, galois::per_iter_alloc,
...
void writePfpGraph(const std::string& inputFile,
...
void initializePreflow(C& initial, Config& app) {
  for (auto ii : app.graph.edges(app.source)) {
    GNode dst   = app.graph.getEdgeDst(ii);
    int64_t cap = app.graph.getEdgeData(ii);
    reduceCapacity(ii, app.source, dst, cap, app);
...
void checkSorting(Config& app) {
  for (auto n : app.graph) {
    ...
    for (auto e : app.graph.edges(n, galois::MethodFlag::UNPROTECTED)) {
      ...
      if (prevDst) {
        Node& prevNode =
            app.graph.getData(*prevDst, galois::MethodFlag::UNPROTECTED);
        Node& currNode =
            app.graph.getData(dst, galois::MethodFlag::UNPROTECTED);
...
void run(Config& app) {
  ...
  while (initial.begin() != initial.end()) {
    ...
    case nondet:
      if (useHLOrder) {
        galois::for_each_local(initial, Process<nondet>(counter, app),
    ...
    case detBase: {
      ...
      galois::for_each_local(
          initial, fn, galois::loopname("Discharge"), galois::wl<DWL>(),
#if defined(__INTEL_COMPILER) && __INTEL_COMPILER <= 1400
          galois::det_parallel_break<Process<detBase>::ParallelBreak>(
              fn.getParallelBreak())
#else
          galois::make_trait_with_args<galois::det_parallel_break>(
              fn.getParallelBreak())
#endif
      );
    } break;
    case detDisjoint: {
      ...
      galois::for_each_local(
          initial, fn, galois::loopname("Discharge"), galois::wl<DWL>(),
#if defined(__INTEL_COMPILER) && __INTEL_COMPILER <= 1400
          galois::det_parallel_break<Process<detDisjoint>::ParallelBreak>(
              fn.getParallelBreak())
#else
          galois::make_trait_with_args<galois::det_parallel_break>(
              fn.getParallelBreak())
