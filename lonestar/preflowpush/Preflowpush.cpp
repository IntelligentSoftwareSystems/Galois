...
struct Config {
  ...
  bool discharge(const GNode& src, galois::UserContext<GNode>& ctx) {
    ...
    while (true) {
      ...
      Graph::edge_iterator ii = graph.edge_begin(src, flag),
  ...
  struct ParallelBreak {
    ...
    bool operator()() {
      if (app.global_relabel_interval > 0 &&
          self.counter.accum.reduce() >= app.global_relabel_interval) {
