...
static cll::opt<uint32_t> min_graph_size("min_graph_size",
cll::desc("Minimum coarsened graph size"),
cll::init(100));
  ...
  for(GNode n : cluster_nodes_to_move){
    ...
    if(subcomm_info[n_data.curr_subcomm_ass].size == 1){
      ...
      if((int64_t) new_subcomm_ass != -1 && new_subcomm_ass != graph.getData(n).curr_subcomm_ass){ 
        ...
        */
        galois::atomicAdd(subcomm_info[new_subcomm_ass].node_wt, n_data.node_wt);
        galois::atomicAdd(subcomm_info[new_subcomm_ass].size, (uint64_t)1);
        galois::atomicAdd(subcomm_info[new_subcomm_ass].degree_wt, n_data.degree_wt);
...
void refinePartition(GraphTy &graph, double constant_for_second_term){
  ...
  for(GNode n: graph){
    ...
    galois::atomicAdd(comm_info[n_data.curr_comm_ass].degree_wt, n_data.degree_wt);
  }
