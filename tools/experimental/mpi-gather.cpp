...
class FileGraph {
  ...
  public:
    ...
    void read(size_t begin, size_t size, int fd) {
      ...
      len = size + align;
      m = mmap(0, len, PROT_READ, MAP_PRIVATE | MAP_POPULATE, fd, begin - align);
      if (m == MAP_FAILED)
...
public:
  ...
  void read(int rank, int total) {
    ...
    size_t outIdxEnd = std::min((rank+1) * bsize, static_cast<size_t>(numNodes));
    size_t len = outIdxEnd - outIdxBegin;
    outIdx.read(4*sizeof(uint64_t) + outIdxBegin * sizeof(uint64_t), len * sizeof(uint64_t), fd);
  ...
  void putMine(int rank, int total, MPI_Win window) {
    ...
    uint64_t* source1 = rank == 0 ? outIdx.begin<uint64_t>(0) : outIdx.begin<uint64_t>(1);
    size_t len1 = std::distance(source1, outIdx.end<uint64_t>()) * sizeof(uint64_t);
    ...
    size_t len2 = std::distance(source2, outs.end<uint32_t>()) * sizeof(uint32_t);
