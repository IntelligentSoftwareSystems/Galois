...
struct multiple_sum {
  ...
  __device__ __host__
  multiple_sum(const T e) {
    for(int i = 0; i < items; i++)
      el[i] = e;
  }

  __device__ __host__
  multiple_sum& operator=(const T rhs) {
    for(int i = 0; i < items; i++)
      el[i] = rhs;
  ...
  __device__ __host__
  multiple_sum& operator+=(const multiple_sum& rhs) {
    for(int i = 0; i < items; i++)
      el[i] += rhs.el[i];
  ...
  __device__ __host__
  friend multiple_sum operator+(multiple_sum lhs, const multiple_sum& rhs) {
    return lhs += rhs;
  }  
};
...
template<const int ITSIZE> struct fg_np {
...
template <typename ts_type, typename index_type,  typename TTB, typename TWP, typename TFG>
union np_shared {
  // for scans
...
struct NPInspector1 {
  ...
  __device__ __host__ bool work() const {
    return total > 0;
  }
