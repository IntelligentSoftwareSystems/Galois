...
public:
   ...
   virtual bool VisitVarDecl(VarDecl * d){
      llvm::outs() << " Declaration[VD] " << d->getType().getAsString() << ", " << d->getNameAsString() << " \n";
      return true;
   }
   virtual bool VisitParmVarDecl(ParmVarDecl* d){
      llvm::outs() << " Declaration[PVD] " << d->getType().getAsString() << ", " << d->getNameAsString() << " \n";
      return true;
   }
   virtual bool VisitFieldDecl(FieldDecl* d){
      llvm::outs() << " Declaration[FD] " << d->getType().getAsString() << ", " << d->getNameAsString() << " \n";
      return true;
   }
};/////////////////////////////////////////////////////////////////////////
...
public:
   explicit OpenCLDeviceVisitor(ASTContext & context, Rewriter & R, std::map<const Type *, string> & t) :
...
//         return qt.getAsString().substr(qt.getAsString().rfind("<")+1,qt.getAsString().rfind(">")-qt.getAsString().rfind("<")-1);
//      }
//      if (qt.getAsString().find ("boost::iterators::counting_iterator") || qt.getAsString().find("GNode")!=string::npos){
//         return "int";
//      }
//      if(qt.getAsString().find("NodeData")!=string::npos){
//         return "__global NodeData *";
...
//         }
//
//         llvm::outs() << " Comparing " << qt.getAsString() << " AND " << QualType::getAsString(t.first,Qualifiers()) << " MAP " << t.second <<"\n";
//      }
//
...
//      }
//      else{
//         return qt.getAsString( ) ; //QualType::getAsString(t, Qualifiers());
//      }*/
//      return qt.getAsString();
//   }
   virtual bool TraverseVarDecl(VarDecl * var) {
      MARKER_CODE(cl_file << "[VARDEL]";)
            ...
            cl_file << OpenCLConversionDB::type_convert(var->getType())<< " "<< var->getNameAsString() << " ";
      if (var->hasInit()) {
         cl_file << " = ";
   ...
   virtual bool TraverseCallExpr(CallExpr * call) {
      ...
      if(const char  * c = OpenCLConversionDB::get_cl_implementation(call->getDirectCallee())){
         cl_file << c ;
         return true;
      }
...
//      llvm::outs()<<call->getDirectCallee()->getNameAsString() << ", "<<  call->getDirectCallee()->getNumParams( );
//      llvm::outs()<<"\n";
//      cl_file << call->getDirectCallee()->getNameAsString() << " (";
//      cl_file << call->getDirectCallee()->getNameAsString() << " (";
      cl_file << OpenCLConversionDB::get_cl_implementation(call->getDirectCallee()) <<"(";
      bool firstArg = true;
...
class OpenCLDeviceRewriter: public RecursiveASTVisitor<OpenCLDeviceRewriter> {
   ...
   std::map<const Type * , string> &type_sub;
public:
   explicit OpenCLDeviceRewriter(ASTContext & context, Rewriter & R, std::map<const Type *, string> & t) :
...
public:
   ...
   virtual bool VisitCXXMethodDecl ( CXXMethodDecl * md){
      string retType = QualType::getAsString (md->getReturnType()->getUnqualifiedDesugaredType(), Qualifiers());
...
//      llvm::outs() << " Method found " << md->getType().getAsString()  << " , " <<md->getNameAsString() << "\n";
      return true;
   }
   void summary(){
      llvm::outs() << " Summary :: \n";
      ...
      llvm::outs()<<"===============\n";
      for(auto i : type_sub){
         llvm::outs() << QualType::getAsString(i.first, Qualifiers()) << "(" << QualType::getAsString(i.first->getCanonicalTypeUnqualified()->getTypePtr(),Qualifiers())<< ")" << " mappedTo " << i.second << "\n";
...
//      gtc.summary();
      OpenCLDeviceRewriter devVisitor(decl->getASTContext(), rewriter, gtc.type_sub);
      devVisitor.GenerateCLCode(decl);
//      llvm::outs() << "REWRITTEN TEXT :: " << decl->getParent()->getNameAsString() << "\n";
...
//      llvm::outs() << "END REWRITTEN TEXT :: " << decl->getParent()->getNameAsString() << "\n";
   }
...
public:
   ...
   virtual void run(const MatchFinder::MatchResult &Results) {
      CXXRecordDecl* decl = const_cast<CXXRecordDecl*>(Results.Nodes.getNodeAs<CXXRecordDecl>("graphClass"));
      graphClass=decl;
      llvm::outs() << " GraphClass Found :: " << decl->getNameAsString() << ", " << decl->getDefinition()->getNameAsString() <<"\n";
//      llvm::outs() << " About to _scan for method declaration in " <<  decl->getNameAsString() <<"\n";
//      GraphTypeCollector gtc;
...
public:
   NodeDataGen(Rewriter &rewriter, ASTContext & c) :
