...
static unsigned getNumaNode(cpuinfo& c) {
  ...
  if (!warnOnce) {
    warnOnce = true;
#ifdef GALOIS_USE_NUMA
    static bool numaAvail = numa_available() >= 0;
    if (!numaAvail)
      Galois::Runtime::gWarn("Numa support configured but not present at runtime.  Assuming numa topology matches socket topology.");
#else
    Galois::Runtime::gWarn("Numa Support Not configured (install libnuma-dev).  Assuming numa topology matches socket topology.");
#endif
  }
  ...
  if (i < 0) {
    assert(0 && "failed finding numa node");
    Galois::Runtime::gDie("failed finding numa node for ", c.proc);
  }
  return i;
#else
  return c.physid;
...
static std::vector<cpuinfo> parseCPUInfo() {
  ...
  if (!procInfo) {
    assert(0 && "Failed opening /proc/cpuinf");
    Galois::Runtime::gDie("failed opening /proc/cpuinfo");
  }
...
std::pair<Galois::Runtime::machineTopoInfo, std::vector<Galois::Runtime::threadTopoInfo> > Galois::Runtime::getHWTopo() {
...
bool Galois::Runtime::bindThreadSelf(unsigned osContext) {
#ifndef __CYGWIN__
  cpu_set_t mask;
  ...
  if (sched_setaffinity(0, sizeof(mask), &mask ) == -1) {
    Galois::Runtime::gWarn("Could not set CPU affinity to ", osContext, "(", strerror(errno), ")");
    return false;
  }
  return true;
#else
  Galois::Runtime::gWarn("No cpu affinity on Cygwin.  Performance will be bad.");
  return false;
#endif
}
