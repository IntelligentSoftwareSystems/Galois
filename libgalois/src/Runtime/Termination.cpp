 ...
 * @section Copyright
 *
 * Copyright (C) 201g, The University of Texas at Austin. All rights
 * reserved.
 *
 * @section Description
 *
...
class LocalTerminationDetection : public TerminationDetection {
  ...
  void propToken(bool isBlack) {
    ...
    TokenHolder& th = *data.get((id + 1) % activeThreads);
...
class TreeTerminationDetection : public TerminationDetection {
  ...
  void processToken() {
	...
	data.get(th.parent)->up_token[th.parent_offset] = black;
      }
    }
...
std::unique_ptr<Galois::Runtime::TerminationDetection> Galois::Runtime::createTermination() {
  //cache line storage doesn't like new
