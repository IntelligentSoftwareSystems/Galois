...
void Galois::Runtime::detail::largeFreer::operator()(void* ptr) const {
  largeFree(ptr, bytes);
}
...
LAptr Galois::Runtime::largeMallocInterleaved(size_t bytes, unsigned numThreads) {
...
LAptr Galois::Runtime::largeMallocLocal(size_t bytes) {
  //round up to hugePageSize
  bytes = roundup(bytes, allocSize());
...
LAptr Galois::Runtime::largeMallocFloating(size_t bytes) {
  //round up to hugePageSize
  bytes = roundup(bytes, allocSize());
  //Get a non-prefaulted allocation
...
LAptr Galois::Runtime::largeMallocBlocked(size_t bytes, unsigned numThreads) {
  //round up to hugePageSize
  bytes = roundup(bytes, allocSize());
  //Get a non-prefaulted allocation
  void* data = allocPages(bytes/allocSize(), false);
