...
#include "Galois/Runtime/gslist.h"
#include "Galois/Threads.h"
#include "Galois/Runtime/TwoLevelIteratorA.h"
#include "Galois/Runtime/UnionFind.h"
...
public:
  typedef DItem<OptionsTy> Item;
  typedef concurrent_gslist<DeterministicContextBase*,8> ContextList;
  Item item;
  ContextList edges;
  ContextList succs;
...
class DAGManagerBase<OptionsTy,true> {
  ...
  Runtime::PerThreadStorage<ThreadLocalData> data;
  WL1 taskList;
  WL2 taskList2;
  WL3 sourceList;
...
public:
  DAGManagerBase(): term(Runtime::createTermination(getActiveThreads())), barrier(getActiveThreads()) { }
...
class IntentToReadManagerBase<OptionsTy, true> {
  typedef DeterministicContext<OptionsTy> Context;
  typedef gdeque<Context*> WL;
  Runtime::PerThreadStorage<WL> pending;
  Runtime::Barrier barrier;

public:
  IntentToReadManagerBase(): barrier(getActiveThreads()) { }
...
public:
  NewWorkManager(const OptionsTy& o): 
    IdManager<OptionsTy>(o), alloc(&heap), mergeBuf(alloc), distributeBuf(alloc), barrier(getActiveThreads()) 
  {
    numActive = getActiveThreads();
  }
  ...
  void addInitialWork(WindowManager<OptionsTy>& wm, InputIteratorTy b, InputIteratorTy e, WL* wl) {
    ...
    if (OptionsTy::hasId) {
      ...
      if (OptionsTy::hasFixedNeighborhood) {
        copyMine(b, e, dist, wl, window, Runtime::ThreadPool::getTID());
      } else {
        copyMine(
            ...
            mergeBuf.size(), wl, window, Runtime::ThreadPool::getTID());
      }
    } else {
      size_t window = wm.initialWindow(dist, OptionsTy::MinDelta);
      copyMineAfterRedistribute(b, e, dist, wl, window, Runtime::ThreadPool::getTID());
    }
  }
  ...
  void distributeNewWork(WindowManager<OptionsTy>& wm, WL* wl) {
    parallelSort(wm, wl, Runtime::ThreadPool::getTID());
  }
};
  ...
  public IntentToReadManager<OptionsTy>
{
  ...
  PendingWork pending;
  const char* loopname;
  Runtime::CacheLineStorage<volatile long> innerDone;
  Runtime::CacheLineStorage<volatile long> outerDone;
  Runtime::CacheLineStorage<volatile long> hasNewWork;
...
public:
  Executor(const OptionsTy& o):
    ...
    options(o),
    barrier(Galois::getActiveThreads()),
...
bool Executor<OptionsTy>::executeTask(ThreadLocalData& tld, Context* ctx) 
{
  ...
  switch (result) {
    ...
    default: gDie("Unknown conflict flag"); break;
  }
