...
public:
  ...
  const_reference operator[](size_type x) const { assert(x < m_size+1);  return m_data[x]; }
  reference operator[](size_type x) { assert(x < m_size+1); return m_data[x]; }
  ...
  template<typename... Args>
  void create(size_type n, unsigned numThreads, Args&&... args) {
    allocateInterleaved(n, numThreads);
