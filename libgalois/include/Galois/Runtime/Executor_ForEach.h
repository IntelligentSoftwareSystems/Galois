...
public:
  ...
  ForEachExecutor(T2, const FunctionTy& f, unsigned activeThreads, const ArgsTy& args, WArgsTy... wargs):
    term(createTermination(activeThreads)),
    wl(wargs...),
    origFunction(f),
  ...
  ForEachExecutor(T1, const FunctionTy& f, unsigned activeThreads,
                  const ArgsTy& args, const WArgsTy& wlargs, int_seq<Is...>):
    ForEachExecutor(T2{}, f, activeThreads, args, std::get<Is>(wlargs)...) {}

  template<typename WArgsTy>
  ForEachExecutor(T1, const FunctionTy& f, unsigned activeThreads,
                  const ArgsTy& args, const WArgsTy& wlargs, int_seq<>):
    ForEachExecutor(T2{}, f, activeThreads, args) {}

public:
  ForEachExecutor(const FunctionTy& f, unsigned activeThreads, const ArgsTy& args):
    ForEachExecutor(T1{}, f, activeThreads, args, 
...
void for_each_impl(const RangeTy& range, const FunctionTy& fn, unsigned activeThreads, const ArgsTy& args) {
...
void for_each_gen(const RangeTy& r, const FunctionTy& fn, unsigned activeThreads, const TupleTy& tpl) {
  ...
  if (forceNew) {
    ...
    Runtime::for_each_impl(r, fn, activeThreads,
        std::tuple_cat(xtpl, 
