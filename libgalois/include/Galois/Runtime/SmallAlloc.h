 ...
 */
#ifndef GALOIS_RUNTIME_SMALLALLOC_H
#define GALOIS_RUNTIME_SMALLALLOC_H
...
public:
  ...
  inline void* allocate(size_t size, Args&&... args) {
    return heaps->allocate(size, std::forward<Args>(args)...);
  }

  inline void deallocate(void* ptr) {
    heaps->deallocate(ptr);
  }
...
class SerialNumaHeap {
  enum { offset = (sizeof(LAptr) + (sizeof(double) - 1)) & ~(sizeof(double) - 1) };

public:
  enum { AllocSize = 0 };

  void* allocate(size_t size, unsigned activeThreads) {
