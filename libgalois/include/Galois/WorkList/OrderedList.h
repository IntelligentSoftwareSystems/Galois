...
class OrderedList : private boost::noncopyable, private Runtime::PaddedLock<concurrent> {
  typedef Runtime::flat_map<T, std::deque<T>, Compare> Map;

  Map map;

  using Runtime::PaddedLock<concurrent>::lock;
  using Runtime::PaddedLock<concurrent>::try_lock;
  using Runtime::PaddedLock<concurrent>::unlock;
...
public:
  ...
  boost::optional<value_type> pop() {
    ...
    boost::optional<value_type> v(list.front());
    list.pop_front();
    if (list.empty())
      map.erase(ii);
    unlock();
