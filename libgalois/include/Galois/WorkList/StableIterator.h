...
#include "Galois/Runtime/split.h"
...
private:
  struct shared_state {
    Iterator stealBegin;
    Iterator stealEnd;
    Runtime::SimpleLock stealLock;
    bool stealAvail;
  };

  struct state {
    Runtime::CacheLineStorage<shared_state> stealState;
    Iterator localBegin;
    Iterator localEnd;
	...
	s.stealLock.lock();
	s.stealEnd = localEnd;
	s.stealBegin = localEnd = Runtime::split_range(localBegin, localEnd);
  ...
  bool doSteal(state& dst, state& src, bool wait) {
	...
	s.stealBegin = dst.localEnd = Runtime::split_range(s.stealBegin, s.stealEnd);
  ...
  boost::optional<value_type> pop_steal(state& data) {
    ...
    if (doSteal(data, *TLDS.getRemote(data.nextVictim), false)) {
      //share the wealth
      if (data.nextVictim != Runtime::ThreadPool::getTID())
	data.populateSteal();
      return *data.localBegin++;
    }
    ++data.nextVictim;
    ++data.numStealFailures;
    data.nextVictim %= Galois::getActiveThreads();
...
public:
  ...
  void push_initial(const RangeTy& r) {
    ...
    data.nextVictim = Runtime::ThreadPool::getTID();
  ...
  boost::optional<value_type> pop() {
    ...
    if (Steal && 2 * data.numStealFailures > Galois::getActiveThreads())
