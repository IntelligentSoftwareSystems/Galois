...
protected:
  ...
  boost::optional<T> popStored(ThreadData&, Index) { return {}; }
};
...
protected:
  ...
  OrderedByIntegerMetricData(): barrier(Galois::getActiveThreads()) { }
  ...
  boost::optional<T> popStored(ThreadData& p, Index idx) {
...
struct OrderedByIntegerMetricComparator {
  ...
  struct with_local_map {
    typedef Runtime::flat_map<Index, C, std::less<Index> > type;
  };
...
struct OrderedByIntegerMetricComparator<Index, true> {
  ...
  template<typename C>
  struct with_local_map {
    typedef Runtime::flat_map<Index, C, std::greater<Index> > type;
  };
...
private:
  ...
  Runtime::PaddedLock<Concurrent> masterLock;
  MasterLog masterLog;
  ...
  boost::optional<T> slowPop(ThreadData& p) {
    bool localLeader = Runtime::ThreadPool::isLeader();
    ...
    if (BSP && !UseMonotonic) {
      ...
      if (localLeader) {
        ...
        Index o = data.getRemote(Runtime::ThreadPool::getLeader())->scanStart;
        if (this->compare(o, msS))
          msS = o;
      }
    }
