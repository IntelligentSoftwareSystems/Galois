...
class AltChunkedQueue {
  Runtime::PtrLock<ChunkHeader> head;
  ChunkHeader* tail;
...
class StealingQueue : private boost::noncopyable {
  ...
  ChunkHeader* doSteal() {
    ...
    unsigned pkg = Runtime::ThreadPool::getPackage();
    ...
    if (Runtime::ThreadPool::isLeader()) {
...
private:
  class Chunk : public ChunkHeader, public Runtime::FixedSizeRing<T, ChunkSize> {};
