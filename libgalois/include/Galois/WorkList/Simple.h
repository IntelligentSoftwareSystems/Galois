...
public:
  Wrapper(unsigned n, std::tuple<> t) {}
  Wrapper() {}
  ...
  void push(const value_type& val) {
    std::lock_guard<Runtime::SimpleLock > lg(lock);
    wl.push_back(val);
  }
...
template<typename T = int>
using GFIFO = Wrapper<T, Runtime::gdeque<T>, false >;

template<typename T = int>
using LIFO = Wrapper<T, std::deque<T>, true >;

template<typename T = int>
using GLIFO = Wrapper<T, Runtime::gdeque<T>, true >;
