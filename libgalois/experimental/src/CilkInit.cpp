...
struct BusyBarrier {
  ...
  BusyBarrier(unsigned val) : entered(val) { check(); }
...
static void initOne(BusyBarrier& busybarrier, unsigned tid) {
  ...
  if (id != 0 ||
      !galois::runtime::LL::EnvCheck("GALOIS_DO_NOT_BIND_MAIN_THREAD")) {
...
#else
void galois::CilkInit(void) { GALOIS_DIE("Cilk not found\n"); }
#endif // HAVE_CILK
