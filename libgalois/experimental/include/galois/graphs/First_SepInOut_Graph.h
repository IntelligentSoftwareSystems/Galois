...
public:
  ...
  struct with_no_lockable { typedef First_SepInOut_Graph<NodeTy,EdgeTy,Directional,InOut,_has_no_lockable,SortedNeighbors,FileEdgeTy> type; };

  template<typename _node_data>
  struct with_node_data { typedef First_SepInOut_Graph<_node_data,EdgeTy,Directional,InOut,HasNoLockable,SortedNeighbors,FileEdgeTy> type; };

  template<typename _edge_data>
  struct with_edge_data { typedef First_SepInOut_Graph<NodeTy,_edge_data,Directional,InOut,HasNoLockable,SortedNeighbors,FileEdgeTy> type; };

  template<typename _file_edge_data>
  struct with_file_edge_data { typedef First_SepInOut_Graph<NodeTy,EdgeTy,Directional,InOut,HasNoLockable,SortedNeighbors,_file_edge_data> type; };

  template<bool _directional>
  struct with_directional { typedef First_SepInOut_Graph<NodeTy,EdgeTy,_directional,InOut,HasNoLockable,SortedNeighbors,FileEdgeTy> type; };

  template<bool _sorted_neighbors>
  struct with_sorted_neighbors { typedef First_SepInOut_Graph<NodeTy,EdgeTy,Directional,InOut,HasNoLockable,_sorted_neighbors,FileEdgeTy> type; };
    ...
    public gNodeTypes
  {
    ...
    iterator createEdge(gNode* N, EdgeTy* v, bool inEdge, Args&&... args) {
      ...
      if (SortedNeighbors) {
        ...
        ii = edgelist.end();
      return edgelist.insert(ii, EdgeInfo(N, v, inEdge, std::forward<Args>(args)...));
    }
    ...
    iterator createEdgeWithReuse(gNode* N, EdgeTy* v, bool inEdge, Args&&... args) {
      ...
      if (SortedNeighbors) {
        ...
        ii = std::lower_bound(edgelist.begin(), edgelist.end(), N,
                              first_lt<gNode*>());
        ei = std::upper_bound(ii, edgelist.end(), N,
                              first_lt<gNode*>());
      }
      else {
	...
	*ii = EdgeInfo(N, v, inEdge, std::forward<Args>(args)...);
	return ii;
      ...
      return edgelist.insert(ei, EdgeInfo(N, v, inEdge, std::forward<Args>(args)...));
    }
...
public:
  ...
  struct ReadGraphAuxData {
    ...
    galois::PerThreadMap<FileGraph::GraphNode, galois::gstl::Vector<std::pair<GraphNode, EdgeTy*> > > inNghs;
  };
#else
  struct AuxNode {
...
private:
   ...
   */
  template<typename... Args>
  EdgeTy* createOutEdge(GraphNode src, GraphNode dst, galois::MethodFlag mflag, Args&&... args) {
    assert(src);
    assert(dst);
   ...
   */
  template<typename... Args>
  void createInEdge(GraphNode src, GraphNode dst, EdgeTy* e, galois::MethodFlag mflag, Args&&... args) {
    assert(src);
    assert(dst);
  ...
  EdgeTy* constructOutEdgeValue(FileGraph& graph, typename FileGraph::edge_iterator nn,
    ...
    if (ED::has_value) {
      return createOutEdge(src, dst, galois::MethodFlag::UNPROTECTED, graph.getEdgeData<FEDV>(nn));
    } else {
  ...
  void constructInEdgeValue(FileGraph& graph, EdgeTy* e, GraphNode src, GraphNode dst) {
...
public:
  ...
  void removeInEdge(GraphNode dst, in_edge_iterator src, galois::MethodFlag mflag = MethodFlag::WRITE, 
                              ...
                              typename std::enable_if<_DirectedInOut>::type* = 0)
  {
    ...
    assert(dst);
    dst->acquire(mflag);
    typename gNodeTypes::iterator ii = dst->find(src, true), ei = dst->in_edges_end();
  ...
  void sortEdgesByDst(GraphNode N, galois::MethodFlag mflag = MethodFlag::WRITE) {
    acquire(N, mflag);
    typedef typename gNode::EdgeInfo EdgeInfo;
    auto eDstCompare = [=] (const EdgeInfo& e1, const EdgeInfo& e2) { return e1.first() < e2.first(); };
                                 ...
                                 typename std::enable_if<!_Undirected>::type* = 0)
  {
    ...
    if (galois::runtime::shouldLock(mflag)) {
      for (typename gNode::iterator ii = N->in_edges_begin(), ee = N->in_edges_end(); ii != ee; ++ii) {
	if (ii->first()->active && ii->isInEdge())
	  ii->first()->acquire(mflag);
      }
    }
    return boost::make_filter_iterator(is_in_edge(), N->in_edges_begin(), N->in_edges_end());
  }
                               ...
                               typename std::enable_if<!_Undirected>::type* = 0)
  {
    ...
    // ever require it
    // N->acquire(mflag);
    return boost::make_filter_iterator(is_in_edge(), N->in_edges_end(), N->in_edges_end());
  }
   ...
   * edges of N.
   */
  internal::EdgesIterator<First_SepInOut_Graph> out_edges(GraphNode N, MethodFlag mflag = MethodFlag::WRITE) {
