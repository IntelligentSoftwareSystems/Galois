...
// 4. One thread trying to abort or add an item after commit, while other trying
// to schedule an item from pending 5.
...
// on self aborts
// TODO: memory management: other threads may refer to an iteration context that
// has been deallocated after commit or abort
...
private:
  ROBcontext(const ROBcontext& that) {
  } // TODO: remove since SimpleRuntimeContext inherits from boost::noncopyable

public:
  explicit ROBcontext(const T& x, Exec& e)
      : Base(x), state(State::UNSCHEDULED), executor(e), lostConflict(false),
        executed(false), owner(substrate::ThreadPool::getTID())
    ...
    for (bool succ = false; !succ;) {
      ...
      case Base::FAIL: {
        ...
        if (that != nullptr) {
          ...
          dbg::debug("owner found to be null, current value: ",
                     Base::getOwner(l), " for lock: ", l);
        }
        break;
      }
  ...
  GALOIS_ATTRIBUTE_PROF_NOINLINE void doCommit() {
    ...
    userHandle.commit();
    releaseLocks();
    executor.push(userHandle.getPushBuffer().begin(),
                  userHandle.getPushBuffer().end());
    userHandle.reset();
...
private:
  GALOIS_ATTRIBUTE_PROF_NOINLINE bool resolveConflict(ROBcontext* volatile that,
                                                      const Lockable* const l) {
    // precond: this could not acquire lock. lock owned by that
    // this can only be in state SCHEDULED or ABORT_SELF
    // that can be in SCHEDULED, ABORT_SELF, ABORT_HELP, READY_TO_COMMIT,
    // ABORT_DONE returns true if this loses the conflict and signals itself to
    // abort
    ...
    if (executor.getCtxtCmp()(this, that)) {
      ...
      assert(!that->hasState(State::COMMIT_DONE) &&
             !that->hasState(State::COMMITTING));
      // abort that
      if (that->hasState(State::ABORT_DONE)) {
        // do nothing

      } else if (that->casState(State::SCHEDULED, State::ABORT_SELF) ||
                 that->hasState(State::ABORT_SELF)) {
        // signalled successfully
        // now wait for it to abort or abort yourself if 'that' missed the
        // signal and completed execution
        ...
        while (true) {
          ...
          if (that->hasExecuted()) {
            if (that->casState(State::ABORT_SELF, State::ABORT_HELP)) {
              ...
              dbg::debug(this,
                         " aborting ABORT_SELF->ABORT_HELP missed signal ",
                         that, " on lock ", l);
            }
          }
...
public:
  ...
  ROBexecutor(const Cmp& cmp, const NhFunc& nhFunc, const OpFunc& opFunc)
      : itemCmp(cmp), nhFunc(nhFunc), opFunc(opFunc), ctxtCmp(itemCmp),
        pending(itemCmp), rob(ctxtCmp),
  ...
  GALOIS_ATTRIBUTE_PROF_NOINLINE void push_initial(const R& range) {
    ...
    galois::do_all(range, [this](const T& x) { pending.get().push(x); });

    assert(!pending.empty_all());

    const T& dummy = *pending[0].begin();

    galois::on_each([&dummy, this](const unsigned tid, const unsigned numT) {
    ...
    do {
      ...
      do {
        ...
        if (ctx != nullptr) {
          ...
          if (!ctx->casState(Ctxt::State::SCHEDULED,
                             Ctxt::State::READY_TO_COMMIT)) {
  ...
  void printStats() {
    ...
    double ar = double(numTotal.reduce() - numCommitted.reduce()) /
                double(numTotal.reduce());
    double totalAborts =
        double(abortSelfByConflict.reduce() + abortSelfBySignal.reduce() +
               abortByOther.reduce());
    gPrint("Total Iterations: ", numTotal.reduce(), "\n");
    gPrint("Number Committed: ", numCommitted.reduce(), "\n");
    gPrint("Abort Ratio: ", ar, "\n");
    gPrint("abortSelfByConflict: ", abortSelfByConflict.reduce(), ", ",
           (100.0 * abortSelfByConflict.reduce()) / totalAborts, "%", "\n");
    gPrint("abortSelfBySignal: ", abortSelfBySignal.reduce(), ", ",
           (100.0 * abortSelfBySignal.reduce()) / totalAborts, "%", "\n");
    gPrint("abortByOther: ", abortByOther.reduce(), ", ",
           (100.0 * abortByOther.reduce()) / totalAborts, "%", "\n");
      ...
      while (!rob.empty()) {
        ...
        if (head->hasState(Ctxt::State::ABORT_DONE)) {
          ...
          if (isEarliest(head->getActive())) {
            ...
            didWork = true;
            numCommitted += 1;
            dbg::debug(__ctxt, " committed: ", head,
                       ", with active: ", head->getActive());
...
public:
  ...
  ROBparamContext(const T& x, Exec& e, const size_t _step)
      : Base(x, e), step(_step) {}
  ...
  virtual void subAcquire(Lockable* l, galois::MethodFlag m) {
    for (bool succ = false; !succ;) {
      ...
      switch (acq) {
      case Base::FAIL: {
        ROBparamContext* that =
            static_cast<ROBparamContext*>(Base::getOwner(l));
        assert(that != nullptr);
        bool abortSelf = resolveConflict(that, l);
        succ           = abortSelf;
        break;
      ...
      case Base::ALREADY_OWNER: {
        assert(std::find(Base::nhood.begin(), Base::nhood.end(), l) !=
               Base::nhood.end());
        succ = true;
        break;
      }
...
private:
  bool resolveConflict(ROBparamContext* that, const Lockable* const l) {
    ...
    assert(this->hasState(Base::State::SCHEDULED) ||
           this->hasState(Base::State::ABORT_SELF));
...
public:
  ...
  ROBparaMeter(const Cmp& cmp, const NhFunc& nhFunc, const OpFunc& opFunc,
               const char* loopname)
      : itemCmp(cmp), nhFunc(nhFunc), opFunc(opFunc), loopname(loopname),
        ctxtCmp(itemCmp) {
  ...
  void push(const T& x) { nextPending->push(x); }

  void push_abort(const T& x, const unsigned owner) { nextPending->push(x); }
  ...
  void execute() {
    ...
    while (!nextPending->empty() || !rob.empty()) {
      ...
      nextPending->clear();
      execRcrds.emplace_back(
          steps - 1,
          currPending->size()); // create record entry for current step;
    ...
    std::printf("OptimParaMeterExecutor: steps=%zd, totalIter=%zd, "
                "totalCommits=%zd, avg=%f\n",
                steps, totalIter, totalCommits,
                float(totalCommits) / float(steps));
...
private:
  ...
  size_t clearROB(void) {
    ...
    auto new_end = std::partition(rob.begin(), rob.end(), [](Ctxt* c) {
      assert(c != nullptr);
...
void for_each_ordered_rob(const R& range, Cmp cmp, NhFunc nhFunc, OpFunc opFunc,
                          const char* loopname = 0) {

  using T = typename R::value_type;

  ParaMeter::ROBparaMeter<T, Cmp, NhFunc, OpFunc> exec(cmp, nhFunc, opFunc,
                                                       loopname);
