...
static cll::opt<bool>
    addRemWinArg("addRemWin", cll::desc("enable windowing in add-rem executor"),
                 cll::init(false));
...
public:
  KDGaddRemContext(const T& active, NhoodMgr& nhmgr)
      : OrderedContextBase<T>(
            active), // to make acquire call virtual function sub_acquire
        nhmgr(nhmgr), onWL(false) {}
...
template <typename T, typename Cmp, typename NhFunc, typename OpFunc,
          typename SourceTest, typename ArgsTuple, typename Ctxt>
class KDGaddRemAsyncExec
...
protected:
  ...
  struct DummyWinWL {
    void push(const T&) const { std::abort(); }
  };
  ...
  struct ApplyOperator {
    ...
    GALOIS_ATTRIBUTE_PROF_NOINLINE void operator()(Ctxt* const in_src, WL& wl) {
      ...
      for (unsigned local_iter = 0;
           (local_iter < UNROLL_FACTOR) && !exec.ctxtLocalQ.get().empty();
           ++local_iter) {

        Ctxt* src = exec.ctxtLocalQ.get().front();
        exec.ctxtLocalQ.get().pop_front();
        ...
        if (ThisClass::NEEDS_PUSH) {
          ...
          for (auto a    = userCtxt.getPushBuffer().begin(),
                    enda = userCtxt.getPushBuffer().end();
               a != enda; ++a) {
      ...
      while (exec.ctxtDelQ.get().size() >= DELETE_CONTEXT_SIZE) {
        ...
        Ctxt* c = exec.ctxtDelQ.get().front();
        exec.ctxtDelQ.get().pop_front();
...
public:
  KDGaddRemAsyncExec(const Cmp& cmp, const NhFunc& nhFunc, const OpFunc& opFunc,
                     const ArgsTuple& argsTuple, NhoodMgr& nhmgr,
                     const SourceTest& sourceTest)
      : Base(cmp, nhFunc, internal::DummyExecFunc(), opFunc, argsTuple),
        nhmgr(nhmgr), sourceTest(sourceTest) {}

  template <typename R>
  void expandNhoodpickSources(const R& range, CtxtWL& sources,
                              Accumulator& nInit) {

    addCtxtWL.clear_all_parallel();

    galois::runtime::do_all_gen(
        range, CreateCtxtExpandNhood{*this, nInit},
        std::make_tuple(galois::loopname("create_contexts"),
                        galois::chunk_size<NhFunc::CHUNK_SIZE>()));

    galois::runtime::do_all_gen(
        makeLocalRange(this->addCtxtWL),
        [this, &sources](Ctxt* ctxt) {
          if (sourceTest(ctxt) && ctxt->onWL.cas(false, true)) {
            sources.get().push_back(ctxt);
          }
        },
        std::make_tuple(galois::loopname("find_sources"),
  ...
  void applyOperator(CtxtWL& sources, A op) {
    ...
    galois::for_each(galois::iterate(sources), op,
                     galois::loopname("apply_operator"),
                     galois::wl<SrcWL_ty>());

    galois::runtime::do_all_gen(
        makeLocalRange(ctxtDelQ),
        [this](Ctxt* ctxt) {
          ThisClass::ctxtAlloc.destroy(ctxt);
          ThisClass::ctxtAlloc.deallocate(ctxt, 1);
        },
        std::make_tuple(galois::loopname("delete_all_ctxt"),
                        galois::chunk_size<DEFAULT_CHUNK_SIZE>()));

    galois::on_each([this, &sources](const unsigned tid, const unsigned numT) {
      sources.get().clear();
      ctxtDelQ.get().clear();
    });
  }
  ...
  void execute(const R& range) {
    ...
    t_for.start();
    applyOperator(initSrc, ApplyOperator<DummyWinWL>{*this, winWL, minWinWL,
                                                     nsrc, nInitCtxt});
    t_for.stop();
    ...
    reportStat_Single(
        Base::loopname,
        "Time taken in creating intial contexts: ", t_create.get());
    reportStat_Single(Base::loopname,
                      "Time taken in finding intial sources: ", t_find.get());
    reportStat_Single(Base::loopname,
                      "Time taken in for_each loop: ", t_for.get());
    reportStat_Single(
        Base::loopname,
        "Time taken in destroying all the contexts: ", t_destroy.get());
  }
};

template <typename T, typename Cmp, typename NhFunc, typename OpFunc,
          typename SourceTest, typename ArgsTuple, typename Ctxt>
class KDGaddRemWindowExec
    : public KDGaddRemAsyncExec<T, Cmp, NhFunc, OpFunc, SourceTest, ArgsTuple,
                                Ctxt> {

  using Base =
      KDGaddRemAsyncExec<T, Cmp, NhFunc, OpFunc, SourceTest, ArgsTuple, Ctxt>;
  ...
  void beginRound(void) { ThisClass::refillRound(winWL, pending); }

  void expandNhoodPending(void) {

    ThisClass::expandNhoodpickSources(makeLocalRange(pending), sources,
                                      ThisClass::roundTasks);
  ...
  void applyOperator(void) {
    ...
    Base::applyOperator(sources,
                        Op{*this, winWL, minWinWL, ThisClass::roundCommits,
                           ThisClass::roundTasks});
  }
...
public:
  KDGaddRemWindowExec(const Cmp& cmp, const NhFunc& nhFunc,
...
template <
    template <typename _one, typename _two, typename _three, typename _four,
              typename _five, typename _six, typename _seven> class Executor,
    typename R, typename Cmp, typename NhFunc, typename OpFunc, typename ST,
    typename ArgsTuple>
void for_each_ordered_ar_impl(const R& range, const Cmp& cmp,
...
template <typename R, typename Cmp, typename NhFunc, typename OpFunc,
          typename StableTest, typename ArgsTuple>
void for_each_ordered_ar(const R& range, const Cmp& cmp, const NhFunc& nhFunc,
                         const OpFunc& opFunc, const StableTest& stabilityTest,
                         const ArgsTuple& argsTuple) {

  if (addRemWinArg) {
    for_each_ordered_ar_impl<KDGaddRemWindowExec>(
        range, cmp, nhFunc, opFunc, SourceTest<StableTest>(stabilityTest),
        argsTuple);
  } else {
    for_each_ordered_ar_impl<KDGaddRemAsyncExec>(
        range, cmp, nhFunc, opFunc, SourceTest<StableTest>(stabilityTest),
        argsTuple);
  }
}
...
void for_each_ordered_ar(const R& range, const Cmp& cmp, const NhFunc& nhFunc,
  ...
  if (addRemWinArg) {
    for_each_ordered_ar_impl<KDGaddRemWindowExec>(
        range, cmp, nhFunc, opFunc, SourceTest<void>(), argsTuple);
  } else {
    for_each_ordered_ar_impl<KDGaddRemAsyncExec>(range, cmp, nhFunc, opFunc,
                                                 SourceTest<void>(), argsTuple);
  }
}
