...
protected:
  ...
  public:
    ...
    void spawn(const T& elem) { executor.spawn(elem, parent); }
  };
  ...
  void push(Task* t) { workList.push(t); }
...
public:
  TreeExecutorTwoFunc(const DivFunc& divFunc, const ConqFunc& conqFunc,
                      const char* loopname)
      : divFunc(divFunc), conqFunc(conqFunc), loopname(loopname) {}
  ...
  void execute(const R& initRange) {
    ...
    galois::runtime::do_all_gen(
        ...
        std::make_tuple(galois::loopname("create_initial_tasks"),
                        galois::chunk_size<4>()));

    typedef worklists::ExternalReference<WL_ty> WL;
    typename WL::value_type* it = nullptr;

    galois::for_each(galois::iterate(it, it), ApplyOperatorSinglePhase{*this},
...
void for_each_ordered_tree(I beg, I end, const DivFunc& divFunc,
                           const ConqFunc& conqFunc, const char* loopname) {

  typedef typename std::iterator_traits<I>::value_type T;

  TreeExecutorTwoFunc<T, DivFunc, ConqFunc, false> executor{divFunc, conqFunc,
                                                            loopname};
  executor.execute(galois::runtime::makeStandardRange(beg, end));
}

template <typename T, typename DivFunc, typename ConqFunc>
void for_each_ordered_tree(const T& rootItem, const DivFunc& divFunc,
                           const ConqFunc& conqFunc,
...
public:
  ...
  public:
    ...
    void spawn(F& f) { executor->spawn(f, parent); }

    void sync() { executor->syncLoop(*this); }

    unsigned getNumChild(void) const { return parent->numChild; }
  };
...
protected:
  struct PerThreadData {
    ...
    PerThreadData(const char* loopname)
        : stat_iterations(0), stat_pushes(0), didWork(false), loopname(loopname)
...
public:
  TreeExecStack(const char* loopname)
      : loopname(loopname), perThreadData(loopname),
        term(substrate::getSystemTermination(activeThreads)) {}

  void initThread(void) { term.initializeThread(); }

  void initWork(F& initTask) { push(WorkItem(nullptr, &initTask)); }
...
void for_each_ordered_tree_generic(TreeTaskBase& initTask,
                                   const char* loopname = nullptr);
