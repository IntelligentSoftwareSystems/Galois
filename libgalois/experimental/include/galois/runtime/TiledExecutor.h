...
class Fixed2DGraphTiledExecutor {
  ...
  void executeBlock(Function& fn, Task& task, 
    ...
    for (auto ii = task.startX; ii != task.endX; ++ii) {
      no_deref_iterator 
        nbegin(g.edge_begin(*ii, galois::MethodFlag::UNPROTECTED));
      no_deref_iterator 
        nend(g.edge_end(*ii, galois::MethodFlag::UNPROTECTED));
      ...
      for (auto jj = std::lower_bound(dbegin, dend, task.startY); 
           jj != dend; ) {
        //if (UseExp) {
        ...
        //} else {
          ...
          if (*jj > task.endYInclusive) break;
  ...
  void executeLoopExp(Function fn, unsigned tid, unsigned total) {
    ...
    for (size_t rounds = 0; rounds < maxRounds; ++rounds) {
      ...
      for (size_t tries = 0; tries < ntries; ++tries) {
        ...
        for (int i = 0; i < numDims; ++i) nextPoint(p, i, 1);
      }
   ...
   */
  void initializeTasks(iterator firstX, iterator lastX, 
                       iterator firstY, iterator lastY, 
                       size_t sizeX, size_t sizeY) {
    const size_t numXBlocks = (std::distance(firstX, lastX) + sizeX - 1) / 
                              sizeX;
    const size_t numYBlocks = (std::distance(firstY, lastY) + sizeY - 1) / 
                              sizeY;
...
public:
   ...
   */
  template<typename Function>
  void executeDense(iterator firstX, iterator lastX, 
                    iterator firstY, iterator lastY,
                    size_t sizeX, size_t sizeY,
