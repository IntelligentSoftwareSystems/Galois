...
struct DAGnhoodItem : public OrdLocBase<DAGnhoodItem<Ctxt, SharerWrapper>, Ctxt,
                                        std::less<Ctxt*>> {
  ...
  void addSharer(Ctxt* ctxt) { wrapper.addSharer(ctxt); }

  void removeMin(Ctxt* ctxt) {
    if (head != end) {
      assert(*head == ctxt);
      ++head;
      assert(std::find(head, wrapper.sharers.end(), ctxt) ==
             wrapper.sharers.end());
    }
  }
...
struct DAGcontext : public OrderedContextBase<T> {
  ...
  explicit DAGcontext(const T& t, NhoodMgr& nhmgr)
      : OrderedContextBase<T>{t}, onWL{false}, nhmgr{nhmgr} {}
  ...
  size_t findNewSources(WL& workList) {
    ...
    for (auto i = nhood.begin(), end_i = nhood.end(); i != end_i; ++i) {
      ...
      if (min != nullptr && !bool(min->onWL) && min->isSrc() &&
...
protected:
  typedef galois::ThreadSafeOrderedSet<DAGcontext*, std::less<DAGcontext*> >
AdjSet;
...
public:
  DAGexecutor(const Cmp& cmp, const NhoodFunc& nhVisitor, const OpFunc& opFunc)
      : cmp(cmp), nhVisitor(nhVisitor), opFunc(opFunc), nitemFactory(CtxtCmp()),
        nhmgr(nitemFactory) {}
  ...
  void initialize(const R& range) {
    ...
    galois::runtime::do_all_gen(range,
                                [this](const T& x) {
                                  ...
                                  // printf ("Created context:%p for item:
                                  // %d\n", ctxt, x);
                                },
                                "create_ctxt",
                                galois::chunk_size<DEFAULT_CHUNK_SIZE>());
  ...
  void resetDAG(void) {
    ...
    t_reset.start();
    galois::runtime::do_all_gen(
        nhmgr.getAllRange(), [](NItem* nitem) { nitem->reset(); }, "reset_dag",
        galois::chunk_size<DEFAULT_CHUNK_SIZE>());
...
Exp::DAGexecutor<typename R::value_type, Cmp, OpFunc, NhoodFunc>
make_dag_executor_alt(const R& range, const Cmp& cmp,
                      const NhoodFunc& nhVisitor, const OpFunc& opFunc,
                      const char* loopname = nullptr) {

  return new Exp::DAGexecutor<typename R::value_type, Cmp, OpFunc, NhoodFunc>(
      cmp, nhVisitor, opFunc);
}

template <typename R, typename Cmp, typename OpFunc, typename NhoodFunc>
void destroy_dag_executor_alt(Exp::DAGexecutor<typename R::value_type, Cmp,
                                               OpFunc, NhoodFunc>*& exec_ptr) {
  delete exec_ptr;
  exec_ptr = nullptr;
}

template <typename R, typename Cmp, typename OpFunc, typename NhoodFunc>
void for_each_ordered_dag_alt(const R& range, const Cmp& cmp,
                              const NhoodFunc& nhVisitor, const OpFunc& opFunc,
                              const char* loopname = nullptr) {
