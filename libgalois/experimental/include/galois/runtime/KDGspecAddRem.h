...
struct SpecAddRemNhoodItem: public OrdLocBase<SpecAddRemNhoodItem<Ctxt, CtxtCmp>, Ctxt, CtxtCmp> {
...
protected:
  ...
  NhoodItem (Lockable* l, const CtxtCmp& ctxtcmp):  
    Base (l), 
    sharers (ctxtcmp)
  {}
...
struct SpecAddRemContext: public SpecContextBase<T, Cmp, Exec> {
  ...
  void addChild (SpecAddRemContext* child) {
...
class KDGspecAddRemExecutor: public OrdSpecExecBase<T, Cmp, NhFunc, ExFunc, OpFunc, ArgsTuple,
  SpecAddRemContext<T, Cmp, KDGspecAddRemExecutor<T, Cmp, NhFunc, ExFunc, OpFunc, ArgsTuple> > > {


  void expandNhoodPending (void) {
    galois::runtime::do_all_gen (makeLocalRange (pendingWL),
        [this] (Ctxt* c) {
        ...
        std::make_tuple (
          galois::loopname ("include-pending"),
  ...
  void applyOperator (void) {
    ...
    galois::runtime::do_all_gen (makeLocalRange (Base::currWL),
        [this] (Ctxt* c) {
    ...
    galois::runtime::do_all_gen (makeLocalRange (Base::getCurrWL()),
        [this, minWinWL] (Ctxt* c) {
          ...
          for (Ctxt* child: c->children) {
            ...
            if (!minWinWL || (Base::targetCommitRatio == 0.0) || ctxtCmp (child, minWinWL)) {
        ...
        [this] (Ctxt* c) {
          for (Ctxt* child: c->children) {
            ...
            if (child->hasState (ContextState::SCHEDULED) 
              && child->isSrcSlowCheck()
              && child->onWL.cas (false, true)) {
